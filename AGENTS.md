...
- 上位仮説のIDは `HYP-YYYYMMDD-NN` 形式とし、同じIDを再利用しません。発行前にリポジトリ全体を検索して未使用であることを確認します。新しい上位仮説を追加するときは、`KAGGLE_DIRECTION.md` の「検証中の仮説」に、仮説、対応する未着手候補、対応する実験、残っている問いを記録します。
