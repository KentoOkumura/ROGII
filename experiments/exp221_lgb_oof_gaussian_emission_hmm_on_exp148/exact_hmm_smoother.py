...
EXPERIMENT_NAME = "exp221_lgb_oof_gaussian_emission_hmm_on_exp148"
OUTPUT_PREFIX = "exp221_lgb_oof_gaussian_emission_hmm_on_exp148"
VARIANT = "lgb_oof_gaussian_emission_hmm"
...
def prepare_lgb_emission_variants(root: str | Path, config: dict[str, Any] | None) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if not active_sources or not sigma_values or not lambda_values:
        raise ValueError("lgb_emission.enabled requires active_sources, sigma_grid, and lambda_grid")
    ...
    for source in active_sources:
        ...
        if source not in loaded_sources:
            loaded_sources[source] = load_lgb_prediction_series(root, source, sources_config[source])
    ...
    *,
    ...
    summary = {
        ...
        "notes": [
            ...
            "LGB OOF predictions are used as target-free Gaussian observation centers; true TVT is not used to tune row-wise sigma or lambda.",
