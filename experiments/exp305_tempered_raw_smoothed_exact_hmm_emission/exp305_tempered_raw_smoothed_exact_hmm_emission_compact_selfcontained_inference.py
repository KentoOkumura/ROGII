# %% [markdown]
# # exp305 tempered raw-smoothed exact-HMM emission inference
#
# This experiment is a train-side decoder audit. Inference, raw-test
# regeneration, submission creation, and automatic promotion are fail-closed.
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    contract = {
        ...
        "inference_run": bool(get_nested(config, "execution.run_inference")),
    ...
    if contract["experiment"] != EXPERIMENT_NAME or contract["route"] != "pf_beam":
        raise ValueError("unexpected exp305 inference config")
    if contract["mode"] != "disabled_train_side_tempered_exact_hmm_audit_only":
        raise ValueError("exp305 inference mode contract changed")
    if any(
        contract[key]
        for key in (
            "inference_enabled",
        ...
        raise ValueError("exp305 inference and submission must remain disabled")
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp305 is a train-side tempered exact-HMM audit; inference and submission are disabled"
    )
