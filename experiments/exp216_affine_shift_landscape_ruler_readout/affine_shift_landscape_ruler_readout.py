...
OUTPUT_PREFIX = "exp216_affine_shift_landscape_ruler_readout"
...
def scan_filter_for_region(
    *,
    ...
    calibration_residual_scale: float | None,
    bimodal_min_separation_ft: float,
    bimodal_gap_threshold: float,
    ...
    sorted_shift_indices = np.argsort(cost, axis=1)
    sorted_cost = np.take_along_axis(cost, sorted_shift_indices, axis=1)
    best_pos = sorted_shift_indices[:, 0]
    best_cost = sorted_cost[:, 0]
    if cost.shape[1] > 1:
        second_pos = sorted_shift_indices[:, 1]
        second_cost = sorted_cost[:, 1]
        second_shift = shifts[second_pos].astype(np.float32)
    else:
        second_pos = np.full(len(row_idx), -1, dtype=np.int32)
        second_cost = best_cost
        second_shift = np.full(len(row_idx), np.nan, dtype=np.float32)
    ...
    best_delta = best_shift.astype(np.float32)
    second_delta = second_shift.astype(np.float32)
    second_delta_vs_best = second_shift - best_delta
    zero_pos = int(np.abs(shifts).argmin())
    zero_shift = float(shifts[zero_pos])
    zero_cost = cost[:, zero_pos]
    zero_rank = (cost <= zero_cost[:, None]).sum(axis=1)
    separated = np.abs(shifts[None, :] - best_shift[:, None]) >= float(
        bimodal_min_separation_ft
    )
    separated_cost = np.where(separated, cost, np.inf)
    secondary_mode_pos = np.argmin(separated_cost, axis=1)
    secondary_mode_cost = separated_cost[np.arange(len(row_idx)), secondary_mode_pos]
    has_secondary_mode = np.isfinite(secondary_mode_cost)
    secondary_mode_cost = np.where(has_secondary_mode, secondary_mode_cost, np.nan)
    secondary_mode_shift = np.where(
        has_secondary_mode,
        shifts[secondary_mode_pos],
        np.nan,
    ).astype(np.float32)
    secondary_mode_gap = np.where(
        has_secondary_mode,
        secondary_mode_cost - best_cost,
        np.nan,
    ).astype(np.float32)
    bimodal_flag = (
        has_secondary_mode & (secondary_mode_gap <= float(bimodal_gap_threshold))
    ).astype(np.int8)
    ...
    calibration_scale_value = (
        float(calibration_residual_scale) if calibration_residual_scale is not None else np.nan
    )
    calibration_scale = np.full(len(row_idx), calibration_scale_value, dtype=np.float32)
    ...
    result = pd.DataFrame(
        {
            ...
            "second_shift_ft": second_shift,
            "second_delta": second_delta.astype(np.float32),
            "second_delta_vs_best": second_delta_vs_best.astype(np.float32),
            "second_cost": second_cost.astype(np.float32),
            "zero_shift_ft": np.full(len(row_idx), zero_shift, dtype=np.float32),
            "zero_cost": zero_cost.astype(np.float32),
            "zero_rank": zero_rank.astype(np.float32),
            "zero_gap_vs_top1": (zero_cost - best_cost).astype(np.float32),
            "zero_delta": (zero_shift - best_shift).astype(np.float32),
            "secondary_mode_shift_ft": secondary_mode_shift,
            "secondary_mode_delta": secondary_mode_shift.astype(np.float32),
            "secondary_mode_delta_vs_best": (
                secondary_mode_shift - best_shift
            ).astype(np.float32),
            "secondary_mode_cost": secondary_mode_cost.astype(np.float32),
            "secondary_mode_gap": secondary_mode_gap,
            "bimodal_flag": bimodal_flag,
            "calibration_residual_scale": calibration_scale,
            "prefix_holdout_error": np.where(region == "prefix_backtest", error, np.nan).astype(
                np.float32
            ),
            "prefix_holdout_abs_error": np.where(
                region == "prefix_backtest",
                np.abs(error),
                np.nan,
            ).astype(np.float32),
    ...
    shift_curve_metrics = pd.DataFrame(
        {
            "well": well,
            "eval_region": region,
            "shift_ft": shifts.astype(np.float32),
            "rows": int(len(row_idx)),
            "mean_cost": cost.mean(axis=0).astype(np.float32),
            "median_cost": np.median(cost, axis=0).astype(np.float32),
            "mean_mae": mae.mean(axis=0).astype(np.float32),
            "mean_ncc": ncc.mean(axis=0).astype(np.float32),
            "mean_weight": weights.mean(axis=0).astype(np.float32),
            "top1_rate": (best_pos[:, None] == np.arange(len(shifts))[None, :])
            .mean(axis=0)
            .astype(np.float32),
        }
    )

    return result, shift_curve_metrics
...
def metric_row(group: pd.DataFrame, *, keys: dict[str, Any]) -> dict[str, Any]:
    ...
    if "prefix_holdout_abs_error" in group.columns:
        prefix_holdout_abs_error = pd.to_numeric(
            group["prefix_holdout_abs_error"], errors="coerce"
        )
    else:
        prefix_holdout_abs_error = pd.Series(dtype="float64")
    ...
    if not finite.any():
        metrics = {
            ...
            "margin_mean": None,
            "zero_rank_mean": None,
            "zero_gap_vs_top1_mean": None,
            "secondary_mode_gap_mean": None,
            "bimodal_rate": None,
            "prefix_holdout_mae": None,
        }
    else:
        ...
        metrics = {
            ...
            "zero_rank_mean": mean_or_none("zero_rank"),
            "zero_gap_vs_top1_mean": mean_or_none("zero_gap_vs_top1"),
            "secondary_mode_gap_mean": mean_or_none("secondary_mode_gap"),
            "bimodal_rate": mean_or_none("bimodal_flag"),
            "prefix_holdout_mae": (
                float(prefix_holdout_abs_error.mean())
                if prefix_holdout_abs_error.notna().any()
                else None
            ),
        }
    return {**keys, **metrics}
...
def pearson_or_none(left: pd.Series, right: pd.Series) -> float | None:
    x = pd.to_numeric(left, errors="coerce").to_numpy(np.float64)
    y = pd.to_numeric(right, errors="coerce").to_numpy(np.float64)
    finite = np.isfinite(x) & np.isfinite(y)
    if finite.sum() < 3:
        return None
    if float(np.std(x[finite])) < 1e-12 or float(np.std(y[finite])) < 1e-12:
        return None
    return float(np.corrcoef(x[finite], y[finite])[0, 1])


def summarize_error_correlations(row_context: pd.DataFrame) -> pd.DataFrame:
            ...
            rows.append(
                {
                    **base,
                    "signal": signal,
                    "rows": int(len(group)),
                    "corr_with_abs_error": pearson_or_none(group[signal], group["abs_error"]),
                }
            )
    return pd.DataFrame(rows).sort_values(
        ["eval_region", "distance_bucket", "surface", "signal"]
    )
...
def build_well_audit(
    *,
    ...
    shift_curve_frames: list[pd.DataFrame] = []
    ...
    for region, region_known_end, row_idx in regions:
        ...
        for filtered in filters:
            for mode in calibration_modes:
                ...
                frame, shift_curve = scan_filter_for_region(
                    well=well,
                    region=region,
                    row_idx=row_idx,
                    md=md,
                    ...
                    calibration_residual_scale=calibration_meta.get("residual_mad"),
                    bimodal_min_separation_ft=float(
                        audit_config.get("bimodal_min_separation_ft", 25.0)
                    ),
                    bimodal_gap_threshold=float(
                        audit_config.get("bimodal_gap_threshold", 1.0)
                    ),
                )
                if frame.empty:
                    continue
                ...
                shift_curve.insert(2, "filter", filtered.name)
                shift_curve.insert(3, "calibration_mode", mode)
                shift_curve.insert(4, "surface", surface)
                shift_curve_frames.append(shift_curve)
                input_rows.append(
                    {
                        "well": well,
    ...
    if not row_frames:
        return pd.DataFrame(), input_rows, pd.DataFrame()
    return (
        concat_frames(row_frames),
        input_rows,
        concat_frames(shift_curve_frames),
    )


def run_affine_shift_landscape_ruler_readout(
    *,
    config: dict[str, Any],
    train_dir: str | Path,
    output_dir: str | Path,
    ...
    for i, well in enumerate(wells, start=1):
        ...
        try:
            frame, well_inputs, shift_curve = build_well_audit(
                well=well,
                train_dir=train_dir,
                audit_config=audit_config,
        ...
        if not frame.empty:
            row_frames.append(frame)
        if not shift_curve.empty:
            shift_curve_frames.append(shift_curve)
        input_rows.extend(well_inputs)

    if not row_frames:
        raise RuntimeError(
            "No row contexts were generated for affine shift-landscape ruler readout"
        )
    row_context = concat_frames(row_frames)
    shift_curve_metrics = concat_frames(shift_curve_frames)
    surface_metrics, bucket_metrics, well_metrics = summarize_metrics(row_context)
    gain_vs_raw = summarize_gain_vs_raw(row_context)
    error_correlation_metrics = summarize_error_correlations(row_context)
    ...
    shift_curve_path = output_dir / f"{output_prefix}_shift_curve_metrics.csv"
    error_correlation_path = output_dir / f"{output_prefix}_error_correlation_metrics.csv"
    ...
    shift_curve_metrics.to_csv(shift_curve_path, index=False)
    error_correlation_metrics.to_csv(error_correlation_path, index=False)
    ...
    best_calibrated = (
        surface_metrics[
            ...
            & ~surface_metrics["calibration_mode"].eq("raw")
        ]
        .sort_values("rmse_tvt")
        .head(1)
        .to_dict("records")
    ...
    summary: dict[str, Any] = {
        "experiment": get_nested(
            config, "experiment.name", "exp216_affine_shift_landscape_ruler_readout"
        ),
        "status": get_nested(config, "experiment.status", "implemented_pending_kaggle_train"),
        ...
        "best_calibrated_all": best_calibrated[0] if best_calibrated else None,
        "decision": {
            "recommendation": "diagnostic_only_until_kaggle_result_review",
            "reason": (
                "Affine shift-ruler readout is only a train-side shift-scan and "
                "observation-cost audit. "
        ...
        "artifacts": {
            ...
            "shift_curve_metrics": str(shift_curve_path),
            "error_correlation_metrics": str(error_correlation_path),
        ...
        "artifact_sha256": {
            ...
            "shift_curve_metrics": sha256_file(shift_curve_path),
            "error_correlation_metrics": sha256_file(error_correlation_path),
    ...
    if metrics_path is not None:
        write_json(
            Path(metrics_path),
            {
                ...
                "best_calibrated_all": summary["best_calibrated_all"],
                "rows": summary["rows"],
                "wells_with_rows": summary["wells_with_rows"],
                "shift_curve_metrics_path": str(shift_curve_path),
                "error_correlation_metrics_path": str(error_correlation_path),
    ...
    return {
        ...
        "shift_curve_metrics": shift_curve_metrics,
        "error_correlation_metrics": error_correlation_metrics,
