...
EXP183_SELECTOR_OOF_PREDICTIONS = (
    "exp183_cluster_outlier_prior_confidence_addonly_on_exp158_selector_oof_predictions.csv.gz"
)
EXP183_SELECTOR_SUMMARY = (
    "exp183_cluster_outlier_prior_confidence_addonly_on_exp158_selector_summary.json"
)
EXP148_OOF_PREDICTIONS = "exp148_learned_likelihood_fulltrain_addonly_on_exp092_predictions.csv.gz"
OUTPUT_PREFIX = "exp188_exp183_selector_confidence_addonly_on_exp148"
...
def load_exp183_selector_oof_predictions(
    ...
    *,
    ...
    source = find_artifact(
        EXP183_SELECTOR_OOF_PREDICTIONS,
        prediction_path,
        local_artifacts=EXP183_TRAIN_ARTIFACTS,
    )
    usecols = [
        "id",
        "well",
    ...
    if selected.empty:
        ...
        raise ValueError(
            f"exp183 selector OOF variant={variant!r} mode={mode!r} not found; "
            f"available examples={available}"
        )
    duplicated = selected.duplicated(["id", "well"]).sum()
    if duplicated:
        raise ValueError(f"exp183 selector OOF has duplicated id/well rows: {duplicated}")
    unused_columns = {
        "variant",
        "mode",
        "true_tvt",
    ...
    return selected, {
        ...
        "source_sha256": sha256_file(source),
        "source_decompressed_sha256": sha256_gzip_decompressed(source),
        "source_kind": "exp183_oof_best_viterbi_selected_path",
        "variant": variant,
        "mode": mode,
...
def build_exp183_selector_confidence_features(
    base_frame: pd.DataFrame,
    config: dict[str, Any] | None = None,
) -> tuple[pd.DataFrame, dict[str, list[str]], pd.DataFrame, dict[str, Any]]:
    config = config or {}
    prefix = str(config.get("prefix") or "exp183_")
    key_cols = ["id", "well"]
    group_name = "exp183_selector_confidence"
    ...
    selector_frame, selector_meta = load_exp183_selector_oof_predictions(
    ...
    if missing_rate > max_missing_rate:
        raise ValueError(
            f"exp183 selector feature join missing_rate={missing_rate:.6f} "
    ...
    add_feature(f"{prefix}is_default_likpf", selected_is_default)
    add_feature(f"{prefix}is_dense_family", selected_is_dense)
    add_feature(f"{prefix}is_pfbeam_family", selected_is_pfbeam)
    add_feature(f"{prefix}selected_tvt_minus_last_known_tvt", selected_tvt - last_tvt)
    add_feature(f"{prefix}selected_tvt_abs_delta_from_last_known_tvt", np.abs(selected_tvt - last_tvt))
    add_feature(
        f"{prefix}selected_tvt_delta_from_last_norm_md",
    ...
    if available_candidate_count >= 3:
        candidate_sum = np.zeros(len(base_frame), dtype=np.float32)
        candidate_sumsq = np.zeros(len(base_frame), dtype=np.float32)
        candidate_min = np.full(len(base_frame), np.inf, dtype=np.float32)
        candidate_max = np.full(len(base_frame), -np.inf, dtype=np.float32)
        for _, values in available_candidates:
            values = values.astype(np.float32, copy=False)
            candidate_sum += values
            candidate_sumsq += values * values
            candidate_min = np.minimum(candidate_min, values)
            candidate_max = np.maximum(candidate_max, values)
        candidate_mean = (candidate_sum / np.float32(available_candidate_count)).astype(np.float32)
        candidate_var = np.maximum(
            candidate_sumsq / np.float32(available_candidate_count) - candidate_mean * candidate_mean,
            np.float32(0.0),
        )
        candidate_std = np.sqrt(candidate_var).astype(np.float32)
        add_feature(f"{prefix}selected_minus_candidate_mean", selected_tvt - candidate_mean)
        add_feature(
            f"{prefix}selected_abs_z_vs_candidate_mean",
            _safe_divide(selected_tvt - candidate_mean, candidate_std + 1.0),
        )
        add_feature(f"{prefix}candidate_tvt_std", candidate_std)
        add_feature(
            f"{prefix}candidate_tvt_range",
            (candidate_max - candidate_min).astype(np.float32),
        )
        del candidate_sum, candidate_sumsq, candidate_min, candidate_max, candidate_mean
        gc.collect()
    del available_candidates, candidate_map, default_tvt
    gc.collect()
    ...
    if exp148_frame is not None:
        ...
        add_feature(f"{prefix}selected_tvt_minus_exp148_oof", np.where(valid_exp148, selected_tvt - exp148_oof, 0.0))
        add_feature(f"{prefix}selected_tvt_abs_minus_exp148_oof", np.where(valid_exp148, np.abs(selected_tvt - exp148_oof), 0.0))
    ...
    if feature_cols:
        _assert_finite_columns(
            feature_matrix,
            feature_cols,
            "exp183 selector confidence feature frame",
        )
...
def build_sp45_bimodal_selector_features(
    ...
    features, groups, summary, _meta = build_exp183_selector_confidence_features(
        base_frame,
        config,
    )
...
def run_exp183_selector_confidence_addonly_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    exp183_feature_config: dict[str, Any] | None = None,
    ...
    exp183_features, exp183_group_columns, exp183_summary, exp183_meta = (
        build_exp183_selector_confidence_features(
            full_frame,
            exp183_feature_config or {},
        )
    )
    exp183_feature_columns = [col for col in exp183_features.columns if col not in {"id", "well"}]
    if not full_frame["id"].equals(exp183_features["id"]) or not full_frame["well"].equals(
        exp183_features["well"]
    ):
        raise ValueError(
            "exp183 selector features are not row-order aligned with train feature frame"
        )
    _assign_aligned_float32_columns(
        full_frame,
        exp183_features,
        exp183_feature_columns,
    )
    del exp183_features
    gc.collect()
    print(
        json.dumps(
            {
                "stage": "added_exp183_selector_features",
                "rows": int(len(full_frame)),
                "exp183_selector_features": int(len(exp183_feature_columns)),
            },
            sort_keys=True,
        ),
        flush=True,
    ...
    exp183_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_exp183_selector_feature_summary.csv",
        index=False,
    )
            ...
            feature_schema_rows.append(
                {
                    ...
                    "is_exp183_selector_feature": bool(feature in exp183_feature_columns),
                }
            )
    ...
    manifest = {
        "experiment": "exp188_exp183_selector_confidence_addonly_on_exp148",
        ...
        "selector_parent": "exp183_cluster_outlier_prior_confidence_addonly_on_exp158_selector",
        "cache_parent": "exp072_exp063_full_replay_feature_cache",
        "mode": "exp183_selector_confidence_addonly_on_exp148_full_train_rows",
        "feature_source": feature_meta,
        "learned_likelihood_feature_source": learned_source_meta,
        "exp183_selector_feature_source": exp183_meta,
        ...
        "exp183_feature_config": exp183_feature_config or {},
        "projection_feature_groups": projection_group_columns,
        "learned_feature_groups": learned_group_columns,
        "exp183_selector_feature_groups": exp183_group_columns,
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "exp183_selector_feature_summary": f"{OUTPUT_PREFIX}_exp183_selector_feature_summary.csv",
