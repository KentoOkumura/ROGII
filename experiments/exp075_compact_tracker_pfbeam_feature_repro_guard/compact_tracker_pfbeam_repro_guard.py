...
OUTPUT_PREFIX = "compact_tracker_pfbeam_repro_guard"
...
def sha256_csv_content(path: str | Path) -> str:
    """Hash decompressed CSV bytes so gzip metadata cannot affect reproducibility checks."""
    path = Path(path)
    hasher = hashlib.sha256()
...
def load_exp063_tracker_frame(
    tracker_path: str | Path | None = None,
    *,
    max_rows: int | None = None,
) -> tuple[pd.DataFrame, list[str], dict[str, Any]]:
    source = find_artifact(TRACKER_TRAIN_FEATURES, tracker_path)
    ...
    if not np.isfinite(frame[["target", *feature_columns]].to_numpy(np.float32)).all():
        raise ValueError("exp063 tracker frame contains non-finite numeric values")
    metadata = {
        "source": str(source),
        "source_sha256": sha256_file(source),
        "source_csv_content_sha256": sha256_csv_content(source),
        "rows": int(len(frame)),
...
def load_exp063_tracker_test_frame(
    ...
    source = find_artifact(TRACKER_TEST_FEATURES, tracker_path)
...
def generate_exp063_tracker_train_frame(
    *,
    data_dir: str | Path,
    output_dir: str | Path,
    ...
    _, pixiux_frame, generation_meta = build_replay_train_frames(max_wells=max_wells)
    tracker_path = output_dir / TRACKER_TRAIN_FEATURES
    tracker_meta = save_tracker_feature_frame(pixiux_frame, tracker_path)
    tracker_frame, _, read_meta = load_exp063_tracker_frame(tracker_path)
    read_meta.update(
        {
            "source_kind": "raw_train_generated_exp063_public_replay",
            "feature_generation": generation_meta,
            "tracker_frame": tracker_meta,
        }
    )
    return tracker_frame, read_meta


def run_pfbeam_feature_generation(
    *,
    data_dir: str | Path,
    output_dir: str | Path,
    n_jobs: int | None = None,
    ...
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    frame, feature_meta = generate_exp063_tracker_train_frame(
        data_dir=data_dir,
        output_dir=output_dir,
        n_jobs=n_jobs,
        pf_seeds=pf_seeds,
    ...
    summary = {
        "experiment": "exp075_compact_tracker_pfbeam_feature_repro_guard",
        "status": "train_feature_generation_completed",
        "mode": "raw_train_pfbeam_likelihoodpf_compact_feature_generation",
        "feature_source": feature_meta,
        "rows": int(len(frame)),
        "wells": int(frame["well"].nunique()),
        "features": int(len([c for c in frame.columns if c not in META_COLUMNS])),
        "artifacts": {
            "tracker_features_train": TRACKER_TRAIN_FEATURES,
            "summary": f"{OUTPUT_PREFIX}_feature_generation_summary.json",
        },
        "elapsed_seconds": round(time.time() - t0, 3),
    }
    (output_dir / f"{OUTPUT_PREFIX}_feature_generation_summary.json").write_text(
...
def generate_exp063_tracker_test_frame(
    *,
    ...
    tracker_path = output_dir / TRACKER_TEST_FEATURES
    tracker_meta = save_tracker_feature_frame(raw_test_frame, tracker_path)
    tracker_frame, read_meta = load_exp063_tracker_test_frame(tracker_path)
    read_meta.update(
        {
...
def _plot_mean_feature_importance(
    mean_importance: pd.DataFrame,
    output_path: Path,
    *,
    top_n: int = 40,
    title: str = "Compact PF/Beam LightGBM mean feature importance",
    xlabel: str = "mean feature_importances_",
) -> None:
    import matplotlib.pyplot as plt

    if mean_importance.empty:
        return
    top = mean_importance.nlargest(int(top_n), "mean_importance")
    top = top.sort_values("mean_importance", ascending=True)
    fig_height = max(6.0, min(20.0, len(top) * 0.32 + 1.5))
    fig, ax = plt.subplots(figsize=(10.5, fig_height))
    ax.barh(top["feature"], top["mean_importance"], color="#2f6f8f")
    ax.set_title(title)
...
def run_reproducibility_guard(
    *,
    ...
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    frame, feature_columns, feature_meta = load_exp063_tracker_frame(
        tracker_path,
        max_rows=max_rows,
    )
    mode_map = modes or {}
    ...
    for mode_name in selected_modes:
        ...
        metrics, importance, by_well, predictions, models, mode_summary = _fit_one_mode(
            mode_name=mode_name,
    ...
    fold_mean_importance = (
        importance.groupby(["mode", "fold", "feature"], as_index=False)
        .agg(
            mean_importance=("importance", "mean"),
            model_records=("importance", "size"),
        )
        .sort_values(["mode", "fold", "mean_importance"], ascending=[True, True, False])
    )
    fold_mean_summary = (
        fold_mean_importance.groupby(["feature"], as_index=False)
        .agg(
            mean_importance=("mean_importance", "mean"),
            std_importance=("mean_importance", "std"),
            fold_records=("fold", "nunique"),
            fold_model_records=("model_records", "sum"),
        )
        .sort_values("mean_importance", ascending=False)
    )
    ...
    fold_mean_importance.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_feature_importance_fold_mean_by_fold.csv",
        index=False,
    )
    fold_mean_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_feature_importance_fold_mean.csv",
        index=False,
    )
    _plot_mean_feature_importance(
        mean_importance,
        output_dir / f"{OUTPUT_PREFIX}_feature_importance_mean_top.png",
    )
    _plot_mean_feature_importance(
        fold_mean_summary,
        output_dir / f"{OUTPUT_PREFIX}_feature_importance_fold_mean_top.png",
        title=(
            "Compact PF/Beam LightGBM feature importance\n"
            "mean of per-fold model-averaged importance"
        ),
        xlabel="mean of fold-level model-averaged feature_importances_",
    )
    ...
    manifest = {
        ...
        "mode": "compact_tracker_pfbeam_lgbm_from_generated_train_features",
        "feature_source": feature_meta,
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "mean_feature_importance": f"{OUTPUT_PREFIX}_feature_importance_mean.csv",
            "mean_feature_importance_plot": f"{OUTPUT_PREFIX}_feature_importance_mean_top.png",
            "fold_mean_feature_importance_by_fold": (
                f"{OUTPUT_PREFIX}_feature_importance_fold_mean_by_fold.csv"
            ),
            "fold_mean_feature_importance": f"{OUTPUT_PREFIX}_feature_importance_fold_mean.csv",
            "fold_mean_feature_importance_plot": (
                f"{OUTPUT_PREFIX}_feature_importance_fold_mean_top.png"
            ),
...
def run_saved_model_inference(
    *,
    ...
    summary = {
        ...
        "mode": "saved_lgb_booster_inference_with_raw_test_pfbeam_regeneration",
