...
# %% [markdown]
# # exp302 exp226 multiscale K-segment candidate audit inference
#
# exp302 is a train-side direct/candidate-novelty audit. This notebook is
...
def validate_disabled_inference(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not all(checks.values()):
        raise ValueError(f"exp302 disabled inference contract failed: {checks}")
    return checks
...
def stop_without_inference(config: Mapping[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp302 is a train-side multiscale candidate audit. Raw-test inference "
