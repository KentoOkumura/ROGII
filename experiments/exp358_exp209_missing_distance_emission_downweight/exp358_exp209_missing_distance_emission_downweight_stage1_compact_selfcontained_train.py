# %% [markdown]
# # exp358 exp209 missing-distance emission downweight Stage 1 train
#
# Train-side exact-HMM audit of one preregistered missing-distance weight.
# The exp209 absolute-TVT grid, rate states, transitions, priors, GR scale,
# Type Well interpolation, and posterior-mean output are unchanged. Only the
# capped Gaussian log emission on raw-missing rows is multiplied once by
# max(0.25, 2^(-nearest-finite-row-distance/8)). Saved exp209 HMM and exp072
# LikPF predictions are read-only controls; no control, PF, Beam, model,
# inference, or submission is regenerated.
...
# 4. Exp209 observation model and missing-distance emission preparation
...
VARIANT_ORDER = ("missing_distance_half_life_8_floor_0p25",)
...
def validate_scientific_contract(
    ...
    expected = {
        ...
        "implementation.scope": "stage_0_passed_and_stage_1_train_side_candidate_audit",
        ...
        "model.emission.control_family": "capped_gaussian",
        "model.emission.candidate_family": "missing_distance_weighted_capped_gaussian",
        "model.emission.emission_clip_z2": 600.0,
        "model.emission.application_count": 1,
        ...
        "execution_contract.reporting_folds": 5,
        ...
        "outcome.technical_gate_passed": True,
    }
    ...
    for key, value in fixed_hmm.items():
        if float(hmm.get(key, -1.0)) != value:
            raise ValueError(f"exp358 fixes model.fixed_hmm.{key}={value}")
    if (
        hmm.get("emission") != "missing_distance_weighted_capped_gaussian"
        or hmm.get("rate_center") != "zero"
    ):
        raise ValueError(
            "exp358 fixes missing-distance weighted capped Gaussian emission "
            "and a zero-centered rate grid"
        )
    fixed_policies = {
    ...
    if [float(value) for value in hmm.get("sigma_clip", [])] != [10.0, 60.0]:
        raise ValueError("exp358 fixes the exp209 GR sigma clip to [10, 60]")
    forbidden = set(str(value) for value in get_nested(config, "guards.forbidden") or [])
    required_forbidden = {
        "exp307_finite_mad_observation",
        "zero_weight_or_hard_missing_mask",
        "half_life_or_floor_grid",
        "sigma_rescaling",
        "transition_or_prior_change",
    ...
    if not required_forbidden.issubset(forbidden):
        raise ValueError("exp358 no-rescue and single-change exclusions are incomplete")
    if not bool(get_nested(config, "execution.implementation_approved")):
        raise ValueError("exp358 implementation approval must be recorded")
    if require_run_approval and not (
        bool(get_nested(config, "execution.stage_1_kaggle_push_approved"))
        and bool(get_nested(config, "execution.train_run_approved"))
        and bool(get_nested(config, "execution.run_stage_1"))
        and bool(get_nested(config, "execution.run_hmm"))
    ):
        raise RuntimeError("exp358 Stage 1 Kaggle package/push/run is not approved")
...
def build_scientific_contract(config: dict[str, Any]) -> dict[str, Any]:
    contract = {
        ...
        "forbidden": [
            "exp307_finite_mad_observation",
            "zero_weight_hard_mask_or_half_life_floor_grid",
            "sigma_temperature_clip_or_gr_preprocessing_change",
...
def preflight_controls_and_assignments(config: dict[str, Any]) -> dict[str, Any]:
    ...
    forbidden_decoder_columns = {str(value) for value in fold.get("forbidden_decoder_columns", [])}
    if set(safe_columns) != {"well_id", "row_idx", "suffix_offset", "fold"}:
        raise ValueError("exp358 reporting decoder allowlist must contain identity/fold only")
    if set(safe_columns) & forbidden_decoder_columns:
        raise ValueError("exp358 reporting decoder allowlist contains forbidden columns")
...
def compute_exp209_zero_fill_sigma_audit(
    ...
    *,
    ...
    if len(known_index) == 0 or not np.array_equal(known_index, np.arange(len(known_index))):
        raise ValueError("exp358 requires one contiguous known TVT_input prefix")
...
def build_weighted_gaussian_emission(
    ...
    if observed.ndim != 1 or states.ndim != 1 or weight.shape != observed.shape:
        raise ValueError("exp358 weighted emission shape mismatch")
    if not np.isfinite(observed).all() or not np.isfinite(states).all():
        raise ValueError("exp358 weighted emission requires finite GR arrays")
    if not math.isfinite(float(sigma_gr)) or float(sigma_gr) <= 0.0:
        raise ValueError("exp358 weighted emission requires a positive finite sigma")
    if not np.isfinite(weight).all() or bool(((weight < 0.25) | (weight > 1.0)).any()):
        raise ValueError("exp358 confidence weight must stay in [0.25, 1]")
    zscore = (observed[:, None] - states[None, :]) / float(sigma_gr)
    base = -0.5 * np.minimum(np.square(zscore), float(emission_clip_z2))
...
def prepare_hmm_inputs(
    ...
    if (
        ...
        raise ValueError("each exp358 well must contain one contiguous known prefix and suffix")
    ...
    raw_gr_observed = confidence["raw_gr_finite"][eval_index]
    observed_gr = parent_interpolated_gr(raw_gr, typewell_gr)[eval_index]
    ...
    return {
        ...
        "no_finite_gr_fallback": bool(confidence["no_finite_gr_fallback"]),
        "sigma_gr": sigma_gr,
        "dm": dm,
        "dz": dz,
        "rates": rates,
...
def run_exact_hmm_variant(
    ...
    if not math.isfinite(float(sigma_gr)) or not 10.0 <= float(sigma_gr) <= 60.0:
        raise ValueError("exp358 exp209 sigma_gr must satisfy the frozen [10, 60] contract")
    ...
    return {
        ...
        "emission_clip_z2": float(hmm["emission_clip_z2"]),
    ...
    schedule = pd.DataFrame(
        {
            ...
            "raw_gr_missing": prepared["raw_gr_missing"].astype(np.int8),
            ...
            "no_finite_gr_fallback": int(prepared["no_finite_gr_fallback"]),
        ...
        runtime_rows.append(
            {
                ...
                "rows": len(eval_index),
                "exp209_zero_fill_sigma": float(observation_audit["exp209_zero_fill_sigma"]),
                "emission_clip_z2": float(result["emission_clip_z2"]),
                "weight_application_count": int(result["weight_application_count"]),
                "minimum_confidence_weight": float(np.min(prepared["confidence_weight"])),
                "maximum_confidence_weight": float(np.max(prepared["confidence_weight"])),
...
def generate_and_freeze_predictions(
    ...
    if not NUMBA_AVAILABLE:
        raise RuntimeError("numba is required for the exp358 exact-HMM audit")
    ...
    def build_one(index: int, well: str):
        print(f"[{index}/{len(wells)}] exp358 well={well}", flush=True)
    ...
    observation_audit = observation_audit.sort_values("well_id", kind="mergesort").reset_index(
        drop=True
    )
    ...
    if (
        ...
        raise ValueError("exp358 prediction/observation/runtime coverage mismatch")
    ...
    frame = pd.DataFrame(
        {
            ...
            .to_numpy(np.int8)
            .astype(bool),
            "raw_gr_missing": pd.to_numeric(schedule["raw_gr_missing"], errors="raise")
            .to_numpy(np.int8)
            .astype(bool),
            "nearest_finite_row_distance": pd.to_numeric(
                schedule["nearest_finite_row_distance"], errors="raise"
            ).to_numpy(np.int64),
            "confidence_weight": pd.to_numeric(
                schedule["confidence_weight"], errors="raise"
            ).to_numpy(np.float64),
            "missing_run_length": pd.to_numeric(
                schedule["missing_run_length"], errors="raise"
            ).to_numpy(np.int64),
            "gap_bucket": schedule["gap_bucket"].astype(str).to_numpy(),
            "interpolated_gr": pd.to_numeric(schedule["interpolated_gr"], errors="raise").to_numpy(
                np.float64
            ),
...
def evaluate_promotion_gate(
    ...
    emission_clip_values = runtime["emission_clip_z2"].to_numpy(np.float64)
    application_counts = runtime["weight_application_count"].to_numpy(np.int64)
    observed_mask = frame["raw_gr_observed"].to_numpy(bool)
    missing_mask = frame["raw_gr_missing"].to_numpy(bool)
    observed_weight = frame.loc[observed_mask, "confidence_weight"].to_numpy(np.float64)
    missing_weight = frame.loc[missing_mask, "confidence_weight"].to_numpy(np.float64)
    missing_distance = frame.loc[missing_mask, "nearest_finite_row_distance"].to_numpy(np.int64)
    ...
    technical = {
        ...
        "weighted_gaussian_emission_finite_coverage": emission_finite_coverage,
        "emission_clip_contract_passed": bool(
            np.isfinite(emission_clip_values).all() and np.all(emission_clip_values == 600.0)
        ),
        "weight_applied_exactly_once": bool(np.all(application_counts == 1)),
        ...
        "raw_mask_partition_passed": bool(
            observed_rows > 0
            and missing_rows > 0
            and observed_rows + missing_rows == expected_rows
            and np.array_equal(observed_mask, ~missing_mask)
        ),
        ...
        "missing_weight_unique_count": int(np.unique(missing_weight).size),
        "missing_weight_has_above_floor_values": bool(
            len(missing_weight) > 0 and np.any(missing_weight > 0.25)
        ),
        "interpolated_gr_finite": bool(
            np.isfinite(frame["interpolated_gr"].to_numpy(np.float64)).all()
        ),
    ...
    technical["passed"] = bool(
        ...
        and technical["weighted_gaussian_emission_finite_coverage"] == 1.0
        and technical["emission_clip_contract_passed"]
        and technical["weight_applied_exactly_once"]
        ...
        and technical["missing_weight_min"] == 0.25
        and technical["missing_weight_max"] < 1.0
        and technical["missing_weight_unique_count"] >= 2
        and technical["missing_weight_has_above_floor_values"]
        and technical["interpolated_gr_finite"]
    ...
    def scope_record(scope: str) -> pd.Series:
        ...
        if len(selected) != 1:
            raise ValueError(f"exp358 expected exactly one direct metric row for scope={scope}")
    ...
    fold_readout = [
        {
            ...
            "improved": bool(row.delta_rmse_candidate_minus_control < -tolerance),
        }
    ...
    scope_readout = {
        ...
        for row in direct.loc[
            ~direct["scope"].isin(["overall"]) & ~direct["scope"].str.startswith("fold_")
    ...
    worst_well = primary_by_well.sort_values(
        ["delta_rmse_candidate_minus_control", "well_id"],
        ascending=[False, True],
        kind="mergesort",
    ).iloc[0]
    worst_delta = float(worst_well["delta_rmse_candidate_minus_control"])
    by_well_readout = {
        "wells": len(primary_by_well),
        "improved_wells": int((delta_values < -tolerance).sum()),
        "regressed_wells": int((delta_values > tolerance).sum()),
        "within_tolerance_wells": int((delta_values.abs() <= tolerance).sum()),
        "median_delta_rmse_candidate_minus_control": float(delta_values.median()),
        "p95_delta_rmse_candidate_minus_control": p95_delta,
        "worst_well_id": str(worst_well["well_id"]),
        "worst_candidate_rmse": float(worst_well["candidate_rmse"]),
        "worst_control_rmse": float(worst_well["control_rmse"]),
        "worst_delta_rmse_candidate_minus_control": worst_delta,
    }
    primary_direct = {
        ...
        "raw_observed_improvement_ft": float(scope_record("raw_gr_observed")["improvement_ft"]),
        "raw_missing_improvement_ft": float(scope_record("raw_gr_missing")["improvement_ft"]),
        "high_missing_fraction_improvement_ft": float(
            scope_record("missing_fraction_high")["improvement_ft"]
        ),
        "required_non_regression_scopes": non_regression_scopes,
        "by_well_readout": by_well_readout,
    ...
    return {
        ...
        "passed": passed,
        "decision": (
            "missing_distance_exp209_passed_train_side_only_no_automatic_downstream"
            if passed
            else "missing_distance_exp209_failed_close_without_rescue"
        ),
        "technical_gate": technical,
        "primary_direct_gate": primary_direct,
        "fixed_likpf_50_50_guard": blend_guard,
        "failure_action": (
            "close_without_half_life_floor_hard_mask_sigma_temperature_clip_"
            "transition_grid_prior_blend_rescue_inference_or_submission"
        ),
    }
...
def run_full_experiment(config: dict[str, Any]) -> dict[str, Any]:
    if not KAGGLE_WORKING_ROOT.exists() and os.environ.get("EXPERIMENT_ALLOW_LOCAL") != "1":
        raise RuntimeError(
            "Full exp358 Stage 1 must run first on Kaggle; "
    ...
    observation_audit = pd.read_csv(output_paths["observation_audit"], dtype={"well_id": str})
    ...
    status = (
        "train_side_missing_distance_exp209_gate_passed_no_automatic_downstream"
        if promotion_gate["passed"]
        else "train_side_missing_distance_exp209_gate_failed_closed"
    )
    summary = {
        "experiment": EXPERIMENT_NAME,
        "status": status,
    ...
    metrics = {
        ...
        "raw_gr_emission_contract_sha256": frozen["raw_gr_emission_contract"]["content_sha256"],
