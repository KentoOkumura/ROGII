...
EXP073_LOCAL_INFERENCE_OUTPUTS = (
    Path("/tmp")
    / "kaggle-output"
    / "exp073_gpu_reproducibility_guard_for_exp063_full_replay"
    / "inference_v2"
...
EXP075_LOCAL_INFERENCE_OUTPUTS = (
    Path("/tmp")
    / "kaggle-output"
    / "exp075_compact_tracker_pfbeam_feature_repro_guard"
    / "inference_v3_gpu"
...
POSTPROCESS_PREFIX = "exp078_compact_surface_longtail_gate"
COMPACT_OUTPUT_PREFIX = "compact_tracker_pfbeam_repro_guard"
...
def load_exp073_oof_predictions(
    ...
    *,
    ...
    for chunk in pd.read_csv(source, dtype={"id": str, "well": str}, chunksize=500_000):
        mask = chunk["mode"].astype(str).eq(mode_name) & chunk["model"].astype(str).eq(model_name)
        if mask.any():
...
def find_compact_train_artifact(
    filename: str,
    explicit_path: str | Path | None = None,
) -> Path:
    candidates: list[Path] = []
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    checked = "\n".join(str(path) for path in candidates[:80])
    raise FileNotFoundError(f"compact train artifact not found: {filename}. Checked:\n{checked}")


def load_exp075_oof_predictions(
    predictions_path: str | Path | None = None,
    *,
    mode_name: str = "gpu_repro_guard_dp_threads8",
    model_name: str = "lgb_mean",
) -> tuple[pd.DataFrame, dict[str, Any]]:
    source = find_compact_train_artifact(
        f"{COMPACT_OUTPUT_PREFIX}_predictions.csv.gz",
        predictions_path,
    )
    frames: list[pd.DataFrame] = []
    for chunk in pd.read_csv(source, dtype={"id": str, "well": str}, chunksize=500_000):
    ...
    if frame.empty:
        raise ValueError(
            f"No compact predictions for mode={mode_name} model={model_name} in {source}"
        )
    required = {
        "id",
        "well",
    ...
    missing = sorted(required - set(frame.columns))
    if missing:
        raise ValueError(f"{source} is missing compact prediction columns: {missing}")
    frame["id"] = frame["id"].astype(str)
    frame["well"] = frame["well"].astype(str)
    for col in ["target_tvt", "last_known_tvt", "target_delta", "pred_delta", "pred_tvt"]:
        frame[col] = pd.to_numeric(frame[col], errors="coerce").astype(np.float32)
    metadata = {
        "source": str(source),
        "source_sha256": sha256_file(source),
        "source_experiment": "exp075_compact_tracker_pfbeam_feature_repro_guard",
        "source_kind": "compact_surface_oof_predictions",
        "mode": mode_name,
        "model": model_name,
        "rows": int(len(frame)),
        "wells": int(frame["well"].nunique()),
    ...
    *,
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    checked = "\n".join(str(path) for path in candidates[:80])
    raise FileNotFoundError(f"{label} artifact not found: {filename}. Checked:\n{checked}")
...
def load_inference_predictions(
    *,
    ...
    source = find_prediction_artifact(
        filename,
        explicit_path,
        local_artifacts=local_artifacts,
        label=source_experiment,
    )
    frame = pd.read_csv(source, dtype={"id": str, "well": str})
    if "mode" in frame.columns:
        frame = frame[frame["mode"].astype(str) == mode_name].copy()
    if "model" in frame.columns:
        frame = frame[frame["model"].astype(str) == model_name].copy()
    if frame.empty:
        raise ValueError(
            f"No inference predictions for mode={mode_name} model={model_name} in {source}"
        )
    required = {"id", "well", "pred_tvt"}
    missing = sorted(required - set(frame.columns))
    if missing:
        raise ValueError(f"{source} is missing inference prediction columns: {missing}")
    frame["id"] = frame["id"].astype(str)
    frame["well"] = frame["well"].astype(str)
    for col in ["last_known_tvt", "pred_delta", "pred_delta_raw", "pred_tvt"]:
...
def _postprocess_candidates(
    ...
    if {"pf_ancc_std", "beam_std_d"}.issubset(frame.columns):
        ...
        beam_std = frame["beam_std_d"].fillna(0.0).to_numpy(np.float32)
        instability = np.maximum(
            _safe_divide(pf_std, np.nanpercentile(pf_std, 75), default=0.0),
            _safe_divide(np.abs(beam_std), np.nanpercentile(np.abs(beam_std), 75), default=0.0),
        )
        base_limit = float(np.quantile(np.abs(target_delta), 0.995))
        dynamic_limit = base_limit / np.clip(instability, 1.0, 3.0)
        candidates.append(
            (
                "pf_confidence_residual_clip_q995",
                np.clip(pred_delta, -dynamic_limit, dynamic_limit),
            )
        )
...
def _tail_rank_and_length(frame: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    work = frame[["id", "well"]].copy()
    work["_row_index"] = _row_index_from_id(work["id"])
    work = work.sort_values(["well", "_row_index", "id"]).reset_index()
    work["_tail_rank"] = work.groupby("well").cumcount().astype(np.float32)
    work["_tail_len"] = work.groupby("well")["id"].transform("size").astype(np.float32)
    restored = work.sort_values("index")
    return (
        restored["_tail_rank"].to_numpy(np.float32),
        restored["_tail_len"].to_numpy(np.float32),
        restored["_row_index"].to_numpy(np.float32),
    )


def _compact_gate_weights(
    frame: pd.DataFrame,
    *,
    policy: str,
    diff_p50: float | None = None,
    diff_p75: float | None = None,
) -> np.ndarray:
    tail_rank = frame["_tail_rank"].to_numpy(np.float32)
    tail_len = frame["_tail_len"].to_numpy(np.float32)
    diff = frame["_compact_abs_diff"].to_numpy(np.float32)
    masks: dict[str, np.ndarray] = {
        "tail_rank_ge1000": tail_rank >= 1000.0,
        "tail_or_len_long": (tail_rank >= 1000.0) | (tail_len >= 5000.0),
        "tail_rank_ge1000_diff_p50": (tail_rank >= 1000.0)
        & (diff >= float(diff_p50 if diff_p50 is not None else np.nanpercentile(diff, 50))),
        "tail_rank_ge1000_diff_p75": (tail_rank >= 1000.0)
        & (diff >= float(diff_p75 if diff_p75 is not None else np.nanpercentile(diff, 75))),
    }
    if "md_since" in frame.columns:
        md_since = frame["md_since"].fillna(0).to_numpy(np.float32)
        masks["md_since_ge1000"] = md_since >= 1000.0
        masks["md_since_ge1000_diff_p50"] = masks["md_since_ge1000"] & (
            diff >= float(diff_p50 if diff_p50 is not None else np.nanpercentile(diff, 50))
        )
    name_to_weight = {"w005": 0.05, "w010": 0.10, "w020": 0.20}
    for suffix, weight in name_to_weight.items():
        marker = f"_{suffix}"
        if policy.endswith(marker):
            gate_name = policy[: -len(marker)]
            if gate_name not in masks:
                raise ValueError(f"Unsupported compact gate policy: {policy}")
            return np.where(masks[gate_name], weight, 0.0).astype(np.float32)
    if policy in {"baseline_exp073", "none"}:
        return np.zeros(len(frame), dtype=np.float32)
    raise ValueError(f"Unsupported compact gate policy: {policy}")


def _compact_gate_candidate_policies(weights: list[float] | tuple[float, ...]) -> list[str]:
    policies = ["baseline_exp073"]
    weight_suffix = {0.05: "w005", 0.10: "w010", 0.20: "w020"}
    gates = [
        "tail_rank_ge1000",
        "tail_or_len_long",
        "tail_rank_ge1000_diff_p50",
        "tail_rank_ge1000_diff_p75",
    ]
    for weight in weights:
        suffix = weight_suffix.get(round(float(weight), 2))
        if suffix is None:
            suffix = f"w{int(round(float(weight) * 1000)):03d}"
        for gate in gates:
            policies.append(f"{gate}_{suffix}")
    return policies


def _compact_bucket_metrics(
    frame: pd.DataFrame,
    pred_tvt: np.ndarray,
    *,
    policy: str,
    ...
    work = frame[["id", "well", "target_tvt", "_tail_rank", "_tail_len"]].copy()
    work["error"] = pred_tvt.astype(np.float32) - work["target_tvt"].to_numpy(np.float32)
    work["base_error"] = base_error.astype(np.float32)
    distance_bins = [-np.inf, 50, 250, 1000, 2500, np.inf]
    distance_labels = ["0000_0050", "0050_0250", "0250_1000", "1000_2500", "2500_plus"]
    length_bins = [-np.inf, 1000, 2500, 5000, np.inf]
    length_labels = ["0000_1000", "1000_2500", "2500_5000", "5000_plus"]
    frames: list[pd.DataFrame] = []
    for segment_type, values, bins, labels in [
        ("tail_rank_bucket", work["_tail_rank"], distance_bins, distance_labels),
        ("tail_length_bucket", work["_tail_len"], length_bins, length_labels),
    ]:
        work["_segment"] = pd.cut(values, bins=bins, labels=labels)
        grouped = (
            work.groupby("_segment", observed=False)
            .agg(
                rows=("id", "size"),
                sse=("error", lambda value: float(np.sum(np.square(value)))),
                base_sse=("base_error", lambda value: float(np.sum(np.square(value)))),
                rmse_tvt=("error", lambda value: float(np.sqrt(np.mean(np.square(value))))),
                base_rmse_tvt=(
                    "base_error",
    ...
    result.insert(0, "policy", policy)
    result["delta_sse_vs_base"] = result["sse"] - result["base_sse"]
    result["delta_rmse_vs_base"] = result["rmse_tvt"] - result["base_rmse_tvt"]
    return result


def _compact_well_metrics(
    frame: pd.DataFrame,
    pred_tvt: np.ndarray,
    *,
    policy: str,
    ...
    work = frame[["id", "well", "target_tvt"]].copy()
    work["error"] = pred_tvt.astype(np.float32) - work["target_tvt"].to_numpy(np.float32)
    work["base_error"] = base_error.astype(np.float32)
    grouped = (
    ...
    grouped.insert(0, "policy", policy)
    grouped["delta_sse_vs_base"] = grouped["sse"] - grouped["base_sse"]
    grouped["delta_rmse_vs_base"] = grouped["rmse_tvt"] - grouped["base_rmse_tvt"]
    return grouped


def build_compact_gate_frame(
    *,
    base_predictions_path: str | Path | None = None,
    compact_predictions_path: str | Path | None = None,
    feature_cache_path: str | Path | None = None,
    mode_name: str = "gpu_repro_guard_dp_threads8",
    model_name: str = "lgb_mean",
) -> tuple[pd.DataFrame, dict[str, Any]]:
    base, base_meta = load_exp073_oof_predictions(
        base_predictions_path,
        mode_name=mode_name,
        model_name=model_name,
    )
    compact, compact_meta = load_exp075_oof_predictions(
    ...
    compact = compact.rename(
        columns={
            "pred_delta": "compact_pred_delta",
            "pred_tvt": "compact_pred_tvt",
        }
    )[
        [
            "id",
    ...
    base = base.rename(
        columns={
            "pred_delta": "base_pred_delta",
            "pred_tvt": "base_pred_tvt",
        }
    )[
        [
            "id",
    ...
    for col in ["target_tvt", "last_known_tvt", "target_delta"]:
        left = f"{col}_base"
        right = f"{col}_compact"
        if left in frame.columns and right in frame.columns:
            max_diff = float(
                np.nanmax(np.abs(frame[left].to_numpy(float) - frame[right].to_numpy(float)))
            )
            if max_diff > 1e-3:
                raise ValueError(f"Base/compact {col} mismatch: max diff {max_diff}")
            frame[col] = frame[left].astype(np.float32)
    ...
    try:
        ...
        keep = [
            col
            for col in ["id", "md_since", "eval_len", "z_span", "tw_range"]
            if col in features.columns
        ]
        if "id" in keep:
            frame = frame.merge(features[keep], on="id", how="left", validate="one_to_one")
            feature_missing = int(frame[keep[1]].isna().sum()) if len(keep) > 1 else 0
    except (FileNotFoundError, pd.errors.EmptyDataError, EOFError, OSError):
        feature_meta = None
    tail_rank, tail_len, row_index = _tail_rank_and_length(frame)
    frame["_tail_rank"] = tail_rank
    frame["_tail_len"] = tail_len
    frame["_row_index"] = row_index
    frame["_compact_abs_diff"] = np.abs(
        frame["compact_pred_tvt"].to_numpy(np.float32)
        - frame["base_pred_tvt"].to_numpy(np.float32)
    )
    metadata = {
        "base_prediction_source": base_meta,
        "compact_prediction_source": compact_meta,
        "feature_source": feature_meta,
...
def run_compact_surface_longtail_gate(
    *,
    output_dir: str | Path,
    base_predictions_path: str | Path | None = None,
    compact_predictions_path: str | Path | None = None,
    ...
    max_allowed_well_rmse_regression: float = 0.25,
) -> dict[str, Any]:
    t0 = time.time()
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    frame, source_meta = build_compact_gate_frame(
    ...
    target = frame["target_tvt"].to_numpy(np.float32)
    base_pred = frame["base_pred_tvt"].to_numpy(np.float32)
    compact_pred = frame["compact_pred_tvt"].to_numpy(np.float32)
    base_error = base_pred - target
    base_sse = float(np.sum(np.square(base_error)))
    base_rmse = rmse(target, base_pred)
    compact_diff = np.abs(compact_pred - base_pred)
    diff_p50 = float(np.nanpercentile(compact_diff, 50))
    diff_p75 = float(np.nanpercentile(compact_diff, 75))
    policies = _compact_gate_candidate_policies(weights or [0.05, 0.10, 0.20])
    metric_rows: list[dict[str, Any]] = []
    ...
    for policy in policies:
        gate_weight = _compact_gate_weights(
            frame,
            policy=policy,
            diff_p50=diff_p50,
            diff_p75=diff_p75,
        )
        pred = base_pred + gate_weight * (compact_pred - base_pred)
        error = pred - target
        sse = float(np.sum(np.square(error)))
        bucket_metrics = _compact_bucket_metrics(
            frame,
            pred,
            policy=policy,
            base_error=base_error,
        )
        well_metrics = _compact_well_metrics(
            frame,
            pred,
            policy=policy,
            base_error=base_error,
        )
        long_tail_bucket = bucket_metrics[
            (bucket_metrics["segment_type"] == "tail_rank_bucket")
            & (bucket_metrics["segment"] == "1000_2500")
        ]
        far_tail_bucket = bucket_metrics[
            (bucket_metrics["segment_type"] == "tail_rank_bucket")
            & (bucket_metrics["segment"] == "2500_plus")
        ]
        long_tail_delta_sse = float(
            pd.concat([long_tail_bucket, far_tail_bucket], ignore_index=True)[
                "delta_sse_vs_base"
            ].sum()
        )
        max_well_regression = float(well_metrics["delta_rmse_vs_base"].max())
        worsened_wells = int((well_metrics["delta_rmse_vs_base"] > 0).sum())
        improved_wells = int((well_metrics["delta_rmse_vs_base"] < 0).sum())
        passes_guard = bool(
            sse <= base_sse
            and long_tail_delta_sse <= 0.0
            and max_well_regression <= float(max_allowed_well_rmse_regression)
        )
        metric_rows.append(
            {
                "policy": policy,
                "rows": int(len(frame)),
                "wells": int(frame["well"].nunique()),
                "adjusted_rows": int((gate_weight > 0).sum()),
                "mean_weight": float(gate_weight.mean()),
                "max_weight": float(gate_weight.max()) if len(gate_weight) else 0.0,
                "rmse_tvt": rmse(target, pred),
                "base_rmse_tvt": base_rmse,
                "delta_rmse_vs_base": rmse(target, pred) - base_rmse,
                "sse": sse,
                "base_sse": base_sse,
                "delta_sse_vs_base": sse - base_sse,
                "mae_tvt": float(np.mean(np.abs(error))),
                "error_mean": float(np.mean(error)),
                "long_tail_delta_sse_vs_base": long_tail_delta_sse,
                "improved_wells": improved_wells,
                "worsened_wells": worsened_wells,
                "max_well_rmse_regression": max_well_regression,
                "passes_discussion_metric_guard": passes_guard,
                "prediction_sha256": prediction_sha256(
                    frame["id"],
                    pred,
                    label=f"exp078/{policy}",
                ),
            }
        )
        bucket_frames.append(bucket_metrics)
        well_frames.append(well_metrics)
        previous_best_rmse = min(
            (row["rmse_tvt"] for row in metric_rows[:-1]),
            default=float("inf"),
        )
        if best_pred is None or metric_rows[-1]["rmse_tvt"] < previous_best_rmse:
            best_pred = pred.astype(np.float32)
            best_weights = gate_weight.astype(np.float32)
            best_policy = policy

    metrics = pd.DataFrame(metric_rows).sort_values(
        ["passes_discussion_metric_guard", "rmse_tvt"],
        ascending=[False, True],
    )
    buckets = pd.concat(bucket_frames, ignore_index=True)
    wells = pd.concat(well_frames, ignore_index=True)
    if best_pred is None or best_weights is None:
        raise RuntimeError("No compact gate candidates were evaluated")
    best_policy = str(metrics.iloc[0]["policy"])
    best_weights = _compact_gate_weights(
        frame,
        policy=best_policy,
        diff_p50=diff_p50,
        diff_p75=diff_p75,
    )
    best_pred = (base_pred + best_weights * (compact_pred - base_pred)).astype(np.float32)
    best_predictions = pd.DataFrame(
        {
            ...
            "policy": best_policy,
            "target_tvt": target,
            "base_pred_tvt": base_pred,
            "compact_pred_tvt": compact_pred,
            "gate_weight": best_weights,
            "pred_tvt": best_pred,
        }
    )
    metrics_path = output_dir / f"{POSTPROCESS_PREFIX}_metrics.csv"
    bucket_path = output_dir / f"{POSTPROCESS_PREFIX}_bucket_metrics.csv"
    well_path = output_dir / f"{POSTPROCESS_PREFIX}_well_metrics.csv"
    predictions_path = output_dir / f"{POSTPROCESS_PREFIX}_best_predictions.csv.gz"
    metrics.to_csv(metrics_path, index=False)
    buckets.to_csv(bucket_path, index=False)
    wells.to_csv(well_path, index=False)
    best_predictions.to_csv(predictions_path, index=False, compression="gzip")
    summary = {
        "experiment": "exp078_compact_surface_longtail_gate",
        "status": "compact_surface_gate_audit_completed",
        "parent": "exp073_gpu_reproducibility_guard_for_exp063_full_replay",
        "compact_parent": "exp075_compact_tracker_pfbeam_feature_repro_guard",
        "mode": "compact_surface_longtail_gate_oof_audit",
        "metric_context": {
            "rmse_vs_ssr_discussion": (
                "docs/discussions/"
                "rogii-wellbore-geology-prediction-698860-rmse-vs-ssr-error.md"
            ),
            "worst_well_discussion": (
                "docs/discussions/"
        ...
        "source": source_meta,
        "diff_thresholds": {
            "compact_abs_diff_p50": diff_p50,
            "compact_abs_diff_p75": diff_p75,
        },
        "best_policy": metrics.iloc[0].to_dict(),
        "artifacts": {
            "metrics": metrics_path.name,
            "bucket_metrics": bucket_path.name,
            "well_metrics": well_path.name,
            "best_predictions": predictions_path.name,
...
def run_compact_surface_longtail_inference(
    *,
    output_dir: str | Path,
    ...
    base_predictions_path: str | Path | None = None,
    compact_predictions_path: str | Path | None = None,
    policy: str = "tail_rank_ge1000_diff_p50_w005",
    mode_name: str = "gpu_repro_guard_dp_threads8",
    model_name: str = "lgb_mean",
    ...
    base, base_meta = load_inference_predictions(
        filename=f"{OUTPUT_PREFIX}_inference_test_predictions.csv.gz",
        explicit_path=base_predictions_path,
        local_artifacts=EXP073_LOCAL_INFERENCE_OUTPUTS,
        source_experiment="exp073_gpu_reproducibility_guard_for_exp063_full_replay",
        source_kind="base_inference_predictions",
        mode_name=mode_name,
        model_name=model_name,
    )
    compact, compact_meta = load_inference_predictions(
        filename=f"{COMPACT_OUTPUT_PREFIX}_inference_test_predictions.csv.gz",
        explicit_path=compact_predictions_path,
        local_artifacts=EXP075_LOCAL_INFERENCE_OUTPUTS,
        source_experiment="exp075_compact_tracker_pfbeam_feature_repro_guard",
        source_kind="compact_surface_inference_predictions",
        mode_name=mode_name,
        model_name=model_name,
    )
    frame = base[["id", "well", "pred_tvt"]].rename(columns={"pred_tvt": "base_pred_tvt"}).merge(
        compact[["id", "well", "pred_tvt"]].rename(columns={"pred_tvt": "compact_pred_tvt"}),
        on=["id", "well"],
        validate="one_to_one",
    )
    tail_rank, tail_len, row_index = _tail_rank_and_length(frame)
    ...
    weights = _compact_gate_weights(
        frame,
        policy=policy,
        diff_p50=float(np.nanpercentile(frame["_compact_abs_diff"], 50)),
        diff_p75=float(np.nanpercentile(frame["_compact_abs_diff"], 75)),
    )
    pred = (
        frame["base_pred_tvt"].to_numpy(np.float32)
        + weights
    ...
    predictions = pd.DataFrame(
        {
            ...
            "well": frame["well"].to_numpy(),
            "policy": policy,
            "base_pred_tvt": frame["base_pred_tvt"].to_numpy(np.float32),
            "compact_pred_tvt": frame["compact_pred_tvt"].to_numpy(np.float32),
            "gate_weight": weights,
            "pred_tvt": pred,
        }
    )
    predictions_path = output_dir / f"{POSTPROCESS_PREFIX}_inference_test_predictions.csv.gz"
    ...
    metrics = {
        ...
        "adjusted_rows": int((weights > 0).sum()),
        ...
        "prediction_sha256": prediction_sha256(
            predictions["id"],
            pred,
            label=f"exp078/{policy}/test",
        ),
        "submission_sha256": sha256_file(submission_path),
    }
    pd.DataFrame([metrics]).to_csv(
        output_dir / f"{POSTPROCESS_PREFIX}_inference_metrics.csv",
        index=False,
    )
    summary = {
        "experiment": "exp078_compact_surface_longtail_gate",
        "status": "inference_completed",
        "mode": "saved_prediction_compact_surface_longtail_gate",
        "base_prediction_source": base_meta,
        "compact_prediction_source": compact_meta,
        "metrics": metrics,
        "artifacts": {
            "predictions": predictions_path.name,
            "metrics": f"{POSTPROCESS_PREFIX}_inference_metrics.csv",
            "summary": f"{POSTPROCESS_PREFIX}_inference_summary.json",
            "submission": str(submission_path),
        },
        "elapsed_seconds": round(time.time() - t0, 3),
    }
    (output_dir / f"{POSTPROCESS_PREFIX}_inference_summary.json").write_text(
        json.dumps(summary, indent=2)
    )
    print(json.dumps(summary, indent=2), flush=True)
    return summary
