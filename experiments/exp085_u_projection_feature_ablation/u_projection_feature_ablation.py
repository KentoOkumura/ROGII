...
OUTPUT_PREFIX = "exp085_u_projection_feature_ablation"
...
def _source_tvt(frame: pd.DataFrame, name: str, spec: dict[str, Any]) -> np.ndarray:
    ...
    if delta_column:
        ...
        return (
            ...
            + frame[delta_column].to_numpy(np.float32)
        )
...
def build_u_projection_features(
    ...
    *,
    ...
    u0 = (
        frame["anchor_t0"].to_numpy(np.float32)
        + frame["anchor_z0"].to_numpy(np.float32)
    )
    ...
    for left_i, left in enumerate(source_names):
        for right in source_names[left_i + 1 :]:
            ...
            result[diff_col] = (
                result[left_col].to_numpy(np.float32)
                - result[right_col].to_numpy(np.float32)
            )
...
def run_u_projection_feature_ablation(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
