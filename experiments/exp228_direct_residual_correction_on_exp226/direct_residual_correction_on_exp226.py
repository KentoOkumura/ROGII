...
EXP226_INFERENCE_SUMMARY = (
    "exp226_connortynan_k16_spline_kernel_knn_adaptive_kappa_reproduction_inference_summary.json"
)
OUTPUT_PREFIX = "exp228_direct_residual_correction_on_exp226"
...
def find_model_manifests(explicit_path: str | Path | None = None) -> list[Path]:
    candidates: list[Path] = []
    if explicit_path is not None:
...
def find_model_manifest(explicit_path: str | Path | None = None) -> Path:
    return find_model_manifests(explicit_path)[0]


def load_exp226_oof_predictions(explicit_path: str | Path | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    source = find_artifact(EXP226_TRAIN_OOF_PREDICTIONS, explicit_path)
    usecols = [
        "well_id",
        "row_idx",
        "suffix_offset",
    ...
    frame = pd.read_csv(source, usecols=usecols, dtype={"well_id": str})
    frame = frame.rename(
        columns={
            "well_id": "well",
            "tvt_true": "exp226_tvt_true",
            "tvt_pred": "exp226_oof_pred",
            "tvt_geop": "exp226_geop",
            "gr_delta": "exp226_gr_delta",
            "error": "exp226_error_pred_minus_true",
            "fold": "exp226_fold",
        }
    )
    frame["well"] = frame["well"].astype(str)
    ...
    numeric_cols = [
        ...
        "exp226_geop",
        "exp226_gr_delta",
        "exp226_error_pred_minus_true",
    ]
    for col in numeric_cols:
        frame[col] = pd.to_numeric(frame[col], errors="raise").astype(np.float32)
    if frame.duplicated(["well", "row_idx"]).any():
        raise ValueError("exp226 OOF has duplicate (well, row_idx) keys")
    meta = {
        "source": str(source),
        "source_sha256_decompressed": sha256_gzip_decompressed(source),
        "rows": int(len(frame)),
        "wells": int(frame["well"].nunique()),
    }
    return frame, meta
...
def add_exp226_residual_target(
    full_frame: pd.DataFrame,
    exp226_oof_path: str | Path | None = None,
) -> tuple[pd.DataFrame, dict[str, Any]]:
    exp226_oof, meta = load_exp226_oof_predictions(exp226_oof_path)
    frame = full_frame.copy()
    frame["row_idx"] = _row_indices_from_ids(frame["id"]).astype(np.int32)
    before_rows = len(frame)
    before_wells = int(frame["well"].nunique())
    frame = frame.merge(
        exp226_oof,
        on=["well", "row_idx"],
        how="left",
    ...
    missing = int(frame["exp226_oof_pred"].isna().sum())
    if missing:
        raise ValueError(f"exp226 OOF did not cover {missing} train rows")
    frame["target_tvt"] = (
        frame["last_known_tvt"].to_numpy(np.float32)
        + frame["target"].to_numpy(np.float32)
    ).astype(np.float32)
    target_delta = np.abs(
        frame["target_tvt"].to_numpy(np.float32)
        - frame["exp226_tvt_true"].to_numpy(np.float32)
    )
    max_target_abs_diff = float(target_delta.max()) if len(target_delta) else 0.0
    if max_target_abs_diff > 0.05:
        raise ValueError(
            "exp226 OOF true TVT does not align with exp218 feature-surface target: "
            f"max abs diff={max_target_abs_diff}"
        )
    frame["exp226_residual_target"] = (
        frame["target_tvt"].to_numpy(np.float32)
        - frame["exp226_oof_pred"].to_numpy(np.float32)
    ).astype(np.float32)
    residual_meta = {
        **meta,
        "joined_rows": int(len(frame)),
        "joined_wells": int(frame["well"].nunique()),
        "dropped_rows": int(before_rows - len(frame)),
        "dropped_wells": int(before_wells - frame["well"].nunique()),
        "missing_rows": missing,
        "target_tvt_vs_exp226_true_max_abs_diff": max_target_abs_diff,
        "target": "target_tvt - exp226_oof_pred",
        "leakage_guard": "exp226 base prediction is group-safe OOF, not full-train prediction",
    }
    return frame, residual_meta


def load_exp226_inference_submission(
    explicit_path: str | Path | None = None,
) -> tuple[pd.DataFrame, dict[str, Any]]:
    candidates: list[Path] = []
    if explicit_path is not None:
    ...
    summary_path = find_optional_artifact(EXP226_INFERENCE_SUMMARY)
    if summary_path is not None:
        candidates.append(summary_path.parent.parent / "submission.csv")
        candidates.append(summary_path.parent / "submission.csv")
    kaggle_input = Path("/kaggle/input")
    if kaggle_input.exists():
        candidates.extend(
            path
            for path in kaggle_input.glob("**/submission.csv")
            if "exp226" in str(path).lower()
            or "k16" in str(path).lower()
            or "kappa" in str(path).lower()
        )
    for candidate in candidates:
        if candidate.exists() and candidate.stat().st_size > 0:
            frame = pd.read_csv(candidate, dtype={"id": str})
            if "id" not in frame.columns:
                continue
            target_col = "tvt" if "tvt" in frame.columns else str(frame.columns[-1])
            frame = frame[["id", target_col]].rename(columns={target_col: "exp226_pred"})
            frame["id"] = frame["id"].astype(str)
            frame["exp226_pred"] = pd.to_numeric(
                frame["exp226_pred"], errors="raise"
            ).astype(np.float32)
            if frame["id"].duplicated().any():
                raise ValueError(f"exp226 inference submission has duplicate ids: {candidate}")
            return frame, {
    ...
    checked = "\n".join(str(path) for path in candidates[:120])
    raise FileNotFoundError(f"exp226 inference submission not found. Checked:\n{checked}")
...
def build_sp45_bimodal_selector_features(
    ...
    raise NotImplementedError(
            "exp228 does not implement the legacy sp45 inference feature wrapper. "
...
def _fit_one_variant_mode(
    *,
    ...
    save_models: bool,
    selected_lgb_indices: list[int] | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, list[dict[str, Any]], dict[str, Any]]:
    from lightgbm import LGBMRegressor, early_stopping, log_evaluation

    variant_name = str(variant["name"])
    if "exp226_residual_target" not in frame.columns or "exp226_oof_pred" not in frame.columns:
        raise ValueError("exp226 residual columns are required for exp228 training")
    y = frame["exp226_residual_target"].to_numpy(np.float32)
    base = frame["exp226_oof_pred"].to_numpy(np.float32)
    ...
    if selected_lgb_indices is not None:
        wanted = {int(index) for index in selected_lgb_indices}
        indexed_configs = [(index, params) for index, params in indexed_configs if index in wanted]
        if not indexed_configs:
            raise ValueError(f"No LightGBM configs selected by indices: {sorted(wanted)}")
        ...
        json.dumps(
            {
                ...
                "selected_lgb_indices": [int(index) for index, _ in indexed_configs],
                "use_gpu": bool(mode_config.get("use_gpu", False)),
            },
            sort_keys=True,
        ),
        ...
        pd.DataFrame(
            {
                ...
                "mode": mode_name,
                "model": "lgb_mean",
                "exp226_oof_pred": base,
                "target": y,
                "target_name": "tvt_minus_exp226_oof_pred",
...
def _selected_lgb_indices_from_names(values: list[str] | tuple[str, ...] | None) -> list[int] | None:
    if not values:
        return None
    indices: list[int] = []
    for value in values:
...
def run_direct_residual_correction_on_exp226(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    full_frame, exp226_oof_meta = add_exp226_residual_target(frame, exp226_oof_path)
    del frame
    gc.collect()
    print(
        json.dumps(
            {
                "stage": "joined_exp226_group_safe_oof_residual_target",
                "rows": int(len(full_frame)),
                "exp226_oof_rows": int(exp226_oof_meta["rows"]),
                "target": exp226_oof_meta["target"],
            },
            sort_keys=True,
        ),
        flush=True,
    ...
    selected_lgb_indices = _selected_lgb_indices_from_names(selected_lgb_models)
    ...
    for variant in selected_variants:
        ...
        for mode_name in selected_modes:
            ...
            metrics, predictions, importance, models, mode_summary = _fit_one_variant_mode(
                ...
                selected_lgb_indices=selected_lgb_indices,
            )
    ...
    manifest = {
        "experiment": "exp228_direct_residual_correction_on_exp226",
        "parent": "exp226_connortynan_k16_spline_kernel_knn_adaptive_kappa_reproduction",
        "ml_feature_surface_parent": "exp218_gr_wavelet_rotation_confidence_features_on_exp148",
        ...
        "mode": "exp218_surface_direct_residual_correction_on_exp226",
        "feature_source": feature_meta,
        "exp226_oof_source": exp226_oof_meta,
        "target": "target_tvt - exp226_oof_pred",
        "base_prediction": "exp226_oof_pred",
        ...
        "selected_lgb_models": list(selected_lgb_models or []),
        "selected_lgb_indices": selected_lgb_indices,
...
def run_saved_model_inference(
    *,
    ...
    exp226_submission_path: str | Path | None = None,
    learned_feature_path: str | Path | None = None,
    learned_schema_path: str | Path | None = None,
    learned_summary_path: str | Path | None = None,
    projection_config: dict[str, Any] | None = None,
    learned_feature_config: dict[str, Any] | None = None,
    grwr_feature_config: dict[str, Any] | None = None,
    variant_name: str = "exp218_surface_direct_residual",
    mode_name: str = "cpu_threads8",
    ...
    test_dir = Path(test_dir)
    manifest_paths = find_model_manifests(model_manifest_path)
    ...
    print(
        ...
        + ", ".join(str(path.parent) for path in manifest_paths),
        flush=True,
    )
    configure_public_runtime(
        data_dir=data_dir,
    ...
    for current_manifest_path in manifest_paths:
        ...
        for item in current_manifest.get("models", []):
            if (
                ...
                row = dict(item)
                row["_model_root"] = str(current_manifest_path.parent)
                row["_manifest"] = str(current_manifest_path)
    ...
    for item in model_rows:
        model_file = Path(str(item["_model_root"])) / str(item["file"])
        ...
        loaded_rows.append(
            {
                ...
                "manifest": str(item.get("_manifest")),
                "sha256": item.get("sha256"),
                "rows": int(len(pred)),
            }
        )

    exp226_submission, exp226_submission_meta = load_exp226_inference_submission(
        exp226_submission_path
    )
    test_frame = test_frame.merge(
        exp226_submission,
        on="id",
    ...
    missing_exp226 = int(test_frame["exp226_pred"].isna().sum())
    if missing_exp226:
        raise ValueError(f"exp226 inference submission missing {missing_exp226} test rows")
    base = test_frame["exp226_pred"].to_numpy(np.float32)
    ...
    summary = {
        ...
        "mode": "saved_split_lgb_booster_inference_with_exp226_base_and_raw_test_feature_replay",
        "train_manifest": str(manifest_path),
        "train_manifests": [str(path) for path in manifest_paths],
        "exp226_submission_source": exp226_submission_meta,
        ...
        "known_followup_risk": "Direct residual correction can improve CV while damaging LB; submit only after split OOF aggregate and stress readouts are checked.",
        "elapsed_seconds": round(time.time() - t0, 3),
    }
