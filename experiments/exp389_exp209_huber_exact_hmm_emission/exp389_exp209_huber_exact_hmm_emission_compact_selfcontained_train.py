...
#
# Train-side exact-HMM audit of one preregistered fixed-delta Huber emission.
...
# 4. Exp209 observation model and Huber emission preparation
...
EXPERIMENT_NAME = "exp389_exp209_huber_exact_hmm_emission"
OUTPUT_PREFIX = EXPERIMENT_NAME
VARIANT_ORDER = ("huber_delta1p345_on_exp209_absolute_tvt",)
...
EXECUTE_NOTEBOOK = os.environ.get("EXP389_IMPORT_ONLY", "0") != "1" and in_notebook_runtime()
...
def load_experiment_config() -> dict[str, Any]:
    ...
    for path in candidates:
        ...
        if get_nested(config, "experiment.name") == EXPERIMENT_NAME:
            return config
    raise FileNotFoundError(f"exp389 config not found in {[str(path) for path in candidates]}")
...
def validate_scientific_contract(
    ...
    expected = {
        ...
        "model.emission.candidate_family": "huber",
        "model.emission.delta": 1.345,
    ...
    for key, value in expected.items():
        if get_nested(config, key) != value:
            raise ValueError(f"exp389 fixed contract mismatch: {key} must be {value!r}")
    ...
    for key, value in fixed_hmm.items():
        if float(hmm.get(key, -1.0)) != value:
            raise ValueError(f"exp389 fixes model.fixed_hmm.{key}={value}")
    if (
        hmm.get("emission") != "huber_quadratic_center_linear_tail"
        or hmm.get("rate_center") != "zero"
    ):
        raise ValueError("exp389 fixes delta=1.345 Huber emission and a zero-centered rate grid")
    fixed_policies = {
    ...
    if not required_forbidden.issubset(forbidden):
        raise ValueError("exp389 no-rescue and single-change exclusions are incomplete")
    if not bool(get_nested(config, "execution.implementation_approved")):
        raise ValueError("exp389 implementation approval must be recorded")
    if require_run_approval and not (
        ...
        raise RuntimeError("exp389 Kaggle package/push/run is not approved")
...
def build_scientific_contract(config: dict[str, Any]) -> dict[str, Any]:
    contract = {
        ...
        "forbidden": [
            "exp357_shift_rank_or_exp281_residual_offset_coordinate",
            "delta_scale_temperature_clip_mixture_or_student_t_search",
...
def preflight_controls_and_assignments(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(safe_columns) != {"well_id", "row_idx", "suffix_offset", "fold"}:
        raise ValueError("exp389 reporting decoder allowlist must contain identity/fold only")
    if set(safe_columns) & forbidden_decoder_columns:
        raise ValueError("exp389 reporting decoder allowlist contains forbidden columns")
...
def compute_exp209_zero_fill_sigma_audit(
    ...
    *,
    ...
    if len(known_index) == 0 or not np.array_equal(known_index, np.arange(len(known_index))):
        raise ValueError("exp389 requires one contiguous known TVT_input prefix")
...
def huber_log_likelihood(
    ...
    absolute = np.abs(np.asarray(zscore, dtype=np.float64))
    return np.where(
        absolute <= delta,
...
def prepare_hmm_inputs(
    ...
    if (
        ...
        raise ValueError("each exp389 well must contain one contiguous known prefix and suffix")
...
def run_exact_hmm_variant(
    ...
    if not math.isfinite(float(sigma_gr)) or not 10.0 <= float(sigma_gr) <= 60.0:
        raise ValueError("exp389 exp209 sigma_gr must satisfy the frozen [10, 60] contract")
    delta = float(hmm["delta"])
    zscore = (
        prepared["observed_gr"][:, None] - prepared["state_gr"][None, :]
    ) / float(sigma_gr)
    emission = huber_log_likelihood(zscore, delta).astype(np.float32)
    ...
    schedule = pd.DataFrame(
        {
            ...
            "huber_delta": float(
                get_nested(config, "model.emission.delta")
            ),
        ...
        runtime_rows.append(
            {
                ...
                "rows": len(eval_index),
                "exp209_zero_fill_sigma": float(observation_audit["exp209_zero_fill_sigma"]),
                "huber_delta": float(result["delta"]),
...
def generate_and_freeze_predictions(
    ...
    if not NUMBA_AVAILABLE:
        raise RuntimeError("numba is required for the exp389 exact-HMM audit")
    ...
    def build_one(index: int, well: str):
        print(f"[{index}/{len(wells)}] exp389 well={well}", flush=True)
    ...
    if (
        ...
        raise ValueError("exp389 prediction/observation/runtime coverage mismatch")
...
def evaluate_promotion_gate(
    ...
    delta_values = runtime["huber_delta"].to_numpy(np.float64)
    technical = {
        ...
        "huber_emission_finite_coverage": emission_finite_coverage,
        "huber_delta_contract_passed": bool(
            np.isfinite(delta_values).all() and np.all(delta_values == 1.345)
        ),
    ...
    technical["passed"] = bool(
        ...
        and technical["huber_emission_finite_coverage"] == 1.0
        and technical["huber_delta_contract_passed"]
    ...
    def scope_record(scope: str) -> pd.Series:
        ...
        if len(selected) != 1:
            raise ValueError(f"exp389 expected exactly one direct metric row for scope={scope}")
    ...
    return {
        ...
        "passed": passed,
        "decision": (
            "huber_exp209_passed_train_side_only_no_automatic_downstream"
            if passed
            else "huber_exp209_failed_close_without_rescue"
        ),
        "technical_gate": technical,
        "primary_direct_gate": primary_direct,
        "fixed_likpf_50_50_guard": blend_guard,
        "failure_action": (
            "close_without_delta_scale_temperature_clip_mixture_student_t_sigma_missing_weight_"
...
def run_full_experiment(config: dict[str, Any]) -> dict[str, Any]:
    if not KAGGLE_WORKING_ROOT.exists() and os.environ.get("EXPERIMENT_ALLOW_LOCAL") != "1":
        raise RuntimeError(
            "Full exp389 must run first on Kaggle; local execution requires explicit smoke approval"
        )
    ...
    status = (
        "train_side_huber_exp209_gate_passed_no_automatic_downstream"
        if promotion_gate["passed"]
        else "train_side_huber_exp209_gate_failed_closed"
    )
    summary = {
        "experiment": EXPERIMENT_NAME,
        "status": status,
