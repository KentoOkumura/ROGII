# %% [markdown]
# # exp389 exp209 Huber exact-HMM emission inference
#
# exp389 is a train-side fixed-delta Huber emission audit. Raw-test prediction,
# inference, promotion, and submission creation remain fail-closed until a
# successful scientific gate and separate approval.
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if contract["experiment"] != EXPERIMENT_NAME or contract["route"] != "pf_beam":
        raise ValueError("unexpected exp389 inference config")
    if contract["mode"] != "disabled_until_train_side_promotion_and_separate_approval":
        raise ValueError("exp389 inference mode contract changed")
    if any(
        contract[key]
        for key in (
            "inference_enabled",
        ...
        raise ValueError("exp389 inference and submission must remain disabled")
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp389 is a train-side Huber exact-HMM emission audit; "
