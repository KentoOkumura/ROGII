...
class CFG:
    ...
    SHOW_FIGS = False
    SOFT_PRIOR_NAME = "pct50_strong2_pct70_weak0p5"
    SOFT_PRIOR_WEAK_PCT = 0.70
    SOFT_PRIOR_STRONG_PCT = 0.50
    SOFT_PRIOR_WEAK_PENALTY = 0.5
    SOFT_PRIOR_STRONG_PENALTY = 2.0
    SOFT_PRIOR_KNOWN_LAST_PCT_THRESHOLD = 0.75
    SOFT_PRIOR_KNOWN_LAST_MULTIPLIER = 1.0
...
PUBLIC_SOURCE_PROVENANCE = {
    ...
    "note": "The base feature builder is the public Ravaghi-style builder copied inside the Pixiux notebook; this experiment rebuilds it from raw files and adds a typewell late-range soft prior inside PF/Beam/likelihood-PF generation.",
}
...
def build_replay_train_frames(max_wells: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    print(
        f"late-soft public replay train wells: {len(train_wids)} | n_jobs={CFG.n_jobs} "
        f"| PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES} | soft_prior={CFG.SOFT_PRIOR_NAME}",
        flush=True,
    )
...
def build_replay_inference_frames(max_wells: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    print(
        f"late-soft public replay inference wells: train={len(train_wids)} test={len(test_wids)} "
        f"| n_jobs={CFG.n_jobs} | PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES} "
        f"| soft_prior={CFG.SOFT_PRIOR_NAME}",
        flush=True,
    )
    init_imputers(train_wids)
...
def build_replay_test_frame() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    test_wids = list_test_wells()
    print(
        f"late-soft public replay test wells: train_imputer_wells={len(train_wids)} test={len(test_wids)} "
        f"| n_jobs={CFG.n_jobs} | PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES} "
        f"| soft_prior={CFG.SOFT_PRIOR_NAME}",
        flush=True,
    )
...
def feature_columns_for_variant(df: pd.DataFrame, variant: str) -> list[str]:
    ...
    if variant in {"pixiux_likpf_public_replay", "pixiux_likpf_late_soft_prior_public_replay"}:
