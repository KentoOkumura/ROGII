...
OUTPUT_PREFIX = "exp185_last50_first_prefix_feature_rebuild_on_exp148"
PREFIX_CROP_TRAIN_FEATURES = (
    "exp185_last50_first_prefix_feature_rebuild_on_exp148_prefix_crop_train_features.csv.gz"
)
PREFIX_CROP_FEATURE_SCHEMA = (
    "exp185_last50_first_prefix_feature_rebuild_on_exp148_prefix_crop_feature_schema.csv"
)
PREFIX_CROP_FEATURE_SUMMARY = (
    "exp185_last50_first_prefix_feature_rebuild_on_exp148_prefix_crop_feature_summary.csv"
)
PREFIX_CROP_FEATURE_MANIFEST = (
    "exp185_last50_first_prefix_feature_rebuild_on_exp148_prefix_crop_feature_manifest.json"
)
...
def _finite_stat(values: np.ndarray, reducer: str, default: float = 0.0) -> float:
...
def _last_delta(values: np.ndarray) -> float:
    values = np.asarray(values, dtype=np.float32)
    if values.size < 2:
        return 0.0
...
def build_prefix_crop_window_features(
    ...
    *,
    ...
    windows = dict(
        config.get("windows")
        or {
            "last50": {"kind": "last_n", "rows": 50, "min_rows": 8},
        }
    )
    candidate_specs = [
        dict(item)
    ...
    for well_i, (well, index) in enumerate(well_groups, start=1):
        ...
        prefix_md = (
            pd.to_numeric(prefix_frame["MD"], errors="coerce")
            .interpolate(limit_direction="both")
            .ffill()
            .bfill()
            .to_numpy(np.float32)
        ...
        prefix_x = (
            pd.to_numeric(prefix_frame["X"], errors="coerce")
            .interpolate(limit_direction="both")
            .ffill()
            .bfill()
            .to_numpy(np.float32)
        ...
        prefix_y = (
            pd.to_numeric(prefix_frame["Y"], errors="coerce")
            .interpolate(limit_direction="both")
            .ffill()
            .bfill()
            .to_numpy(np.float32)
        ...
        prefix_z = (
            pd.to_numeric(prefix_frame["Z"], errors="coerce")
            .interpolate(limit_direction="both")
            .ffill()
            .bfill()
            .to_numpy(np.float32)
        ...
        for window_name, window_config in windows.items():
            ...
            kx = prefix_x[mask].astype(np.float32)
            ky = prefix_y[mask].astype(np.float32)
            ...
            md_span = _finite_stat(kmd, "range")
            x_span = _finite_stat(kx, "range")
            y_span = _finite_stat(ky, "range")
            z_span = _finite_stat(kz, "range")
            xy_step = np.sqrt(np.square(np.diff(kx)) + np.square(np.diff(ky))).astype(np.float32)
            xy_path = float(np.nansum(xy_step)) if xy_step.size else 0.0
            xy_chord = float(np.sqrt((_last_delta(kx) ** 2) + (_last_delta(ky) ** 2)))
            xy_straightness = xy_chord / max(xy_path, 1e-6)
            dz = _last_delta(kz)
            dmd = _last_delta(kmd)
            dxy = xy_chord
            heading = float(np.arctan2(_last_delta(ky), _last_delta(kx))) if dxy > 1e-6 else 0.0
            ktvt_range = _finite_stat(ktvt, "range")
            ktvt_std = _finite_stat(ktvt, "std")
            ktvt_delta = _last_delta(ktvt)
            gr_mean = _finite_stat(kgr, "mean", fallback)
            gr_std = _finite_stat(kgr, "std")
            gr_range = _finite_stat(kgr, "range")
            column_data[f"{label}_known_rows"] = np.full(
                len(part), np.float32(len(ktvt)), dtype=np.float32
            )
            column_data[f"{label}_known_row_fraction"] = np.full(
                len(part), np.float32(len(ktvt) / max(prefix_len, 1)), dtype=np.float32
            )
            ...
            column_data[f"{label}_x_span"] = np.full(
                len(part), np.float32(x_span), dtype=np.float32
            )
            column_data[f"{label}_y_span"] = np.full(
                len(part), np.float32(y_span), dtype=np.float32
            )
            column_data[f"{label}_z_span"] = np.full(
                len(part), np.float32(z_span), dtype=np.float32
            )
            column_data[f"{label}_xy_path_len"] = np.full(
                len(part), np.float32(xy_path), dtype=np.float32
            )
            column_data[f"{label}_xy_chord"] = np.full(
                len(part), np.float32(xy_chord), dtype=np.float32
            )
            column_data[f"{label}_xy_straightness"] = np.full(
                len(part), np.float32(xy_straightness), dtype=np.float32
            )
            column_data[f"{label}_dz"] = np.full(
                len(part), np.float32(dz), dtype=np.float32
            )
            column_data[f"{label}_dxy"] = np.full(
                len(part), np.float32(dxy), dtype=np.float32
            )
            column_data[f"{label}_dz_dmd"] = np.full(
                len(part), np.float32(dz / max(abs(dmd), 1e-6)), dtype=np.float32
            )
            column_data[f"{label}_dxy_dmd"] = np.full(
                len(part), np.float32(dxy / max(abs(dmd), 1e-6)), dtype=np.float32
            )
            column_data[f"{label}_heading_sin"] = np.full(
                len(part), np.float32(np.sin(heading)), dtype=np.float32
            )
            column_data[f"{label}_heading_cos"] = np.full(
                len(part), np.float32(np.cos(heading)), dtype=np.float32
            )
            ...
            column_data[f"{label}_ktvt_range"] = np.full(
                len(part),
                np.float32(ktvt_range),
                dtype=np.float32,
            )
            column_data[f"{label}_ktvt_std"] = np.full(
                len(part), np.float32(ktvt_std), dtype=np.float32
            )
            column_data[f"{label}_ktvt_delta"] = np.full(
                len(part), np.float32(ktvt_delta), dtype=np.float32
            )
            column_data[f"{label}_ktvt_mean_minus_last_known"] = (
                np.float32(_finite_stat(ktvt, "mean")) - last_known
            ).astype(np.float32)
            column_data[f"{label}_gr_mean"] = np.full(
                len(part), np.float32(gr_mean), dtype=np.float32
            )
            column_data[f"{label}_gr_std"] = np.full(
                len(part), np.float32(gr_std), dtype=np.float32
            )
            column_data[f"{label}_gr_range"] = np.full(
                len(part), np.float32(gr_range), dtype=np.float32
            )
            ...
            for cand_i, cand_name in enumerate(candidate_names):
                ...
                column_data[f"{label}_candidate_minus_last_known_{cand_name}"] = (
                    candidate - last_known
                ).astype(np.float32)
                column_data[f"{label}_candidate_minus_ktvt_mean_{cand_name}"] = (
                    candidate - np.float32(_finite_stat(ktvt, "mean"))
                ).astype(np.float32)
            ...
            summary_rows.append(
                {
                    ...
                    "ktvt_range": ktvt_range,
                    "md_span": md_span,
                    "xy_path_len": xy_path,
                    "xy_straightness": xy_straightness,
                    "z_span": z_span,
                    "gr_std": gr_std,
                    "pfx_rmse": pfx_rmse,
...
def _write_prefix_crop_feature_cache(
    *,
    ...
    manifest = {
        "experiment": "exp185_last50_first_prefix_feature_rebuild_on_exp148",
...
def load_prefix_crop_feature_cache(
    ...
    *,
    ...
    schema_features = (
        schema["feature"].astype(str).tolist()
    ...
    if selected_feature_columns is not None:
        ...
        dtype_map = {"id": str, "well": str}
        dtype_map.update({col: np.float32 for col in requested_columns})
        ...
        dtype_map = {"id": str, "well": str}
        dtype_map.update({col: np.float32 for col in schema_features})
        features = pd.read_csv(resolved_feature, dtype=dtype_map)
    required = {"id", "well"}
    ...
    for col in feature_columns:
        if features[col].dtype != np.float32:
            features[col] = pd.to_numeric(features[col], errors="coerce").astype(np.float32)
        values = features[col].to_numpy(dtype=np.float32, copy=False)
        if not np.isfinite(values).all():
            raise ValueError(f"prefix crop feature cache contains non-finite values: {col}")
...
def run_last50_first_prefix_feature_rebuild_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    del frame, projection_features, learned_features
    ...
    for window_name in (prefix_crop_config or {}).get("windows", {}) or {}:
        ...
        feature_group_columns[f"prefix_crop_{window_name}"] = window_columns
        ...
        feature_group_columns[f"prefix_crop_{window_name}_multiobs"] = [
            col
            for col in window_columns
            if "_multiobs_" in col or "_candidate_outside_prefix_tvt_range_" in col
        ]
        feature_group_columns[f"prefix_crop_{window_name}_sc_ncc"] = [
            col for col in window_columns if "_sc" in col
        ]
        feature_group_columns[f"prefix_crop_{window_name}_slope_calibration"] = [
            col
            for col in window_columns
            if any(
                token in col
    ...
    for variant in selected_variants:
        ...
        nonprefix_feature_columns = [
            col for col in feature_columns if col not in prefix_crop_feature_columns_all
        ]
        variant_base_columns = []
        for col in ["id", "well", "target", "last_known_tvt", *nonprefix_feature_columns]:
            if col not in variant_base_columns:
                variant_base_columns.append(col)
        missing_base_columns = sorted(set(variant_base_columns) - set(full_frame.columns))
        if missing_base_columns:
            raise ValueError(
                f"variant={variant_name} missing non-prefix feature columns: "
                f"{missing_base_columns[:40]}"
            )
        variant_frame = full_frame.loc[:, variant_base_columns].reset_index(drop=True).copy(
            deep=False
        )
        prefix_frame = variant_prefix_features[variant_prefix_columns].reset_index(drop=True)
        for col in variant_prefix_columns:
            variant_frame[col] = prefix_frame[col].to_numpy(dtype=np.float32, copy=False)
        ...
        _log_event(
            ...
            prefix_features=int(len(variant_prefix_columns)),
            nonprefix_features=int(len(nonprefix_feature_columns)),
            frame_columns=int(len(variant_frame.columns)),
        )
        for index, feature in enumerate(feature_columns):
    ...
    manifest = {
        ...
        "mode": "last50_first_prefix_feature_rebuild_on_exp148_full_train_rows",
        "feature_source": feature_meta,
        "learned_likelihood_feature_source": learned_source_meta,
...
def run_saved_model_inference(
    *,
    ...
    learned_feature_config: dict[str, Any] | None = None,
    prefix_crop_config: dict[str, Any] | None = None,
    variant_name: str = "last50_first_prefix_rebuild",
