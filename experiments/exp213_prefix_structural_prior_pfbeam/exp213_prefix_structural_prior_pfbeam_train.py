...
# ---
# %% [markdown]
# # exp213_prefix_structural_prior_pfbeam train
#
# Scoped train-side audit for using prefix TVT_input + Z structural surface as
# PF/Beam initial-state and transition prior while keeping raw GR observation.
...
# 4. Prefix structural prior and PF / Beam variant contract
# 5. Prefix structural prior PF / Beam generation audit
...
from prefix_structural_prior_pfbeam import (
    parse_filter_specs,
    read_exp072_eval_cache,
    run_prefix_structural_prior_pfbeam,
    select_target_wells,
)
...
if target_wells.empty:
    raise RuntimeError("No target wells selected for prefix structural prior PF/Beam audit.")
...
if not any(spec.transition == "prefix_structural" for spec in filters):
    raise RuntimeError("At least one prefix_structural transition variant is required.")
...
# %%
result = run_prefix_structural_prior_pfbeam(config=config, paths=paths)
...
display(
    ...
    .agg(
        ...
        structural_velocity_blend=("structural_velocity_blend", "mean"),
        structural_velocity_pull=("structural_velocity_pull", "mean"),
