# %% [markdown]
# # exp346 exp209 observed-only finite-sigma GR HMM inference
#
# exp346 is a train-side row-scheduled scale/decoder audit. Raw-test prediction,
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if contract["experiment"] != EXPERIMENT_NAME or contract["route"] != "pf_beam":
        raise ValueError("unexpected exp346 inference config")
    if contract["mode"] != "disabled_until_train_side_promotion_and_separate_approval":
        raise ValueError("exp346 inference mode contract changed")
    if any(
        contract[key]
        for key in (
            "inference_enabled",
        ...
        raise ValueError("exp346 inference and submission must remain disabled")
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp346 is a train-side observed-only finite-sigma exact-HMM audit; "
