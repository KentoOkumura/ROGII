# %% [markdown]
# # exp346 exp209 observed-only finite-sigma GR HMM train
#
# Train-side exact-HMM audit of one preregistered row-wise GR scale schedule.
# Raw finite evaluation rows use a finite-prefix population standard deviation;
# raw missing rows retain the exact exp209 zero-fill scale. Saved exp209 HMM and
# exp072 LikPF controls are read-only; no control, PF, Beam, model, inference,
# or submission is regenerated.
...
# 4. Observed-only finite scale schedule and exact-HMM input preparation
...
EXPERIMENT_NAME = "exp346_exp209_observed_only_finite_sigma_gr_hmm"
OUTPUT_PREFIX = EXPERIMENT_NAME
VARIANT_ORDER = ("observed_finite_std_missing_exp209_sigma",)
...
EXECUTE_NOTEBOOK = os.environ.get("EXP346_IMPORT_ONLY", "0") != "1" and in_notebook_runtime()
...
def load_experiment_config() -> dict[str, Any]:
    ...
    for path in candidates:
        ...
        if get_nested(config, "experiment.name") == EXPERIMENT_NAME:
            return config
    raise FileNotFoundError(f"exp346 config not found in {[str(path) for path in candidates]}")
...
def validate_scientific_contract(
    ...
    expected = {
        ...
        "model.confidence_contract.certain_row_definition": (
            "raw_horizontal_gr_is_finite_before_interpolation"
        ),
        "model.confidence_contract.uncertain_row_definition": (
            "raw_horizontal_gr_is_missing_before_interpolation"
        ),
        "model.confidence_contract.learned_confidence": False,
        "model.scale_schedule.observed_scale_estimator": (
            "population_standard_deviation_finite_known_prefix_pairs"
        ),
        "model.scale_schedule.exp209_scale_estimator": (
            "population_standard_deviation_zero_filled_known_prefix_residuals"
        ),
        "model.scale_schedule.minimum_finite_pairs": 20,
        "model.scale_schedule.insufficient_or_nonfinite_fallback": (
            "exp209_zero_fill_sigma_same_well"
        ),
        "model.scale_schedule.affine_a": 1.0,
        "model.scale_schedule.affine_b": 0.0,
        "model.scale_schedule.raw_finite_evaluation_row": "observed_finite_std",
        "model.scale_schedule.raw_missing_evaluation_row": ("exp209_zero_fill_sigma_exact_parity"),
        "execution_contract.schedule_audits": 1,
        ...
        "promotion_gates.require_raw_missing_emission_schedule_parity": True,
    }
    for key, value in expected.items():
        if get_nested(config, key) != value:
            raise ValueError(f"exp346 fixed contract mismatch: {key} must be {value!r}")
    if [float(value) for value in get_nested(config, "model.scale_schedule.clip")] != [10.0, 60.0]:
        raise ValueError("exp346 fixes the GR scale clip to [10, 60]")
    ...
    for key, value in fixed_hmm.items():
        if float(hmm.get(key, -1.0)) != value:
            raise ValueError(f"exp346 fixes model.fixed_hmm.{key}={value}")
    if (
        hmm.get("emission") != "gaussian_negative_half_min_z2_600_without_log_sigma"
        or hmm.get("rate_center") != "zero"
    ):
        raise ValueError("exp346 fixes Gaussian emission and zero-centered rate grid")
    fixed_policies = {
    ...
    if not bool(get_nested(config, "execution.implementation_approved")):
        raise ValueError("exp346 implementation approval must be recorded")
    if require_run_approval and not (
        bool(get_nested(config, "execution.kaggle_push_approved"))
        and bool(get_nested(config, "execution.run_scale_schedule_audit"))
        and bool(get_nested(config, "execution.run_hmm"))
    ):
        raise RuntimeError("exp346 Kaggle package/push/run is not approved")
...
def build_scientific_contract(config: dict[str, Any]) -> dict[str, Any]:
    contract = {
        ...
        "confidence_contract": get_nested(config, "model.confidence_contract"),
        "scale_schedule": get_nested(config, "model.scale_schedule"),
        ...
        "forbidden": [
            "finite_mad_or_robust_scale",
            "sigma_multiplier_or_clip_grid",
            "learned_confidence_or_threshold_search",
            "missing_row_scale_or_weight_change",
            "evaluation_gr_change",
            "typewell_change",
            "affine_or_center_change",
            "structure_variance",
            "temperature_student_t_huber_or_mixture_emission",
...
def _finalize_observed_scale(
    ...
    *,
    ...
    """Apply the frozen no-op fallback and clip to the observed-row scale."""
    fallback_used = pair_count < minimum_pairs or not math.isfinite(raw_scale)
    source = exp209_sigma if fallback_used else raw_scale
...
def compute_prefix_scale_audit(
    ...
    *,
    ...
    observed_sigma_raw = float(np.std(residual, ddof=0)) if pair_count else float("nan")
    zero_fill_residual = (
    ...
    observed_sigma, fallback_used, clip_low, clip_high = _finalize_observed_scale(
        observed_sigma_raw,
        pair_count,
        minimum_pairs=minimum_pairs,
        exp209_sigma=exp209_sigma,
        clip=clip,
    )
    ...
    return {
        ...
        "observed_finite_std_raw": observed_sigma_raw,
        "observed_finite_std": observed_sigma,
        "observed_finite_std_fallback": fallback_used,
        "observed_finite_std_clip_low": clip_low,
        "observed_finite_std_clip_high": clip_high,
        "fallback_policy": "exp209_zero_fill_sigma_same_well",
        "affine_a": 1.0,
        "affine_b": 0.0,
    }
...
def prepare_hmm_inputs(
    ...
    hmm = get_nested(config, "model.fixed_hmm") or {}
    scale_config = get_nested(config, "model.scale_schedule") or {}
    tvt_input = pd.to_numeric(horizontal["TVT_input"], errors="coerce").to_numpy(np.float64)
    ...
    if not known_mask.any() or not eval_mask.any():
        raise ValueError("each exp346 well must contain known prefix and unknown suffix rows")
    ...
    scale_audit = compute_prefix_scale_audit(
        ...
        typewell_tvt,
        typewell_gr,
        minimum_pairs=int(scale_config["minimum_finite_pairs"]),
    ...
    sigma_base = float(scale_audit["exp209_zero_fill_sigma"])
    sigma_observed = float(scale_audit["observed_finite_std"])
    sigma_schedule = np.where(raw_gr_observed, sigma_observed, sigma_base).astype(np.float64)
    if bool(scale_audit["observed_finite_std_fallback"]):
        sigma_schedule.fill(sigma_base)
...
def run_exact_hmm_variant(
    ...
    sigma_gr: np.ndarray | float,
    config: dict[str, Any],
) -> dict[str, Any]:
    hmm = get_nested(config, "model.fixed_hmm") or {}
    sigma_schedule = np.asarray(sigma_gr, dtype=np.float64)
    if sigma_schedule.ndim == 0:
        sigma_schedule = np.full(len(prepared["observed_gr"]), float(sigma_schedule))
    if sigma_schedule.shape != (len(prepared["observed_gr"]),):
        raise ValueError("exp346 sigma schedule must contain exactly one value per evaluation row")
    if not np.isfinite(sigma_schedule).all() or not bool(
        ((sigma_schedule >= 10.0) & (sigma_schedule <= 60.0)).all()
    ):
        raise ValueError("exp346 sigma schedule must satisfy the frozen [10, 60] contract")
    zscore = (prepared["observed_gr"][:, None] - prepared["state_gr"][None, :]) / sigma_schedule[
        :, None
    ]
    emission = (-0.5 * np.minimum(zscore**2, float(hmm["emission_clip_z2"]))).astype(np.float32)
    missing_mask = ~np.asarray(prepared["raw_gr_observed"], dtype=bool)
    base_sigma = float(prepared["scale_audit"]["exp209_zero_fill_sigma"])
    base_zscore = (prepared["observed_gr"][:, None] - prepared["state_gr"][None, :]) / base_sigma
    base_emission = (-0.5 * np.minimum(base_zscore**2, float(hmm["emission_clip_z2"]))).astype(
        np.float32
    )
    missing_emission_max_abs_diff = (
        float(np.max(np.abs(emission[missing_mask] - base_emission[missing_mask])))
    ...
    return {
        ...
        "raw_missing_emission_max_abs_diff_vs_exp209": missing_emission_max_abs_diff,
    }
...
def decode_well(
    ...
    schedule = pd.DataFrame(
        {
            ...
            "exp209_zero_fill_sigma": float(scale_audit["exp209_zero_fill_sigma"]),
            "observed_finite_std": float(scale_audit["observed_finite_std"]),
            "sigma_gr_row": prepared["sigma_schedule"].astype(np.float64),
            "observed_scale_fallback": int(scale_audit["observed_finite_std_fallback"]),
        ...
        runtime_rows.append(
            {
                ...
                "exp209_zero_fill_sigma": float(scale_audit["exp209_zero_fill_sigma"]),
                "observed_finite_std": float(scale_audit["observed_finite_std"]),
                "observed_scale_fallback": bool(scale_audit["observed_finite_std_fallback"]),
                ...
                "raw_missing_emission_max_abs_diff_vs_exp209": float(
                    result["raw_missing_emission_max_abs_diff_vs_exp209"]
                ),
    ...
    if not np.array_equal(
        schedule.loc[schedule["raw_gr_observed"] == 0, "sigma_gr_row"].to_numpy(np.float64),
        schedule.loc[schedule["raw_gr_observed"] == 0, "exp209_zero_fill_sigma"].to_numpy(
            np.float64
        ),
    ):
        raise ValueError(f"well={well} raw-missing sigma schedule lost exp209 parity")
    scale_row = {
        "well_id": well,
        **scale_audit,
    ...
    return predictions, schedule, scale_row, pd.DataFrame(runtime_rows)
...
def generate_and_freeze_predictions(
    ...
    if not NUMBA_AVAILABLE:
        raise RuntimeError("numba is required for the exp346 exact-HMM audit")
    ...
    def build_one(index: int, well: str):
        print(f"[{index}/{len(wells)}] exp346 well={well}", flush=True)
    ...
    scale_audit = pd.DataFrame([row[2] for row in results])
    ...
    if (
        ...
        raise ValueError("exp346 prediction/scale/runtime coverage mismatch")
    _assert_same_order("prediction vs raw-mask schedule", prediction["id"], schedule["id"])
    prediction_path = artifacts / f"{OUTPUT_PREFIX}_predictions.csv.gz"
    schedule_path = artifacts / f"{OUTPUT_PREFIX}_raw_mask_scale_schedule.csv.gz"
    ...
    scale_report["schema_sha256"] = dataframe_schema_sha(scale_audit)
    ...
    reports = {
        ...
        "raw_mask_scale_schedule": schedule_report,
    ...
    return (
        reports,
        {
            ...
            "raw_mask_scale_schedule": schedule_path,
            "scale_audit": scale_path,
...
def load_late_readout_frame(
    ...
    _require_frozen_prediction(frozen["raw_mask_scale_schedule"])
    prediction = (
        pd.read_csv(
    ...
    schedule = (
        pd.read_csv(
            output_paths["raw_mask_scale_schedule"],
            dtype={"id": str, "well_id": str},
        )
        .sort_values(["well_id", "row_idx"], kind="mergesort")
        .reset_index(drop=True)
    ...
    return frame, {
        "truth_attachment_stage": (
            "after_raw_mask_scale_schedule_and_prediction_gzip_content_sha_freeze"
        ),
        "prediction_content_sha256": frozen["prediction"]["content_sha256"],
        "raw_mask_scale_schedule_content_sha256": frozen["raw_mask_scale_schedule"][
            "content_sha256"
        ],
...
def evaluate_promotion_gate(
    ...
    scale_columns = ["exp209_zero_fill_sigma", "observed_finite_std"]
    fallback_fraction = float(scale_audit["observed_finite_std_fallback"].astype(bool).mean())
    emission_parity_max = float(runtime["raw_missing_emission_max_abs_diff_vs_exp209"].max())
    observed_rows = int(runtime["raw_observed_rows"].sum())
    missing_rows = int(runtime["raw_missing_rows"].sum())
    scale_values = scale_audit[scale_columns].to_numpy(np.float64)
    technical = {
        ...
        "scale_finite_coverage": float(np.isfinite(scale_values).mean()),
        "scale_clip_contract_passed": bool(
            np.isfinite(scale_values).all()
            and (scale_values >= 10.0).all()
            and (scale_values <= 60.0).all()
        ),
        ...
        "scale_fallback_well_fraction": fallback_fraction,
        "maximum_scale_fallback_well_fraction": float(gate["maximum_scale_fallback_well_fraction"]),
        "raw_missing_emission_schedule_max_abs_diff_vs_exp209": emission_parity_max,
        "raw_missing_emission_schedule_parity_passed": bool(emission_parity_max == 0.0),
    ...
    technical["passed"] = bool(
        ...
        and technical["scale_clip_contract_passed"]
        and technical["raw_mask_partition_passed"]
        and fallback_fraction <= technical["maximum_scale_fallback_well_fraction"]
        and technical["raw_missing_emission_schedule_parity_passed"]
    ...
    def scope_record(scope: str) -> pd.Series:
        ...
        if len(selected) != 1:
            raise ValueError(f"exp346 expected exactly one direct metric row for scope={scope}")
    ...
    primary_direct["passed"] = bool(
        ...
        and p95_delta <= float(gate["require_by_well_p95_delta_max"])
    ...
    return {
        ...
        "passed": passed,
        "decision": (
            "observed_only_finite_sigma_passed_train_side_only_no_automatic_downstream"
            if passed
            else "observed_only_finite_sigma_failed_close_without_rescue"
        ),
        "technical_gate": technical,
        "primary_direct_gate": primary_direct,
        "fixed_likpf_50_50_guard": blend_guard,
        "failure_action": (
            "close_without_sigma_confidence_emission_hmm_blend_rescue_inference_or_submission"
        ),
    }
...
def run_full_experiment(config: dict[str, Any]) -> dict[str, Any]:
    if not KAGGLE_WORKING_ROOT.exists() and os.environ.get("EXPERIMENT_ALLOW_LOCAL") != "1":
        raise RuntimeError(
            "Full exp346 must run first on Kaggle; local execution requires explicit smoke approval"
        )
    ...
    status = (
        "train_side_observed_only_finite_sigma_gate_passed_no_automatic_downstream"
        if promotion_gate["passed"]
        else "train_side_observed_only_finite_sigma_gate_failed_closed"
    )
    summary = {
        "experiment": EXPERIMENT_NAME,
        "status": status,
    ...
    summary["generated_files"] = output_file_reports(
        {
            **output_metric_paths,
            ...
            "raw_mask_scale_schedule": Path(output_paths["raw_mask_scale_schedule"]),
    ...
    metrics = {
        ...
        "raw_mask_scale_schedule_sha256": frozen["raw_mask_scale_schedule"]["content_sha256"],
        "scale_audit_sha256": frozen["scale_audit"]["content_sha256"],
        "model_sha256": None,
        "submission_sha256": None,
