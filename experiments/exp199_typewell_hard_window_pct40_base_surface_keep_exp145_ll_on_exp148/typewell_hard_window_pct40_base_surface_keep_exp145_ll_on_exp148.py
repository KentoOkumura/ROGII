...
BASE_SURFACE_EXPERIMENT = "exp196_typewell_late_range_hard_window_pct40_full_cache_replacement"
BASE_SURFACE_ARTIFACTS = Path("experiments") / BASE_SURFACE_EXPERIMENT / "artifacts"
...
OUTPUT_PREFIX = "exp199_typewell_hard_window_pct40_base_surface_keep_exp145_ll_on_exp148"
...
def find_artifact(
    ...
    *,
    local_artifacts: Path = BASE_SURFACE_ARTIFACTS,
) -> Path:
...
def load_base_surface_cache_frame(
    ...
    *,
    ...
    if not np.isfinite(frame[["target", *feature_columns]].to_numpy(np.float32)).all():
        raise ValueError("exp196 pct40 base surface cache contains non-finite numeric values")
    ...
    metadata = {
        ...
        "source_experiment": BASE_SURFACE_EXPERIMENT,
        "source_kind": "typewell_hard_window_pct40_full_public_replay_train_feature_cache",
        "rows": int(len(frame)),
...
load_exp072_full_replay_cache_frame = load_base_surface_cache_frame
...
def run_typewell_hard_window_pct40_base_surface_keep_exp145_ll_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    frame, base_feature_columns, feature_meta = load_base_surface_cache_frame(
        cache_path,
        max_rows=max_rows,
    )
    ...
    if full_frame.empty:
        raise ValueError(
            "No shared rows between exp196 pct40 base surface and learned likelihood features"
        )
    coverage_meta = {
    ...
    manifest = {
        "experiment": "exp199_typewell_hard_window_pct40_base_surface_keep_exp145_ll_on_exp148",
        "parent": "exp148_learned_likelihood_fulltrain_addonly_on_exp092",
        "base_surface_parent": BASE_SURFACE_EXPERIMENT,
        "learned_likelihood_parent": "exp145_learned_likelihood_rawtest_feature_generator_parity",
        "cache_parent": BASE_SURFACE_EXPERIMENT,
        "mixed_provenance": {
            "base_surface": BASE_SURFACE_EXPERIMENT,
            "learned_likelihood_features": "exp145_learned_likelihood_rawtest_feature_generator_parity",
            "direct_submit_candidate": False,
        },
        "mode": "typewell_hard_window_pct40_base_surface_keep_exp145_ll_on_exp148_full_train_rows",
...
def run_saved_model_inference(
    *,
    ...
    projection_config: dict[str, Any] | None = None,
    learned_feature_config: dict[str, Any] | None = None,
    variant_name: str = "pct40_base_surface_keep_exp145_ll_mixed_provenance",
