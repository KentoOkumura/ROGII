...
OUTPUT_PREFIX = "exp161_prefix_crop_window_features_on_exp148"
PREFIX_CROP_TRAIN_FEATURES = (
    "exp161_prefix_crop_window_features_on_exp148_prefix_crop_train_features.csv.gz"
)
PREFIX_CROP_FEATURE_SCHEMA = (
    "exp161_prefix_crop_window_features_on_exp148_prefix_crop_feature_schema.csv"
)
PREFIX_CROP_FEATURE_SUMMARY = (
    "exp161_prefix_crop_window_features_on_exp148_prefix_crop_feature_summary.csv"
)
PREFIX_CROP_FEATURE_MANIFEST = (
    "exp161_prefix_crop_window_features_on_exp148_prefix_crop_feature_manifest.json"
)
    ...
    *,
    ...
    windows = dict(
        ...
        or {
            ...
            "tail2000": {"kind": "md_tail", "md_back": 2000.0, "min_rows": 8},
...
def _write_prefix_crop_feature_cache(
    *,
    ...
    manifest = {
        "experiment": "exp161_prefix_crop_window_features_on_exp148",
...
def load_prefix_crop_feature_cache(
    ...
    *,
    ...
    for col in feature_columns:
        features[col] = pd.to_numeric(features[col], errors="coerce").astype(np.float32)
    if not np.isfinite(features[feature_columns].to_numpy(np.float32)).all():
...
def run_prefix_crop_window_features_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    if require_prefix_crop_cache or prefix_crop_feature_path is not None:
        prefix_crop_features, prefix_crop_group_columns, prefix_crop_summary, prefix_crop_cache_meta = (
    ...
    prefix_keys_match = (
        len(prefix_crop_features) == len(full_frame)
        and prefix_crop_features["id"].astype(str).reset_index(drop=True).equals(
            full_frame["id"].astype(str).reset_index(drop=True)
        )
        and prefix_crop_features["well"].astype(str).reset_index(drop=True).equals(
    ...
    _log_event(
        "prefix_crop_join_start",
        rows=int(before_prefix_rows),
        prefix_features=int(len(prefix_crop_feature_columns)),
    )
    full_frame = pd.concat(
        [
            full_frame.reset_index(drop=True),
            prefix_crop_features[prefix_crop_feature_columns].reset_index(drop=True),
        ],
        axis=1,
        copy=False,
    )
    del prefix_crop_features
    gc.collect()
    _log_event(
        "prefix_crop_join_done",
    ...
    manifest = {
        ...
        "mode": "prefix_crop_window_features_on_exp148_full_train_rows",
        "feature_source": feature_meta,
        "learned_likelihood_feature_source": learned_source_meta,
...
def run_saved_model_inference(
    *,
    ...
    learned_feature_config: dict[str, Any] | None = None,
    prefix_crop_config: dict[str, Any] | None = None,
    variant_name: str = "prefix_crop_window_addonly",
