...
import numpy as np
import pandas as pd
from pf_model_diff_model_audit import (
    MODEL_DIFF_FEATURE_COLUMNS,
    VariantSpec,
    add_derived_features,
    add_model_diff_features,
    ...
    load_features,
    make_estimator,
    predict_source_well_cutoff,
    resolve_feature_path,
    seedbag_distance_gate_prediction,
    source_model_config,
    stable_fold,
...
def selected_inference_spec(config: dict[str, Any]) -> VariantSpec:
    selected_name = str(
        get_nested(config, "inference.selected_variant", "lgbm_capacity_pf_model_diff_foldsafe")
    )
...
def fit_final_candidate_model(
    ...
    *,
    ...
    well_folds = int(get_nested(config, "audit.well_holdout_folds", 5))
    split_codes = frame["well_id"].map(lambda value: stable_fold(str(value), well_folds))
    split_codes = split_codes.to_numpy(dtype=np.int16)
    frame, _, source_rows = add_model_diff_features(
        audit="final_well_hash_foldout_train",
        frame=frame,
        config=config,
        paths=paths,
    ...
    return model, spec, source_rows, info
...
def fit_final_source_models(
    ...
    *,
    ...
    model_files = train_files(paths, max_wells)
    fitted: dict[str, dict[str, Any]] = {}
    source_rows: list[dict[str, Any]] = []
    for source in list(get_nested(config, "audit.model_diff_sources", []) or []):
        ...
        for raw_variant in get_nested(source_config, "audit.training_variants.variants", []):
            if isinstance(raw_variant, dict) and str(raw_variant.get("name")) == selected:
                variant = dict(raw_variant)
                break
        if variant is None:
            raise ValueError(f"selected training variant not found for source {name}: {selected}")
        ...
        seed = int(source.get("seed", get_nested(source_config, "validation.seed", 42)))
        max_rows_per_well = int(
            source.get(
                "max_rows_per_well",
                get_nested(source_config, "model.training.max_train_rows_per_well", 800),
            )
        )
        max_rows_total = int(
            source.get(
                "max_rows_total",
                get_nested(source_config, "model.training.max_train_rows_final", 450000),
            )
        )
        kind = str(variant.get("kind", "single_model"))
        if kind == "seed_bagging":
            models, n_train_rows, rows, member_seeds = fit_pseudo_tail_models_from_files(
                model_files,
                source_config,
                variant,
                seed=seed,
            ...
            model, n_train_rows, rows = fit_pseudo_tail_model_from_files(
                model_files,
                source_config,
                variant,
                seed=seed,
        ...
        for row in rows:
            updated = dict(row)
            updated["source"] = name
            updated["phase"] = "final_inference_source_fit"
            updated["prediction_method"] = method
            updated["train_rows_total"] = n_train_rows
            updated["member_seeds"] = "|".join(str(value) for value in member_seeds)
            source_rows.append(updated)
        fitted[name] = {
            "models": models,
    ...
    return fitted, pd.DataFrame(source_rows)


def add_inference_model_diff_features(
    frame: pd.DataFrame,
    source_predictions: dict[str, np.ndarray],
) -> pd.DataFrame:
    required = {"exp052_foldout", "exp054_foldout"}
    missing = sorted(required - set(source_predictions))
    if missing:
        raise ValueError(f"missing inference source predictions: {missing}")
    frame = frame.copy()
    ...
    exp052 = source_predictions["exp052_foldout"]
    exp054 = source_predictions["exp054_foldout"]
...
def predict_hidden_frame(
    *,
    ...
    row_indices = frame["row_idx"].to_numpy(dtype=int)
    for name, info in source_models.items():
        source_predictions[name] = predict_source_well_cutoff(
            path=path,
            cutoff_row=cutoff_row,
    ...
    frame = add_inference_model_diff_features(frame, source_predictions)
    ...
    raw_pred = apply_prediction_policy(
        residual=residual,
        last_anchor=frame["last_anchor_tvt"].to_numpy(dtype=float),
        config=config,
    )
    postprocess = str(get_nested(config, "inference.selected_postprocess", "raw"))
    if postprocess == "seedbag_distance_gate":
        profile_name = str(get_nested(config, "inference.selected_gate_profile"))
        pred, _, _ = seedbag_distance_gate_prediction(
            raw_pred=raw_pred,
            seedbag_anchor=source_predictions["exp054_foldout"],
        ...
        raise ValueError(f"unsupported inference postprocess: {postprocess}")
    return pred, residual, source_predictions


def generate_pf_model_diff_submission(
    paths: ExperimentPaths,
    config: dict[str, Any],
    *,
    max_wells: int | None = None,
    ...
    source_models, final_source_rows = fit_final_source_models(
        paths,
        config,
        max_wells=max_wells,
    )
    ...
    for well_id in sorted(test_path_by_well):
        ...
        source_by_row: dict[str, dict[int, float]] = {}
        if tvt_phys is None:
            ...
            pred, residual, source_predictions = predict_hidden_frame(
                frame=feature_frame,
            ...
            for source_name, values in source_predictions.items():
                source_by_row[source_name] = {
                    int(idx): float(value) for idx, value in zip(row_indices, values, strict=True)
                }
            print(f"  PF-model-diff LGBM correction OK rows={len(feature_frame)}")
        ...
        for _, row in well_sample.iterrows():
            ...
            if tvt_phys is not None:
                ...
                source = str(
                    get_nested(
                        config,
                        "inference.selected_candidate",
                        "lgbm_capacity_pf_model_diff_foldsafe_seedbag_gate_near_mid_a0p25_far0",
                    )
                )
            ...
            for source_name, by_row in source_by_row.items():
                audit_row[f"{source_name}_prediction"] = by_row.get(row_idx)
    ...
    summary = {
        ...
        "candidate": str(get_nested(config, "inference.selected_candidate", f"{spec.name}_raw")),
        "selected_postprocess": str(get_nested(config, "inference.selected_postprocess", "raw")),
        "selected_gate_profile": str(get_nested(config, "inference.selected_gate_profile", "")),
        ...
        "final_source_models": {
            name: {
                "kind": info["kind"],
                "method": info["method"],
                "train_rows": int(info["train_rows"]),
                "member_seeds": [int(value) for value in info["member_seeds"]],
            }
            for name, info in source_models.items()
        },
    }
    ...
    anchor_submission.to_csv(
        paths.output_root / "pf_model_diff_original_selector_submission.csv",
        index=False,
    )
    audit.to_csv(paths.output_root / "lgbm_pf_model_diff_corrected_diff.csv", index=False)
    pd.DataFrame(well_rows).to_csv(
        paths.artifacts_dir / "pf_model_diff_inference_wells.csv",
        index=False,
    )
    pd.concat([foldout_source_rows, final_source_rows], ignore_index=True).to_csv(
        paths.artifacts_dir / "pf_model_diff_inference_source_summary.csv",
        index=False,
    )
    (paths.output_root / "lgbm_pf_model_diff_corrected_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n"
    )
    (paths.artifacts_dir / "pf_model_diff_inference_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n"
    )
    return summary
