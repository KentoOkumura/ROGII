...
OUTPUT_PREFIX = "exp139_exp092_exp098_small_rank_slot_merge"
...
def run_exp092_exp098_small_rank_slot_merge(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    if rank_slot_config.get("include_lgb_oof_features", False):
        raise NotImplementedError("LGB OOF rank-slot features are disabled for exp139")
...
def run_saved_model_inference(
    *,
    ...
    projection_config: dict[str, Any] | None = None,
    rank_slot_config: dict[str, Any] | None = None,
    variant_name: str = "u_projection_rank_slot_small_merge",
    ...
    if projection_config.get("include_lgb_oof_features", False):
        raise NotImplementedError("LGB OOF U-projection features are disabled for exp139 inference")
    rank_slot_config = rank_slot_config or dict(manifest.get("rank_slot_config") or {})
    if rank_slot_config.get("include_lgb_oof_features", False):
        raise NotImplementedError("LGB OOF rank-slot features are disabled for exp139 inference")
    ...
    summary = {
        ...
        "known_followup_risk": (
            ...
            "checks support the small merge."
        ),
        "elapsed_seconds": round(time.time() - t0, 3),
    }
...
run_u_projection_feature_ablation = run_exp092_exp098_small_rank_slot_merge
