# %% [markdown]
# # exp307 finite-only robust sigma GR inference
#
# exp307 is a train-side scale/decoder audit. Raw-test prediction, inference,
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if contract["experiment"] != EXPERIMENT_NAME or contract["route"] != "pf_beam":
        raise ValueError("unexpected exp307 inference config")
    if contract["mode"] != "disabled_train_side_finite_sigma_audit_only":
        raise ValueError("exp307 inference mode contract changed")
    if any(
        contract[key]
        for key in (
            "inference_enabled",
        ...
        raise ValueError("exp307 inference and submission must remain disabled")
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp307 is a train-side finite-sigma exact-HMM audit; inference and submission are disabled"
    )
