...
# %% [markdown]
# # exp287 fold-safe formation add-only inference
#
# exp264 inferenceと同じraw-test再生成からclean 273 + outer-fold別compact 74を作る。
# 全train wellだけをreferenceにformation 74を再生成し、保存済みexp287 421-feature model 15本で
# TVTを予測する。train guard FAILの明示overrideであり、学習、hard selector、Viterbi、submitは行わない。
...
if not (PACKAGE_DIR / "config.yaml").exists():
    PACKAGE_DIR = Path("experiments/exp287_fold_safe_formation_74_addonly_on_exp264")
paths = ExperimentPaths()
paths.require_kaggle_runtime()
paths.ensure_output_dirs()
experiment_config = load_config()
parent_config_path = PACKAGE_DIR / "inputs/exp264_parent_config.yaml"
parent_config = read_yaml(parent_config_path)
config = dict(parent_config)
config["experiment"] = dict(experiment_config["experiment"])
config["execution"] = dict(experiment_config["execution"])
config["inference"] = dict(experiment_config["inference"])
...
# ## 2. User authorization and saved-model contracts
#
# exp287のworst-well / 悪化well数guard FAILは保持する。2026-07-20のユーザー指示を、
# 保存済み15 modelによるinferenceだけの明示overrideとして扱う。再学習や外部submitへは拡張しない。

# %%
inference_cfg = dict(config["inference"])
if not bool(config["execution"]["inference_enabled"]):
    raise RuntimeError("exp287 inference is disabled")
if inference_cfg.get("status") != "user_authorized_guard_override_2026_07_20":
    raise RuntimeError("exp287 inference does not have the fixed user authorization")
if bool(inference_cfg.get("stage_d_guard_passed")):
    raise RuntimeError("exp287 train guard failure must remain explicit during this override")
...
candidate_contract_path = PACKAGE_DIR / "candidate_contract.yaml"
candidate_contract = read_yaml(candidate_contract_path)
names = candidate_ids(candidate_contract)
if len(names) != 12:
    raise ValueError("exp287 inference requires exactly 12 candidates")
sample = pd.read_csv(paths.sample_submission_path, dtype={"id": str})

configured_bundle_path = Path(str(inference_cfg["selector_model_bundle"]))
...
if sha256_file(stage_c_bundle) != str(inference_cfg["selector_model_bundle_sha256"]):
    raise ValueError("Stage C inference bundle SHA mismatch")
stage_c_root = Path("/tmp/exp287_stage_c_inference_models")
...
expected_stage_c_files = {
    stage_c_manifest_path: inference_cfg["selector_model_manifest_sha256"],
    selector_schema_path: inference_cfg["selector_feature_schema_file_sha256"],
    compact_schema_path: inference_cfg["compact_meta_schema_file_sha256"],
}
...
if stage_c_manifest.get("candidate_order") != names:
    raise ValueError("Stage C candidate order differs from the pinned parent contract")
if int(stage_c_manifest.get("model_count", -1)) != int(inference_cfg["selector_model_count"]):
...
if compact_schema.get("compact_meta_schema_sha256") != str(
    inference_cfg["compact_meta_schema_logical_sha256"]
):
...
if len(compact_features) != int(inference_cfg["expected_compact_feature_count"]):
...
for outer, by_objective in selector_models.items():
    for objective, models in by_objective.items():
        if len(models) != int(inference_cfg["selector_models_per_outer_objective"]):
...
if sha256_file(stage_d_manifest_path) != str(inference_cfg["tvt_model_manifest_sha256"]):
    raise ValueError("exp287 TVT model manifest SHA mismatch")
stage_d_manifest = json.loads(stage_d_manifest_path.read_text())
if stage_d_manifest.get("status") != "fold_safe_formation_15_gpu_boosters_completed":
    raise ValueError("exp287 model manifest status is not the completed 15-booster run")
if int(stage_d_manifest.get("feature_count", -1)) != int(
    inference_cfg["expected_final_feature_count"]
):
    raise ValueError("exp287 model manifest feature count mismatch")
if stage_d_manifest.get("feature_schema_sha256") != str(
    inference_cfg["tvt_model_feature_schema_sha256"]
):
    raise ValueError("exp287 model manifest logical feature SHA mismatch")
stage_d_rows = [
    dict(item)
...
if len(stage_d_rows) != int(inference_cfg["tvt_model_count"]):
    raise ValueError("exp287 inference requires exactly 15 add-only models")
...
compact_feature_count = int(inference_cfg["expected_compact_feature_count"])
model_compact_features = final_feature_columns[
    base_feature_count : base_feature_count + compact_feature_count
]
model_formation_features = final_feature_columns[
    base_feature_count + compact_feature_count :
]
...
if base_feature_columns != base_allowlist:
    raise ValueError("exp287 model base feature order differs from clean 273 allowlist")
...
if model_compact_features != compact_features:
    raise ValueError("exp287 model compact feature order differs from Stage C schema")
...
if len(formation_features) != int(inference_cfg["expected_formation_feature_count"]):
    raise ValueError("exp287 canonical formation feature count mismatch")
if model_formation_features != formation_features:
    raise ValueError("exp287 model formation feature order differs from canonical 74 schema")
if len(final_feature_columns) != int(inference_cfg["expected_final_feature_count"]):
    raise ValueError("exp287 final feature count mismatch")
...
source_work = Path("/tmp/exp287_trusted_upstream_sources")
...
# ## 4. Candidate-long context and Stage C compact features
#
# candidate-long matrixはchunkごとに一度だけ作る。同じmatrixへouter-fold別8 selectorを適用し、
# scoreをファイルへ保存・再読込せず74 compact列へ決定変換する。
...
chunk_size = int(config["model"]["training"]["predict_base_row_chunk_size"])
...
for outer in range(5):
    ...
    if [column for column in compact if column.startswith("selector__")] != compact_features:
        raise ValueError(f"compact schema mismatch for outer fold {outer}")
    if not np.isfinite(compact[compact_features].to_numpy(np.float32)).all():
        raise ValueError(f"compact features are non-finite for outer fold {outer}")
    compact_path = output_dir / f"compact_meta_current_test_outer{outer}.parquet"
...
del compact_parts, score_sample
gc.collect()
...
# current testから再計算する。保存済みpublic-test feature artifactは入力に使わず、exp287 modelの
...
# ## 6. Current-test all-train-reference formation features
#
# 全773 train wellsからFormationPlaneKNN / DenseANCCImputer referenceを構築する。target側は
# raw testのMD/X/Y/Z/TVT_inputだけを読み、formation列は読まない。public testでtrainとwell名が
# 重なる場合は、そのwellのtrain referenceだけをqueryからself-excludeする。
...
if len(train_reference_wells) != int(experiment_config["validation"]["expected_wells"]):
    raise ValueError("exp287 inference train-reference well count mismatch")
formation_surface, formation_generation = build_current_test_formation_surface(
    ...
    generator_config=experiment_config["formation_generator"],
)
if not formation_surface[["id", "well"]].equals(
    test_frame[["id", "well"]].reset_index(drop=True)
):
    raise ValueError("exp287 current-test formation alignment mismatch")
...
# ## 7. Exp287 saved-booster inference
#
# 各TVT modelは学習時と同じouter foldのcompact 74列と、共通のall-train-reference formation
# 74列を使う。3 config × 5 foldの15 residual predictionを等重み平均し、`last_known_tvt`へ加算する。
...
for outer in range(5):
    ...
    aligned_compact = test_frame[["id"]].merge(
        compact[["id", *compact_features]],
        on="id",
        how="left",
        validate="one_to_one",
    )
    if aligned_compact[compact_features].isna().any().any():
        raise ValueError(f"compact alignment introduced missing values for outer fold {outer}")
    matrix_frame = pd.concat(
        [
            test_frame[base_feature_columns].reset_index(drop=True),
            aligned_compact[compact_features].reset_index(drop=True),
            formation_surface[formation_features].reset_index(drop=True),
        ],
        axis=1,
    )
    if list(matrix_frame.columns) != final_feature_columns:
        raise ValueError(f"exp287 feature order mismatch for outer fold {outer}")
    matrix = matrix_frame.to_numpy(np.float32, copy=False)
    if not np.isfinite(matrix).all():
        raise ValueError(f"exp287 feature matrix is non-finite for outer fold {outer}")
    fold_models = [
        (item, model_path)
    ...
    if len(fold_models) != 3:
        raise ValueError(f"exp287 outer fold {outer} must have three add-only models")
    for item, model_path in fold_models:
        booster = lgb.Booster(model_file=str(model_path))
        if list(booster.feature_name()) != final_feature_columns:
            raise ValueError(f"exp287 model feature schema mismatch: {model_path.name}")
        prediction = booster.predict(
            matrix, num_iteration=int(item["best_iteration"])
        ).astype(np.float32)
        if not np.isfinite(prediction).all():
            raise ValueError(f"exp287 model prediction is non-finite: {model_path.name}")
        ...
        gc.collect()
    del compact, aligned_compact, matrix_frame, matrix
    gc.collect()
if len(tvt_model_audit) != 15:
    raise ValueError("exp287 inference did not use all 15 add-only models")
pred_tvt = test_frame["last_known_tvt"].to_numpy(np.float32) + pred_delta
if not np.isfinite(pred_tvt).all():
    raise ValueError("final exp287 TVT prediction contains non-finite values")
...
prediction_path = output_dir / "exp287_current_test_predictions.csv.gz"
feature_schema_path = output_dir / "exp287_inference_feature_schema.csv"
...
pd.DataFrame(
    {
        ...
        "feature_group": [
            ...
            if index < len(base_feature_columns) + len(compact_features)
...
metrics = {
    "experiment": EXPERIMENT_NAME,
    "route": config["experiment"]["route"],
    "status": "guard_override_inference_completed_submit_not_authorized",
    "authorization": {
        ...
        "competition_submit_authorized": False,
        "competition_submit_requires_submit_check_pass": True,
    },
    ...
    "selector_model_count": len(selector_model_audit),
    ...
    "train_guard_evidence": {
        "parent_exp264_rmse": float(inference_cfg["parent_exp264_rmse"]),
        "fold_safe_formation_rmse": float(inference_cfg["fold_safe_formation_rmse"]),
        "delta_rmse_new_minus_parent": float(
            inference_cfg["delta_rmse_new_minus_parent"]
        ),
        "improved_folds": int(inference_cfg["improved_folds"]),
        "worst_well_delta_rmse": float(inference_cfg["worst_well_delta_rmse"]),
        "worsened_well_threshold_counts": dict(
            inference_cfg["worsened_well_threshold_counts"]
        ),
        "guard_passed": False,
    },
    ...
    "selector_models": selector_model_audit,
    "tvt_models": tvt_model_audit,
    "sha256": {
        "candidate_contract": candidate_contract_sha(candidate_contract),
        "exp264_parent_config": sha256_file(parent_config_path),
        ...
        "compact_meta_schema": sha256_file(compact_schema_path),
        "stage_d_model_manifest": sha256_file(stage_d_manifest_path),
        "formation_current_test_parquet": sha256_file(formation_path),
        "formation_current_test_logical": logical_feature_content_sha256(
            formation_surface, formation_features
        ),
        "compact_parquet_by_outer": compact_sha,
    ...
    "notes": [
        "All 12 exp263 candidates and 21 native-confidence columns are regenerated from raw test in this run.",
        ...
        "Each exp287 model receives compact features from its matching Stage C downstream outer fold.",
        "Current-test formation features use all train wells as reference and never read target formation columns.",
        "All 40 saved selectors and 15 saved exp287 TVT models are SHA-verified; no model is fitted.",
        ...
        "submission.csv is generated for submit-check, but external competition submission is not authorized by this run.",
        "The pre-registered exp287 worst-well and worsened-well-count guard failures remain recorded and are not reclassified as a pass.",
    ],
}
