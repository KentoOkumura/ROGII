...
# %% [markdown]
# # exp492 Huber exact-HMM full replacement on exp264 — train
#
# exp264の固定12候補ID・順序・legal domainを変えず、Gaussian `exact_hmm`
# semantic slotをSHA固定済みexp389 Huber exact HMMへ全面置換する。
...
EXPERIMENT_NAME = "exp492_huber_exact_hmm_full_replacement_on_exp264"
...
EXECUTE_NOTEBOOK = (
    os.environ.get("EXP492_IMPORT_ONLY", "0") != "1"
...
# ## 3. Authorization, candidate, and compute contract
#
# 実装承認とKaggle実行承認を分離する。この候補Notebookは、`run_approved`
# と固定scopeが同時に設定されるまで40 booster学習を開始しない。
...
expected_approval_scope = (
    "fixed12_huber_replacement_stage_a_stage_c_"
...
if EXECUTE_NOTEBOOK:
    if not bool(CONFIG["authorization"]["kaggle_run_approved"]):
        raise RuntimeError(
            "exp492 Kaggle train is not authorized. Confirm 1 variant / "
    ...
    if not bool(CONFIG["execution"]["run_approved"]):
        raise RuntimeError("exp492 execution.run_approved is false")
    if CONFIG["execution"].get("approved_scope") != expected_approval_scope:
        raise RuntimeError("exp492 approved scope differs from the frozen cost")

# %% [markdown]
# ## 4. Frozen inputs and SHA checks
#
# exp389はallowlist 6列だけを読み、gzip decompressed SHAとpost-read logical SHAを
...
if EXECUTE_NOTEBOOK:
    ...
    input_paths = {
        ...
        "parent_runtime_contract": {
            "source": "exp492_frozen_copy_of_exp264_stage_c_settings",
            "parent_exp264_config_sha256": parent_spec["config_sha256"],
        },
...
# ## 5. Fixed12 replacement cache and Stage A schema rebuild
#
# Huber予測をglobal `(well_id,row_idx)` joinしてからexp263 outer foldへ再分割する。
...
# %%
if EXECUTE_NOTEBOOK:
    replacement_cache = Exp389Fixed12ReplacementCache(
        cache_root,
        CONTRACT,
        exp389_predictions=replacement_predictions,
        exp389_manifest=replacement_manifest,
    )
    ...
    write_exp492_input_contract(
        OUTPUT_DIR / "exp492_input_contract.json",
        config=CONFIG,
        contract=CONTRACT,
...
if EXECUTE_NOTEBOOK:
    ...
    if not all(technical_checks.values()):
        raise RuntimeError(
            f"exp492 technical checks failed: {technical_checks}"
        )
    display(
        {
            "stage_c": stage_c,
...
# Huber依存4候補の利用率とfixed fallback 7候補はreport-onlyとする。
...
if EXECUTE_NOTEBOOK:
    ...
    decision = (
        "PASS_FIXED12_HUBER_REPLACEMENT_SELECTOR"
        if bool(gate["passed"])
        else "FAIL_CLOSE_FIXED12_HUBER_REPLACEMENT_SELECTOR"
    )
...
if EXECUTE_NOTEBOOK:
    ...
    importance_mean.to_csv(
        OUTPUT_DIR / "exp492_feature_importance_mean.csv", index=False
    )
    for objective, part in importance_mean.groupby("objective", sort=True):
        ...
        ax = top.plot.barh(
            ...
            figsize=(10, 11),
            legend=False,
            title=f"exp492 {objective} mean gain importance",
        )
        ax.set_xlabel("mean gain across outer/inner models")
        plt.tight_layout()
        plt.savefig(
            OUTPUT_DIR / f"exp492_{objective}_feature_importance_top30.png",
            dpi=140,
        )
        plt.show()
...
if EXECUTE_NOTEBOOK:
    ...
    write_json(OUTPUT_DIR / "exp492_summary.json", summary)
    ...
    reproducibility.update(
        {
            ...
            "exp492_summary_sha256": sha256_file(
                OUTPUT_DIR / "exp492_summary.json"
            ),
        }
    )
