...
EXPERIMENT_NAME = "exp223_joint_typewell_self_gr_hmm_likelihood_probe"
...
def compute_self_gr_signal_buckets(
    hmm: pd.DataFrame,
    predictions: dict[str, np.ndarray],
    true_tvt: np.ndarray,
    baselines: dict[str, np.ndarray],
    ...
    signals = [
        column
        for column in ["self_gr_quality", "self_gr_typewell_agreement", "self_gr_peak_gap"]
        if column in hmm.columns
    ]
    rows: list[dict[str, Any]] = []
    if not signals:
        return pd.DataFrame(rows)
    hmm_candidates = [name for name in predictions if name.startswith("hmm_selfgr")]
    for signal in signals:
        values = pd.to_numeric(hmm[signal], errors="coerce").to_numpy(np.float64)
        finite = np.isfinite(values)
        if int(finite.sum()) < max(10, n_bins):
            continue
        if np.unique(values[finite]).size < 2:
            continue
        ranks = pd.Series(values[finite]).rank(method="first")
        bins = pd.qcut(ranks, q=n_bins, labels=False, duplicates="drop").to_numpy()
        finite_indices = np.flatnonzero(finite)
        for bin_id in sorted(set(int(v) for v in bins)):
            idx = finite_indices[bins == bin_id]
            for name in hmm_candidates:
                ...
                row = metric_row(
                    ...
                    pred[idx],
                    int(len(idx)),
                    {base_name: base_pred[idx] for base_name, base_pred in baselines.items()},
                )
                row["signal"] = signal
                row["signal_bin"] = int(bin_id)
                row["signal_min"] = float(np.min(values[idx]))
                row["signal_mean"] = float(np.mean(values[idx]))
                row["signal_max"] = float(np.max(values[idx]))
                rows.append(row)
    return pd.DataFrame(rows)
...
def run_direct_comparison(
    *,
    ...
    output_prefix = str(comparison.get("output_prefix") or "exp223_joint_typewell_self_gr_hmm_likelihood_probe")
    ...
    if hmm_frame is None:
        ...
        hmm_source = hmm_source or "in_memory_self_gr_emission_hmm_cache"
    ...
    if not baselines:
        for fallback_name in ("exp072_likpf_mean",):
    ...
    self_gr_signal_buckets = compute_self_gr_signal_buckets(
        hmm,
        predictions,
        true_tvt,
        baselines,
        int(comparison.get("self_gr_signal_bins", 5)),
    )
    ...
    for name, pred in predictions.items():
        if name.startswith("hmm_selfgr"):
            enriched[name] = pred
    for column in ["self_gr_quality", "self_gr_peak_tvt", "self_gr_peak_gap", "self_gr_typewell_agreement", "self_gr_valid"]:
        if column in hmm.columns:
            enriched[column] = hmm[column]
    ...
    self_gr_signal_path = paths.artifacts_dir / f"{output_prefix}_self_gr_signal_bucket_metrics.csv"
    ...
    self_gr_signal_buckets.to_csv(self_gr_signal_path, index=False)

    best = overall.iloc[0].to_dict() if len(overall) else {}
    hmm_overall = overall[overall["candidate"].astype(str).str.startswith("hmm_selfgr")]
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "self_gr_signal_bucket_metrics": str(self_gr_signal_path),
        ...
        "sha256": {
            ...
            "self_gr_signal_bucket_metrics": sha256_path(self_gr_signal_path),
        },
    }
