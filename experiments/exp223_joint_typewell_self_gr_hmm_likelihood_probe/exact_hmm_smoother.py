...
OUTPUT_PREFIX = "exp223_joint_typewell_self_gr_hmm_likelihood_probe"
VARIANT = "joint_typewell_self_gr_hmm_likelihood_probe"
...
def self_gr_variant_name(alpha: float, clip_value: float, mode: str) -> str:
    alpha_token = f"a{int(round(float(alpha) * 1000)):03d}"
    clip_token = f"c{int(round(float(clip_value) * 100)):03d}"
    return f"hmm_selfgr_{sanitize_token(mode)}_{alpha_token}_{clip_token}"
    ...
    for alpha in alpha_values:
        for clip_value in clip_values:
            for mode in modes:
                ...
                name = self_gr_variant_name(alpha, clip_value, mode)
                variant = {
                    "name": name,
                    "alpha": float(alpha),
...
def run_train_feature_cache(
    *,
    ...
    summary = {
        ...
        "mode": "joint_typewell_self_gr_hmm_likelihood_probe_train_feature_cache",
