# %% [markdown]
# # exp404 paired scale-5 GR likelihood-PF ablation inference
#
...
def validate_inference_is_disabled(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if status["experiment"] != EXPERIMENT_NAME or status["route"] != "pf_beam":
        raise ValueError(f"Unexpected exp404 inference contract: {status}")
    enabled = [
        key
        for key in (
            "inference_enabled",
    ...
    if enabled:
        raise ValueError(
            "exp404 inference cannot be enabled before all train-side gates PASS "
...
if __name__ == "__main__":
    ...
    raise RuntimeError(
        "exp404 inference is fail-closed: train-side gates have not run and "
