...
EXPERIMENT_NAME = "exp209_exp072_exp205_joint_exact_parity_fast_cache_generation"
...
def run_train_feature_cache(
    *,
    ...
    summary = {
        ...
        "reference_experiment": "exp072_exp063_full_replay_feature_cache",
    ...
    if return_frame:
        if return_columns is not None:
            missing_return = [col for col in return_columns if col not in train_frame.columns]
            if missing_return:
                raise ValueError(f"Requested return_columns are missing: {missing_return[:20]}")
            return summary, train_frame.loc[:, return_columns].copy()
        return summary, train_frame
    return summary
