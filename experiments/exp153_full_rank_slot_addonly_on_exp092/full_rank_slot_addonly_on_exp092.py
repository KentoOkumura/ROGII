...
EXPERIMENT_NAME = "exp153_full_rank_slot_addonly_on_exp092"
...
def run_full_rank_slot_addonly_on_exp092(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    if rank_slot_config.get("include_lgb_oof_features", False):
        raise NotImplementedError("LGB OOF rank-slot features are disabled for this experiment")
    ...
    manifest = {
        ...
        "mode": "full_rank_slot_addonly_on_exp092_from_exp072_cache",
...
def run_saved_model_inference(
    *,
    ...
    projection_config: dict[str, Any] | None = None,
    rank_slot_config: dict[str, Any] | None = None,
    variant_name: str = "u_projection_full_rank_slot_addonly",
    ...
    if projection_config.get("include_lgb_oof_features", False):
        raise NotImplementedError(
            "LGB OOF U-projection features are disabled for this experiment inference"
        )
    rank_slot_config = rank_slot_config or dict(manifest.get("rank_slot_config") or {})
    if rank_slot_config.get("include_lgb_oof_features", False):
        raise NotImplementedError(
            "LGB OOF rank-slot features are disabled for this experiment inference"
        )
    ...
    summary = {
        ...
        "known_followup_risk": (
            ...
            "checks support the full rank-slot add-only merge."
        ),
        "elapsed_seconds": round(time.time() - t0, 3),
    }
...
run_u_projection_feature_ablation = run_full_rank_slot_addonly_on_exp092
