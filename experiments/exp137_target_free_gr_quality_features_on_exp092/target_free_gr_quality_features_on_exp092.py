...
OUTPUT_PREFIX = "exp137_target_free_gr_quality_features_on_exp092"
...
EXP065_NATIVE_PAIRS = "typewell_native_overlap_pairs.csv"
...
def find_exp065_artifact(filename: str, explicit_path: str | Path | None = None) -> Path | None:
    candidates: list[Path] = []
    if explicit_path is not None:
        candidates.append(Path(explicit_path))
    candidates.extend(
        [
            EXP065_ARTIFACTS / filename,
            Path.cwd() / filename,
            Path.cwd() / "artifacts" / filename,
        ]
    )
...
def _run_lengths(mask: np.ndarray) -> np.ndarray:
    mask = np.asarray(mask, dtype=bool)
    lengths = np.zeros(mask.shape[0], dtype=np.float32)
    start = None
    for idx, value in enumerate(mask):
        ...
        if start is not None and (not value or idx == len(mask) - 1):
            ...
            lengths[start:end] = float(end - start)
            start = None
    return lengths


def _nearest_finite_gap(md: np.ndarray, finite: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    md = np.asarray(md, dtype=np.float64)
    finite = np.asarray(finite, dtype=bool)
    gap = np.full(len(md), np.nan, dtype=np.float32)
    bracketed = np.zeros(len(md), dtype=np.float32)
    finite_idx = np.flatnonzero(finite & np.isfinite(md))
    if len(finite_idx) == 0:
        return np.full(len(md), 1e6, dtype=np.float32), bracketed
    for pos in range(len(md)):
        if finite[pos]:
            gap[pos] = 0.0
            bracketed[pos] = 1.0
            continue
        insert = np.searchsorted(finite_idx, pos)
        left = finite_idx[insert - 1] if insert > 0 else None
        right = finite_idx[insert] if insert < len(finite_idx) else None
        distances: list[float] = []
        if left is not None:
            distances.append(abs(float(md[pos] - md[left])))
        if right is not None:
            distances.append(abs(float(md[right] - md[pos])))
        gap[pos] = min(distances) if distances else 1e6
        bracketed[pos] = 1.0 if left is not None and right is not None else 0.0
    return np.nan_to_num(gap, nan=1e6, posinf=1e6, neginf=1e6), bracketed


def _robust_median_shift(left: np.ndarray, right: np.ndarray) -> tuple[float, float]:
    ...
    denom = max(float(left_q75 - left_q25), 1.0)
    median_shift = abs(float(np.median(right) - np.median(left))) / denom
    scale_ratio = float(np.std(right) / max(np.std(left), 1.0))
    return float(median_shift), float(np.clip(scale_ratio, 0.0, 20.0))


def _load_exp065_group_context(
    wells: pd.Series,
    config: dict[str, Any],
) -> tuple[pd.DataFrame, dict[str, Any]]:
    wells_frame = pd.DataFrame({"well": sorted(set(map(str, wells)))})
    assignments_path = find_exp065_artifact(
        EXP065_ASSIGNMENTS,
        config.get("exp065_assignments_path"),
    )
    pairs_path = find_exp065_artifact(EXP065_NATIVE_PAIRS, config.get("exp065_native_pairs_path"))
    group_methods = list(
        config.get("group_methods")
        or [
            {"name": "native_overlap_0p999", "method": "native_overlap", "threshold": "0.999"},
            {"name": "native_overlap_1", "method": "native_overlap", "threshold": "1"},
            {"name": "exact_hash", "method": "exact_hash", "threshold": "byte_hash"},
            {"name": "shifted_ncc_0p98", "method": "shifted_ncc", "threshold": "0.98"},
            {"name": "dtw_0p94", "method": "dtw", "threshold": "0.94"},
        ]
    )
    context = wells_frame.copy()
    metadata: dict[str, Any] = {
        "assignments_path": str(assignments_path) if assignments_path else None,
        "assignments_sha256": sha256_file(assignments_path) if assignments_path else None,
        "pairs_path": str(pairs_path) if pairs_path else None,
        "pairs_sha256": sha256_file(pairs_path) if pairs_path else None,
        "group_methods": group_methods,
    }
    if assignments_path is None:
        for method in group_methods:
            name = str(method["name"])
            context[f"grq_{name}_cluster_size"] = 1.0
            context[f"grq_{name}_multiwell"] = 0.0
        metadata["loaded_assignments"] = False
    else:
        assignments = pd.read_csv(assignments_path, dtype=str)
        assignments["cluster_size"] = pd.to_numeric(
            assignments["cluster_size"],
            errors="coerce",
        ).fillna(1)
        metadata["loaded_assignments"] = True
        metadata["assignment_rows"] = int(len(assignments))
        for method in group_methods:
            name = str(method["name"])
            method_name = str(method["method"])
            threshold = str(method["threshold"])
            subset = assignments[
                assignments["method"].astype(str).eq(method_name)
                & assignments["threshold"].astype(str).eq(threshold)
            ][["well_id", "cluster_id", "cluster_size"]].copy()
            subset = subset.rename(columns={"well_id": "well"})
            subset[f"grq_{name}_cluster_size"] = pd.to_numeric(
                subset["cluster_size"],
                errors="coerce",
            ).fillna(1.0)
            subset[f"grq_{name}_multiwell"] = (
                subset[f"grq_{name}_cluster_size"].to_numpy(np.float32) > 1
            ).astype(np.float32)
            context = context.merge(
                subset[["well", f"grq_{name}_cluster_size", f"grq_{name}_multiwell"]],
                on="well",
                how="left",
                validate="one_to_one",
            )
            context[f"grq_{name}_cluster_size"] = (
                pd.to_numeric(context[f"grq_{name}_cluster_size"], errors="coerce")
                .fillna(1.0)
                .astype(np.float32)
            )
            context[f"grq_{name}_multiwell"] = (
                pd.to_numeric(context[f"grq_{name}_multiwell"], errors="coerce")
                .fillna(0.0)
                .astype(np.float32)
            )
    if pairs_path is None:
        context["grq_native_overlap_pair_count"] = 0.0
        context["grq_native_overlap_max_exact_match_rate"] = 0.0
        context["grq_native_overlap_max_containment"] = 0.0
        metadata["loaded_native_pairs"] = False
    else:
        pairs = pd.read_csv(pairs_path)
        pair_rows: list[dict[str, Any]] = []
        for side in ["a", "b"]:
            well_col = f"well_id_{side}"
            if well_col not in pairs.columns:
                continue
            temp = pairs[[well_col, "exact_match_rate", "overlap_fraction_shorter"]].copy()
            temp = temp.rename(columns={well_col: "well"})
            pair_rows.extend(temp.to_dict("records"))
        if pair_rows:
            pair_df = pd.DataFrame(pair_rows)
            pair_df["exact_match_rate"] = pd.to_numeric(
                pair_df["exact_match_rate"],
                errors="coerce",
            ).fillna(0.0)
            pair_df["overlap_fraction_shorter"] = pd.to_numeric(
                pair_df["overlap_fraction_shorter"],
                errors="coerce",
            ).fillna(0.0)
            pair_summary = (
                pair_df.groupby("well", as_index=False)
                .agg(
                    grq_native_overlap_pair_count=("well", "size"),
                    grq_native_overlap_max_exact_match_rate=("exact_match_rate", "max"),
                    grq_native_overlap_max_containment=("overlap_fraction_shorter", "max"),
                )
                .astype(
                    {
                        "grq_native_overlap_pair_count": np.float32,
                        "grq_native_overlap_max_exact_match_rate": np.float32,
                        "grq_native_overlap_max_containment": np.float32,
                    }
                )
            )
            context = context.merge(pair_summary, on="well", how="left", validate="one_to_one")
        else:
            context["grq_native_overlap_pair_count"] = 0.0
            context["grq_native_overlap_max_exact_match_rate"] = 0.0
            context["grq_native_overlap_max_containment"] = 0.0
        for col in [
            "grq_native_overlap_pair_count",
            "grq_native_overlap_max_exact_match_rate",
            "grq_native_overlap_max_containment",
        ]:
            context[col] = (
                pd.to_numeric(context[col], errors="coerce").fillna(0.0).astype(np.float32)
            )
        metadata["loaded_native_pairs"] = True
        metadata["native_pair_rows"] = int(len(pairs))
    return context, metadata


def build_target_free_gr_quality_features(
    frame: pd.DataFrame,
    train_dir: str | Path,
    config: dict[str, Any] | None = None,
) -> tuple[pd.DataFrame, dict[str, list[str]], pd.DataFrame]:
    config = config or {}
    prefix = str(config.get("prefix") or "grq_")
    min_md_scale = float(config.get("min_md_scale", 25.0))
    key_cols = ["id", "well"]
    group_columns: dict[str, list[str]] = {
        "target_free_gr_quality": [],
    }
    ...
    features[f"{prefix}md_since_norm"] = s_norm
    features[f"{prefix}tail_md_scale"] = tail_md_scale
    group_columns["target_free_gr_quality"].extend(
        [
            f"{prefix}md_since_norm",
            f"{prefix}tail_md_scale",
        ]
    )

    summary_rows: list[dict[str, Any]] = []
    train_dir = Path(train_dir)
    row_index = _tail_rank(frame["id"])
    ...
    per_row_parts: list[pd.DataFrame] = []
    for well, subset in row_features.groupby("well", sort=True):
        path = train_dir / f"{well}__horizontal_well.csv"
        if not path.exists():
            raise FileNotFoundError(f"raw train horizontal well not found for GR quality: {path}")
        raw = pd.read_csv(path, usecols=["MD", "GR", "TVT_input"])
        md = pd.to_numeric(raw["MD"], errors="coerce").to_numpy(np.float64)
        gr = pd.to_numeric(raw["GR"], errors="coerce").to_numpy(np.float64)
        tvt_input = pd.to_numeric(raw["TVT_input"], errors="coerce").to_numpy(np.float64)
        prefix_mask = np.isfinite(tvt_input)
        if not prefix_mask.any():
            raise ValueError(f"No finite TVT_input prefix rows for well {well}")
        first_eval = int(np.flatnonzero(prefix_mask)[-1]) + 1
        eval_mask = np.arange(len(raw)) >= first_eval
        finite_gr = np.isfinite(gr)
        prefix_gr = gr[prefix_mask]
        eval_gr = gr[eval_mask]
        prefix_finite = np.isfinite(prefix_gr)
        eval_finite = np.isfinite(eval_gr)
        missing_run_len = _run_lengths(~finite_gr)
        nearest_gap, bracketed = _nearest_finite_gap(md, finite_gr)
        median_shift, scale_ratio = _robust_median_shift(prefix_gr, eval_gr)
        prefix_missing_rate = 1.0 - float(prefix_finite.mean()) if len(prefix_finite) else 1.0
        eval_missing_rate = 1.0 - float(eval_finite.mean()) if len(eval_finite) else 1.0
        full_missing_rate = 1.0 - float(finite_gr.mean()) if len(finite_gr) else 1.0
        known_prefix_tail_gap_rows = int(len(raw) - 1 - np.flatnonzero(prefix_mask)[-1])

        safe_index = np.clip(subset["_row_index"].to_numpy(np.int64), 0, len(raw) - 1)
        part = subset[["id", "well"]].copy()
        part[f"{prefix}row_gr_missing"] = (~finite_gr[safe_index]).astype(np.float32)
        part[f"{prefix}row_missing_run_len"] = missing_run_len[safe_index].astype(np.float32)
        part[f"{prefix}row_missing_run_len_norm"] = (
            missing_run_len[safe_index] / np.maximum(tail_md_scale[subset.index.to_numpy()], 1.0)
        ).astype(np.float32)
        part[f"{prefix}row_nearest_finite_gr_gap_md"] = nearest_gap[safe_index].astype(np.float32)
        part[f"{prefix}row_nearest_finite_gr_gap_norm"] = (
            nearest_gap[safe_index] / np.maximum(tail_md_scale[subset.index.to_numpy()], 1.0)
        ).astype(np.float32)
        part[f"{prefix}row_gr_bracketed_by_finite"] = bracketed[safe_index].astype(np.float32)
        per_row_parts.append(part)
        summary_rows.append(
            {
                "well": well,
                ...
                "prefix_gr_coverage": float(1.0 - prefix_missing_rate),
                "eval_gr_coverage": float(1.0 - eval_missing_rate),
                "full_gr_coverage": float(1.0 - full_missing_rate),
                "prefix_eval_gr_median_shift_norm": median_shift,
                "prefix_eval_gr_scale_ratio": scale_ratio,
            }
        )
        well_rows.append(
            {
                "well": well,
                f"{prefix}prefix_gr_missing_rate": prefix_missing_rate,
                f"{prefix}eval_gr_missing_rate": eval_missing_rate,
                f"{prefix}full_gr_missing_rate": full_missing_rate,
                f"{prefix}prefix_eval_missing_rate_gap": abs(
                    eval_missing_rate - prefix_missing_rate
                ),
                f"{prefix}prefix_gr_missing_run_max": float(
                    _max_true_run(~finite_gr & prefix_mask)
                ),
                f"{prefix}eval_gr_missing_run_max": float(_max_true_run(~finite_gr & eval_mask)),
                f"{prefix}prefix_eval_gr_median_shift_norm": median_shift,
                f"{prefix}prefix_eval_gr_scale_ratio": scale_ratio,
                f"{prefix}known_prefix_tail_gap_rows": float(known_prefix_tail_gap_rows),
                f"{prefix}eval_has_no_gr": float(not eval_finite.any()),
                f"{prefix}prefix_has_no_gr": float(not prefix_finite.any()),
            }
        )

    row_quality = pd.concat(per_row_parts, ignore_index=True)
    well_quality = pd.DataFrame(well_rows)
    group_context, group_meta = _load_exp065_group_context(features["well"], config)
    features = features.merge(well_quality, on="well", how="left", validate="many_to_one")
    features = features.merge(row_quality, on=["id", "well"], how="left", validate="one_to_one")
    features = features.merge(group_context, on="well", how="left", validate="many_to_one")
    quality_cols = [col for col in features.columns if col not in key_cols]
    for col in quality_cols:
        if col.startswith(prefix) and col not in group_columns["target_free_gr_quality"]:
            group_columns["target_free_gr_quality"].append(col)
    ...
    summary_rows.append(
        {
            ...
            "prefix_rows": None,
            "eval_rows": None,
            "prefix_gr_coverage": float(1.0 - features[f"{prefix}prefix_gr_missing_rate"].mean()),
            "eval_gr_coverage": float(1.0 - features[f"{prefix}eval_gr_missing_rate"].mean()),
            "full_gr_coverage": float(1.0 - features[f"{prefix}full_gr_missing_rate"].mean()),
            "prefix_eval_gr_median_shift_norm": float(
                features[f"{prefix}prefix_eval_gr_median_shift_norm"].mean()
            ),
            "prefix_eval_gr_scale_ratio": float(
                features[f"{prefix}prefix_eval_gr_scale_ratio"].mean()
            ),
            "generated_features": int(len(group_columns["target_free_gr_quality"])),
            "rows": int(len(features)),
            "wells": int(features["well"].nunique()),
            "exp065_assignments_path": group_meta.get("assignments_path"),
            "exp065_assignments_sha256": group_meta.get("assignments_sha256"),
            "exp065_native_pairs_path": group_meta.get("pairs_path"),
            "exp065_native_pairs_sha256": group_meta.get("pairs_sha256"),
            "exp065_loaded_assignments": group_meta.get("loaded_assignments"),
            "exp065_loaded_native_pairs": group_meta.get("loaded_native_pairs"),
            "exp065_group_methods": json.dumps(group_meta.get("group_methods", [])),
        }
    )
    ...
    if not np.isfinite(features[feature_cols].to_numpy(np.float32)).all():
        raise ValueError("target-free GR quality feature frame contains non-finite values")
...
def _fit_one_variant_mode(
    *,
    ...
    active_model_indices: list[int] | tuple[int, ...] | None,
    save_models: bool,
    ...
    if active_model_indices is not None:
        selected_configs: list[dict[str, Any]] = []
        for index in active_model_indices:
            model_index = int(index)
            if model_index < 0 or model_index >= len(configs):
                raise ValueError(
                    f"active_model_indices contains {model_index}, "
                    f"but only {len(configs)} configs are available"
                )
            selected_configs.append(configs[model_index])
...
def run_target_free_gr_quality_features_on_exp092(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    active_model_indices: list[int] | tuple[int, ...] | None = None,
    ...
    diagnostic_features, diagnostic_group_columns, diagnostic_summary = (
        build_target_free_gr_quality_features(
            full_frame,
            train_dir,
            diagnostic_config or {},
        )
    )
    diagnostic_feature_columns = [
        col for col in diagnostic_features.columns if col not in {"id", "well"}
    ]
    ...
    if full_frame.empty:
        raise ValueError("No rows after merging target-free GR quality features")
    diagnostic_meta = {
        "rows": int(len(diagnostic_features)),
        "wells": int(diagnostic_features["well"].nunique()),
        "generated_features": int(len(diagnostic_feature_columns)),
    ...
    diagnostic_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_diagnostic_feature_summary.csv",
        index=False,
    )
            ...
            feature_schema_rows.append(
                {
                    ...
                    "is_target_free_gr_quality_feature": bool(
                        feature in diagnostic_feature_columns
                    ),
                }
            )
    ...
    manifest = {
        "experiment": "exp137_target_free_gr_quality_features_on_exp092",
        "parent": "exp092_u_projection_correction_disagreement_fullrun",
        "cache_parent": "exp072_exp063_full_replay_feature_cache",
        "mode": "target_free_gr_quality_features_on_exp092_on_exp092_surface",
        "feature_source": feature_meta,
        "diagnostic_feature_source": diagnostic_meta,
        "anchor_source": {
        ...
        "diagnostic_feature_groups": diagnostic_group_columns,
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "diagnostic_feature_summary": f"{OUTPUT_PREFIX}_diagnostic_feature_summary.csv",
