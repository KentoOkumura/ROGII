...
# ---
# %% [markdown]
# # exp192_typewell_late_range_hard_window_pct50_full_cache_replacement train
#
...
if hard_window["name"] != "pct50":
    raise ValueError(f"exp192 runs only pct50, got {hard_window['name']}")
if not hard_window["enabled"]:
    raise ValueError("exp192 hard-window filtering must be enabled")
if float(hard_window["min_typewell_pct"]) != 0.50:
    raise ValueError("exp192 must use min_typewell_pct == 0.50")
...
# %%
schema_path = paths.artifacts_dir / "exp192_typewell_late_range_hard_window_pct50_full_cache_replacement_full_replay_cache_feature_schema.csv"
summary_path = paths.artifacts_dir / "exp192_typewell_late_range_hard_window_pct50_full_cache_replacement_full_replay_cache_summary.json"
feature_path = (
    paths.artifacts_dir
    / "exp192_typewell_late_range_hard_window_pct50_full_cache_replacement_full_replay_cache_pixiux_likpf_hard_window_pct50_public_replay_train_features.csv.gz"
)
