...
OUTPUT_PREFIX = "exp130_pfbeam_normalized_diagnostic_score"
...
def _candidate_shape_metrics(
    ...
    for _, idx in frame.groupby("well", sort=False).indices.items():
        ...
        if len(ordered_idx) < 2 or np.ptp(x) <= 1e-8:
            continue
        local_slope = np.gradient(y, x, edge_order=1).astype(np.float32)
        if len(ordered_idx) >= 3:
            local_curvature = np.gradient(local_slope.astype(np.float64), x, edge_order=1).astype(
                np.float32
            )
        else:
...
def build_pfbeam_normalized_diagnostic_features(
    frame: pd.DataFrame,
    config: dict[str, Any] | None = None,
) -> tuple[pd.DataFrame, dict[str, list[str]], pd.DataFrame]:
    config = config or {}
    prefix = str(config.get("prefix") or "pfbn_")
    candidate_specs = {
        str(name): dict(spec)
        for name, spec in (config.get("candidates") or {}).items()
        if dict(spec).get("enabled", True)
    }
    if len(candidate_specs) < 2:
        raise ValueError("pfbeam normalized diagnostic requires at least two candidates")

    min_md_scale = float(config.get("min_md_scale", 25.0))
    min_u_scale = float(config.get("min_u_scale", 10.0))
    score_temperature = float(config.get("score_temperature", 0.65))
    high_threshold = float(config.get("high_confidence_threshold", 0.70))
    low_threshold = float(config.get("low_confidence_threshold", 0.35))
    key_cols = ["id", "well"]
    group_columns: dict[str, list[str]] = {
        "pfbeam_normalized_diagnostic": [],
        "pfbeam_normalized_candidate_shape": [],
        "pfbeam_normalized_disagreement": [],
    }
    ...
    candidate_u: dict[str, np.ndarray] = {}
    for name, spec in candidate_specs.items():
        tvt = _source_tvt(frame, name, spec)
        candidate_tvt[name] = tvt.astype(np.float32)
        candidate_u[name] = (tvt + z - u0).astype(np.float32)

    u_matrix_raw = np.column_stack([candidate_u[name] for name in candidate_specs]).astype(
        np.float32
    )
    row_abs_u = np.max(np.abs(u_matrix_raw), axis=1)
    row_range_u = np.max(u_matrix_raw, axis=1) - np.min(u_matrix_raw, axis=1)
    z_rel = np.abs(z - frame["anchor_z0"].to_numpy(np.float32))
    scale_source = np.maximum.reduce(
        [row_abs_u, row_range_u, z_rel, np.full(len(frame), min_u_scale)]
    )
    u_scale = (
        pd.DataFrame({"well": frame["well"], "scale": scale_source})
        .groupby("well")["scale"]
        .transform(lambda value: float(np.nanquantile(value, 0.90)))
        .to_numpy(np.float32)
    )
    u_scale = np.maximum(u_scale, min_u_scale).astype(np.float32)
    prefix_u_slope_norm = (
        frame["prefix_u_slope_per_md"].to_numpy(np.float32) * tail_md_scale / u_scale
    ).astype(np.float32)
    prefix_line = (prefix_u_slope_norm * s_norm).astype(np.float32)

    features[f"{prefix}md_since_norm"] = s_norm
    features[f"{prefix}tail_md_scale"] = tail_md_scale
    features[f"{prefix}u_scale"] = u_scale
    features[f"{prefix}prefix_u_slope_norm"] = prefix_u_slope_norm
    features[f"{prefix}prefix_u_roughness_norm"] = (
        frame["prefix_u_roughness"].to_numpy(np.float32) / u_scale
    ).astype(np.float32)
    group_columns["pfbeam_normalized_diagnostic"].extend(
        [
            f"{prefix}md_since_norm",
            f"{prefix}tail_md_scale",
            f"{prefix}u_scale",
            f"{prefix}prefix_u_slope_norm",
            f"{prefix}prefix_u_roughness_norm",
        ]
    )

    candidate_u_norm: dict[str, np.ndarray] = {}
    candidate_losses: list[np.ndarray] = []
    summary_rows: list[dict[str, Any]] = []
    for name in candidate_specs:
        u_norm = (candidate_u[name] / u_scale).astype(np.float32)
        candidate_u_norm[name] = u_norm
        slope, curvature, roughness = _candidate_shape_metrics(frame, u_norm, s_norm)
        prefix_resid = (u_norm - prefix_line).astype(np.float32)
        abs_prefix_resid = np.abs(prefix_resid).astype(np.float32)
        abs_curvature = np.abs(curvature).astype(np.float32)
        shape_loss = (
            abs_prefix_resid
            + 0.50 * np.abs(roughness).astype(np.float32)
            + 0.25 * abs_curvature
        ).astype(np.float32)
        candidate_losses.append(shape_loss)

        for suffix, values in [
            ("u_norm", u_norm),
            (
                "minus_last_norm",
                (candidate_tvt[name] - frame["last_known_tvt"].to_numpy(np.float32)) / u_scale,
            ),
            ("slope_norm", slope),
            ("curvature_norm", curvature),
            ("roughness_norm", roughness),
            ("prefix_resid_norm", prefix_resid),
            ("abs_prefix_resid_norm", abs_prefix_resid),
            ("shape_loss", shape_loss),
        ]:
            col = f"{prefix}{name}_{suffix}"
            features[col] = np.asarray(values, dtype=np.float32)
            group_columns["pfbeam_normalized_candidate_shape"].append(col)

        summary_rows.append(
            {
                "candidate": name,
                "u_norm_abs_mean": float(np.mean(np.abs(u_norm))),
                "prefix_resid_abs_mean": float(np.mean(abs_prefix_resid)),
                "roughness_mean": float(np.mean(np.abs(roughness))),
                "curvature_abs_mean": float(np.mean(abs_curvature)),
                "shape_loss_mean": float(np.mean(shape_loss)),
            }
        )

    candidate_names = list(candidate_specs)
    u_norm_matrix = np.column_stack([candidate_u_norm[name] for name in candidate_names]).astype(
        np.float32
    )
    loss_matrix = np.column_stack(candidate_losses).astype(np.float32)
    score_matrix = np.exp(-np.clip(loss_matrix, 0.0, 40.0)).astype(np.float32)
    prob_matrix = _softmax(-loss_matrix, score_temperature)
    sorted_scores = np.sort(score_matrix, axis=1)

    for idx, name in enumerate(candidate_names):
        score_col = f"{prefix}{name}_shape_score"
        prob_col = f"{prefix}{name}_shape_prob"
        features[score_col] = score_matrix[:, idx].astype(np.float32)
        features[prob_col] = prob_matrix[:, idx].astype(np.float32)
        group_columns["pfbeam_normalized_diagnostic"].extend([score_col, prob_col])

    for left_i, left in enumerate(candidate_names):
        for right in candidate_names[left_i + 1 :]:
            diff = (candidate_u_norm[left] - candidate_u_norm[right]).astype(np.float32)
            diff_col = f"{prefix}diff_{left}_minus_{right}_u_norm"
            abs_col = f"{prefix}absdiff_{left}_{right}_u_norm"
            features[diff_col] = diff
            features[abs_col] = np.abs(diff).astype(np.float32)
            group_columns["pfbeam_normalized_disagreement"].extend([diff_col, abs_col])

    features[f"{prefix}candidate_u_norm_std"] = np.std(u_norm_matrix, axis=1).astype(np.float32)
    features[f"{prefix}candidate_u_norm_range"] = (
        np.max(u_norm_matrix, axis=1) - np.min(u_norm_matrix, axis=1)
    ).astype(np.float32)
    features[f"{prefix}shape_loss_mean"] = np.mean(loss_matrix, axis=1).astype(np.float32)
    features[f"{prefix}shape_loss_min"] = np.min(loss_matrix, axis=1).astype(np.float32)
    features[f"{prefix}shape_loss_margin"] = (
        np.partition(loss_matrix, kth=1, axis=1)[:, 1] - np.min(loss_matrix, axis=1)
    ).astype(np.float32)
    features[f"{prefix}shape_score_top1"] = sorted_scores[:, -1].astype(np.float32)
    features[f"{prefix}shape_score_top1_margin"] = (
        sorted_scores[:, -1] - sorted_scores[:, -2]
    ).astype(np.float32)
    entropy = -np.sum(prob_matrix * np.log(np.maximum(prob_matrix, 1e-8)), axis=1)
    features[f"{prefix}shape_score_entropy"] = entropy.astype(np.float32)

    instability = np.mean(
        np.column_stack(
            [
                _rank01(features[f"{prefix}candidate_u_norm_range"].to_numpy(np.float32)),
                _rank01(features[f"{prefix}candidate_u_norm_std"].to_numpy(np.float32)),
                _rank01(features[f"{prefix}shape_loss_mean"].to_numpy(np.float32)),
                _rank01(features[f"{prefix}shape_score_entropy"].to_numpy(np.float32)),
                1.0 - _rank01(features[f"{prefix}shape_score_top1_margin"].to_numpy(np.float32)),
            ]
        ),
        axis=1,
    ).astype(np.float32)
    diagnostic_score = (1.0 - instability).astype(np.float32)
    features[f"{prefix}instability_score"] = instability
    features[f"{prefix}normalized_diagnostic_score"] = diagnostic_score
    features[f"{prefix}high_confidence"] = (diagnostic_score >= high_threshold).astype(np.float32)
    features[f"{prefix}low_confidence"] = (diagnostic_score <= low_threshold).astype(np.float32)
    group_columns["pfbeam_normalized_diagnostic"].extend(
        [
            f"{prefix}candidate_u_norm_std",
            f"{prefix}candidate_u_norm_range",
            f"{prefix}shape_loss_mean",
            f"{prefix}shape_loss_min",
            f"{prefix}shape_loss_margin",
            f"{prefix}shape_score_top1",
            f"{prefix}shape_score_top1_margin",
            f"{prefix}shape_score_entropy",
            f"{prefix}instability_score",
            f"{prefix}normalized_diagnostic_score",
            f"{prefix}high_confidence",
            f"{prefix}low_confidence",
        ]
    )
    group_columns["pfbeam_normalized_disagreement"].extend(
        [f"{prefix}candidate_u_norm_std", f"{prefix}candidate_u_norm_range"]
    )
    ...
    if not np.isfinite(features[feature_cols].to_numpy(np.float32)).all():
        raise ValueError("PF/Beam normalized diagnostic feature frame contains non-finite values")

    summary_rows.append(
        {
            "candidate": "__aggregate__",
            "u_norm_abs_mean": float(np.mean(np.abs(u_norm_matrix))),
            "prefix_resid_abs_mean": None,
            "roughness_mean": None,
            "curvature_abs_mean": None,
            "shape_loss_mean": float(np.mean(loss_matrix)),
            "generated_features": int(len(feature_cols)),
            "rows": int(len(features)),
            "wells": int(features["well"].nunique()),
            "diagnostic_score_mean": float(np.mean(diagnostic_score)),
            "diagnostic_score_p10": float(np.quantile(diagnostic_score, 0.10)),
            "diagnostic_score_p90": float(np.quantile(diagnostic_score, 0.90)),
            "high_confidence_rate": float(np.mean(diagnostic_score >= high_threshold)),
            "low_confidence_rate": float(np.mean(diagnostic_score <= low_threshold)),
        }
    )
...
def run_pfbeam_normalized_diagnostic_score(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    diagnostic_features, diagnostic_group_columns, diagnostic_summary = (
        build_pfbeam_normalized_diagnostic_features(
            full_frame,
    ...
    if full_frame.empty:
        raise ValueError("No rows after merging PF/Beam normalized diagnostic features")
    diagnostic_meta = {
            ...
            feature_schema_rows.append(
                {
                    ...
                    "is_pfbeam_normalized_diagnostic_feature": bool(
