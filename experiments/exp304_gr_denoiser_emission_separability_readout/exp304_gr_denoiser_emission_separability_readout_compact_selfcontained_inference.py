# %% [markdown]
# # exp304 GR-denoiser emission separability readout inference
#
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(
        ...
        raise ValueError("exp304 inference and submission must remain disabled")
    if contract["mode"] != "disabled_train_side_separability_readout_only":
        raise ValueError("exp304 inference mode contract changed")
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp304 is a train-side separability readout only; inference and submission are disabled"
    )
