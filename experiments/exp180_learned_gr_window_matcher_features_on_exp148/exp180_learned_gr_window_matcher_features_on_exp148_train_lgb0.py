...
# %% [markdown]
# # exp180_learned_gr_window_matcher_features_on_exp148 train lgb0
#
...
from learned_gr_window_matcher_features_on_exp148 import (
    ...
    find_artifact,
    run_learned_gr_window_matcher_features_on_exp148,
)
...
gr_matcher_path = find_artifact(
    GR_MATCHER_TRAIN_FEATURES,
    cfg_get(config, "data.gr_matcher_train_features_local"),
)
gr_matcher_summary_path = find_artifact(
    GR_MATCHER_FEATURE_SUMMARY,
    cfg_get(config, "data.gr_matcher_train_summary_local"),
)
gr_matcher_summary = pd.read_csv(gr_matcher_summary_path)
...
print("learned GR matcher cache:", gr_matcher_path, "bytes=", gr_matcher_path.stat().st_size)
display(pd.read_csv(cache_path, nrows=5, dtype={"id": str, "well": str}))
display(gr_matcher_summary.head(20))
...
# %%
summary = run_learned_gr_window_matcher_features_on_exp148(
    ...
    gr_matcher_feature_path=cfg_get(config, "data.gr_matcher_train_features_local"),
    gr_matcher_schema_path=cfg_get(config, "data.gr_matcher_train_feature_schema_local"),
    gr_matcher_summary_path=cfg_get(config, "data.gr_matcher_train_summary_local"),
    require_gr_matcher_cache=True,
