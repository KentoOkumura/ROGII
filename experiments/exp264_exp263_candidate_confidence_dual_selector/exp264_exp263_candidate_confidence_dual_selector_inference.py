...
# %% [markdown]
# # exp264 hidden-safe Stage C → Stage D inference
#
# exp263の12候補と21 confidence列をraw competition testから同じrunで再生成する。保存済み
# 修正版Stage C v6 nested selector 40本からouter-fold別74 compact特徴を作り、保存済みStage D v3
# `selector_compact_addonly` 15本でTVTを予測する。学習、hard selector、Viterbiは行わない。
...
# 6. Stage D saved-booster inference
# 7. Prediction artifacts and reproducibility manifest
...
if not (PACKAGE_DIR / "config.yaml").exists():
    PACKAGE_DIR = Path("experiments/exp264_exp263_candidate_confidence_dual_selector")
...
# ## 2. User authorization and saved-model contracts
#
# Stage Dのworst-well guard FAILは保持する。2026-07-19のユーザー指示を、修正版保存modelによる
# 推論と、submit-check PASS後の外部competition submitの例外承認として扱う。再学習へは拡張しない。

# %%
inference_cfg = dict(config["inference"])
if not bool(config["execution"]["inference_enabled"]):
    raise RuntimeError("exp264 hidden-safe inference is disabled")
if inference_cfg.get("status") != "user_authorized_2026_07_19":
    raise RuntimeError("exp264 inference does not have the fixed user authorization")
if bool(inference_cfg.get("stage_d_guard_passed")):
    raise RuntimeError("Stage D guard failure must remain explicit during this override")
if bool(inference_cfg.get("submit_to_kaggle")):
    raise RuntimeError("the inference notebook must not call the Kaggle submit API")
if not bool(inference_cfg.get("competition_submit_authorized")):
    raise RuntimeError("external Kaggle competition submit is not authorized")
if not bool(inference_cfg.get("competition_submit_requires_submit_check_pass")):
    raise RuntimeError("competition submit must remain gated by submit-check")
...
if len(names) != 12:
    raise ValueError("exp264 inference requires exactly 12 candidates")
...
if len(stage_d_rows) != int(inference_cfg["tvt_model_count"]):
    raise ValueError("Stage D inference requires exactly 15 add-only models")
...
if model_compact_features != compact_features:
    raise ValueError("Stage D model compact feature order differs from Stage C schema")
...
display(
    {
        ...
        "competition_submit_authorized_after_submit_check": True,
    }
)
...
# ## 6. Stage D saved-booster inference
#
# 各TVT modelは学習時と同じouter foldのcompact特徴だけを使う。3 config × 5 foldの15 residual
# predictionを等重み平均し、`last_known_tvt`へ加算する。
...
for outer in range(5):
    ...
    if len(fold_models) != 3:
        raise ValueError(f"Stage D outer fold {outer} must have three add-only models")
...
if len(tvt_model_audit) != 15:
    raise ValueError("Stage D inference did not use all 15 add-only models")
...
prediction_path = output_dir / "stage_d_compact_current_test_predictions.csv.gz"
feature_schema_path = output_dir / "stage_d_inference_feature_schema.csv"
...
pd.DataFrame(
    {
        ...
        "feature_group": [
            "exp218_base" if index < len(base_feature_columns) else "selector_compact"
...
metrics = {
    "experiment": EXPERIMENT_NAME,
    "route": config["experiment"]["route"],
    "status": "corrected_hidden_safe_inference_completed_submit_authorized_pending_external_submit",
    "authorization": {
    ...
    "stage_d_guard_evidence": {
        "matched_control_rmse": float(
            config["model"]["downstream_tvt_stage"]["matched_control_lgb_mean_rmse"]
        ),
        "selector_compact_addonly_rmse": float(
            config["model"]["downstream_tvt_stage"]
            ["selector_compact_addonly_lgb_mean_rmse"]
        ),
        "delta_rmse": float(
            config["model"]["downstream_tvt_stage"]
        ...
        "worst_well_delta_rmse": float(
            config["model"]["downstream_tvt_stage"]["worst_well_delta_rmse"]
        ),
        "guard_passed": False,
    },
    ...
    "notes": [
        ...
        "Each Stage D model receives compact features from its matching Stage C downstream outer fold.",
        "All 40 saved selectors and 15 saved add-only TVT models are SHA-verified; no model is fitted.",
        ...
        "submission.csv is generated in this notebook; external submission is authorized only after submit-check PASS.",
        "The pre-registered Stage D worst-well guard failure remains recorded and is not reclassified as a pass.",
    ],
}
