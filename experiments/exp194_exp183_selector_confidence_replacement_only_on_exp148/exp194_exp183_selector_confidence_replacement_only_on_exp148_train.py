...
# %% [markdown]
# # exp194_exp183_selector_confidence_replacement_only_on_exp148 train
#
# Full-train replacement-only audit that drops the exp148 learned-likelihood
# confidence block and uses exp183 selector selected-path confidence features.
#
...
# 3. Train full-row exp183 selector replacement-only variant
...
from exp183_selector_confidence_replacement_only_on_exp148 import (
    EXP145_TRAIN_ML_FEATURES,
    EXP183_SELECTOR_OOF_PREDICTIONS,
    FULL_REPLAY_TRAIN_FEATURES,
    OUTPUT_PREFIX,
    find_artifact,
    run_exp183_selector_confidence_replacement_only_on_exp148,
)
...
print("Active feature groups:", [v.get("feature_groups", []) for v in active_variants])
...
selector_path = find_artifact(
    EXP183_SELECTOR_OOF_PREDICTIONS,
    cfg_get(config, "data.exp183_selector_oof_predictions_local"),
)
print("exp072 full replay train cache:", cache_path)
print("exp145 full-train learned likelihood feature cache for exclusion inventory:", learned_path)
print("exp183 selector OOF selected path:", selector_path)
...
if {"variant", "mode"}.issubset(selector_preview.columns):
    selector_preview = selector_preview[
        selector_preview["variant"].astype(str).eq(
            str(cfg_get(config, "model.exp183_selector_features.selected_variant"))
        )
        & selector_preview["mode"].astype(str).eq(
            str(cfg_get(config, "model.exp183_selector_features.selected_mode", "viterbi"))
        )
    ].head(5)
else:
...
# %% [markdown]
# ## 3. Train full-row replacement-only variant

# %%
summary = run_exp183_selector_confidence_replacement_only_on_exp148(
    ...
    exp183_feature_config=cfg_get(config, "model.exp183_selector_features", {}),
...
exp183_summary = pd.read_csv(paths.artifacts_dir / f"{OUTPUT_PREFIX}_exp183_selector_feature_summary.csv")
