# %% [markdown]
# # exp363 sticky GR reliability exact HMM — inference
#
# exp363 currently implements only the zero-HMM train-side Stage 0 reliability
# readout. Stage 1 decoding, raw-test inference, and submission are fail-closed.
...
def validate_disabled_inference(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key, expected in checks.items():
        ...
        if actual != expected:
            raise ValueError(
                f"exp363 disabled inference contract changed: "
...
def stop_disabled_inference(config: Mapping[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp363 implements only the target-free Stage 0 reliability readout. "
        "Stage 1 exact HMM, inference, and submission require separate approval."
    )
