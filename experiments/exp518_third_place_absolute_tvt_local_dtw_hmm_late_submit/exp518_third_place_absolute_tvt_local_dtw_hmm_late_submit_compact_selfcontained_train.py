# %% [markdown]
# # exp518 — corrected absolute-TVT three-family HMM LATE SUBMIT audit (train)
#
# This is a post-competition, writeup-based reconstruction audit.  The original
# source code was not public. The absolute-TVT state corrects exp515's invalid
# baseline-relative range, while unreported lattice, transition, and Local-DTW
# details remain fixed approximations recorded in `config.yaml`.
...
EXECUTE_NOTEBOOK = os.environ.get("EXP518_IMPORT_ONLY", "0") != "1" and in_notebook_runtime()
...
def load_experiment_config() -> dict[str, Any]:
    ...
    for path in candidates:
        ...
        if get_nested(config, "experiment.name") == EXPERIMENT_NAME:
            return config
    raise FileNotFoundError(f"exp518 config not found in {[str(path) for path in candidates]}")


def artifact_dir() -> Path:
    override = os.environ.get("EXP518_OUTPUT_DIR")
    if override:
        path = Path(override)
...
def resolve_absolute_tvt_guide(config: dict[str, Any]) -> Path:
    guide = get_nested(config, "data.absolute_tvt_guide") or {}
    filename = str(guide["filename"])
    candidates = []
    override = os.environ.get("EXP518_GUIDE_PATH")
    if override:
        candidates.append(override)
    candidates.extend(guide.get("candidates") or [])
    root = project_root()
    for raw in candidates:
        path = Path(str(raw))
        for candidate in (path, root / path, Path.cwd() / path):
    ...
    if KAGGLE_INPUT_ROOT.exists():
        ...
        if found:
            return found[0]
    raise FileNotFoundError(f"could not resolve saved absolute-TVT guide {filename}")


def load_absolute_tvt_guide(
    config: dict[str, Any],
) -> tuple[pd.DataFrame, dict[str, Any]]:
    guide_config = get_nested(config, "data.absolute_tvt_guide") or {}
    prediction_column = str(guide_config["prediction_column"])
    std_column = str(guide_config["std_column"])
    path = resolve_absolute_tvt_guide(config)
    columns = ["well_id", "row_idx", prediction_column, std_column]
    ...
    frame[prediction_column] = pd.to_numeric(frame[prediction_column], errors="raise")
    frame[std_column] = pd.to_numeric(frame[std_column], errors="raise")
    frame = frame.sort_values(["well_id", "row_idx"]).reset_index(drop=True)
    if frame[["well_id", "row_idx"]].duplicated().any():
        raise ValueError("saved absolute-TVT guide contains duplicate well/row pairs")
    numeric = frame[[prediction_column, std_column]].to_numpy(np.float64)
    if not np.isfinite(numeric).all() or np.any(frame[std_column].to_numpy(np.float64) < 0.0):
        raise ValueError("saved absolute-TVT guide contains invalid predictions or uncertainty")
    expected_rows = int(guide_config["expected_rows"])
    expected_wells = int(guide_config["expected_wells"])
    if len(frame) != expected_rows or frame["well_id"].nunique() != expected_wells:
        raise ValueError(
            f"saved absolute-TVT guide expected {expected_rows} rows/{expected_wells} wells, "
            f"found {len(frame)} rows/{frame['well_id'].nunique()} wells"
        )
    return frame, {
        "path": str(path),
        "raw_sha256": sha256_path(path),
        "content_sha256": dataframe_content_sha(frame, columns),
        "rows": len(frame),
...
def load_horizontal(data_dir: Path, well: str, *, include_truth: bool) -> pd.DataFrame:
    columns = ["MD", "Z", "GR", "TVT_input"]
    if include_truth:
        columns.append("TVT")
    frame = pd.read_csv(data_dir / f"{well}__horizontal_well.csv", usecols=columns)
...
def validate_frozen_contract(config: dict[str, Any]) -> None:
    expected = {
        ...
        "model.state.coordinate": "absolute_tvt_around_absolute_hmm_guide",
        "model.state.position_step_ft": 1.0,
        "model.state.guide_band_pad_ft": 60.0,
        "model.state.guide_prior_sigma_floor_ft": 10.0,
        "model.state.guide_prior_std_multiplier": 3.0,
        "model.state.rate_states": 9,
        "model.state.bias_states": 3,
        "model.reference.local_dtw_reference_offsets_ft": [-2.0, 0.0, 2.0],
...
def prepare_transition_logs(
    ...
    position_step = float(get_nested(config, "model.state.position_step_ft"))
    sigma_position = max(float(transition["position_sigma_ft"]), 0.35 * position_step)
    ...
    elif family == "local_dtw_reconstruction":
        switch = float(transition["local_dtw_neighbor_switch_probability"])
        reference_log = np.full((reference_count, reference_count), -1e18, dtype=np.float64)
        for source in range(reference_count):
            neighbours = int(source > 0) + int(source + 1 < reference_count)
            reference_log[source, source] = math.log(max(1.0 - switch, 1e-30))
            if source > 0:
                reference_log[source, source - 1] = math.log(max(switch / neighbours, 1e-30))
            if source + 1 < reference_count:
                reference_log[source, source + 1] = math.log(max(switch / neighbours, 1e-30))
    else:
...
def joint_hmm_forward_backward(
    ...
    """Exact sum-product on (TVT coordinate, rate, GR bias, reference identity)."""
    ...
    mean_position = np.empty(t_count, np.float64)
    std_position = np.empty(t_count, np.float64)
    ...
    for t in range(t_count - 1, -1, -1):
        ...
        for p in range(p_count):
            ...
            total += p_total
            weighted += p_total * positions[p]
            weighted2 += p_total * positions[p] * positions[p]
            if p == 0 or p == p_count - 1:
                edge += p_total
        mean = weighted / total
        mean_position[t] = mean
        std_position[t] = math.sqrt(max(weighted2 / total - mean * mean, 0.0))
    ...
    return mean_position, std_position, boundary_mass, normalization_error, loglik


def warm_joint_hmm_kernel() -> None:
    """Compile the Numba signature once before well-level thread parallelism."""
    neg = -1e18
    nearest_log = np.full((1, 1, 3), neg, dtype=np.float64)
    nearest_log[0, 0, 1] = 0.0
    position_log = np.full((1, 1, 5), neg, dtype=np.float64)
    position_log[0, 0, 2] = 0.0
    outputs = joint_hmm_forward_backward(
    ...
    if not all(np.isfinite(np.asarray(value)).all() for value in outputs):
        raise RuntimeError("joint HMM warm-up produced a non-finite value")
...
def sequence_contract(horizontal: pd.DataFrame, config: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(dm) < 2 or np.any(dm[1:] <= 0) or not np.isfinite(dm).all():
        raise ValueError("MD must be finite and strictly increasing inside the HMM sequence")
    if not np.isfinite(dz).all():
        raise ValueError("Z must be finite inside the HMM sequence")
    return {
        ...
        "dm": dm,
        "dz": dz,
        "first_prefix_tvt": float(horizontal.loc[int(prefix_index[0]), "TVT_input"]),
        "last_known_tvt": float(horizontal.loc[int(known_index[-1]), "TVT_input"]),
    }
...
def build_absolute_tvt_states(
    horizontal: pd.DataFrame,
    sequence: dict[str, Any],
    well_guide: pd.DataFrame,
    config: dict[str, Any],
) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    state = get_nested(config, "model.state") or {}
    step = float(state["position_step_ft"])
    pad = float(state["guide_band_pad_ft"])
    guide_config = get_nested(config, "data.absolute_tvt_guide") or {}
    prediction_column = str(guide_config["prediction_column"])
    std_column = str(guide_config["std_column"])
    ordered = well_guide.sort_values("row_idx").reset_index(drop=True)
    expected_index = sequence["eval_index"].astype(np.int64)
    actual_index = ordered["row_idx"].to_numpy(np.int64)
    if not np.array_equal(actual_index, expected_index):
        raise ValueError("saved absolute-TVT guide rows do not match the unknown suffix")
    prefix_tvt = pd.to_numeric(
        horizontal.loc[sequence["prefix_index"], "TVT_input"], errors="coerce"
    ).to_numpy(np.float64)
    eval_guide = ordered[prediction_column].to_numpy(np.float64)
    eval_std = ordered[std_column].to_numpy(np.float64)
    if not np.isfinite(prefix_tvt).all() or not np.isfinite(eval_guide).all():
        raise ValueError("HMM prefix TVT_input must be finite")
    guide = np.concatenate([prefix_tvt, eval_guide])
    guide_std = np.concatenate([np.zeros(len(prefix_tvt), dtype=np.float64), eval_std])
    offsets = np.arange(-pad, pad + 0.5 * step, step, dtype=np.float64)
    absolute_tvt = guide[:, None] + offsets[None, :]
    return absolute_tvt, guide, guide_std, pad
...
def build_reference_tensor(
    ...
    if family == "exp417_base_reconstruction":
        sibling = sibling_values(absolute_tvt, "coarse")
        type_ref = type_values(absolute_tvt)
        references = []
        ...
        for type_weight, sibling_weight in mixtures:
            references.append(
                with_self(
                    absolute_tvt,
                    float(type_weight) * type_ref + float(sibling_weight) * sibling,
                )
            )
        return np.stack(references, axis=-1).astype(np.float64)
    if family == "local_dtw_reconstruction":
        references = []
        for local_offset in get_nested(config, "model.reference.local_dtw_reference_offsets_ft"):
            query = absolute_tvt + float(local_offset)
    ...
    if family == "fine_bin_reconstruction":
        sibling = sibling_values(absolute_tvt, "fine")
        return with_self(absolute_tvt, sibling)[..., None].astype(np.float64)
...
def build_emission(
    ...
    state = get_nested(config, "model.state") or {}
    guide_scale = np.maximum(
        float(state["guide_prior_sigma_floor_ft"]),
        float(state["guide_prior_std_multiplier"]) * guide_std,
    )
    guide_log_prior = -0.5 * (
        (absolute_tvt - guide[:, None]) / guide_scale[:, None]
    ) ** 2
    emission += guide_log_prior[:, :, None, None]
    ...
    clamp = np.maximum(
        -0.5
        * ((absolute_tvt[:prefix_count] - observed[:, None]) / clamp_sigma) ** 2,
        -60.0,
    )
...
def decode_family(
    ...
    state = get_nested(config, "model.state") or {}
    absolute_tvt, guide, guide_std, guide_pad = build_absolute_tvt_states(
        horizontal, sequence, well_guide, config
    )
    offsets = absolute_tvt[0] - guide[0]
    ...
    started = time.time()
    mean_offset, std_position, boundary_mass, norm_error, loglik = joint_hmm_forward_backward(
        emission,
        offsets,
        rate_log,
        bias_log,
    ...
    prefix_count = int(sequence["prefix_count"])
    raw = guide[prefix_count:] + mean_offset[prefix_count:]
    ...
    return {
        ...
        "raw_prediction": raw,
        "guide_prediction": guide[prefix_count:],
        "guide_std": guide_std[prefix_count:],
        "posterior_std": std_position[prefix_count:],
        ...
        "position_states": len(offsets),
        "guide_band_pad_ft": guide_pad,
        "rate_delta_min": float(rate_delta[0]),
        "rate_delta_max": float(rate_delta[-1]),
        "guide_std_max": float(np.max(guide_std[prefix_count:])),
...
def decode_well_target_free(
    ...
    frame: dict[str, Any] = {
        ...
        "absolute_tvt_guide": family_results["exp417_base_reconstruction"]["guide_prediction"],
        "absolute_tvt_guide_std": family_results["exp417_base_reconstruction"]["guide_std"],
    }
    for family in FAMILY_ORDER:
...
def run_oof(config: dict[str, Any]) -> dict[str, Any]:
    ...
    guide_frame, guide_meta = load_absolute_tvt_guide(config)
    guide_by_well = {
        str(well): frame.reset_index(drop=True)
        for well, frame in guide_frame.groupby("well_id", sort=False)
    }

    debug_max = int(os.environ.get("EXP518_DEBUG_MAX_WELLS", "0"))
    ordered_wells = sorted(wells)
    debug_selection = os.environ.get("EXP518_DEBUG_SELECTION", "first")
    if debug_max > 0 and debug_selection == "evenly_spaced":
        selected_index = np.linspace(0, len(ordered_wells) - 1, debug_max, dtype=np.int64)
        target_wells = [ordered_wells[int(index)] for index in np.unique(selected_index)]
    else:
        target_wells = ordered_wells[:debug_max] if debug_max > 0 else ordered_wells
    ...
    def build_one(index: int, well: str) -> tuple[pd.DataFrame, dict[str, Any]]:
        safe_horizontal = load_horizontal(train_dir, well, include_truth=False)
        typewell = load_typewell(train_dir, well)
        fold = fold_map[well]
        if well not in guide_by_well:
            raise ValueError(f"saved absolute-TVT guide is missing well {well}")
        ...
        scored_truth = decoded["tvt_true"].to_numpy(np.float64)
        guide_prediction = decoded["absolute_tvt_guide"].to_numpy(np.float64)
        guide_pad = float(get_nested(config, "model.state.guide_band_pad_ft"))
        outside_distance = np.maximum(guide_prediction - guide_pad - scored_truth, 0.0) + np.maximum(
            scored_truth - guide_prediction - guide_pad, 0.0
        )
        runtime_row = {
            "well_id": well,
            "fold": fold,
            ...
            "truth_rows_outside_absolute_tvt_grid": int(np.count_nonzero(outside_distance)),
            "truth_max_distance_outside_absolute_tvt_grid": float(outside_distance.max(initial=0.0)),
            **meta,
        }
        print(
        ...
        return decoded, runtime_row

    outer_workers = max(1, int(get_nested(config, "runtime.num_workers") or 1))
    if outer_workers > 1:
        try:
            from joblib import Parallel, delayed
        except ModuleNotFoundError as exc:
            raise RuntimeError("joblib is required when runtime.num_workers > 1") from exc
    ...
    predictions = [decoded for decoded, _ in results]
    runtime_rows = [runtime_row for _, runtime_row in results]
...
def summarize_and_write(run: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    guide_rmse = rmse(oof["tvt_true"], oof["absolute_tvt_guide"])
    fold_rows = [
        {
            "fold": int(fold),
            "rows": int(len(frame)),
            ...
            lambda frame: pd.Series(
                {
                    "rows": len(frame),
                    "rmse": rmse(frame["tvt_true"], frame["prediction"]),
                    "absolute_tvt_guide_rmse": rmse(
                        frame["tvt_true"], frame["absolute_tvt_guide"]
                    ),
                    **{
    ...
    manifest = {
        ...
        "absolute_tvt_guide": run["guide_meta"],
        ...
        "truth_rows_outside_absolute_tvt_grid": int(
            runtime["truth_rows_outside_absolute_tvt_grid"].sum()
        ),
        "trained_models": 0,
        "lightgbm_configs": 0,
        "boosters": 0,
    ...
    technical = {
        ...
        "truth_max_distance_outside_absolute_tvt_grid": float(
            runtime["truth_max_distance_outside_absolute_tvt_grid"].max()
        ),
    ...
    technical["passed"] = bool(
        ...
        and technical["truth_rows_outside_absolute_tvt_grid"] == 0
    ...
    metrics = {
        "experiment": EXPERIMENT_NAME,
        "status": (
            "debug_smoke_complete_inference_blocked"
            if run["debug"]
            else "train_oof_complete_inference_blocked_pending_reproduction_review"
        ),
        "method_fidelity": "proxy",
        ...
        "absolute_tvt_guide_rmse": guide_rmse,
        "folds": fold_rows,
        "public_lb": None,
        "private_lb": None,
        "inference_blocked": True,
        "third_place_oof_absolute_difference": abs(pooled - 5.9703),
...
def run_train(config: dict[str, Any]) -> dict[str, Any]:
    ...
    print(
        f"EXP518 LATE SUBMIT RECONSTRUCTION TRAIN COMPLETE elapsed={time.time() - started:.3f}s",
        flush=True,
    )
    return metrics
