# %% [markdown]
# # exp373 exp355 fixed13 dual selector on exp264 — train
#
# Add the saved exp355 Stage 1 OOF as exactly one new candidate to the corrected
# exp264 candidate-long dual selector. Stage A refreezes the raw-test-safe
# selector schema, then Stage C trains the unchanged nested dual selector.
# The saved fixed12 selector score is read only after fitting for a paired
# add-one audit. Downstream TVT training, inference, and submission are excluded.
...
from src.exp355_fixed13_candidate_cache import (
    ADDED_CANDIDATE_ID,
    BASE_CANDIDATE_IDS,
...
EXPERIMENT_NAME = "exp373_exp355_fixed13_dual_selector_on_exp264"
...
EXECUTE_NOTEBOOK = (
    os.environ.get("EXP373_IMPORT_ONLY", "0") != "1" and in_notebook_runtime()
)
...
# ## 4. Input resolution and SHA checks
#
# Exp355 is opened through four target-free columns only. All other diagnostic
# columns are deliberately not read. The parent exp264 score is a post-fit
# audit input and never enters Stage A or selector fitting.

# %%
if EXECUTE_NOTEBOOK:
    if not bool(execution["run_approved"]):
        raise RuntimeError(
            "exp373 Kaggle train is not authorized. Set execution.run_approved "
    ...
    if execution.get("approved_scope") != (
        ...
        raise RuntimeError("exp373 approval scope does not match the fixed cost contract")
    ...
    exp355_cfg = CONFIG["data"]["exp355_oof"]
    exp355_path = resolve_file_by_sha(
        exp355_cfg["patterns"],
        roots,
        expected_file_sha256=exp355_cfg["expected_file_sha256"],
        expected_decompressed_sha256=exp355_cfg["expected_decompressed_sha256"],
        label="saved exp355 Stage 1 OOF",
    )
    exp355_oof, exp355_manifest = load_exp355_oof(
        exp355_path,
        expected_rows=int(CONFIG["validation"]["expected_rows"]),
        expected_wells=int(CONFIG["validation"]["expected_wells"]),
        expected_file_sha256=exp355_cfg["expected_file_sha256"],
        expected_decompressed_sha256=exp355_cfg["expected_decompressed_sha256"],
        expected_prediction_logical_sha256=exp355_cfg[
            "expected_prediction_logical_sha256"
        ],
    )
    write_json(OUTPUT_DIR / "exp373_exp355_oof_manifest.json", exp355_manifest)
...
# globally joins exp355 by `(well_id, row_idx)`, preserves its saved-exp226
# source fold as provenance, and repartitions rows by the exp263 selector fold.
# Source fold is never a model feature. Stage A then refreezes the
# candidate-long schema using the same audit policy as corrected exp264.

# %%
if EXECUTE_NOTEBOOK:
    fixed13_cache = Exp355Fixed13CandidateCache(
        cache_root,
        CONTRACT,
        exp355_oof=exp355_oof,
        exp355_manifest=exp355_manifest,
    )
    ...
    exp355_repartition_manifest = fixed13_cache.selector_repartition_manifest(
        expected_rows=int(CONFIG["validation"]["expected_rows"])
    )
    if not bool(exp355_repartition_manifest["passed"]):
        raise RuntimeError(
            "exp355 global-key selector-fold repartition audit failed: "
            f"{exp355_repartition_manifest['checks']}"
        )
    exp355_manifest["selector_fold_repartition"] = exp355_repartition_manifest
    write_json(
        OUTPUT_DIR / "exp373_exp355_selector_fold_repartition.json",
        exp355_repartition_manifest,
    )
    write_json(OUTPUT_DIR / "exp373_exp355_oof_manifest.json", exp355_manifest)

    write_exp373_input_contract(
        OUTPUT_DIR / "exp373_input_contract.json",
        config=CONFIG,
        contract=CONTRACT,
        exp355_manifest=exp355_manifest,
...
if EXECUTE_NOTEBOOK:
    ...
    technical_checks = {
        ...
        "exp355_truth_or_error_columns_loaded": int(
            exp355_manifest["truth_or_error_columns_loaded"]
        )
        == 0,
        "exp355_global_key_selector_fold_repartition": bool(
            exp355_repartition_manifest["passed"]
        ),
        "exp355_source_fold_not_used_as_model_feature": not bool(
            exp355_repartition_manifest["source_fold_used_as_model_feature"]
        ),
    }
    if not all(technical_checks.values()):
        raise RuntimeError(f"exp373 technical checks failed: {technical_checks}")
...
if EXECUTE_NOTEBOOK:
    ...
    scope_metrics = pd.read_csv(
        OUTPUT_DIR / "exp373_fixed13_vs_fixed12_scope_metrics.csv"
    )
...
if EXECUTE_NOTEBOOK:
    ...
    summary = {
        ...
        "exp355_oof": exp355_manifest,
        "exp355_selector_fold_repartition": exp355_repartition_manifest,
    ...
    write_json(OUTPUT_DIR / "exp373_summary.json", summary)
    ...
    reproducibility.update(
        {
            ...
            "exp373_summary_sha256": sha256_file(
                OUTPUT_DIR / "exp373_summary.json"
            ),
        }
    )
