# %% [markdown]
# # exp398 all-well sigma_gr ×1.3 exact-HMM inference
#
# exp398 is a train-side fixed global GR-scale audit. Raw-test prediction,
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if contract["experiment"] != EXPERIMENT_NAME or contract["route"] != "pf_beam":
        raise ValueError("unexpected exp398 inference config")
    if contract["mode"] != "terminally_disabled_after_train_side_gate_failure":
        raise ValueError("exp398 inference mode contract changed")
    if any(
        contract[key]
        for key in (
            "inference_enabled",
        ...
        raise ValueError("exp398 inference and submission must remain disabled")
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp398 is a train-side all-well sigma_gr x1.3 exact-HMM audit; "
