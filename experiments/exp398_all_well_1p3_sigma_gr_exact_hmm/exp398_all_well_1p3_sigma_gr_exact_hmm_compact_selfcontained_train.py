# %% [markdown]
# # exp398 all-well sigma_gr ×1.3 exact-HMM train
#
# Train-side exact-HMM audit of one preregistered global GR-scale intervention.
...
# unchanged except that every well's already-clipped sigma_gr is multiplied by
# exactly 1.3 without a second clip. Saved exp209 HMM and exp072 LikPF
# predictions are read-only controls; no control, PF, Beam, model, inference,
...
# 4. Exp209 observation model and global sigma preparation
...
EXPERIMENT_NAME = "exp398_all_well_1p3_sigma_gr_exact_hmm"
OUTPUT_PREFIX = EXPERIMENT_NAME
VARIANT_ORDER = ("all_well_sigma_gr_x1p3_on_exp209_absolute_tvt",)
...
EXECUTE_NOTEBOOK = os.environ.get("EXP398_IMPORT_ONLY", "0") != "1" and in_notebook_runtime()
...
def load_experiment_config() -> dict[str, Any]:
    ...
    for path in candidates:
        ...
        if get_nested(config, "experiment.name") == EXPERIMENT_NAME:
            return config
    raise FileNotFoundError(f"exp398 config not found in {[str(path) for path in candidates]}")
...
def validate_scientific_contract(
    ...
    expected = {
        ...
        "model.emission.candidate_family": "gaussian",
        ...
        "model.sigma.multiplier": 1.3,
        "model.sigma.application_scope": "all_wells",
        "model.sigma.application_order": "clip_base_then_multiply_exactly_once",
    ...
    for key, value in expected.items():
        if get_nested(config, key) != value:
            raise ValueError(f"exp398 fixed contract mismatch: {key} must be {value!r}")
    ...
    for key, value in fixed_hmm.items():
        if float(hmm.get(key, -1.0)) != value:
            raise ValueError(f"exp398 fixes model.fixed_hmm.{key}={value}")
    if (
        hmm.get("emission") != "capped_gaussian"
        or hmm.get("rate_center") != "zero"
    ):
        raise ValueError("exp398 fixes capped Gaussian emission and a zero-centered rate grid")
    fixed_policies = {
        "coordinate": "absolute_tvt",
        "sigma_mode": "known_prefix_zero_fill_population_std_then_global_x1p3",
    ...
    if [float(value) for value in hmm.get("sigma_clip", [])] != [10.0, 60.0]:
        raise ValueError("exp398 fixes the exp209 GR sigma clip to [10, 60]")
    if hmm.get("post_multiplier_clip") is not None:
        raise ValueError("exp398 forbids a post-multiplier sigma clip")
    forbidden = set(str(value) for value in get_nested(config, "guards.forbidden") or [])
    required_forbidden = {
        "exp397_prefix_selector_or_threshold",
        "exp226_tvt_geop_or_exp281_residual_offset_coordinate",
        "gaussian_parent_hmm_rerun",
        "multiplier_or_clip_search",
        "likelihood_temperature_or_emission_clip_search",
        "well_or_row_adaptive_sigma",
    ...
    if not required_forbidden.issubset(forbidden):
        raise ValueError("exp398 no-rescue and single-change exclusions are incomplete")
    if not bool(get_nested(config, "execution.implementation_approved")):
        raise ValueError("exp398 implementation approval must be recorded")
    if require_run_approval and not (
        ...
        raise RuntimeError("exp398 Kaggle package/push/run is not approved")
...
def build_scientific_contract(config: dict[str, Any]) -> dict[str, Any]:
    contract = {
        ...
        "forbidden": [
            "exp397_prefix_selector_or_exp281_residual_offset_coordinate",
            "multiplier_clip_temperature_emission_or_family_search",
            "well_or_row_adaptive_sigma_or_missing_weight_change",
...
def preflight_controls_and_assignments(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(safe_columns) != {"well_id", "row_idx", "suffix_offset", "fold"}:
        raise ValueError("exp398 reporting decoder allowlist must contain identity/fold only")
    if set(safe_columns) & forbidden_decoder_columns:
        raise ValueError("exp398 reporting decoder allowlist contains forbidden columns")
...
def compute_exp209_zero_fill_sigma_audit(
    ...
    *,
    ...
    if len(known_index) == 0 or not np.array_equal(known_index, np.arange(len(known_index))):
        raise ValueError("exp398 requires one contiguous known TVT_input prefix")
...
def capped_gaussian_log_likelihood(
    zscore: np.ndarray,
    clip_z2: float = 600.0,
) -> np.ndarray:
    """Exp209 capped Gaussian score with its state-independent constant omitted."""
    clip_z2 = float(clip_z2)
    if not math.isfinite(clip_z2) or clip_z2 <= 0.0:
        raise ValueError("Gaussian squared-z clip must be positive and finite")
    squared = np.square(np.asarray(zscore, dtype=np.float64))
    return -0.5 * np.minimum(squared, clip_z2)
...
def prepare_hmm_inputs(
    ...
    if (
        ...
        raise ValueError("each exp398 well must contain one contiguous known prefix and suffix")
    ...
    sigma_base = float(observation_audit["exp209_zero_fill_sigma"])
    sigma_multiplier = float(hmm["sigma_multiplier"])
    sigma_gr = sigma_base * sigma_multiplier
    if not 13.0 <= sigma_gr <= 78.0:
        raise ValueError("exp398 effective sigma_gr must remain in [13, 78]")
    observation_audit.update(
        {
            "sigma_multiplier": sigma_multiplier,
            "effective_sigma_gr": sigma_gr,
...
def run_exact_hmm_variant(
    ...
    hmm = get_nested(config, "model.fixed_hmm") or {}
    if not math.isfinite(float(sigma_gr)) or not 13.0 <= float(sigma_gr) <= 78.0:
        raise ValueError("exp398 effective sigma_gr must satisfy [13, 78]")
    clip_z2 = float(hmm["emission_clip_z2"])
    zscore = (
        prepared["observed_gr"][:, None] - prepared["state_gr"][None, :]
    ) / float(sigma_gr)
    emission = capped_gaussian_log_likelihood(zscore, clip_z2).astype(np.float32)
    ...
    return {
        ...
        "sigma_multiplier": float(hmm["sigma_multiplier"]),
        "effective_sigma_gr": float(sigma_gr),
        "emission_clip_z2": clip_z2,
    }
    ...
    schedule = pd.DataFrame(
        {
            ...
            "sigma_multiplier": float(get_nested(config, "model.sigma.multiplier")),
            "effective_sigma_gr": float(prepared["sigma_gr"]),
        ...
        runtime_rows.append(
            {
                ...
                "rows": len(eval_index),
                "exp209_zero_fill_sigma": float(observation_audit["exp209_zero_fill_sigma"]),
                "sigma_multiplier": float(result["sigma_multiplier"]),
                "effective_sigma_gr": float(result["effective_sigma_gr"]),
...
def generate_and_freeze_predictions(
    ...
    if not NUMBA_AVAILABLE:
        raise RuntimeError("numba is required for the exp398 exact-HMM audit")
    ...
    def build_one(index: int, well: str):
        print(f"[{index}/{len(wells)}] exp398 well={well}", flush=True)
    ...
    if (
        ...
        raise ValueError("exp398 prediction/observation/runtime coverage mismatch")
...
def evaluate_promotion_gate(
    ...
    runtime_sigma_values = runtime["exp209_zero_fill_sigma"].to_numpy(np.float64)
    emission_finite_coverage = float(runtime["emission_finite_coverage"].min())
    multiplier_values = runtime["sigma_multiplier"].to_numpy(np.float64)
    effective_sigma_values = runtime["effective_sigma_gr"].to_numpy(np.float64)
    sigma_roundtrip_atol = 1.0e-12
    sigma_audit_roundtrip_max_abs_error = float(
        np.max(np.abs(runtime_sigma_values - sigma_values))
    )
    effective_sigma_multiplier_max_abs_error = float(
        np.max(np.abs(effective_sigma_values - runtime_sigma_values * 1.3))
    )
    technical = {
        ...
        "gaussian_emission_finite_coverage": emission_finite_coverage,
        "sigma_roundtrip_atol": sigma_roundtrip_atol,
        "sigma_audit_roundtrip_max_abs_error": sigma_audit_roundtrip_max_abs_error,
        "effective_sigma_multiplier_max_abs_error": (
            effective_sigma_multiplier_max_abs_error
        ),
        "global_sigma_multiplier_contract_passed": bool(
            np.isfinite(multiplier_values).all()
            and np.all(multiplier_values == 1.3)
            and np.isfinite(runtime_sigma_values).all()
            and np.isfinite(effective_sigma_values).all()
            and np.allclose(
                sigma_values,
                runtime_sigma_values,
                rtol=0.0,
                atol=sigma_roundtrip_atol,
            )
            and np.allclose(
                effective_sigma_values,
                runtime_sigma_values * 1.3,
                rtol=0.0,
                atol=sigma_roundtrip_atol,
            )
        ),
        ...
        "effective_sigma_range_contract_passed": bool(
            (effective_sigma_values >= 13.0).all()
            and (effective_sigma_values <= 78.0).all()
        ),
    ...
    technical["passed"] = bool(
        ...
        and technical["gaussian_emission_finite_coverage"] == 1.0
        and technical["global_sigma_multiplier_contract_passed"]
        and technical["exp209_sigma_finite_coverage"] == 1.0
        and technical["exp209_sigma_clip_contract_passed"]
        and technical["effective_sigma_range_contract_passed"]
    ...
    def scope_record(scope: str) -> pd.Series:
        ...
        if len(selected) != 1:
            raise ValueError(f"exp398 expected exactly one direct metric row for scope={scope}")
    ...
    return {
        ...
        "passed": passed,
        "decision": (
            "all_well_sigma_x1p3_passed_train_side_only_no_automatic_downstream"
            if passed
            else "all_well_sigma_x1p3_failed_close_without_rescue"
        ),
        "technical_gate": technical,
        "primary_direct_gate": primary_direct,
        "fixed_likpf_50_50_guard": blend_guard,
        "failure_action": (
            "close_without_multiplier_clip_temperature_emission_family_missing_weight_"
...
def run_full_experiment(config: dict[str, Any]) -> dict[str, Any]:
    if not KAGGLE_WORKING_ROOT.exists() and os.environ.get("EXPERIMENT_ALLOW_LOCAL") != "1":
        raise RuntimeError(
            "Full exp398 must run first on Kaggle; local execution requires explicit smoke approval"
        )
    ...
    status = (
        "train_side_all_well_sigma_x1p3_gate_passed_no_automatic_downstream"
        if promotion_gate["passed"]
        else "train_side_all_well_sigma_x1p3_gate_failed_closed"
    )
    summary = {
        "experiment": EXPERIMENT_NAME,
        "status": status,
