...
OUTPUT_PREFIX = "exp160_sp45_bimodal_selector_confidence_features_on_exp148"
...
def build_sp45_bimodal_selector_features(
    ...
    config = config or {}
    prefix = str(config.get("prefix") or "sp45_")
    key_cols = ["id", "well"]
    feature_data: dict[str, np.ndarray] = {}
    group_name = "sp45_bimodal_selector_confidence"
    ...
    for column in direct_columns:
        if column not in base_frame.columns:
            continue
        out = f"{prefix}{column}"
        add_feature(out, _numeric_array(base_frame, column))
    ...
    is_near = (md_since <= float(config.get("near_md_threshold", 250.0))).astype(np.float32)
    is_longtail = (md_since >= float(config.get("longtail_md_threshold", 1000.0))).astype(
        np.float32
    )
    add_feature(f"{prefix}near_row_flag", is_near)
    add_feature(f"{prefix}longtail_flag", is_longtail)

    scale_score_columns = [str(col) for col in config.get("scale_score_columns") or []]
    available_scale_scores = [col for col in scale_score_columns if col in base_frame.columns]
    if len(available_scale_scores) >= 2:
        scores = np.column_stack(
            [_numeric_array(base_frame, col) for col in available_scale_scores]
        )
        probs = _rowwise_softmax(scores.astype(np.float32))
        order = np.argsort(-scores, axis=1)
        best = scores[np.arange(len(scores)), order[:, 0]]
        second = scores[np.arange(len(scores)), order[:, 1]]
        entropy = -(probs * np.log(np.maximum(probs, 1e-6))).sum(axis=1).astype(np.float32)
        max_entropy = np.float32(np.log(float(len(available_scale_scores))))
        add_feature(f"{prefix}scale_best_code", order[:, 0].astype(np.float32))
        add_feature(f"{prefix}scale_best_score", best.astype(np.float32))
        add_feature(f"{prefix}scale_second_score", second.astype(np.float32))
        add_feature(f"{prefix}scale_margin", (best - second).astype(np.float32))
        add_feature(f"{prefix}scale_entropy_norm", _safe_divide(entropy, max_entropy))
        for index, column in enumerate(available_scale_scores):
            add_feature(f"{prefix}{column}_posterior", probs[:, index].astype(np.float32))
    ...
    for spec in candidate_specs:
        if str(spec.get("column")) not in base_frame.columns:
            continue
        name = str(spec["name"])
        tvt = _candidate_tvt(base_frame, spec)
        available_candidates.append((name, tvt))
        minus_last = (tvt - last_tvt).astype(np.float32)
        out_delta = f"{prefix}{name}_minus_last_known_tvt"
        out_abs = f"{prefix}{name}_abs_delta_from_last_known_tvt"
        out_norm = f"{prefix}{name}_delta_from_last_norm_md"
        add_feature(out_delta, minus_last)
        add_feature(out_abs, np.abs(minus_last).astype(np.float32))
        add_feature(out_norm, _safe_divide(minus_last, md_since + 1.0))

    candidate_map = {name: values for name, values in available_candidates}
    reference_name = str(config.get("reference_candidate") or "likpf_mean")
    if reference_name in candidate_map:
        reference = candidate_map[reference_name]
    elif "likpf_mean_d" in base_frame.columns:
        reference = last_tvt + _numeric_array(base_frame, "likpf_mean_d")
    else:
        reference = last_tvt

    for name, tvt in available_candidates:
        if name == reference_name:
            continue
        diff = (tvt - reference).astype(np.float32)
        out = f"{prefix}{name}_minus_{reference_name}"
        out_abs = f"{prefix}{name}_abs_minus_{reference_name}"
        add_feature(out, diff)
        add_feature(out_abs, np.abs(diff).astype(np.float32))

    if len(available_candidates) >= 3:
        names = [name for name, _ in available_candidates]
        matrix = np.column_stack([values for _, values in available_candidates]).astype(np.float32)
        spread_std = np.std(matrix, axis=1).astype(np.float32)
        spread_range = (np.max(matrix, axis=1) - np.min(matrix, axis=1)).astype(np.float32)
        low = np.min(matrix, axis=1).astype(np.float32)
        high = np.max(matrix, axis=1).astype(np.float32)
        midpoint = ((low + high) * 0.5).astype(np.float32)
        centered = (matrix - reference[:, None]).astype(np.float32)
        sign_positive_rate = (centered > 0).mean(axis=1).astype(np.float32)
        order_abs = np.argsort(np.abs(centered), axis=1)
        near1 = matrix[np.arange(len(matrix)), order_abs[:, 0]]
        near2 = matrix[np.arange(len(matrix)), order_abs[:, 1]]

        derived = {
            f"{prefix}candidate_count": np.full(len(base_frame), len(names), dtype=np.float32),
            f"{prefix}candidate_tvt_std": spread_std,
            f"{prefix}candidate_tvt_range": spread_range,
            f"{prefix}candidate_tvt_range_norm_md": _safe_divide(spread_range, md_since + 1.0),
            f"{prefix}bimodal_midpoint_minus_{reference_name}": midpoint - reference,
            f"{prefix}bimodal_midpoint_abs_minus_{reference_name}": np.abs(midpoint - reference),
            f"{prefix}bimodal_extreme_gap": spread_range,
            f"{prefix}candidate_sign_positive_rate_vs_{reference_name}": sign_positive_rate,
            f"{prefix}closest_candidate_gap": np.abs(near1 - near2).astype(np.float32),
        }
        for column, values in derived.items():
            add_feature(column, values.astype(np.float32))

    interaction_pairs = [
        ("pf_beam_gap", "pf_ancc", "beam_mean"),
        ("beam_likpf_gap", "beam_mean", "likpf_mean"),
        ("sc15_likpf_gap", "sc15", "likpf_mean"),
        ("dense_likpf_gap", "tvt_dense", "likpf_mean"),
    ]
    for label, left, right in interaction_pairs:
        if left not in candidate_map or right not in candidate_map:
            continue
        gap = np.abs(candidate_map[left] - candidate_map[right]).astype(np.float32)
        cols = {
            f"{prefix}{label}_abs": gap,
            f"{prefix}{label}_abs_norm_md": _safe_divide(gap, md_since + 1.0),
            f"{prefix}{label}_near": (gap * is_near).astype(np.float32),
            f"{prefix}{label}_longtail": (gap * is_longtail).astype(np.float32),
        }
        for column, values in cols.items():
            add_feature(column, values)

    shape_columns = [str(col) for col in config.get("shape_columns") or []]
    ...
    if feature_cols:
        feature_matrix = pd.DataFrame(feature_data, copy=False)
        feature_matrix = feature_matrix.fillna(0.0).astype(np.float32, copy=False)
    else:
        feature_matrix = pd.DataFrame(index=base_frame.index)
    if feature_cols and not np.isfinite(feature_matrix.to_numpy(np.float32)).all():
        raise ValueError("SP45/bimodal feature frame contains non-finite values")
        ...
        [
            {
                ...
                "configured_direct_columns": len(direct_columns),
                "available_direct_columns": len([c for c in direct_columns if c in base_frame]),
                "configured_scale_score_columns": len(scale_score_columns),
                "available_scale_score_columns": len(available_scale_scores),
                "configured_candidate_tvt_specs": len(candidate_specs),
                "available_candidate_tvt_specs": len(available_candidates),
                "generated_features": len(feature_cols),
                "rows": int(len(result)),
...
def run_sp45_bimodal_selector_confidence_features_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    sp45_features, sp45_group_columns, sp45_summary = build_sp45_bimodal_selector_features(
        full_frame,
        sp45_feature_config or {},
    )
    sp45_feature_columns = [col for col in sp45_features.columns if col not in {"id", "well"}]
    if not full_frame[["id", "well"]].reset_index(drop=True).equals(
        sp45_features[["id", "well"]].reset_index(drop=True)
    ):
        raise ValueError(
            "SP45/bimodal features are not row-order aligned with train feature frame"
        )
    full_frame = pd.concat(
        [
    ...
    sp45_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_sp45_bimodal_feature_summary.csv",
        index=False,
    )
    ...
    manifest = {
        "experiment": "exp160_sp45_bimodal_selector_confidence_features_on_exp148",
        ...
        "mode": "sp45_bimodal_selector_confidence_features_on_exp148_full_train_rows",
        ...
        "sp45_feature_config": sp45_feature_config or {},
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "sp45_bimodal_feature_summary": f"{OUTPUT_PREFIX}_sp45_bimodal_feature_summary.csv",
...
def run_saved_model_inference(
    *,
    ...
    if not test_frame[["id", "well"]].reset_index(drop=True).equals(
        ...
        raise ValueError(
            "Raw-test SP45/bimodal features are not row-order aligned with test feature frame"
        )
    test_frame = pd.concat(
        [
    ...
    if configured_sp45_groups and {
        ...
        raise ValueError("SP45/bimodal feature groups differ from train manifest")
    feature_group_columns = {
        **feature_group_columns,
        **sp45_group_columns,
    }
