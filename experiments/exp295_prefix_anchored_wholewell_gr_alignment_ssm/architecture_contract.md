...
この文書はexp295の入力境界、数理モデル、stage分岐、成功条件を実装都合や中間結果から独自変更しないための正規契約である。変更する場合はユーザーの明示承認を得て、`config.yaml`、この文書、`backlog/KAGGLE_DIRECTION.md`を更新し、新しい実験へ分ける場合はその`requirements.md`へ契約を移す。
