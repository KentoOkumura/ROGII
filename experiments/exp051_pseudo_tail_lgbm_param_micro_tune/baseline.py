...
def make_drift_model(config: dict[str, Any], *, random_state: int | None = None) -> Any:
    ...
    if estimator == "LGBMRegressor":
        ...
        return LGBMRegressor(
            ...
            subsample_freq=int(config_get(config, "model.drift_model.params.subsample_freq", 0)),
    ...
    if estimator == "XGBRegressor":
        try:
            ...
            raise ImportError(
                "model.drift_model.estimator=XGBRegressor requires xgboost in the runtime"
            ) from exc
        params = {
            "n_estimators": int(config_get(config, "model.drift_model.params.n_estimators", 900)),
            "learning_rate": float(
                config_get(config, "model.drift_model.params.learning_rate", 0.03)
            ),
            "max_depth": int(config_get(config, "model.drift_model.params.max_depth", 6)),
            "min_child_weight": float(
                config_get(config, "model.drift_model.params.min_child_weight", 8.0)
            ),
            "subsample": float(config_get(config, "model.drift_model.params.subsample", 0.9)),
            "colsample_bytree": float(
                config_get(config, "model.drift_model.params.colsample_bytree", 0.9)
            ),
            "reg_lambda": float(config_get(config, "model.drift_model.params.reg_lambda", 1.0)),
            "reg_alpha": float(config_get(config, "model.drift_model.params.reg_alpha", 0.0)),
            "objective": "reg:squarederror",
            "eval_metric": "rmse",
            "tree_method": str(config_get(config, "model.drift_model.params.tree_method", "hist")),
            "random_state": seed,
            "n_jobs": int(config_get(config, "model.drift_model.params.n_jobs", 2)),
            "verbosity": int(config_get(config, "model.drift_model.params.verbosity", 0)),
        }
        device = config_get(config, "model.drift_model.params.device", None)
        if device:
