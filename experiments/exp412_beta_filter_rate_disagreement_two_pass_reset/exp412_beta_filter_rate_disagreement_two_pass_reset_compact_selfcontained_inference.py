...
# %% [markdown]
# # exp412 beta-filter rate disagreement two-pass reset — inference
#
...
def validate_disabled_inference(config: Mapping[str, Any]) -> dict[str, bool]:
    ...
    if any(contract.values()):
        raise ValueError("exp412 Stage 0 inference contract must remain disabled")
    if get_nested(config, "execution.run_stage") != "stage_0_fixed32":
        raise ValueError("exp412 inference cannot run outside the fixed32 Stage 0 contract")
    return contract


def run_inference() -> None:
    raise RuntimeError(
        "exp412 inference is disabled: complete Stage 0, obtain separate Stage 1 "
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {
                "event": "exp412_inference_disabled",
