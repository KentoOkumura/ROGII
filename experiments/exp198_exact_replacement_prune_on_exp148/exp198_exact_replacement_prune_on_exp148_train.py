...
# %% [markdown]
# # exp198_exact_replacement_prune_on_exp148 train
#
# Compact self-contained train notebook source for the exp148 exact replacement prune.
...
# 6. Input and full-train coverage contract
# 7. Train exact replacement prune variant
...
# %%
EXPERIMENT_NAME = "exp198_exact_replacement_prune_on_exp148"
...
OUTPUT_PREFIX = "exp198_exact_replacement_prune_on_exp148"
...
def feature_columns_for_variant(
    ...
    if drop_columns:
        if len(set(drop_columns)) != len(drop_columns):
            raise ValueError(
                f"Duplicate drop columns for variant {variant.get('name')}: {drop_columns}"
            )
        missing_drop = sorted(set(drop_columns) - seen)
        if missing_drop:
            raise ValueError(
                f"Drop columns are not present in assembled features for "
                f"{variant.get('name')}: {missing_drop}"
            )
        drop_set = set(drop_columns)
        columns = [col for col in columns if col not in drop_set]
    expected_feature_count = variant.get("expected_feature_count")
    if expected_feature_count is not None and len(columns) != int(expected_feature_count):
        raise ValueError(
            f"Variant {variant.get('name')} expected {expected_feature_count} features "
            f"after pruning, got {len(columns)}"
        )
    return columns
...
# %%
def run_exact_replacement_prune_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    manifest = {
        "experiment": "exp198_exact_replacement_prune_on_exp148",
        ...
        "mode": "exact_replacement_prune_on_exp148_full_train_rows",
...
print("Drop columns:", {v["name"]: v.get("drop_columns", []) for v in active_variants})
...
# %%
summary = run_exact_replacement_prune_on_exp148(
