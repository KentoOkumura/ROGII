...
TRACKER_SCALE_NAMES = ("s3", "s5", "s8", "s12")
TRACKER_SUMMARY_FEATURE_COLUMNS = [
    "tracker_available",
    "tracker_best_score",
    "tracker_second_score",
    "tracker_confidence",
    "tracker_ambiguity",
    "tracker_score_entropy",
    "tracker_best_scale",
    "tracker_best_shift_tvt",
    "tracker_best_direction",
    "tracker_best_slope_scale",
    "tracker_best_dtw_cost",
    "tracker_hold_weight",
    "tracker_recent_delta_tvt",
    "tracker_best_delta_tvt",
    "tracker_mean_delta_tvt",
    "tracker_std_delta_tvt",
    "tracker_range_delta_tvt",
    "tracker_best_minus_recent",
    "tracker_mean_minus_recent",
    "tracker_max_pairwise_delta",
    "tracker_eval_length_selector",
    "tracker_z_span_selector",
]
TRACKER_SCALE_FEATURE_COLUMNS = [
    f"tracker_{name}_{suffix}"
    for name in TRACKER_SCALE_NAMES
    for suffix in (
        "score",
        "shift_tvt",
        "direction",
...
DEFAULT_FEATURE_SETS = {
    ...
    "all_plus_tracker": [*BASE_FEATURE_COLUMNS, *TRACKER_FEATURE_COLUMNS],
    "no_gr_signal_plus_tracker": [
        *[column for column in BASE_FEATURE_COLUMNS if column not in GR_FEATURE_COLUMNS],
        *TRACKER_FEATURE_COLUMNS,
    ],
    "no_gr_signal_plus_tracker_direction": [
        *[column for column in BASE_FEATURE_COLUMNS if column not in GR_FEATURE_COLUMNS],
        *TRACKER_FEATURE_COLUMNS,
        *TRAJECTORY_DIRECTION_FEATURE_COLUMNS,
    ],
...
def empty_formation_guide_features(n_rows: int) -> pd.DataFrame:
    data = {
        column: np.full(n_rows, np.nan, dtype=float) for column in FORMATION_GUIDE_FEATURE_COLUMNS
    }
...
def tracker_enabled(config: dict[str, Any]) -> bool:
    return bool(config_get(config, "model.tracker.enabled", False))


def tracker_candidate_specs(config: dict[str, Any]) -> list[dict[str, Any]]:
    defaults = [
        {"name": "s3", "scale": 3.0, "window": 25, "radius": 90.0, "shift_step": 15.0},
        {"name": "s5", "scale": 5.0, "window": 35, "radius": 120.0, "shift_step": 15.0},
        {"name": "s8", "scale": 8.0, "window": 50, "radius": 160.0, "shift_step": 20.0},
        {"name": "s12", "scale": 12.0, "window": 75, "radius": 220.0, "shift_step": 20.0},
    ]
    raw_specs = config_get(config, "model.tracker.candidates", defaults)
    if not isinstance(raw_specs, list) or not raw_specs:
        raise ValueError("model.tracker.candidates must be a non-empty list")

    specs: list[dict[str, Any]] = []
    for index, raw_spec in enumerate(raw_specs):
        if not isinstance(raw_spec, dict):
            raise ValueError(f"model.tracker.candidates[{index}] must be a mapping")
        name = str(raw_spec.get("name") or "")
        if name not in TRACKER_SCALE_NAMES:
            raise ValueError(
                f"unsupported tracker candidate name {name!r}; "
                f"expected one of {list(TRACKER_SCALE_NAMES)}"
            )
        ...
        specs.append(
            {
                "name": name,
                "scale": scale,
                "slope_scale": float(raw_spec.get("slope_scale", scale / 8.0)),
...
def empty_tracker_features(n_rows: int) -> pd.DataFrame:
    data = {column: np.full(n_rows, np.nan, dtype=float) for column in TRACKER_FEATURE_COLUMNS}
    zero_columns = [
        "tracker_available",
        "tracker_best_score",
        "tracker_second_score",
    ...
    for name in TRACKER_SCALE_NAMES:
        for suffix in (
            ...
            data[f"tracker_{name}_{suffix}"] = np.zeros(n_rows, dtype=float)
    return pd.DataFrame(data, columns=TRACKER_FEATURE_COLUMNS)
...
def score_tracker_candidate(
    *,
    ...
    min_valid_fraction = float(config_get(config, "model.tracker.min_valid_fraction", 0.30))
    allow_reverse = bool(config_get(config, "model.tracker.allow_reverse", True))

    eval_gr_filled = interpolate_finite_by_index(eval_gr)
    eval_smooth = centered_mean(eval_gr_filled, window)
    shifts = np.arange(-radius, radius + shift_step * 0.5, shift_step, dtype=float)
    directions = (1.0, -1.0) if allow_reverse else (1.0,)
    base_slope = effective_ncc_prior_slope(prefix, config) * slope_scale
    delta_md = eval_md - prefix.last_known_md
    ...
    dtw_cost = normalized_banded_dtw_cost(
        ...
        max_points=int(config_get(config, "model.tracker.dtw_max_points", 160)),
        band_fraction=float(config_get(config, "model.tracker.dtw_band_fraction", 0.12)),
    )
    return {
        "score": float(best_score),
        "second_score": float(second_score),
...
def build_tracker_features(
    *,
    ...
    n_eval = int(eval_indices.size)
    features = empty_tracker_features(n_eval)
    if n_eval == 0 or not tracker_enabled(config):
        return features
    ...
    scores: list[float] = []
    for spec in tracker_candidate_specs(config):
        name = str(spec["name"])
        record = score_tracker_candidate(
            eval_md=eval_md,
            eval_gr=eval_gr,
            type_tvt=type_tvt,
            type_gr=type_gr,
        ...
        features[f"tracker_{name}_score"] = record["score"]
        features[f"tracker_{name}_shift_tvt"] = record["shift_tvt"]
        features[f"tracker_{name}_direction"] = record["direction"]
        features[f"tracker_{name}_slope_scale"] = record["slope_scale"]
        features[f"tracker_{name}_dtw_cost"] = record["dtw_cost"]
        features[f"tracker_{name}_pred_tvt"] = pred_tvt
        features[f"tracker_{name}_delta_tvt"] = delta_tvt
        features[f"tracker_{name}_minus_recent"] = pred_tvt - recent_tvt
        features[f"tracker_{name}_typewell_gr"] = typewell_gr
        features[f"tracker_{name}_gr_abs_error"] = np.abs(eval_gr - typewell_gr)

    if not records:
        return features

    best_name, best_record = max(records.items(), key=lambda item: float(item[1]["score"]))
    ...
    eval_length_threshold = float(config_get(config, "model.tracker.long_eval_rows", 5700.0))
    z_span_threshold = float(config_get(config, "model.tracker.high_z_span", 120.0))
    hold_weight = np.clip(
        1.0 - confidence * float(config_get(config, "model.tracker.confidence_hold_scale", 2.0)),
        0.0,
        1.0,
    )

    features["tracker_available"] = 1.0
    features["tracker_best_score"] = best_score
    features["tracker_second_score"] = second_score
    features["tracker_confidence"] = confidence
    features["tracker_ambiguity"] = max(0.0, 1.0 - confidence)
    features["tracker_score_entropy"] = entropy
    features["tracker_best_scale"] = float(best_record["scale"])
    features["tracker_best_shift_tvt"] = float(best_record["shift_tvt"])
    features["tracker_best_direction"] = float(best_record["direction"])
    features["tracker_best_slope_scale"] = float(best_record["slope_scale"])
    features["tracker_best_dtw_cost"] = best_record["dtw_cost"]
    features["tracker_hold_weight"] = float(hold_weight)
    features["tracker_recent_delta_tvt"] = recent_delta
    features["tracker_best_delta_tvt"] = best_delta
    features["tracker_mean_delta_tvt"] = mean_delta
    features["tracker_std_delta_tvt"] = std_pred
    features["tracker_range_delta_tvt"] = range_pred
    features["tracker_best_minus_recent"] = best_pred - recent_tvt
    features["tracker_mean_minus_recent"] = mean_pred - recent_tvt
    features["tracker_max_pairwise_delta"] = range_pred
    features["tracker_eval_length_selector"] = float(n_eval >= eval_length_threshold)
    features["tracker_z_span_selector"] = float(z_span >= z_span_threshold)
    return features[TRACKER_FEATURE_COLUMNS]
    ...
    *,
    ...
    if eval_indices.size:
        ...
        trajectory_delta_xy = float(
            np.sqrt(
                trajectory_delta_x * trajectory_delta_x + trajectory_delta_y * trajectory_delta_y
            )
        )
        trajectory_delta_xyz = float(
            np.sqrt(
                trajectory_delta_xy * trajectory_delta_xy + trajectory_delta_z * trajectory_delta_z
            )
        )
...
def build_drift_feature_frame(
    ...
    *,
    ...
    turn_from_prefix_sin = (
        prefix_azimuth_cos * anchor_azimuth_sin - prefix_azimuth_sin * anchor_azimuth_cos
    )
    turn_from_prefix_cos = (
        prefix_azimuth_cos * anchor_azimuth_cos + prefix_azimuth_sin * anchor_azimuth_sin
    )
    ...
    tracker_features = build_tracker_features(
        md=md,
        z=z,
        gr=gr,
        typewell_df=typewell_df,
    ...
    features = pd.concat(
        [
            ...
            tracker_features.reset_index(drop=True),
        ],
        axis=1,
    )[FEATURE_COLUMNS]
