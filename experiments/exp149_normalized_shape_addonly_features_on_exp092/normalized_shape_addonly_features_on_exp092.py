...
OUTPUT_PREFIX = "exp149_normalized_shape_addonly_features_on_exp092"
...
def build_normalized_shape_features(
    ...
    config = config or {}
    prefix = str(config.get("prefix") or "nshp_")
    candidate_specs = {
        str(name): dict(spec)
    ...
    if len(candidate_specs) < 2:
        raise ValueError("normalized shape features require at least two candidates")

    min_md_scale = float(config.get("min_md_scale", 25.0))
    min_u_scale = float(config.get("min_u_scale", 10.0))
    polynomial_degree = int(config.get("polynomial_degree", 3))
    robust_iters = int(config.get("robust_iters", 3))
    clip_sigma = float(config.get("clip_sigma", 4.0))
    key_cols = ["id", "well"]
    group_columns: dict[str, list[str]] = {
        "normalized_shape_geometry": [],
        "normalized_candidate_shape": [],
        "normalized_shape_disagreement": [],
    }
    ...
    z = frame["z"].to_numpy(np.float32)
    z_rel = (z - frame["anchor_z0"].to_numpy(np.float32)).astype(np.float32)
    u0 = frame["anchor_t0"].to_numpy(np.float32) + frame["anchor_z0"].to_numpy(np.float32)
    ...
    scale_source = np.maximum.reduce(
        [row_abs_u, row_range_u, z_rel_abs, np.full(len(frame), min_u_scale)]
    )
    u_scale = (
    ...
    features[f"{prefix}z_rel_norm"] = (z_rel / u_scale).astype(np.float32)
    features[f"{prefix}z_rel_abs_norm"] = (z_rel_abs / u_scale).astype(np.float32)
    ...
    group_columns["normalized_shape_geometry"].extend(
        [
            f"{prefix}md_since_norm",
            f"{prefix}tail_md_scale",
            f"{prefix}u_scale",
            f"{prefix}z_rel_norm",
            f"{prefix}z_rel_abs_norm",
    ...
    for name in candidate_specs:
        ...
        poly, poly_resid, poly_abs_resid, poly_slope, poly_curvature = (
            _candidate_polynomial_shape_metrics(
                frame,
                u_norm,
                s_norm,
        ...
        for suffix, values in [
            ...
            ("drift_from_prefix_line_norm", prefix_resid),
            ("abs_drift_from_prefix_line_norm", abs_prefix_resid),
            ("slope_norm", slope),
            ("curvature_norm", curvature),
            ("abs_curvature_norm", abs_curvature),
            ("roughness_norm", roughness),
            ("poly_u_norm", poly),
            ("poly_resid_norm", poly_resid),
            ("poly_abs_resid_norm", poly_abs_resid),
            ("poly_slope_norm", poly_slope),
            ("poly_curvature_norm", poly_curvature),
        ]:
            col = f"{prefix}{name}_{suffix}"
            features[col] = np.asarray(values, dtype=np.float32)
            group_columns["normalized_candidate_shape"].append(col)

        summary_rows.append(
            {
                "candidate": name,
                "u_norm_abs_mean": float(np.mean(np.abs(u_norm))),
                "drift_from_prefix_line_abs_mean": float(np.mean(abs_prefix_resid)),
                "roughness_mean": float(np.mean(np.abs(roughness))),
                "curvature_abs_mean": float(np.mean(abs_curvature)),
                "poly_abs_resid_mean": float(np.mean(poly_abs_resid)),
            }
        )
    ...
    for left_i, left in enumerate(candidate_names):
        for right in candidate_names[left_i + 1 :]:
            ...
            group_columns["normalized_shape_disagreement"].extend([diff_col, abs_col])
    ...
    group_columns["normalized_shape_disagreement"].extend(
    ...
    if not np.isfinite(features[feature_cols].to_numpy(np.float32)).all():
        raise ValueError("normalized shape feature frame contains non-finite values")

    summary_rows.append(
        {
            "candidate": "__aggregate__",
            "u_norm_abs_mean": float(np.mean(np.abs(u_norm_matrix))),
            "drift_from_prefix_line_abs_mean": None,
            "roughness_mean": None,
            "curvature_abs_mean": None,
            "poly_abs_resid_mean": None,
            "generated_features": int(len(feature_cols)),
            "rows": int(len(features)),
            "wells": int(features["well"].nunique()),
            "candidate_u_norm_std_mean": float(
                features[f"{prefix}candidate_u_norm_std"].mean()
            ),
            "candidate_u_norm_range_mean": float(
                features[f"{prefix}candidate_u_norm_range"].mean()
            ),
        }
    )
...
def run_normalized_shape_addonly_features_on_exp092(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    shape_features, shape_group_columns, shape_summary = (
        build_normalized_shape_features(
            full_frame,
            shape_config or {},
        )
    )
    shape_feature_columns = [
        col for col in shape_features.columns if col not in {"id", "well"}
    ]
    ...
    if full_frame.empty:
        raise ValueError("No rows after merging normalized shape features")
    shape_meta = {
        "rows": int(len(shape_features)),
        "wells": int(shape_features["well"].nunique()),
        "generated_features": int(len(shape_feature_columns)),
        "feature_columns": shape_feature_columns,
    }
    ...
    shape_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_shape_feature_summary.csv",
        index=False,
    )
            ...
            feature_schema_rows.append(
                {
                    ...
                    "is_normalized_shape_feature": bool(feature in shape_feature_columns),
                }
            )
    ...
    manifest = {
        "experiment": "exp149_normalized_shape_addonly_features_on_exp092",
        "parent": "exp092_u_projection_correction_disagreement_fullrun",
        "cache_parent": "exp072_exp063_full_replay_feature_cache",
        "mode": "normalized_shape_addonly_features_on_exp092_surface",
        "feature_source": feature_meta,
        "shape_feature_source": shape_meta,
        "anchor_source": {
        ...
        "shape_feature_groups": shape_group_columns,
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "shape_feature_summary": f"{OUTPUT_PREFIX}_shape_feature_summary.csv",
