...
class CFG:
    ...
    SHOW_FIGS = False
    SOFT_PRIOR_NAME = "no_prior"
    SOFT_PRIOR_WEAK_PCT = 0.70
    SOFT_PRIOR_STRONG_PCT = 0.50
    SOFT_PRIOR_WEAK_PENALTY = 0.0
    SOFT_PRIOR_STRONG_PENALTY = 0.0
    SOFT_PRIOR_KNOWN_LAST_PCT_THRESHOLD = 0.75
    SOFT_PRIOR_KNOWN_LAST_MULTIPLIER = 0.0
    HARD_WINDOW_NAME = "pct40"
    HARD_WINDOW_ENABLED = True
    HARD_WINDOW_MIN_PCT = 0.40
    HARD_WINDOW_MAX_PCT = 1.00
    HARD_WINDOW_MIN_ROWS = 3
...
def configure_public_runtime(
    *,
    ...
    soft_prior: dict[str, Any] | None = None,
    ...
    if soft_prior is not None:
        CFG.SOFT_PRIOR_NAME = str(soft_prior.get("name", CFG.SOFT_PRIOR_NAME))
        CFG.SOFT_PRIOR_WEAK_PCT = float(soft_prior.get("weak_pct", CFG.SOFT_PRIOR_WEAK_PCT))
        CFG.SOFT_PRIOR_STRONG_PCT = float(soft_prior.get("strong_pct", CFG.SOFT_PRIOR_STRONG_PCT))
        CFG.SOFT_PRIOR_WEAK_PENALTY = float(soft_prior.get("weak_penalty", CFG.SOFT_PRIOR_WEAK_PENALTY))
        CFG.SOFT_PRIOR_STRONG_PENALTY = float(soft_prior.get("strong_penalty", CFG.SOFT_PRIOR_STRONG_PENALTY))
        CFG.SOFT_PRIOR_KNOWN_LAST_PCT_THRESHOLD = float(
            soft_prior.get("known_last_pct_threshold", CFG.SOFT_PRIOR_KNOWN_LAST_PCT_THRESHOLD)
        )
        CFG.SOFT_PRIOR_KNOWN_LAST_MULTIPLIER = float(
            soft_prior.get("known_last_multiplier", CFG.SOFT_PRIOR_KNOWN_LAST_MULTIPLIER)
        )
    if hard_window is not None:
        CFG.HARD_WINDOW_NAME = str(hard_window.get("name", CFG.HARD_WINDOW_NAME))
        CFG.HARD_WINDOW_ENABLED = bool(hard_window.get("enabled", CFG.HARD_WINDOW_ENABLED))
        CFG.HARD_WINDOW_MIN_PCT = float(hard_window.get("min_typewell_pct", CFG.HARD_WINDOW_MIN_PCT))
        CFG.HARD_WINDOW_MAX_PCT = float(hard_window.get("max_typewell_pct", CFG.HARD_WINDOW_MAX_PCT))
        CFG.HARD_WINDOW_MIN_ROWS = int(hard_window.get("min_rows_after_filter", CFG.HARD_WINDOW_MIN_ROWS))


def soft_prior_config() -> dict[str, Any]:
    return {
        "name": CFG.SOFT_PRIOR_NAME,
        "weak_pct": CFG.SOFT_PRIOR_WEAK_PCT,
        "strong_pct": CFG.SOFT_PRIOR_STRONG_PCT,
        "weak_penalty": CFG.SOFT_PRIOR_WEAK_PENALTY,
        "strong_penalty": CFG.SOFT_PRIOR_STRONG_PENALTY,
        "known_last_pct_threshold": CFG.SOFT_PRIOR_KNOWN_LAST_PCT_THRESHOLD,
        "known_last_multiplier": CFG.SOFT_PRIOR_KNOWN_LAST_MULTIPLIER,
    }


def hard_window_config() -> dict[str, Any]:
    return {
        "name": CFG.HARD_WINDOW_NAME,
        "enabled": CFG.HARD_WINDOW_ENABLED,
        "min_typewell_pct": CFG.HARD_WINDOW_MIN_PCT,
        "max_typewell_pct": CFG.HARD_WINDOW_MAX_PCT,
        "min_rows_after_filter": CFG.HARD_WINDOW_MIN_ROWS,
    }


def apply_typewell_hard_window(tw: pd.DataFrame) -> pd.DataFrame:
    tw = tw.sort_values("TVT").copy()
    if not CFG.HARD_WINDOW_ENABLED:
        return tw
    tvt = pd.to_numeric(tw["TVT"], errors="coerce")
    finite = np.isfinite(tvt.to_numpy(np.float64))
    if int(finite.sum()) < CFG.HARD_WINDOW_MIN_ROWS:
        raise ValueError("typewell has too few finite TVT rows before hard-window filtering")
    finite_tvt = tvt.to_numpy(np.float64)[finite]
    tw_min = float(np.nanmin(finite_tvt))
    tw_span = max(float(np.nanmax(finite_tvt) - tw_min), 1e-6)
    pct = (tvt - tw_min) / tw_span
    keep = finite & (pct.to_numpy(np.float64) >= CFG.HARD_WINDOW_MIN_PCT) & (
        pct.to_numpy(np.float64) <= CFG.HARD_WINDOW_MAX_PCT
    )
    filtered = tw.loc[keep].copy()
    if len(filtered) < CFG.HARD_WINDOW_MIN_ROWS:
        raise ValueError(
            f"typewell hard window {CFG.HARD_WINDOW_NAME} left {len(filtered)} rows; "
            f"minimum is {CFG.HARD_WINDOW_MIN_ROWS}"
        )
    filtered["typewell_pct_original_range"] = pct.loc[filtered.index].astype(np.float32)
    return filtered
...
def load_well(wid, split="train"):
    ...
    tw = apply_typewell_hard_window(pd.read_csv(base / f"{wid}__typewell.csv"))
    return hw, tw
...
@njit(cache=True)
def _soft_prior_cost(tvt, tw_min, tw_span, known_last_pct, weak_pct, strong_pct,
                     weak_penalty, strong_penalty, known_last_pct_threshold, known_last_multiplier):
    if weak_penalty == 0.0 and strong_penalty == 0.0:
        return 0.0
    span = tw_span
    if span < 1e-6:
        span = 1e-6
    pct = (tvt - tw_min) / span
    weak_width = weak_pct - strong_pct
    if weak_width < 1e-6:
        weak_width = 1e-6
    weak = (weak_pct - pct) / weak_width
    if weak < 0.0:
        weak = 0.0
    if weak > 1.0:
        weak = 1.0
    weak *= weak_penalty
    strong = strong_pct - pct
    if strong < 0.0:
        strong = 0.0
    denom = strong_pct
    if denom < 1e-6:
        denom = 1e-6
    strong = strong / denom * strong_penalty
    late = 0.0
    if known_last_pct_threshold < 1.0:
        denom2 = 1.0 - known_last_pct_threshold
        if denom2 < 1e-6:
            denom2 = 1e-6
        late = (known_last_pct - known_last_pct_threshold) / denom2
        if late < 0.0:
            late = 0.0
        if late > 1.0:
            late = 1.0
    return (weak + strong) * (1.0 + known_last_multiplier * late)
...
@njit(cache=True)
def _beam_jit(sgr, tw_tvt, tw_gr, si, BS, mc, es, tw_min, tw_span, known_last_pct,
              weak_pct, strong_pct, weak_penalty, strong_penalty,
              known_last_pct_threshold, known_last_multiplier):
    ...
    for step in range(n):
        ...
        for bi in range(bn):
            ...
            for d in range(-2, 3):
                ...
                prior = _soft_prior_cost(
                    tw_tvt[ni], tw_min, tw_span, known_last_pct, weak_pct, strong_pct,
                    weak_penalty, strong_penalty, known_last_pct_threshold, known_last_multiplier,
                )
                tot = cost+(gv-tw_gr[ni])**2/es+mc*(d if d >= 0 else -d)+prior
                fnd = np.int64(-1)
                for ci in range(nc):
...
def _pf_ancc(md_v, z_v, gr_v, gg, vmin, step, gs, ls, ir, N, ALPHA, RN, PN, IS, RP, RR, RESAMP,
             tw_min, tw_span, known_last_pct, weak_pct, strong_pct, weak_penalty, strong_penalty,
             known_last_pct_threshold, known_last_multiplier):
    ...
    for i in range(len(md_v)):
        ...
        if not np.isnan(gr_v[i]):
            ...
            for j in range(N):
                tvt_j = pos[j]-z_v[i]
                eg = _interp1(gg, tvt_j, vmin, step); d = (gr_v[i]-eg)/gs
                dd = d*d
                if dd > 600.:
                    dd = 600.
                prior = _soft_prior_cost(
                    tvt_j, tw_min, tw_span, known_last_pct, weak_pct, strong_pct,
                    weak_penalty, strong_penalty, known_last_pct_threshold, known_last_multiplier,
                )
                lk = np.exp(-0.5*dd-prior)
                if lk < 1e-300:
                    lk = 1e-300
                w[j] *= lk; ws += w[j]
            if ws > 0.:
    ...
    for i in range(len(md_v)):
        ...
        if not np.isnan(gr_v[i]):
            ...
            for j in range(N):
                ...
                prior = _soft_prior_cost(
                    pos[j], tw_min, tw_span, known_last_pct, weak_pct, strong_pct,
                    weak_penalty, strong_penalty, known_last_pct_threshold, known_last_multiplier,
                )
                lk = lk*np.exp(-prior)
...
def beam_search(gr_h, tw_tvt, tw_gr, start_tvt, bs, mc, es, r):
    ...
    tw_tvt64 = tw_tvt.astype(np.float64)
    tw_min = float(np.nanmin(tw_tvt64))
    tw_span = max(float(np.nanmax(tw_tvt64) - tw_min), 1e-6)
    known_last_pct = float((float(start_tvt) - tw_min) / tw_span)
    return tw_tvt[
        _beam_jit(
            sgr,
            tw_tvt64,
            ...
            tw_span,
            known_last_pct,
            CFG.SOFT_PRIOR_WEAK_PCT,
            CFG.SOFT_PRIOR_STRONG_PCT,
            CFG.SOFT_PRIOR_WEAK_PENALTY,
            CFG.SOFT_PRIOR_STRONG_PENALTY,
            CFG.SOFT_PRIOR_KNOWN_LAST_PCT_THRESHOLD,
            CFG.SOFT_PRIOR_KNOWN_LAST_MULTIPLIER,
        )
    ].astype(np.float32)
...
def _pf_ancc_seeded(seed, md_v, z_v, gr_v, gg, vmin, step, gs, ls, ir, N, ALPHA, RN, PN, IS, RP, RR, RESAMP,
    ...
    np.random.seed(seed)
    return _pf_ancc(
        md_v, z_v, gr_v, gg, vmin, step, gs, ls, ir, N, ALPHA, RN, PN, IS, RP, RR, RESAMP,
        tw_min, tw_span, known_last_pct, weak_pct, strong_pct, weak_penalty, strong_penalty,
        known_last_pct_threshold, known_last_multiplier,
    )
...
def run_pf_ancc(hw, tw_tvt, tw_gr, N=ANCC_N, seed=42):
    ...
    tw_min = float(np.nanmin(tw_tvt))
    tw_span = max(float(np.nanmax(tw_tvt) - tw_min), 1e-6)
    known_last_pct = float((float(kn.TVT_input.iloc[-1]) - tw_min) / tw_span)
...
def run_pf_z(hw, tw_tvt, tw_gr, N=PF_N, seed=42):
    ...
    tw_min = float(np.nanmin(tw_tvt))
    tw_span = max(float(np.nanmax(tw_tvt) - tw_min), 1e-6)
    known_last_pct = float((float(kna.TVT_input.iloc[-1]) - tw_min) / tw_span)
...
def _pf_lik_allseeds(md_v, z_v, gr_v, gg, vmin, step, gs, ls, ir, N, n_seeds, seed_base,
    ...
    for s in range(n_seeds):
        ...
        for i in range(n):
            ...
            for j in range(N):
                tvt_j = pos[j]-z_v[i]
                eg = _interp1(gg, tvt_j, vmin, step); d = (gr_v[i]-eg)/gs; dd = d*d
                if dd > 600.: dd = 600.
                prior = _soft_prior_cost(
                    tvt_j, tw_min, tw_span, known_last_pct, weak_pct, strong_pct,
                    weak_penalty, strong_penalty, known_last_pct_threshold, known_last_multiplier,
...
def lik_pf(hw, tw, n_particles=CFG.PF_PARTICLES, n_seeds=CFG.PF_SEEDS, scales=CFG.PF_SCALES,
    ...
    tw_min = float(np.nanmin(tw_tvt))
    tw_span = max(float(np.nanmax(tw_tvt) - tw_min), 1e-6)
    known_last_pct = float((float(last.TVT_input) - tw_min) / tw_span)
...
_pf_ancc(_m, _z, _g, _gg, 45., .1, 20., 50., 0., 8, .998, .002, .005, .3, .1, .001, .5,
         45., 10., 0.5, 0.7, 0.5, 0., 0., 0.75, 1.)
_pf_z(_m, _z, _g, _g, _gg, _gg, 45., .1, 20., 50., 0., -1., 0., .1, 8, .993, .005, .01, .3, .2, .003, .5,
      45., 10., 0.5, 0.7, 0.5, 0., 0., 0.75, 1.)
_beam_jit(np.linspace(-1., 1., 30), np.linspace(45., 55., 50), np.linspace(-1., 1., 50), 25, 8, 15., 100.,
          45., 10., 0.5, 0.7, 0.5, 0., 0., 0.75, 1.)
...
def build_well(hw_path, tw_path, is_train, likpf_map=None):
    ...
    try: hw = pd.read_csv(hw_path); tw = apply_typewell_hard_window(pd.read_csv(tw_path))
    except: return None
...
PUBLIC_SOURCE_PROVENANCE = {
    ...
    "note": "The base feature builder is the public Ravaghi-style builder copied inside the Pixiux notebook; this experiment rebuilds it from raw files after cutting each typewell to typewell_pct >= 0.40 before PF/Beam/likelihood-PF generation.",
}
...
def build_replay_train_frames(max_wells: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    print(
        f"hard-window public replay train wells: {len(train_wids)} | n_jobs={CFG.n_jobs} "
        f"| PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES} | hard_window={hard_window_config()}",
        flush=True,
    )
    ...
    meta = {
        ...
        "soft_prior": soft_prior_config(),
        "hard_window": hard_window_config(),
...
def build_replay_inference_frames(max_wells: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    print(
        f"hard-window public replay inference wells: train={len(train_wids)} test={len(test_wids)} "
        f"| n_jobs={CFG.n_jobs} | PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES} "
        f"| hard_window={hard_window_config()}",
        flush=True,
    )
    init_imputers(train_wids)
...
def build_replay_test_frame() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    test_wids = list_test_wells()
    print(
        f"hard-window public replay test wells: train_imputer_wells={len(train_wids)} test={len(test_wids)} "
        f"| n_jobs={CFG.n_jobs} | PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES} "
        f"| hard_window={hard_window_config()}",
        flush=True,
    )
...
def feature_columns_for_variant(df: pd.DataFrame, variant: str) -> list[str]:
    ...
    if variant in {
        "pixiux_likpf_public_replay",
        "pixiux_likpf_hard_window_pct50_public_replay",
        "pixiux_likpf_hard_window_pct40_public_replay",
    }:
