# %% [markdown]
# # exp331 prefix GR unary local CE exact SSM inference
#
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if bool(inference.get("enabled")):
        raise ValueError("exp331 Stage C inference is not approved")
    if bool(inference.get("create_submission")):
        raise ValueError("exp331 must not create a submission")
