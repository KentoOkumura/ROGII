...
EXPERIMENT_NAME = "exp107_selector_topn_candidate_only_features"
OUTPUT_PREFIX = "exp107_selector_topn_candidate_only_features"
...
def build_selector_topn_candidate_only_features(
    frame: pd.DataFrame,
    *,
    ...
    slot_delta_columns: list[str] = []
    per_slot_feature_columns: list[list[str]] = []
    ...
    for slot in range(top_k):
        ...
        slot_delta_columns.append(f"{prefix}_candidate_minus_last_anchor")
        ...
        for _, idx in frame.groupby("well", sort=False).indices.items():
            idx_array = np.asarray(idx, dtype=np.int64)
            pred, deriv1, deriv2, _used_degree = _weighted_polyfit_predict(
                x_all[idx_array],
                slot_u[idx_array],
        ...
        per_slot_feature_columns.append(
            [
    ...
    for top_n in range(1, top_k + 1):
        group_name = f"top{top_n}_candidate_only"
        columns: list[str] = []
        for slot in range(top_n):
            columns.extend(per_slot_feature_columns[slot])
        if top_n >= 2:
            score_margin_col = f"top{top_n}_score_margin"
            delta_spread_col = f"top{top_n}_candidate_delta_spread"
            u_spread_col = f"top{top_n}_u_spread"
            score_subset = result[slot_score_columns[:top_n]].to_numpy(np.float32)
            delta_subset = result[slot_delta_columns[:top_n]].to_numpy(np.float32)
            u_subset = result[slot_u_columns[:top_n]].to_numpy(np.float32)
            result[score_margin_col] = (
                score_subset[:, 0] - score_subset[:, top_n - 1]
            ).astype(np.float32)
            result[delta_spread_col] = (
                np.max(delta_subset, axis=1) - np.min(delta_subset, axis=1)
            ).astype(np.float32)
            result[u_spread_col] = (
                np.max(u_subset, axis=1) - np.min(u_subset, axis=1)
            ).astype(np.float32)
            columns.extend([score_margin_col, delta_spread_col, u_spread_col])
    ...
    for slot in range(top_k):
        prefix = f"rank{slot + 1}"
        slot_code = result[f"{prefix}_source_code"].to_numpy(np.float32)
        for name in candidate_names:
            mask = slot_code == source_codes[name]
            summary_rows.append(
                {
                    "slot": prefix,
                    "candidate": name,
...
def run_selector_topn_candidate_only_features(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    rank_features, rank_group_columns, rank_summary = build_selector_topn_candidate_only_features(
        frame,
    ...
    manifest = {
        "experiment": "exp107_selector_topn_candidate_only_features",
        "parent": "exp073_gpu_reproducibility_guard_for_exp063_full_replay",
        "cache_parent": "exp072_exp063_full_replay_feature_cache",
        "mode": "selector_topn_candidate_only_features_from_exp072_cache",
...
def run_saved_model_inference(
    *,
    ...
    rank_slot_config: dict[str, Any] | None = None,
    variant_name: str = "top3_candidate_only",
    ...
    summary = {
        ...
        "known_followup_risk": (
            "exp107 inference should only be used after train-side top-n OOF review selects "
            "a variant; no competition submission decision has been made."
        ),
        "elapsed_seconds": round(time.time() - t0, 3),
    }
...
run_topn_candidate_only_feature_ablation = run_selector_topn_candidate_only_features
