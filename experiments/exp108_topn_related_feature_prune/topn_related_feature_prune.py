...
OUTPUT_PREFIX = "exp108_topn_related_feature_prune"
META_COLUMNS = {"id", "well", "target"}
EXPECTED_FULL_REPLAY_FEATURE_COUNT = 196
TOPN_CORE_BASE_FEATURES = {
    "pf_ancc",
    "pf_ancc_std",
    "pf_ancc_delta",
    "beam_mean_d",
    ...
    for item in config.get("candidates") or []:
        specs.append(
            CandidateSpec(
                ...
                family=str(item.get("family", item.get("name", "candidate"))),
                enabled=bool(item.get("enabled", True)),
            )
        )
    if not specs:
        raise ValueError("rank_slot.candidates must configure at least one candidate")
    return specs


def _candidate_tvt(frame: pd.DataFrame, spec: CandidateSpec) -> np.ndarray:
    if spec.source_column not in frame.columns:
        raise ValueError(f"Candidate {spec.name} missing column={spec.source_column}")
    values = frame[spec.source_column].to_numpy(np.float32)
    if spec.transform == "absolute":
        return values.astype(np.float32)
    if spec.transform == "base_plus_delta":
        return (frame["last_known_tvt"].to_numpy(np.float32) + values).astype(np.float32)
...
def _candidate_scores(
    ...
    for name, values in candidate_values.items():
        if name == "pf_ancc":
            pf_std = np.abs(_frame_numeric(frame, "pf_ancc_std", default=50.0))
...
def _entropy_from_scores(score_matrix: np.ndarray) -> np.ndarray:
    clipped = np.clip(score_matrix.astype(np.float64), 1e-9, None)
    prob = clipped / clipped.sum(axis=1, keepdims=True)
    return (-(prob * np.log(prob)).sum(axis=1) / np.log(score_matrix.shape[1])).astype(
        np.float32
    )


def _is_base_candidate_family_feature(column: str) -> bool:
...
def classify_base_feature_columns(base_feature_columns: list[str]) -> dict[str, list[str]]:
    groups: dict[str, list[str]] = {
        "base_196_all": list(base_feature_columns),
        "base_196_candidate_family": [],
        "base_196_non_candidate_context": [],
        "base_196_topn_core_candidate_family": [],
    }
    for column in base_feature_columns:
        if column in TOPN_CORE_BASE_FEATURES:
            groups["base_196_topn_core_candidate_family"].append(column)
        if _is_base_candidate_family_feature(column):
            groups["base_196_candidate_family"].append(column)
        else:
            groups["base_196_non_candidate_context"].append(column)
    return groups
...
def build_selector_rank_slot_features(
    frame: pd.DataFrame,
    *,
    rank_slot_config: dict[str, Any],
) -> tuple[pd.DataFrame, dict[str, list[str]], pd.DataFrame]:
    specs = [spec for spec in candidate_specs_from_config(rank_slot_config) if spec.enabled]
    top_k = int(rank_slot_config.get("top_k", 3))
    if top_k < 1:
        raise ValueError("rank_slot.top_k must be >= 1")
    if len(specs) < top_k:
        raise ValueError(f"rank_slot top_k={top_k} exceeds candidate count={len(specs)}")

    candidate_names = [spec.name for spec in specs]
    source_codes = {name: float(index) for index, name in enumerate(candidate_names)}
    candidate_values = {spec.name: _candidate_tvt(frame, spec) for spec in specs}
    candidate_scores = _candidate_scores(frame, candidate_values)
    value_matrix = np.column_stack([candidate_values[name] for name in candidate_names]).astype(
        np.float32
    )
    score_matrix = np.column_stack([candidate_scores[name] for name in candidate_names]).astype(
        np.float32
    )
    ...
    md_since = frame.get("md_since")
    if md_since is None:
        x_all = np.maximum(_tail_rank(frame["id"]).astype(np.float32), 0.0)
    else:
    ...
    group_columns: dict[str, list[str]] = {
        "rank_slot_delta": [],
        "rank_slot_identity_score": [],
        "rank_slot_u_projection": [],
    ...
    slot_score_columns: list[str] = []
    slot_code_columns: list[str] = []
    per_slot_related_columns: list[list[str]] = []

    for slot in range(top_k):
        prefix = f"rank{slot + 1}"
        col_index = order[:, slot]
        slot_tvt = value_matrix[row_index, col_index].astype(np.float32)
        slot_score = score_matrix[row_index, col_index].astype(np.float32)
        slot_code = np.asarray(
            [source_codes[candidate_names[int(idx)]] for idx in col_index],
            dtype=np.float32,
        )
        slot_u = (slot_tvt + z - u0).astype(np.float32)
        slot_u_columns.append(f"{prefix}_u")
        slot_score_columns.append(f"{prefix}_score")
        slot_code_columns.append(f"{prefix}_source_code")

        result[f"{prefix}_candidate_minus_last_anchor"] = (slot_tvt - last_known).astype(
            np.float32
        )
        result[f"{prefix}_score"] = slot_score
        result[f"{prefix}_source_code"] = slot_code
        result[f"{prefix}_u"] = slot_u
        group_columns["rank_slot_delta"].append(f"{prefix}_candidate_minus_last_anchor")
        group_columns["rank_slot_identity_score"].extend(
            [f"{prefix}_score", f"{prefix}_source_code"]
        )
        for name in candidate_names:
            flag_col = f"{prefix}_is_{name}"
            result[flag_col] = (slot_code == source_codes[name]).astype(np.float32)
            group_columns["rank_slot_identity_score"].append(flag_col)
            group_columns["rank_slot_source_flags"].append(flag_col)
        ...
        for _, idx in frame.groupby("well", sort=False).indices.items():
            ...
            pred, deriv1, deriv2, used_degree = _weighted_polyfit_predict(
                x_all[idx_array],
                slot_u[idx_array],
                degree=int(rank_slot_config.get("degree", 3)),
                robust_iters=int(rank_slot_config.get("robust_iters", 3)),
                clip_sigma=float(rank_slot_config.get("clip_sigma", 4.0)),
            )
            poly[idx_array] = pred
            slope[idx_array] = deriv1
            curvature[idx_array] = deriv2
        ...
        result[f"{prefix}_u_poly"] = poly
        result[f"{prefix}_u_resid"] = resid
        result[f"{prefix}_u_corr"] = (poly - slot_u).astype(np.float32)
        result[f"{prefix}_u_abs_resid"] = abs_resid
        result[f"{prefix}_u_resid_mad"] = mad_by_well
        result[f"{prefix}_u_slope"] = slope
        result[f"{prefix}_u_curvature"] = curvature
        result[f"{prefix}_u_fit_degree"] = fit_degree.astype(np.float32)
        group_columns["rank_slot_u_projection"].extend(
            [
                f"{prefix}_u_corr",
                f"{prefix}_u_resid",
                f"{prefix}_u_abs_resid",
                f"{prefix}_u_resid_mad",
                f"{prefix}_u_slope",
                f"{prefix}_u_curvature",
                f"{prefix}_u_fit_degree",
            ]
        )
        slot_related = [
            f"{prefix}_candidate_minus_last_anchor",
            f"{prefix}_score",
            f"{prefix}_source_code",
            f"{prefix}_u_slope",
            f"{prefix}_u_curvature",
            f"{prefix}_u_resid_mad",
        ]
        per_slot_related_columns.append(slot_related)
        group_columns["rank_slot_topn_slot_features"].extend(slot_related)

    for left in range(top_k):
        for right in range(left + 1, top_k):
            left_prefix = f"rank{left + 1}"
            right_prefix = f"rank{right + 1}"
            ...
            group_columns["rank_slot_delta"].extend([delta_col, abs_col])
            group_columns["rank_slot_identity_score"].append(score_gap_col)
            group_columns["rank_slot_u_disagreement"].extend([u_diff_col, u_abs_col])
            group_columns["rank_slot_pairwise_disagreement"].extend(
                [delta_col, abs_col, score_gap_col, u_diff_col, u_abs_col]
            )

    result["rank_score_entropy"] = _entropy_from_scores(score_matrix)
    result["rank_score_top1_margin"] = (
        result["rank1_score"].to_numpy(np.float32)
        - result["rank2_score"].to_numpy(np.float32)
    ).astype(np.float32)
    group_columns["rank_slot_identity_score"].extend(
        ["rank_score_entropy", "rank_score_top1_margin"]
    )
    group_columns["rank_slot_global_candidate_stats"].extend(["rank_score_entropy"])

    slot_u_matrix = result[slot_u_columns].to_numpy(np.float32)
    result["rank_slot_u_std"] = np.std(slot_u_matrix, axis=1).astype(np.float32)
    result["rank_slot_u_range"] = (
        np.max(slot_u_matrix, axis=1) - np.min(slot_u_matrix, axis=1)
    ).astype(np.float32)
    group_columns["rank_slot_u_disagreement"].extend(["rank_slot_u_std", "rank_slot_u_range"])
    group_columns["rank_slot_global_candidate_stats"].extend(
        ["rank_slot_u_std", "rank_slot_u_range"]
    )

    for top_n in range(1, top_k + 1):
        related_columns: list[str] = []
        for slot in range(top_n):
            related_columns.extend(per_slot_related_columns[slot])
        if top_n >= 2:
            for left in range(top_n):
                for right in range(left + 1, top_n):
                    left_prefix = f"rank{left + 1}"
                    right_prefix = f"rank{right + 1}"
                    related_columns.extend(
                        [
                            f"{left_prefix}_minus_{right_prefix}_candidate_delta",
                            f"{left_prefix}_{right_prefix}_candidate_absdiff",
                            f"{left_prefix}_minus_{right_prefix}_score_gap",
                            f"{left_prefix}_minus_{right_prefix}_u_diff",
                            f"{left_prefix}_{right_prefix}_u_absdiff",
                        ]
                    )
        if top_n >= 3:
            related_columns.extend(["rank_slot_u_std", "rank_slot_u_range"])
        if top_n >= 1:
            related_columns.append("rank_score_top1_margin")
        group_columns[f"rank_slot_top{top_n}_related"] = _dedupe_columns(related_columns)

    numeric_cols = [col for col in result.columns if col not in {"id", "well"}]
    for col in numeric_cols:
        result[col] = pd.to_numeric(result[col], errors="coerce").astype(np.float32)
    if not np.isfinite(result[numeric_cols].to_numpy(np.float32)).all():
        raise ValueError("selector rank-slot feature frame contains non-finite values")

    summary_rows: list[dict[str, Any]] = []
    for slot in range(top_k):
        prefix = f"rank{slot + 1}"
        for name in candidate_names:
            mask = result[f"{prefix}_is_{name}"].to_numpy(np.float32).astype(bool)
            summary_rows.append(
                {
                    "slot": prefix,
                    "candidate": name,
                    ...
                    "score_mean_when_selected": (
                        float(result.loc[mask, f"{prefix}_score"].mean()) if mask.any() else 0.0
                    ),
                }
            )
    return result, group_columns, pd.DataFrame(summary_rows)
...
def feature_columns_for_variant(
    ...
    base_group_columns: dict[str, list[str]],
    rank_group_columns: dict[str, list[str]],
    variant: dict[str, Any],
) -> list[str]:
    base_groups = list(variant.get("base_feature_groups") or [])
    if base_groups:
        columns: list[str] = []
        for group in base_groups:
            if group not in base_group_columns:
                raise ValueError(f"Unknown base feature group for variant {variant}: {group}")
            columns.extend(base_group_columns[group])
    else:
        columns = list(base_feature_columns)
    groups = list(variant.get("feature_groups") or [])
    extra: list[str] = []
    for group in groups:
        if group not in rank_group_columns:
            raise ValueError(f"Unknown rank-slot feature group for variant {variant}: {group}")
        extra.extend(rank_group_columns[group])
...
def run_topn_related_feature_prune(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    rank_slot_config: dict[str, Any] | None = None,
    ...
    base_group_columns = classify_base_feature_columns(base_feature_columns)

    rank_slot_config = rank_slot_config or {}
    if rank_slot_config.get("include_lgb_oof_features", False):
        raise NotImplementedError(
            "LGB OOF rank-slot features require nested fold generation. "
            "This audit keeps them disabled to avoid leakage."
        )
    rank_features, rank_group_columns, rank_summary = build_selector_rank_slot_features(
        frame,
        rank_slot_config=rank_slot_config,
    )
    rank_feature_columns = [col for col in rank_features.columns if col not in {"id", "well"}]
    full_frame = pd.concat(
        [
            frame.reset_index(drop=True),
            rank_features[rank_feature_columns].reset_index(drop=True),
        ],
        axis=1,
    )
    rank_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_rank_slot_feature_summary.csv",
        index=False,
    )
            ...
            feature_schema_rows.append(
                {
                    ...
                    "feature": feature,
                    "is_rank_slot_feature": bool(feature in rank_feature_columns),
                    "base_feature_group": ",".join(
                        group for group, columns in base_group_columns.items() if feature in columns
                    ),
                    "rank_slot_feature_group": ",".join(
                        group for group, columns in rank_group_columns.items() if feature in columns
                    ),
                }
            )
    ...
    manifest = {
        ...
        "rank_slot_config": rank_slot_config,
        "base_feature_groups": base_group_columns,
        "rank_slot_feature_groups": rank_group_columns,
    ...
    summary = {
        ...
        "base_feature_groups": {
            key: len(value) for key, value in base_group_columns.items()
        },
        "rank_slot_feature_groups": {
            key: len(value) for key, value in rank_group_columns.items()
        },
        ...
        "artifacts": {
            ...
            "rank_slot_feature_summary": f"{OUTPUT_PREFIX}_rank_slot_feature_summary.csv",
...
def run_saved_model_inference(
    *,
    ...
    rank_slot_config: dict[str, Any] | None = None,
    variant_name: str = "rank_slot_u_disagreement",
    ...
    rank_slot_config = rank_slot_config or dict(manifest.get("rank_slot_config") or {})
    if rank_slot_config.get("include_lgb_oof_features", False):
        raise NotImplementedError(
            "LGB OOF rank-slot features require nested hidden-test generation and are disabled."
        )
    ...
    base_group_columns = dict(manifest.get("base_feature_groups") or {})
    if not base_group_columns:
        base_group_columns = classify_base_feature_columns(base_feature_columns)
    configured_groups = manifest.get("rank_slot_feature_groups") or {}
    if configured_groups and {
        key: list(value) for key, value in rank_group_columns.items()
    } != {key: list(value) for key, value in configured_groups.items()}:
        raise ValueError("Rank-slot feature groups differ from train manifest")
    ...
    rank_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_inference_rank_slot_feature_summary.csv",
        index=False,
    )
    pd.DataFrame(
        [
    ...
    ).to_csv(output_dir / f"{OUTPUT_PREFIX}_inference_feature_schema.csv", index=False)
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "rank_slot_feature_summary": (
                f"{OUTPUT_PREFIX}_inference_rank_slot_feature_summary.csv"
            ),
            "feature_schema": f"{OUTPUT_PREFIX}_inference_feature_schema.csv",
            "metrics": f"{OUTPUT_PREFIX}_inference_metrics.csv",
            "summary": f"{OUTPUT_PREFIX}_inference_summary.json",
            "submission": str(submission_path),
        },
        "known_followup_risk": (
            "Inference should only be used after train-side pruning review selects a variant."
        ),
        "elapsed_seconds": round(time.time() - t0, 3),
    }
...
run_u_projection_feature_ablation = run_topn_related_feature_prune
run_selector_rank_slot_features_on_exp073 = run_topn_related_feature_prune
