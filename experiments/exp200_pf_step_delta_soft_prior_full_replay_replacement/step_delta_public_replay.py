...
class CFG:
    ...
    SHOW_FIGS = False
    STEP_DELTA_PRIOR_NAME = "delta_free010_cost0025_scale003"
    STEP_DELTA_FREE = 0.10
    STEP_DELTA_COST = 0.025
    STEP_DELTA_SCALE = 0.03
...
def configure_public_runtime(
    *,
    ...
    step_delta_prior: dict[str, Any] | None = None,
) -> None:
    ...
    if step_delta_prior is not None:
        CFG.STEP_DELTA_PRIOR_NAME = str(step_delta_prior.get("name", CFG.STEP_DELTA_PRIOR_NAME))
        CFG.STEP_DELTA_FREE = float(step_delta_prior.get("delta_free", CFG.STEP_DELTA_FREE))
        CFG.STEP_DELTA_COST = float(step_delta_prior.get("delta_cost", CFG.STEP_DELTA_COST))
        CFG.STEP_DELTA_SCALE = float(step_delta_prior.get("delta_scale", CFG.STEP_DELTA_SCALE))


def step_delta_prior_config() -> dict[str, Any]:
    return {
        "name": CFG.STEP_DELTA_PRIOR_NAME,
        "delta_free": CFG.STEP_DELTA_FREE,
        "delta_cost": CFG.STEP_DELTA_COST,
        "delta_scale": CFG.STEP_DELTA_SCALE,
        "formula": "delta_cost * (max(0, abs(current_tvt - previous_tvt) - delta_free) / delta_scale)^2",
    }
...
@njit(cache=True)
def _step_delta_prior_cost(tvt, prev_tvt, delta_free, delta_cost, delta_scale):
    if delta_cost == 0.0:
        return 0.0
    if delta_scale < 1e-9:
        delta_scale = 1e-9
    excess = abs(tvt - prev_tvt) - delta_free
    if excess <= 0.0:
        return 0.0
    cost = delta_cost * (excess / delta_scale) * (excess / delta_scale)
    if cost > 600.0:
        cost = 600.0
    return cost
...
@njit(cache=True)
def _pf_ancc(md_v, z_v, gr_v, gg, vmin, step, gs, ls, last_tvt, ir, N, ALPHA, RN, PN, IS, RP, RR, RESAMP,
             delta_free, delta_cost, delta_scale):
    pos = np.empty(N); rate = np.empty(N); prev_tvt = np.empty(N); w = np.ones(N)/N
    for j in range(N):
        pos[j] = ls+IS*np.random.randn(); rate[j] = ir+0.01*np.random.randn(); prev_tvt[j] = last_tvt
    ...
    for i in range(len(md_v)):
        ...
        for j in range(N):
            tvt_j = pos[j]-z_v[i]
            prior = _step_delta_prior_cost(tvt_j, prev_tvt[j], delta_free, delta_cost, delta_scale)
            lk = np.exp(-prior)
        ...
        std_[i] = va**0.5
        for j in range(N): prev_tvt[j] = pos[j]-z_v[i]
        pm = md_v[i]
    return pts, std_

@njit(cache=True)
def _pf_z(md_v, z_v, gr_v, gr_sm_v, gg_p, gg_s, vmin, step, gs, ip, iv, beta, icpt, zsig, N,
         MOM, VN, PN, GR_WT, RP, RV, RESAMP, delta_free, delta_cost, delta_scale):
    pos = np.empty(N); vel = np.empty(N); prev_tvt = np.empty(N); w = np.ones(N)/N
    for j in range(N):
        pos[j] = ip+0.5*np.random.randn(); vel[j] = iv+0.02*np.random.randn(); prev_tvt[j] = ip
    ...
    for i in range(len(md_v)):
        ...
        ws = 0.
        for j in range(N):
            prior = _step_delta_prior_cost(pos[j], prev_tvt[j], delta_free, delta_cost, delta_scale)
            lk = np.exp(-prior)
            if not np.isnan(gr_v[i]):
        ...
        std_[i] = va**0.5
        for j in range(N): prev_tvt[j] = pos[j]
        pm = md_v[i]; pz = z_v[i]
    return pts, std_
...
@njit(cache=True)
def _pf_ancc_seeded(seed, md_v, z_v, gr_v, gg, vmin, step, gs, ls, last_tvt, ir, N, ALPHA, RN, PN, IS, RP, RR, RESAMP,
                    delta_free, delta_cost, delta_scale):
    np.random.seed(seed)
    return _pf_ancc(
        md_v, z_v, gr_v, gg, vmin, step, gs, ls, last_tvt, ir, N, ALPHA, RN, PN, IS, RP, RR, RESAMP,
        delta_free, delta_cost, delta_scale,
    )
...
def _pf_lik_allseeds(md_v, z_v, gr_v, gg, vmin, step, gs, ls, ir, N, n_seeds, seed_base,
                     MOM, VN, PN, RP, RR, RESAMP, init_spr, last_tvt,
                     delta_free, delta_cost, delta_scale):
    n = len(md_v); preds = np.empty((n_seeds, n)); liks = np.empty(n_seeds); tmax = vmin + len(gg)*step
    ess_sums = np.zeros(n_seeds); resample_counts = np.zeros(n_seeds)
    for s in range(n_seeds):
        np.random.seed(seed_base + s)
        pos = np.empty(N); rate = np.empty(N); prev_tvt = np.empty(N); w = np.ones(N)/N
        for j in range(N):
            pos[j] = ls + init_spr*np.random.randn(); rate[j] = ir + 0.01*np.random.randn(); prev_tvt[j] = last_tvt
        ...
        for i in range(n):
            ...
            prev_md = md_v[i]
        liks[s] = log_lik
    return preds, liks, ess_sums, resample_counts
...
def lik_pf(hw, tw, n_particles=CFG.PF_PARTICLES, n_seeds=CFG.PF_SEEDS, scales=CFG.PF_SCALES,
    ...
    preds, liks, ess_sums, resample_counts = _pf_lik_allseeds(
    ...
    if with_quality:
        q = {"pf_best_ll": float(liks.max())/len(ev), "pf_ll_spread": float(liks.std()),
             ...
             "pf_ess_mean": float(np.mean(ess_sums / max(len(ev), 1))),
             "pf_resampling_rate": float(np.sum(resample_counts) / max(n_seeds * len(ev), 1))}
...
_pf_ancc(_m, _z, _g, _gg, 45., .1, 20., 50., 50., 0., 8, .998, .002, .005, .3, .1, .001, .5,
         .1, .025, .03)
...
def _likpf_rows(wid, split):
    ...
    out, idx, q = lik_pf(hw, tw, seed_base=stable_seed("likpf", split, wid), with_quality=True)
    ...
    diag = {
        ...
        "split": split,
        "rows": int(len(idx)),
        "pf_best_ll": float(q.get("pf_best_ll", np.nan)),
        "pf_ll_spread": float(q.get("pf_ll_spread", np.nan)),
        "pf_gr_sig": float(q.get("pf_gr_sig", np.nan)),
        "pf_ess_mean": float(q.get("pf_ess_mean", np.nan)),
        "pf_resampling_rate": float(q.get("pf_resampling_rate", np.nan)),
    }
    return pd.DataFrame(d), diag

def build_likpf(wids, split, return_diagnostics=False):
    ...
    frame = pd.concat([r[0] for r in valid], ignore_index=True)
    diagnostics = pd.DataFrame([r[1] for r in valid])
...
PUBLIC_SOURCE_PROVENANCE = {
    ...
    "note": "The base feature builder is the public Ravaghi-style builder copied inside the Pixiux notebook. This experiment adds a per-step TVT delta soft prior inside PF_ANCC, PF_Z, and likelihood-PF only; Beam search remains the exp072 baseline implementation.",
}


def summarize_likpf_diagnostics(diagnostics: pd.DataFrame) -> dict[str, Any]:
    if diagnostics.empty:
        return {"rows": 0}
    summary: dict[str, Any] = {
        "rows": int(len(diagnostics)),
        "wells": int(diagnostics["well"].nunique()) if "well" in diagnostics else int(len(diagnostics)),
    }
    for col in ["rows", "pf_best_ll", "pf_ll_spread", "pf_gr_sig", "pf_ess_mean", "pf_resampling_rate"]:
        if col not in diagnostics:
            continue
        values = pd.to_numeric(diagnostics[col], errors="coerce")
...
def build_replay_train_frames(max_wells: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    print(
        f"step-delta public replay train wells: {len(train_wids)} | n_jobs={CFG.n_jobs} "
        f"| PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES} "
        f"| step_delta_prior={CFG.STEP_DELTA_PRIOR_NAME}",
        flush=True,
    )
    ...
    likpf_train, likpf_diag = build_likpf(train_wids, "train", return_diagnostics=True)
    ...
    meta = {
        ...
        "likpf_diagnostics": summarize_likpf_diagnostics(likpf_diag),
        "elapsed_feature_seconds": round(time.time() - t0, 3),
        "step_delta_prior": step_delta_prior_config(),
...
def build_replay_inference_frames(max_wells: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    print(
        f"step-delta public replay inference wells: train={len(train_wids)} test={len(test_wids)} "
...
def build_replay_test_frame() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    test_wids = list_test_wells()
    print(
        f"step-delta public replay test wells: train_imputer_wells={len(train_wids)} test={len(test_wids)} "
        f"| n_jobs={CFG.n_jobs} | PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES} "
        f"| step_delta_prior={CFG.STEP_DELTA_PRIOR_NAME}",
        flush=True,
    )
...
def feature_columns_for_variant(df: pd.DataFrame, variant: str) -> list[str]:
    ...
    if variant in {"pixiux_likpf_public_replay", "pixiux_likpf_step_delta_prior_public_replay"}:
