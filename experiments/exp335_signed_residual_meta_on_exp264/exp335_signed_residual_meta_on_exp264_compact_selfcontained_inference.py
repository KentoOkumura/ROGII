...
# %% [markdown]
# # exp335 signed-residual meta CPU inference
#
...
# outer別signed-residual 23特徴を作り、exp335 Stage D v2の15本でTVTをCPU予測する。
# 学習、hard selector、Viterbi、competition submitは行わない。
...
if not (PACKAGE_DIR / "config.yaml").exists():
    PACKAGE_DIR = Path("experiments/exp335_signed_residual_meta_on_exp264")
...
# ## 2. User authorization and saved-model contracts
#
# Stage Dのtail guard FAILは保持する。2026-07-23のユーザー指示を、保存modelによるCPU推論と
# submission file生成だけの例外承認として扱う。再学習や外部competition submitへは拡張しない。

# %%
inference_cfg = dict(config["inference"])
if not bool(config["execution"]["inference_approved"]):
    raise RuntimeError("exp335 inference is not approved")
if not bool(config["execution"]["run_inference"]):
    raise RuntimeError("exp335 inference run flag is disabled")
if inference_cfg.get("status") != "user_authorized_2026_07_23_cpu":
    raise RuntimeError("exp335 CPU inference does not have the fixed user authorization")
if bool(inference_cfg.get("stage_d_scientific_support_passed")):
    raise RuntimeError("Stage D guard failure must remain explicit during this override")
if bool(inference_cfg.get("stage_d_promotion_passed")):
    raise RuntimeError("Stage D promotion failure must remain explicit during this override")
if str(inference_cfg.get("runtime")) != "kaggle_cpu":
    raise RuntimeError("exp335 inference must use Kaggle CPU")
...
if len(names) != 12:
    raise ValueError("exp335 inference requires exactly 12 candidates")
...
chunk_size = int(config["model"]["selector"]["training"]["predict_base_row_chunk_size"])
...
# %% [markdown]
# ## 6. Exp335 Stage D saved-booster CPU inference
#
# 各TVT modelは学習時と同じouter foldのsaved74 + signed23だけを使う。GPUで学習したtext modelを
...
prediction_path = output_dir / "exp335_current_test_predictions.csv.gz"
feature_schema_path = output_dir / "exp335_inference_feature_schema.csv"
...
metrics = {
    ...
    "authorization": {
        ...
        "stage_d_scientific_support_passed": False,
        "stage_d_promotion_passed": False,
        "retained_guard_failure": inference_cfg["retained_guard_failure"],
    ...
    "stage_d_guard_evidence": {
        "saved_exp264_rmse": float(inference_cfg["stage_d_saved_exp264_rmse"]),
        "signed_residual_meta_rmse": float(inference_cfg["stage_d_variant_rmse"]),
        ...
        "scientific_support_passed": False,
    ...
    "notes": [
        ...
        "Each Stage D model receives saved74 and signed23 features from its matching downstream outer fold.",
        "All 40 parent selectors, 20 signed selectors, and 15 TVT models are SHA-verified; no model is fitted.",
        ...
        "submission.csv is generated for submit-check; external competition submission remains unauthorized.",
        "The pre-registered Stage D by-well tail failures remain recorded and are not reclassified as passes.",
    ],
}
