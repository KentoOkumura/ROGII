# %% [markdown]
# # exp338 well-adaptive transition noise inference
#
# exp338 is a train-side exp209 transition-scale audit. Raw-test prediction,
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if contract["experiment"] != EXPERIMENT_NAME or contract["route"] != "pf_beam":
        raise ValueError("unexpected exp338 inference config")
    if contract["mode"] != "disabled_until_train_side_promotion_and_separate_approval":
        raise ValueError("exp338 inference mode contract changed")
    if any(
        contract[key]
        for key in (
            "inference_enabled",
        ...
        raise ValueError("exp338 inference and submission must remain disabled")
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp338 is a train-side adaptive-transition exact-HMM audit; "
