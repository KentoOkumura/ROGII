# %% [markdown]
# # exp338 exp209 well-adaptive transition noise train
#
# Train-side exact-HMM audit of one preregistered well-specific rate-diffusion
# estimator. The trusted exp209 zero-fill population-standard-deviation GR
# observation model is reproduced exactly; saved exp209 HMM and LikPF caches
...
# 4. Exp209 observation model and adaptive transition-scale helpers
...
# 8. Diagnostic readouts, promotion gate, and generated artifacts
...
EXPERIMENT_NAME = "exp338_exp209_well_adaptive_transition_noise"
OUTPUT_PREFIX = EXPERIMENT_NAME
VARIANT_ORDER = ("robust_prefix_rate_diffusion_on_exp209",)
...
EXECUTE_NOTEBOOK = os.environ.get("EXP338_IMPORT_ONLY", "0") != "1" and in_notebook_runtime()
...
def load_experiment_config() -> dict[str, Any]:
    ...
    for path in candidates:
        ...
        if get_nested(config, "experiment.name") == EXPERIMENT_NAME:
            return config
    raise FileNotFoundError(f"exp338 config not found in {[str(path) for path in candidates]}")
...
def validate_scientific_contract(
    ...
    expected = {
        ...
        "model.transition_scale.minimum_innovations": 20,
        "model.transition_scale.fallback_sig_r": 0.002,
        "model.transition_scale.shrinkage_pseudocount": 100.0,
        "model.transition_scale.raw_scale_floor": 1.0e-6,
        ...
        "data.hidden_like_assignment.allowed_roles.hidden_like_spatial": [
            "train",
            "valid",
        ],
        ...
        "data.hidden_like_assignment.expected_role_counts.hidden_like_spatial": {
            "train": 573,
            "valid": 200,
        },
        "data.hidden_like_assignment.expected_role_counts.hidden_like_typewell_purged": {
            "train": 557,
            "valid": 200,
            "purged_train_excluded": 16,
        },
        "data.saved_exp209.expected_metrics_sha256": (
            "ebf7e24624d8b98f41cdda9c90a5df488031673f44d59a7485c9b2b62e6d67ae"
        ),
        "data.saved_exp209.accepted_metric_parity.expected_best_candidate": (
            "blend_likpf_hmm_w500"
        ),
        "data.saved_exp209.accepted_metric_parity.maximum_best_rmse_abs_difference": (
            1.0e-5
        ),
    ...
    for key, value in expected.items():
        if get_nested(config, key) != value:
            raise ValueError(f"exp338 fixed contract mismatch: {key} must be {value!r}")
    if [float(value) for value in get_nested(config, "model.transition_scale.clip")] != [
        0.001,
        0.004,
    ]:
        raise ValueError("exp338 fixes the sig_r clip to [0.001, 0.004]")
    ...
    for key, value in fixed_hmm.items():
        if float(hmm.get(key, -1.0)) != value:
            raise ValueError(f"exp338 fixes model.fixed_exp209_hmm.{key}={value}")
    if hmm.get("emission") != "gaussian" or hmm.get("rate_center") != "zero":
        raise ValueError("exp338 fixes Gaussian emission and zero-centered rate grid")
    fixed_policies = {
        "sigma_mode": "std",
    ...
    if [float(value) for value in hmm.get("sigma_clip", [])] != [10.0, 60.0]:
        raise ValueError("exp338 fixes exp209 sigma_GR clip to [10, 60]")
    forbidden = set(str(value) for value in get_nested(config, "model.forbidden") or [])
    if not {"finite_only_sigma_gr", "robust_or_mad_sigma_gr", "missing_gr_downweight"}.issubset(
        forbidden
    ):
        raise ValueError("exp338 must explicitly forbid exp307/exp308 observation changes")
    if not bool(get_nested(config, "execution.implementation_approved")):
        raise ValueError("exp338 implementation approval must be recorded")
    if require_run_approval:
        if not (
            bool(get_nested(config, "execution.kaggle_push_approved"))
            and bool(get_nested(config, "execution.run_transition_audit"))
            and bool(get_nested(config, "execution.run_hmm"))
        ):
            raise RuntimeError("exp338 Kaggle package/push/run is not approved")
...
def build_scientific_contract(config: dict[str, Any]) -> dict[str, Any]:
    contract = {
        ...
        "transition_scale": get_nested(config, "model.transition_scale"),
        ...
        "forbidden": [
            "finite_only_or_robust_sigma_gr",
            "missing_gr_downweight",
            "typewell_change",
            "affine_or_center_change",
            "sig_p_or_position_floor_change",
            "rate_grid_momentum_prior_or_output_change",
            "transition_scale_grid_or_rescue",
...
def validate_parent_metrics_contract(
    ...
    *,
    ...
    expected_parent_sha = str(saved["expected_hmm_decompressed_sha256"])
    expected_likpf_sha = str(saved["expected_likpf_decompressed_sha256"])
    if raw_sha256 != str(saved["expected_metrics_sha256"]):
        raise ValueError("exp209 parent metrics raw SHA mismatch")

    hmm_parity = (
        get_nested(
            parent_metrics,
            "reference_parity.exp205_hmm_train_features",
        )
        or {}
    )
    likpf_generation = (
        get_nested(
            parent_metrics,
            "reference_parity.exp072_train_features",
        )
        or {}
    )
    metric_parity = parent_metrics.get("metric_parity") or {}
    acceptance = saved.get("accepted_metric_parity") or {}
    expected_best = str(acceptance.get("expected_best_candidate", ""))
    maximum_difference = float(
        acceptance.get("maximum_best_rmse_abs_difference", -1.0)
    )
    actual_difference = float(metric_parity.get("best_rmse_abs_diff", np.inf))

    if (
        parent_metrics.get("experiment")
        != "exp209_exp072_exp205_joint_exact_parity_fast_cache_generation"
        or int(parent_metrics.get("rows", -1)) != expected_rows
        or int(parent_metrics.get("wells", -1)) != expected_wells
        or not bool(hmm_parity.get("matches_expected_decompressed_sha256"))
        or not bool(hmm_parity.get("matches_reference_decompressed_sha256"))
        or hmm_parity.get("generated_decompressed_sha256") != expected_parent_sha
        or hmm_parity.get("expected_decompressed_sha256") != expected_parent_sha
        or likpf_generation.get("generated_decompressed_sha256") != expected_likpf_sha
        or not bool(metric_parity.get("checked"))
        or not bool(metric_parity.get("best_candidate_matches"))
        or metric_parity.get("actual_best_candidate") != expected_best
        or metric_parity.get("expected_best_candidate") != expected_best
        or maximum_difference < 0.0
        or actual_difference > maximum_difference
    ):
        raise ValueError("exp209 parent exact-parity dependency mismatch")
    return {
        "status_at_generation": parent_metrics.get("status"),
        "hmm_exact_parity_passed": True,
        "metric_parity_accepted": True,
        "metric_parity_acceptance_source": acceptance.get("acceptance_source"),
        "best_candidate": expected_best,
        "best_rmse_abs_difference": actual_difference,
        "accepted_metric_abs_tolerance": maximum_difference,
        "hmm_prediction_decompressed_sha256": expected_parent_sha,
        "likpf_cache_decompressed_sha256": expected_likpf_sha,
    }


def validate_hidden_like_role_contract(
    hidden_frame: pd.DataFrame,
    hidden: dict[str, Any],
) -> dict[str, dict[str, int]]:
    role_columns = {str(key): str(value) for key, value in hidden["role_columns"].items()}
    allowed_roles = hidden.get("allowed_roles") or {}
    expected_role_counts = hidden.get("expected_role_counts") or {}
    report: dict[str, dict[str, int]] = {}
    for scope, column in role_columns.items():
        actual_counts = {
            str(key): int(value)
            for key, value in hidden_frame[column]
            .astype(str)
            .value_counts(dropna=False)
            .sort_index()
            .items()
        }
        allowed = {str(value) for value in allowed_roles.get(scope, [])}
        expected = {
            str(key): int(value)
            for key, value in (expected_role_counts.get(scope) or {}).items()
        }
        if set(actual_counts) != allowed or actual_counts != expected:
            raise ValueError(
                f"late hidden-like role contract mismatch for {scope}: "
                f"actual={actual_counts}, allowed={sorted(allowed)}, expected={expected}"
            )
        report[scope] = actual_counts
    return report


def preflight_controls_and_assignments(config: dict[str, Any]) -> dict[str, Any]:
    saved = get_nested(config, "data.saved_exp209") or {}
    metrics_candidates = [str(value) for value in saved.get("metrics_candidates", [])]
    paths = {
        "saved_hmm": resolve_existing(str(saved["hmm_cache_filename"]), _candidate_paths(saved)),
        "parent_metrics": resolve_existing(str(saved["metrics_filename"]), metrics_candidates),
        "saved_likpf": resolve_existing(
            str(saved["likpf_cache_filename"]), _candidate_paths(saved)
        ),
    }
    parent_report = inspect_gzip_csv(paths["saved_hmm"])
    likpf_report = inspect_gzip_csv(paths["saved_likpf"])
    expected_parent_sha = str(saved["expected_hmm_decompressed_sha256"])
    expected_likpf_sha = str(saved["expected_likpf_decompressed_sha256"])
    if (
        len(expected_parent_sha) != 64
        or parent_report["decompressed_sha256"] != expected_parent_sha
    ):
        raise ValueError("saved exp209 HMM prediction decompressed SHA mismatch")
    if len(expected_likpf_sha) != 64 or likpf_report["decompressed_sha256"] != expected_likpf_sha:
        raise ValueError("saved exp209 LikPF cache decompressed SHA mismatch")
    hmm_columns = set(pd.read_csv(paths["saved_hmm"], nrows=0).columns.astype(str))
    likpf_columns = set(pd.read_csv(paths["saved_likpf"], nrows=0).columns.astype(str))
    if not {"id", "well", str(saved["hmm_prediction_column"])}.issubset(hmm_columns):
        raise ValueError("saved exp209 HMM column contract mismatch")
    if not {
        "id",
        "well",
        "md_since",
    ...
    }.issubset(likpf_columns):
        raise ValueError("saved exp209 LikPF column contract mismatch")
    expected_rows = int(get_nested(config, "validation.expected_rows"))
    expected_wells = int(get_nested(config, "validation.expected_wells"))
    parent_metrics = json.loads(paths["parent_metrics"].read_text())
    parent_metrics_raw_sha = sha256_path(paths["parent_metrics"])
    parent_metrics_contract = validate_parent_metrics_contract(
        parent_metrics,
        raw_sha256=parent_metrics_raw_sha,
        saved=saved,
        expected_rows=expected_rows,
        expected_wells=expected_wells,
    )
    if (
        int(parent_report["data_rows"]) != expected_rows
        or int(likpf_report["data_rows"]) != expected_rows
    ):
        raise ValueError("saved exp209 control row/well coverage mismatch")
    ...
    fold_columns = set(pd.read_csv(fold_path, nrows=0).columns.astype(str))
    required_fold_columns = {
        *[str(value) for value in fold["safe_columns"]],
        *[str(value) for value in fold["truth_columns"]],
    }
    if fold_report["data_rows"] != expected_rows or not required_fold_columns.issubset(
        fold_columns
    ):
        raise ValueError("fold assignment row/column contract mismatch")
    ...
    hidden_columns = set(pd.read_csv(hidden_path, nrows=0).columns.astype(str))
    role_columns = [str(value) for value in hidden["role_columns"].values()]
    if not {"well_id", *role_columns}.issubset(hidden_columns):
    ...
    return {
        ...
        "saved_hmm": {**parent_report, "columns": sorted(hmm_columns)},
        "parent_metrics": {
            "path": str(paths["parent_metrics"]),
            "raw_sha256": parent_metrics_raw_sha,
            **parent_metrics_contract,
        },
        "saved_likpf": {**likpf_report, "columns": sorted(likpf_columns)},
        "fold_assignment": {
            **fold_report,
            "columns": sorted(fold_columns),
            "parsed_before_prediction_freeze": False,
        },
        "hidden_like_assignment": {
            "path": str(hidden_path),
            "raw_sha256": hidden_sha,
            "columns": sorted(hidden_columns),
            "parsed_before_prediction_freeze": False,
        },
    }
...
def compute_exp209_zero_fill_sigma_audit(
    ...
    *,
    ...
    """Reproduce exp209 sigma_mode=std exactly on the known prefix."""
    ...
    if len(known_index) == 0 or not np.array_equal(known_index, np.arange(len(known_index))):
        raise ValueError("exp338 requires one contiguous known TVT_input prefix")
    ...
    residual = np.where(np.isfinite(horizontal_gr[known_mask]), horizontal_gr[known_mask], 0.0)
    residual = residual - typewell_at_known
    raw_sigma = float(np.nanstd(residual, ddof=0))
    if not math.isfinite(raw_sigma):
        raise ValueError("exp209 zero-fill population std must be finite")
    sigma = float(np.clip(raw_sigma, clip[0], clip[1]))
    return {
        "known_prefix_rows": int(known_mask.sum()),
        "finite_known_gr_rows": int(np.isfinite(horizontal_gr[known_mask]).sum()),
        "missing_known_gr_rows": int((~np.isfinite(horizontal_gr[known_mask])).sum()),
        "sigma_gr_raw": raw_sigma,
        "sigma_gr": sigma,
        "sigma_gr_clip_low": bool(raw_sigma < clip[0]),
        "sigma_gr_clip_high": bool(raw_sigma > clip[1]),
        "sigma_mode": "std",
...
def compute_prefix_transition_scale_audit(
    horizontal: pd.DataFrame,
    *,
    minimum_innovations: int = 20,
    fallback_sig_r: float = 0.002,
    shrinkage_pseudocount: float = 100.0,
    raw_scale_floor: float = 1.0e-6,
    clip: tuple[float, float] = (0.001, 0.004),
) -> dict[str, Any]:
    md = pd.to_numeric(horizontal["MD"], errors="coerce").to_numpy(np.float64)
    z = pd.to_numeric(horizontal["Z"], errors="coerce").to_numpy(np.float64)
    ...
    rates = np.full(len(dmd), np.nan, dtype=np.float64)
    rates[pair_valid] = du[pair_valid] / dmd[pair_valid]
    adjacent_valid = pair_valid[:-1] & pair_valid[1:]
    mean_step = 0.5 * (dmd[:-1] + dmd[1:])
    innovations = np.full(max(0, len(rates) - 1), np.nan, dtype=np.float64)
    innovations[adjacent_valid] = (
        rates[1:][adjacent_valid] - rates[:-1][adjacent_valid]
    ) / np.sqrt(mean_step[adjacent_valid])
    finite_innovations = innovations[np.isfinite(innovations)]
    n = int(len(finite_innovations))
    if n:
        center = float(np.median(finite_innovations))
        raw_scale = float(1.4826 * np.median(np.abs(finite_innovations - center)))
        abs_median = float(np.median(np.abs(finite_innovations)))
    else:
        center = float("nan")
        raw_scale = float("nan")
        abs_median = float("nan")
    fallback_used = n < minimum_innovations or not math.isfinite(raw_scale)
    alpha = float(n / (n + shrinkage_pseudocount)) if not fallback_used else 0.0
    if fallback_used:
        unclip_sig_r = float(fallback_sig_r)
    else:
        unclip_sig_r = float(
            math.exp(
                alpha * math.log(max(raw_scale, raw_scale_floor))
                + (1.0 - alpha) * math.log(fallback_sig_r)
            )
        )
    sig_r = float(np.clip(unclip_sig_r, clip[0], clip[1]))
    return {
        "known_prefix_rows": int(np.isfinite(tvt).sum()),
        "valid_rate_pairs": int(pair_valid.sum()),
        "valid_rate_innovations": n,
        "innovation_median": center,
        "innovation_abs_median": abs_median,
        "raw_sig_r": raw_scale,
        "shrinkage_alpha": alpha,
        "unclipped_sig_r": unclip_sig_r,
        "sig_r": sig_r,
        "sig_r_fallback": bool(fallback_used),
        "sig_r_clip_low": bool(unclip_sig_r < clip[0]),
        "sig_r_clip_high": bool(unclip_sig_r > clip[1]),
    }
...
def prepare_hmm_inputs(
    ...
    hmm = get_nested(config, "model.fixed_exp209_hmm") or {}
    transition_config = get_nested(config, "model.transition_scale") or {}
    tvt_input = pd.to_numeric(horizontal["TVT_input"], errors="coerce").to_numpy(np.float64)
    if np.isinf(tvt_input).any():
        raise ValueError("TVT_input may contain finite known values or NaN only")
    known_mask = np.isfinite(tvt_input)
    known_index = np.flatnonzero(known_mask)
    eval_index = np.flatnonzero(~known_mask)
    if (
        len(known_index) == 0
        or len(eval_index) == 0
        or not np.array_equal(known_index, np.arange(len(known_index)))
    ):
        raise ValueError("each exp338 well must contain one contiguous known prefix and suffix")
    ...
    if len(typewell_tvt) < 2 or np.any(np.diff(typewell_tvt) < 0.0):
        raise ValueError("typewell TVT/GR contract is invalid")
    sigma_audit = compute_exp209_zero_fill_sigma_audit(
        horizontal,
        typewell_tvt,
        typewell_gr,
        clip=tuple(float(value) for value in hmm["sigma_clip"]),
    )
    transition_audit = compute_prefix_transition_scale_audit(
        horizontal,
        minimum_innovations=int(transition_config["minimum_innovations"]),
        fallback_sig_r=float(transition_config["fallback_sig_r"]),
        shrinkage_pseudocount=float(transition_config["shrinkage_pseudocount"]),
        raw_scale_floor=float(transition_config["raw_scale_floor"]),
        clip=tuple(float(value) for value in transition_config["clip"]),
    )
    ...
    return {
        ...
        "observation_weight": np.ones(len(eval_index), dtype=np.float64),
        "dm": dm,
        "dz": dz,
        "rates": rates,
...
def run_exact_hmm_variant(
    ...
    if not math.isfinite(float(sigma_gr)) or not 10.0 <= float(sigma_gr) <= 60.0:
        raise ValueError("exp338 exp209 sigma_gr must satisfy the frozen [10, 60] contract")
    if not math.isfinite(float(sig_r)) or not 0.001 <= float(sig_r) <= 0.004:
        raise ValueError("exp338 sig_r must satisfy the frozen [0.001, 0.004] contract")
...
def decode_well(
        ...
        runtime_rows.append(
            {
                ...
                "variant": variant,
                "rows": len(eval_index),
                "sigma_gr": float(prepared["sigma_audit"]["sigma_gr"]),
                "sig_r": float(prepared["transition_audit"]["sig_r"]),
                "observation_weight_min": float(prepared["observation_weight"].min()),
                "observation_weight_mean": float(prepared["observation_weight"].mean()),
    ...
    scale_row = {
        "well_id": well,
        **{f"exp209_{key}": value for key, value in prepared["sigma_audit"].items()},
        **prepared["transition_audit"],
    }
...
def generate_and_freeze_predictions(
    ...
    if not NUMBA_AVAILABLE:
        raise RuntimeError("numba is required for the exp338 exact-HMM audit")
    ...
    def build_one(index: int, well: str):
        print(f"[{index}/{len(wells)}] exp338 well={well}", flush=True)
    ...
    transition_audit = pd.DataFrame([row[1] for row in results])
    ...
    transition_audit = transition_audit.sort_values("well_id", kind="mergesort").reset_index(
        drop=True
    )
    ...
    if (
        ...
        or len(transition_audit) != expected_wells
        or len(runtime) != int(get_nested(config, "execution_contract.hmm_well_runs"))
    ):
        raise ValueError("exp338 prediction/transition-audit/runtime coverage mismatch")
    prediction_path = artifacts / f"{OUTPUT_PREFIX}_predictions.csv.gz"
    audit_path = artifacts / f"{OUTPUT_PREFIX}_transition_scale_audit.csv.gz"
    ...
    transition_audit.to_csv(audit_path, index=False, compression="gzip")
    runtime.to_csv(runtime_path, index=False)
    prediction_report = inspect_gzip_csv(prediction_path)
    audit_report = inspect_gzip_csv(audit_path)
    prediction_report["frozen_before_truth_attachment"] = True
    audit_report["frozen_before_truth_attachment"] = True
    reports = {
        "prediction": prediction_report,
        "transition_audit": audit_report,
        "runtime": {
    ...
    return (
        reports,
        {"prediction": prediction_path, "transition_audit": audit_path, "runtime": runtime_path},
        runtime,
    )
...
def load_late_readout_frame(
    ...
    saved = get_nested(config, "data.saved_exp209") or {}
    likpf_frame = pd.read_csv(
        preflight["controls"]["paths"]["saved_likpf"],
        usecols=[
            "id",
            "well",
            "md_since",
    ...
    parent_frame = pd.read_csv(
        ...
        usecols=["id", "well", str(saved["hmm_prediction_column"])],
        dtype={"id": str, "well": str},
    )
    likpf_frame["row_idx"] = pd.to_numeric(
        likpf_frame["id"].astype(str).str.rsplit("_", n=1).str[-1], errors="raise"
    ).astype(np.int64)
    likpf_frame.sort_values(["well", "row_idx"], kind="mergesort", inplace=True)
    likpf_frame.reset_index(drop=True, inplace=True)
    parent_frame["row_idx"] = pd.to_numeric(
        parent_frame["id"].astype(str).str.rsplit("_", n=1).str[-1], errors="raise"
    ).astype(np.int64)
    parent_frame.sort_values(["well", "row_idx"], kind="mergesort", inplace=True)
    parent_frame.reset_index(drop=True, inplace=True)
    _assert_same_order("prediction vs exp072 LikPF", prediction["id"], likpf_frame["id"])
    _assert_same_order("prediction vs exp209 HMM", prediction["id"], parent_frame["id"])
    fold = get_nested(config, "data.fold_assignment") or {}
    ...
    if (
        len(truth) != expected_rows
        or truth["well_id"].nunique() != expected_wells
        or truth["id"].duplicated().any()
        or sorted(pd.to_numeric(truth["fold"], errors="raise").unique().tolist()) != expected_folds
    ):
        raise ValueError("late fold/truth row, well, identity, or fold coverage mismatch")
    ...
    if hidden_frame.index.duplicated().any() or sorted(hidden_frame.index.astype(str)) != sorted(
        truth["well_id"].astype(str).unique().tolist()
    ):
        raise ValueError("late hidden-like well identity mismatch")
    hidden_role_counts = validate_hidden_like_role_contract(hidden_frame, hidden)
    frame = pd.DataFrame(
        {
            ...
            "md_since": pd.to_numeric(likpf_frame["md_since"], errors="raise").to_numpy(np.float64),
            "parent_hmm_tvt": pd.to_numeric(
                parent_frame[str(saved["hmm_prediction_column"])], errors="raise"
            ).to_numpy(np.float64),
            "likpf_mean": (
                pd.to_numeric(
                    likpf_frame[str(saved["likpf_anchor_column"])], errors="raise"
                ).to_numpy(np.float64)
                + pd.to_numeric(
                    likpf_frame[str(saved["likpf_delta_column"])], errors="raise"
                ).to_numpy(np.float64)
            ),
        }
    )
    ...
    frame["parent_hmm_likpf_50_50"] = 0.5 * frame["parent_hmm_tvt"] + 0.5 * frame["likpf_mean"]
    ...
    return frame, {
        "truth_attachment_stage": "after_transition_audit_and_prediction_gzip_content_sha_freeze",
        "prediction_content_sha256": frozen["prediction"]["content_sha256"],
        "transition_audit_content_sha256": frozen["transition_audit"]["content_sha256"],
        "rows": len(frame),
        "wells": int(frame["well_id"].nunique()),
        "id_mismatches": 0,
        "hidden_like_role_counts": hidden_role_counts,
    }
...
def build_diagnostic_scope_metrics(
    frame: pd.DataFrame,
    transition_audit: pd.DataFrame,
) -> pd.DataFrame:
    audit = transition_audit.copy()
    if audit["well_id"].duplicated().any():
        raise ValueError("transition audit has duplicate wells")
    quantile_count = min(5, len(audit))
    if quantile_count < 1:
        raise ValueError("transition audit is empty")
    audit["sig_r_quintile"] = (
        pd.qcut(audit["sig_r"].rank(method="first"), q=quantile_count, labels=False)
        .astype(np.int64)
        .map(lambda value: f"q{value + 1}")
    )
    turning = pd.to_numeric(audit["innovation_abs_median"], errors="coerce").fillna(-1.0)
    audit["turning_quintile"] = (
        pd.qcut(turning.rank(method="first"), q=quantile_count, labels=False)
        .astype(np.int64)
        .map(lambda value: f"q{value + 1}")
    )
    support = pd.to_numeric(audit["valid_rate_innovations"], errors="raise")
    audit["support_bucket"] = np.select(
        [
            audit["sig_r_fallback"].astype(bool),
            support < 100,
            support < 500,
        ],
        ["fallback", "20_099", "100_499"],
        default="500_plus",
    )
    lookup = audit.set_index("well_id")
    annotated = frame.copy()
    for column in ("sig_r_quintile", "turning_quintile", "support_bucket"):
        annotated[column] = annotated["well_id"].map(lookup[column])
        if annotated[column].isna().any():
            raise ValueError(f"diagnostic audit join failed for {column}")
    candidate = f"{VARIANT_ORDER[0]}_hmm_tvt"
    scopes: list[tuple[str, np.ndarray]] = []
    for column in ("sig_r_quintile", "turning_quintile", "support_bucket"):
        for value in sorted(annotated[column].astype(str).unique()):
            scopes.append((f"{column}_{value}", annotated[column].astype(str).to_numpy() == value))
    distance = annotated["md_since"].to_numpy(np.float64)
    distance_scopes = (
    ...
    return pd.DataFrame(
        [
            paired_metric_row(
                ...
                comparison="direct_diagnostic",
                candidate_column=candidate,
                control_column="parent_hmm_tvt",
                scope=scope,
            )
            for scope, mask in scopes
            if bool(mask.any())
...
def evaluate_promotion_gate(
    ...
    fallback_fraction = float(transition_audit["sig_r_fallback"].astype(bool).mean())
    clip_fraction = float(
        (
            transition_audit["sig_r_clip_low"].astype(bool)
            | transition_audit["sig_r_clip_high"].astype(bool)
        ).mean()
    )
    technical = {
        "input_preflight_passed": True,
        "parent_dependency_gate_passed": bool(
            preflight["controls"]["parent_metrics"]["hmm_exact_parity_passed"]
        ),
        "prediction_rows": len(frame),
        "prediction_wells": int(frame["well_id"].nunique()),
        "transition_audit_wells": len(transition_audit),
        ...
        "audit_finite_coverage": float(
            np.isfinite(
                transition_audit[
                    [
                        "exp209_sigma_gr",
                        "exp209_observation_weight",
                        "sig_r",
                    ]
                ].to_numpy(np.float64)
            ).mean()
        ),
        "sig_r_fallback_fraction": fallback_fraction,
        "maximum_fallback_fraction": float(gate["maximum_fallback_fraction"]),
        "sig_r_total_clip_fraction": clip_fraction,
        "maximum_total_clip_fraction": float(gate["maximum_total_clip_fraction"]),
        ...
        "parent_prediction_sha256": preflight["controls"]["saved_hmm"]["decompressed_sha256"],
    }
    technical["passed"] = bool(
        technical["parent_dependency_gate_passed"]
        and technical["prediction_rows"] == expected_rows
        and technical["prediction_wells"] == expected_wells
        and technical["transition_audit_wells"] == expected_wells
        ...
        and technical["audit_finite_coverage"] == 1.0
        and fallback_fraction < technical["maximum_fallback_fraction"]
        and clip_fraction < technical["maximum_total_clip_fraction"]
    ...
    return {
        ...
        "passed": passed,
        "decision": (
            "adaptive_sig_r_passed_train_side_only_no_automatic_downstream"
            if passed
            else "adaptive_sig_r_failed_close_without_rescue"
        ),
        "technical_gate": technical,
        "primary_direct_gate": primary_direct,
        "fixed_likpf_50_50_guard": blend_guard,
        "failure_action": (
            "close_without_sig_r_grid_shrinkage_change_sig_p_momentum_rate_grid_"
            "pf_beam_inference_or_submission"
        ),
    }
...
def run_full_experiment(config: dict[str, Any]) -> dict[str, Any]:
    if not KAGGLE_WORKING_ROOT.exists() and os.environ.get("EXPERIMENT_ALLOW_LOCAL") != "1":
        raise RuntimeError(
            "Full exp338 must run first on Kaggle; local execution requires explicit smoke approval"
        )
    ...
    wells = [str(value) for value in raw_preflight.pop("well_ids")]
    ...
    transition_audit = pd.read_csv(output_paths["transition_audit"], dtype={"well_id": str})
    ...
    diagnostic_metrics = build_diagnostic_scope_metrics(frame, transition_audit)
    ...
    output_metric_paths = {
        ...
        "diagnostic_scope_metrics": artifacts / f"{OUTPUT_PREFIX}_diagnostic_scope_metrics.csv",
    ...
    diagnostic_metrics.to_csv(output_metric_paths["diagnostic_scope_metrics"], index=False)
    ...
    status = (
        "train_side_adaptive_sig_r_gate_passed_no_automatic_downstream"
        if promotion_gate["passed"]
        else "train_side_adaptive_sig_r_gate_failed_closed"
    )
    summary = {
        "experiment": EXPERIMENT_NAME,
        "status": status,
    ...
    summary["generated_files"] = output_file_reports(
        {
            **output_metric_paths,
            ...
            "transition_scale_audit": Path(output_paths["transition_audit"]),
    ...
    metrics = {
        ...
        "transition_audit_sha256": frozen["transition_audit"],
        "model_sha256": None,
        "submission_sha256": None,
