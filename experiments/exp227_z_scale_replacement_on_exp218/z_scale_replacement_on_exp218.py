...
OUTPUT_PREFIX = "exp227_z_scale_replacement_on_exp218"
...
def find_model_manifest(explicit_path: str | Path | None = None) -> Path:
    ...
    kaggle_input = Path("/kaggle/input")
    if kaggle_input.exists():
        kaggle_manifests = sorted(kaggle_input.glob(f"**/{OUTPUT_PREFIX}_lgb_models/manifest.json"))
        if explicit_path is None and len(kaggle_manifests) > 1:
            found = "\n".join(str(path) for path in kaggle_manifests[:20])
            raise RuntimeError(
                "Multiple split train manifests found. Build an aggregate manifest "
                "or pass an explicit model_manifest_path before saved-booster inference.\n"
                f"{found}"
            )
        candidates.extend(kaggle_manifests)
    for candidate in candidates:
        if candidate.exists() and candidate.stat().st_size > 0:
            return candidate
...
def _robust_stats(values: np.ndarray, min_scale: float) -> dict[str, float]:
    numeric = np.asarray(values, dtype=np.float32)
    finite = numeric[np.isfinite(numeric)]
    if finite.size == 0:
        return {
            "median": 0.0,
            "mad": max(float(min_scale), 1.0),
            "iqr": max(float(min_scale), 1.0),
            "p05": 0.0,
            "p25": 0.0,
            "p75": 0.0,
            "p95": 0.0,
            "p05_p95_range": max(float(min_scale), 1.0),
        }
    median = float(np.median(finite))
    p05, p25, p75, p95 = [float(value) for value in np.percentile(finite, [5, 25, 75, 95])]
    mad = float(np.median(np.abs(finite - median))) * 1.4826
    iqr = p75 - p25
    robust_range = p95 - p05
    return {
        "median": median,
        "mad": max(abs(mad), float(min_scale)),
        "iqr": max(abs(iqr), float(min_scale)),
        "p05": p05,
        "p25": p25,
        "p75": p75,
        "p95": p95,
        "p05_p95_range": max(abs(robust_range), float(min_scale)),
    }
...
def _rank_pct(values: np.ndarray) -> np.ndarray:
    numeric = np.asarray(values, dtype=np.float32)
    if len(numeric) <= 1:
        return np.full(len(numeric), 0.5, dtype=np.float32)
    ranks = pd.Series(numeric).rank(method="average").to_numpy(np.float32)
    return ((ranks - 1.0) / float(len(numeric) - 1)).astype(np.float32)
...
def build_well_scaled_z_dz_features(
    base_frame: pd.DataFrame,
    config: dict[str, Any] | None = None,
) -> tuple[pd.DataFrame, dict[str, list[str]], pd.DataFrame, dict[str, Any]]:
    config = config or {}
    prefix = str(config.get("prefix") or "wsz_")
    group_name = str(config.get("feature_group") or "well_scaled_z_dz")
    source_columns = [str(col) for col in config.get("source_columns") or ["z", "dz", "dzdmd", "slp_z"]]
    scaled_interaction_columns = [
        str(col) for col in config.get("scaled_interaction_columns") or ["z", "dz"]
    ]
    raw_interaction_columns = [str(col) for col in config.get("raw_interaction_columns") or ["gr"]]
    grwr_interaction_columns = [
        str(col)
        for col in config.get("grwr_interaction_columns")
        or [
            "grwr_fft_rotation_ratio_x_log1p_md_since",
            "grwr_candidate_tvt_std",
            "grwr_candidate_tvt_range",
            "grwr_known_prefix_fraction",
        ]
    ]
    min_scale = float(config.get("min_scale", 1.0e-6))
    include_likpf_range_scale = bool(config.get("include_likpf_range_scale", True))
    likpf_base_column = str(config.get("likpf_base_column") or "last_known_tvt")
    likpf_delta_column = str(config.get("likpf_delta_column") or "likpf_mean_d")
    likpf_quantiles = list(config.get("likpf_range_quantiles") or [0.05, 0.95])
    if len(likpf_quantiles) != 2:
        raise ValueError("well_scaled_z_dz_features.likpf_range_quantiles must have two values")
    likpf_q_low = float(likpf_quantiles[0])
    likpf_q_high = float(likpf_quantiles[1])
    if not 0.0 <= likpf_q_low < likpf_q_high <= 1.0:
        raise ValueError(
            "well_scaled_z_dz_features.likpf_range_quantiles must satisfy "
            "0 <= low < high <= 1"
        )

    required = set(source_columns)
    if include_likpf_range_scale:
        required.update([likpf_base_column, likpf_delta_column])
    missing = sorted(required - set(base_frame.columns))
    if missing:
        raise ValueError(f"well-scaled z/dz feature frame missing required columns: {missing}")

    source_values = {column: _numeric_array(base_frame, column) for column in source_columns}
    raw_interactions = {
        column: _numeric_array(base_frame, column)
        for column in raw_interaction_columns
        if column in base_frame.columns
    }
    grwr_interactions = {
        column: _numeric_array(base_frame, column)
        for column in grwr_interaction_columns
        if column in base_frame.columns
    }
    likpf_tvt = None
    if include_likpf_range_scale:
        likpf_tvt = (
            _numeric_array(base_frame, likpf_base_column)
            + _numeric_array(base_frame, likpf_delta_column)
        ).astype(np.float32)
    ...
    for well, positions_raw in base_frame.groupby("well", sort=False).indices.items():
        ...
        scaled_cache: dict[str, dict[str, np.ndarray]] = {}
        well_summary: dict[str, Any] = {
            "well": str(well),
            "rows": int(len(positions)),
        }
        for source_column in source_columns:
            values = source_values[source_column][positions].astype(np.float32)
            stats = _robust_stats(values, min_scale)
            centered = (values - stats["median"]).astype(np.float32)
            mad_scaled = (centered / stats["mad"]).astype(np.float32)
            iqr_scaled = (centered / stats["iqr"]).astype(np.float32)
            range_scaled = (centered / stats["p05_p95_range"]).astype(np.float32)
            rel_range = ((values - stats["p05"]) / stats["p05_p95_range"]).astype(np.float32)
            safe_col = _safe_feature_name(source_column)
            columns[f"{prefix}{safe_col}_centered"] = centered
            columns[f"{prefix}{safe_col}_mad_scaled"] = mad_scaled
            columns[f"{prefix}{safe_col}_iqr_scaled"] = iqr_scaled
            columns[f"{prefix}{safe_col}_p05_p95_scaled"] = range_scaled
            columns[f"{prefix}{safe_col}_rank_pct"] = _rank_pct(values)
            columns[f"{prefix}{safe_col}_rel_p05_p95"] = rel_range
            columns[f"{prefix}{safe_col}_well_mad_log1p"] = np.full(
                len(positions),
                np.log1p(abs(stats["mad"])),
                dtype=np.float32,
            )
            columns[f"{prefix}{safe_col}_well_iqr_log1p"] = np.full(
                len(positions),
                np.log1p(abs(stats["iqr"])),
                dtype=np.float32,
            )
            columns[f"{prefix}{safe_col}_well_p05_p95_range_log1p"] = np.full(
                len(positions),
                np.log1p(abs(stats["p05_p95_range"])),
                dtype=np.float32,
            )
            scaled_cache[source_column] = {
                "centered": centered,
                "p05_p95_scaled": range_scaled,
            }
            well_summary[f"{safe_col}_median"] = stats["median"]
            well_summary[f"{safe_col}_mad"] = stats["mad"]
            well_summary[f"{safe_col}_iqr"] = stats["iqr"]
            well_summary[f"{safe_col}_p05_p95_range"] = stats["p05_p95_range"]

        if likpf_tvt is not None:
            likpf_values = likpf_tvt[positions].astype(np.float32)
            finite = likpf_values[np.isfinite(likpf_values)]
            if finite.size:
                low, high = [
                    float(value)
                    for value in np.quantile(finite, [likpf_q_low, likpf_q_high])
                ]
                likpf_range = max(abs(high - low), min_scale)
            else:
                low = 0.0
                high = 0.0
                likpf_range = max(1.0, min_scale)
            columns[f"{prefix}likpf_tvt_p05_p95_range_log1p"] = np.full(
                len(positions),
                np.log1p(likpf_range),
                dtype=np.float32,
            )
            if "z" in source_values:
                z_values = source_values["z"][positions].astype(np.float32)
                z_centered = scaled_cache.get("z", {}).get("centered", z_values)
                columns[f"{prefix}z_over_likpf_tvt_p05_p95_range"] = (
                    z_values / likpf_range
                ).astype(np.float32)
                columns[f"{prefix}z_centered_over_likpf_tvt_p05_p95_range"] = (
                    z_centered / likpf_range
                ).astype(np.float32)
            if "dz" in source_values:
                dz_values = source_values["dz"][positions].astype(np.float32)
                dz_centered = scaled_cache.get("dz", {}).get("centered", dz_values)
                columns[f"{prefix}dz_over_likpf_tvt_p05_p95_range"] = (
                    dz_values / likpf_range
                ).astype(np.float32)
                columns[f"{prefix}dz_centered_over_likpf_tvt_p05_p95_range"] = (
                    dz_centered / likpf_range
                ).astype(np.float32)
            well_summary["likpf_tvt_q_low"] = low
            well_summary["likpf_tvt_q_high"] = high
            well_summary["likpf_tvt_p05_p95_range"] = likpf_range

        for scaled_column in scaled_interaction_columns:
            if scaled_column not in scaled_cache:
                continue
            safe_scaled = _safe_feature_name(scaled_column)
            scaled_values = scaled_cache[scaled_column]["p05_p95_scaled"]
            for raw_column, raw_values_all in raw_interactions.items():
                raw_values = raw_values_all[positions].astype(np.float32)
                raw_stats = _robust_stats(raw_values, min_scale)
                raw_scaled = ((raw_values - raw_stats["median"]) / raw_stats["p05_p95_range"]).astype(
                    np.float32
                )
                columns[
                    f"{prefix}{safe_scaled}_p05_p95_scaled_x_{_safe_feature_name(raw_column)}_p05_p95_scaled"
                ] = (scaled_values * raw_scaled).astype(np.float32)
            for grwr_column, grwr_values_all in grwr_interactions.items():
                grwr_values = grwr_values_all[positions].astype(np.float32)
                columns[
                    f"{prefix}{safe_scaled}_p05_p95_scaled_x_{_safe_feature_name(grwr_column)}"
                ] = (scaled_values * grwr_values).astype(np.float32)

        feature_frames.append(pd.DataFrame(columns))
    ...
    _assert_finite_columns(features, feature_cols, "well-scaled z/dz feature frame")
    if not base_frame[["id", "well"]].reset_index(drop=True).equals(
        features[["id", "well"]].reset_index(drop=True)
    ):
        features = base_frame[["id", "well"]].merge(
            features,
            on=["id", "well"],
            how="left",
            validate="one_to_one",
        ...
        [
            {
                ...
                "scaled_interaction_columns": ",".join(scaled_interaction_columns),
                "raw_interaction_columns": ",".join(raw_interactions),
                "grwr_interaction_columns": ",".join(grwr_interactions),
                "include_likpf_range_scale": include_likpf_range_scale,
                "likpf_range_quantiles": f"{likpf_q_low},{likpf_q_high}",
            }
        ]
    )
    summary = pd.concat([summary_overview, summary], ignore_index=True, sort=False)
    meta = {
        "source_kind": "target_free_well_scaled_z_dz_features",
        "generated_feature_count": int(len(feature_cols)),
        "generated_feature_columns": feature_cols,
        "source_columns": source_columns,
        "scaled_interaction_columns": scaled_interaction_columns,
        "raw_interaction_columns": list(raw_interactions),
        "grwr_interaction_columns": list(grwr_interactions),
        "include_likpf_range_scale": include_likpf_range_scale,
        "likpf_base_column": likpf_base_column,
        "likpf_delta_column": likpf_delta_column,
        "likpf_range_quantiles": [likpf_q_low, likpf_q_high],
        "min_scale": min_scale,
    }
    return features, group_columns, summary, meta
...
def build_sp45_bimodal_selector_features(
    ...
    raise NotImplementedError(
        "exp227 does not implement the legacy sp45 inference feature wrapper. "
        "Train-side GRWR and well-scaled z/dz features require raw train_dir and are generated by "
...
def feature_columns_for_variant(
    ...
    drop_base_columns = {str(col) for col in variant.get("drop_base_columns") or []}
    unknown_drop_columns = sorted(drop_base_columns - set(base_feature_columns))
    if unknown_drop_columns:
        raise ValueError(
            f"Variant {variant.get('name')} requested unknown base columns to drop: "
            f"{unknown_drop_columns}"
        )
    columns = [col for col in base_feature_columns if col not in drop_base_columns]
...
def run_z_scale_replacement_on_exp218(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    well_scaled_feature_config: dict[str, Any] | None = None,
    ...
    well_scaled_features, well_scaled_group_columns, well_scaled_summary, well_scaled_meta = (
        build_well_scaled_z_dz_features(
            full_frame,
            config=well_scaled_feature_config or {},
        )
    )
    well_scaled_feature_columns = [
        col for col in well_scaled_features.columns if col not in {"id", "well"}
    ]
    if not full_frame["id"].equals(well_scaled_features["id"]) or not full_frame[
        "well"
    ].equals(well_scaled_features["well"]):
        raise ValueError(
            "Well-scaled z/dz features are not row-order aligned with train feature frame"
        )
    _assign_aligned_float32_columns(
        full_frame,
        well_scaled_features,
        well_scaled_feature_columns,
    )
    del well_scaled_features
    gc.collect()
    print(
        json.dumps(
            {
                "stage": "added_well_scaled_z_dz_features",
                "rows": int(len(full_frame)),
                "well_scaled_z_dz_features": int(len(well_scaled_feature_columns)),
            },
            sort_keys=True,
        ),
        flush=True,
    ...
    well_scaled_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_well_scaled_z_dz_feature_summary.csv",
        index=False,
    )
            ...
            feature_schema_rows.append(
                {
                    ...
                    "is_well_scaled_z_dz_feature": bool(
                        feature in well_scaled_feature_columns
                    ),
                }
            )
    ...
    manifest = {
        ...
        "well_scaled_feature_config": well_scaled_feature_config or {},
        ...
        "well_scaled_z_dz_feature_groups": well_scaled_group_columns,
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "well_scaled_z_dz_feature_summary": f"{OUTPUT_PREFIX}_well_scaled_z_dz_feature_summary.csv",
...
def run_saved_model_inference(
    *,
    ...
    grwr_feature_config: dict[str, Any] | None = None,
    well_scaled_feature_config: dict[str, Any] | None = None,
    variant_name: str = "z_scale_replacement",
    ...
    well_scaled_features, well_scaled_group_columns, well_scaled_summary, well_scaled_meta = (
        build_well_scaled_z_dz_features(
            test_frame,
            config=well_scaled_feature_config
            or dict(manifest.get("well_scaled_feature_config") or {}),
        )
    )
    well_scaled_feature_columns = [
        col for col in well_scaled_features.columns if col not in {"id", "well"}
    ...
    if not test_frame[["id", "well"]].reset_index(drop=True).equals(
        well_scaled_features[["id", "well"]].reset_index(drop=True)
    ):
        raise ValueError(
            "Raw-test well-scaled z/dz features are not row-order aligned with test feature frame"
        )
    test_frame = pd.concat(
        [
            test_frame.reset_index(drop=True),
            well_scaled_features[well_scaled_feature_columns].reset_index(drop=True),
        ],
        axis=1,
    )
    configured_well_scaled_groups = manifest.get("well_scaled_z_dz_feature_groups") or {}
    if configured_well_scaled_groups and {
        key: list(value) for key, value in well_scaled_group_columns.items()
    } != {key: list(value) for key, value in configured_well_scaled_groups.items()}:
        raise ValueError("Well-scaled z/dz feature groups differ from train manifest")
    feature_group_columns = {
        **feature_group_columns,
        **well_scaled_group_columns,
    }
    ...
    well_scaled_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_inference_well_scaled_z_dz_feature_summary.csv",
        index=False,
    )
    pd.DataFrame(
        [
            {
                ...
                "is_learned_likelihood_feature": bool(feature in learned_feature_columns),
                "is_grwr_feature": bool(feature in grwr_feature_columns),
                "is_well_scaled_z_dz_feature": bool(feature in well_scaled_feature_columns),
            }
            for index, feature in enumerate(feature_columns)
        ]
    ...
    summary = {
        ...
        "well_scaled_z_dz_feature_source": _jsonable(well_scaled_meta),
        "learned_feature_groups": learned_group_columns,
        "grwr_feature_groups": grwr_group_columns,
        "well_scaled_z_dz_feature_groups": well_scaled_group_columns,
        "selected": {
        ...
        "artifacts": {
            ...
            "well_scaled_z_dz_feature_summary": f"{OUTPUT_PREFIX}_inference_well_scaled_z_dz_feature_summary.csv",
