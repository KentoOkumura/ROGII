...
def source_text_and_metadata(path: Path) -> tuple[str, dict[str, Any]]:
    if path.suffix == ".py":
        try:
...
def inspect_source_files(paths: list[Path], max_files: int = 200) -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
    for base_path in paths:
        if base_path.suffix in {".ipynb", ".py"}:
            source_paths = [base_path]
        else:
            source_paths = sorted(
                path
                for path in base_path.rglob("*")
                if path.is_file() and path.suffix in {".ipynb", ".py"}
            )
        for source_path in source_paths[:max_files]:
            text, metadata = source_text_and_metadata(source_path)
            risk_hits = {
...
def write_markdown_report(summary: dict[str, Any], path: Path) -> None:
    lines = [
        ...
        f"- Source files inspected: {len(summary['notebook_inspections'])}",
...
def run_integrity_audit(
    *,
    ...
    notebook_inspections = inspect_source_files(
        sorted(set(notebook_paths)),
        max_files=int(audit_config.get("max_source_files", 200)),
    )
