...
# %% [markdown]
# # exp269 raw HMM missing-GR observation neutrality — train
#
# This notebook performs one exact-HMM ablation. The saved exp209/exp205 raw
# typewell-GR exact-HMM control is fixed. In `raw_hmm_missing_gr_neutral`, a row
# whose raw horizontal GR is missing contributes zero GR log-likelihood to every
# TVT state. The exp209 transition and all other HMM settings remain fixed. No
# self-GR descriptor, LightGBM unary, likelihood-PF, raw-test prediction, model,
# control regeneration, or submission is produced.
...
# 4. Fixed exp209/exp205 raw-HMM control readout
# 5. Observation-neutral exact-HMM orchestration
...
# 10. Full observation-neutral generation
...
from exact_hmm_smoother import (
    ...
    list_well_ids,
    load_well,
    run_raw_hmm_missing_gr_neutral,
    set_num_threads,
)
...
def load_control_frame(path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Fixed raw-HMM control cache is missing columns: {missing}")
    frame = pd.read_csv(path, usecols=required, dtype={"id": str, "well": str})
    if frame["id"].duplicated().any():
        raise ValueError("Fixed raw-HMM control cache has duplicate ids")
    ...
    if not np.isfinite(frame[numeric].to_numpy(np.float64)).all():
        raise ValueError("Fixed raw-HMM control cache contains non-finite required values")
    return frame
...
# %%
def build_neutral_rows_for_well(
    well: str,
    train_dir: Path,
    ...
    result = run_raw_hmm_missing_gr_neutral(
        horizontal,
        typewell,
        **hmm_config,
    )
    ...
    # exp209/exp205 serialized the raw-HMM cache through float32. Preserve that storage
    ...
    neutral_tvt = np.asarray(result["mean_eval"], dtype=np.float32)
    neutral_std = np.asarray(result["std_eval"], dtype=np.float32)
    if len(neutral_tvt) != len(eval_index):
    ...
    frame = pd.DataFrame(
        {
            ...
            "neutral_tvt": neutral_tvt,
            "neutral_std": neutral_std,
            "neutral_loglik": float(result["loglik"]),
            "neutral_finite": np.isfinite(neutral_tvt) & np.isfinite(neutral_std),
    ...
    finite = bool(frame["neutral_finite"].all() and np.isfinite(float(result["loglik"])))
    meta = {
        "well": well,
        ...
        "neutral_rmse": metric_values(true_tvt, neutral_tvt)["rmse"],
        "finite": finite,
...
def paired_metric_row(frame: pd.DataFrame, group_type: str, group: str) -> dict[str, Any]:
    ...
    neutral = metric_values(target, frame["neutral_tvt"].to_numpy(np.float64))
    return {
        "group_type": group_type,
        "group": group,
        "rows": int(len(frame)),
        ...
        "neutral_rmse": neutral["rmse"],
        "delta_rmse_neutral_minus_control": (
            float(neutral["rmse"] - control["rmse"])
            if neutral["rmse"] is not None and control["rmse"] is not None
            else None
        ),
        "control_mae": control["mae"],
        "neutral_mae": neutral["mae"],
        "delta_mae_neutral_minus_control": (
            float(neutral["mae"] - control["mae"])
            if neutral["mae"] is not None and control["mae"] is not None
            else None
        ),
        "control_within10": control["within10"],
        "neutral_within10": neutral["within10"],
        "delta_within10_neutral_minus_control": (
            float(neutral["within10"] - control["within10"])
            if neutral["within10"] is not None and control["within10"] is not None
            else None
        ),
        "control_finite_coverage": control["finite_coverage"],
        "neutral_finite_coverage": neutral["finite_coverage"],
        "control_std_mean": float(frame["control_std"].mean()),
        "neutral_std_mean": float(frame["neutral_std"].mean()),
        "control_std_p95": float(frame["control_std"].quantile(0.95)),
        "neutral_std_p95": float(frame["neutral_std"].quantile(0.95)),
        "control_loglik_mean": float(frame["control_loglik"].mean()),
        "neutral_loglik_mean": float(frame["neutral_loglik"].mean()),
        "changed_rows": int(frame["prediction_changed"].sum()),
        "mean_abs_neutral_minus_control": float(frame["abs_neutral_minus_control"].mean()),
        "max_abs_neutral_minus_control": float(frame["abs_neutral_minus_control"].max()),
    }
...
def build_by_well_metrics(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for well, group in frame.groupby("well", sort=True):
        ...
        rows.append(row)
    return pd.DataFrame(rows).sort_values("delta_rmse_neutral_minus_control", ascending=False)
                ...
                rows.append(
                    {
                        ...
                        "mean_abs_neutral_minus_control": float(
                            segment["abs_neutral_minus_control"].mean()
                        ),
                        "max_abs_neutral_minus_control": float(
                            segment["abs_neutral_minus_control"].max()
                        ),
                    }
                )
                start = position
...
def evaluate_stage1_guard(
    ...
    *,
    ...
    hidden_like_groups: list[str],
) -> dict[str, Any]:
    """Evaluate the pre-registered HMM gate and fail closed on missing evidence."""

    delta_column = "delta_rmse_neutral_minus_control"
    checks: list[dict[str, Any]] = []

    def group_delta(group_type: str, group: str) -> float | None:
        selected = group_metrics[
            (group_metrics["group_type"] == group_type)
            & (group_metrics["group"].astype(str) == str(group))
        ]
        if len(selected) != 1:
            return None
        value = selected.iloc[0][delta_column]
        return float(value) if pd.notna(value) and np.isfinite(float(value)) else None

    def add_max_check(name: str, actual: float | None, threshold_key: str) -> None:
    ...
    add_max_check(
        "overall_delta_rmse",
        group_delta("overall", "all"),
        "overall_delta_rmse_max_ft",
    )
    add_max_check(
        "raw_missing_delta_rmse",
        group_delta("gr_availability", "missing"),
        "raw_missing_delta_rmse_max_ft",
    )
    add_max_check(
        "observed_delta_rmse",
        group_delta("gr_availability", "observed"),
        "observed_delta_rmse_max_ft",
    )
    add_max_check(
        "distance_1000_plus_delta_rmse",
        group_delta("distance", "1000_plus"),
        "distance_1000_plus_delta_rmse_max_ft",
    )
    for group in hidden_like_groups:
        add_max_check(
            f"hidden_like_{group}_delta_rmse",
            group_delta("hidden_like", group),
            "hidden_like_delta_rmse_max_ft",
        )

    worst_well_delta = None
    if len(by_well) and delta_column in by_well:
        values = pd.to_numeric(by_well[delta_column], errors="coerce")
        if values.notna().any():
            worst_well_delta = float(values.max())
    add_max_check(
        "worst_well_delta_rmse",
        worst_well_delta,
        "worst_well_delta_rmse_max_ft",
    )

    variant_rows = finite_coverage[
        finite_coverage["candidate"] == "raw_hmm_missing_gr_neutral"
    ]
    prediction_coverage = None
    std_coverage = None
    if len(variant_rows) == 1 and int(variant_rows.iloc[0]["rows"]) > 0:
        row = variant_rows.iloc[0]
        prediction_coverage = float(row["prediction_finite_rows"] / row["rows"])
        std_coverage = float(row["std_finite_rows"] / row["rows"])
    for name, actual, threshold_key in (
        (
            "prediction_finite_coverage",
            prediction_coverage,
            "prediction_finite_coverage_min",
        ),
        ("std_finite_coverage", std_coverage, "std_finite_coverage_min"),
    ):
        threshold = float(thresholds[threshold_key])
        checks.append(
            {
    ...
    id_threshold = int(thresholds["id_mismatch_max"])
    checks.append(
        {
            "name": "id_mismatch_count",
            "actual": int(id_mismatch_count),
            "operator": "<=",
            "threshold": id_threshold,
            "passed": bool(id_mismatch_count <= id_threshold),
        }
    )
    passed = bool(checks and all(check["passed"] for check in checks))
    return {
        "passed": passed,
        "pf_stage_eligible": passed,
        "pf_stage_executed": False,
        "fail_close": True,
        "checks": checks,
    }
...
print("Synthetic contract PASS: raw-missing GR unary=0; observed raw-HMM unary unchanged.")
...
expected_control_sha = str(nested("data.control_expected_decompressed_sha256"))
actual_control_sha = sha256_gzip_decompressed(CONTROL_PATH)
if actual_control_sha != expected_control_sha:
    raise ValueError(
        "Fixed raw-HMM control decompressed SHA mismatch: "
        f"expected={expected_control_sha}, actual={actual_control_sha}, path={CONTROL_PATH}"
    )
...
cost_guard = {
    ...
    "pf_stage_enabled": bool(nested("pf_stage.enabled")),
    "pf_requires_separate_user_approval": bool(
        nested("pf_stage.requires_separate_user_approval")
    ),
    "outer_workers": OUTER_WORKERS,
    "numba_threads_requested": NUMBA_THREADS,
    "numba_threads_effective": get_num_threads(),
}
if cost_guard["route"] != "pf_beam":
    raise ValueError("exp269 route must remain pf_beam")
if cost_guard["active_variants"] != ["raw_hmm_missing_gr_neutral"]:
    raise ValueError("exp269 must run exactly the raw_hmm_missing_gr_neutral variant")
if any(
    cost_guard[key] != 0
    for key in ("lightgbm_config_count", "fold_training_count", "booster_count")
):
    raise ValueError("exp269 must not train LightGBM models or boosters")
if (
    cost_guard["parent_control_retraining"]
    or cost_guard["inference_enabled"]
    or cost_guard["pf_stage_enabled"]
):
    raise ValueError("exp269 Stage 1 must reuse the fixed control and keep PF/inference disabled")
if nested("lgb_emission") is not None:
    raise ValueError("exp269 must not configure a LightGBM emission unary")
if bool(nested("model.gr_missing.self_gr_unary")) or bool(
    nested("model.gr_missing.lightgbm_unary")
):
    raise ValueError("exp269 scientific path must remain raw GR-only")
if not cost_guard["pf_requires_separate_user_approval"]:
    raise ValueError("exp269 PF Stage 2 must require separate user approval")
...
def generate_one(
    ...
    print(f"[{index}/{len(wells)}] observation-neutral exact HMM well={well}", flush=True)
    frame, meta = build_neutral_rows_for_well(well, TRAIN_DIR, HMM_CONFIG)
...
if OUTER_WORKERS > 1:
    ...
    generated = Parallel(n_jobs=OUTER_WORKERS, prefer="threads")(
        delayed(generate_one)(index, well) for index, well in enumerate(wells, start=1)
    )
else:
    generated = [generate_one(index, well) for index, well in enumerate(wells, start=1)]

neutral_frames = [frame for frame, _ in generated if len(frame)]
generation_meta = pd.DataFrame([meta for _, meta in generated])
if not neutral_frames:
    raise ValueError("Observation-neutral HMM generation produced no rows")
neutral_frame = pd.concat(neutral_frames, ignore_index=True)
if neutral_frame["id"].duplicated().any():
    raise ValueError("Observation-neutral HMM generation produced duplicate ids")
neutral_ids = set(neutral_frame["id"])
control_ids = set(control["id"])
id_mismatch_count = len(neutral_ids.symmetric_difference(control_ids))
merged = neutral_frame.merge(
    control,
    on=["id", "well"],
    how="inner",
    validate="one_to_one",
)
if id_mismatch_count:
    print(
        "WARNING: observation-neutral/fixed-control ID coverage mismatch; "
        "Stage-1 guard will fail closed: "
        f"neutral={len(neutral_frame)}, control={len(control)}, joined={len(merged)}, "
        f"symmetric_difference={id_mismatch_count}",
        flush=True,
    )
if not len(merged):
    raise ValueError("Observation-neutral/fixed-control join produced no paired rows")
...
merged["abs_neutral_minus_control"] = np.abs(
    merged["neutral_tvt"].to_numpy(np.float64) - merged["control_tvt"].to_numpy(np.float64)
)
change_tolerance = float(nested("audit.prediction_change_tolerance_ft", 1e-6))
merged["prediction_changed"] = merged["abs_neutral_minus_control"] > change_tolerance
...
no_missing_control_parity_max_abs = (
    float(merged.loc[no_eval_missing, "abs_neutral_minus_control"].max())
...
del neutral_frame, neutral_frames, generated, control
...
focus_well = str(nested("audit.focus_well"))
focus_selected = merged["well"].astype(str) == focus_well
if focus_selected.any():
    metric_parts.append(
        pd.DataFrame([paired_metric_row(merged.loc[focus_selected], "focus_well", focus_well)])
    )
...
finite_coverage = pd.DataFrame(
    [
        {
            "candidate": "exp209_raw_hmm_control",
            "rows": int(len(merged)),
        ...
        },
        {
            ...
            "rows": int(len(merged)),
            "prediction_finite_rows": int(np.isfinite(merged["neutral_tvt"]).sum()),
            "std_finite_rows": int(np.isfinite(merged["neutral_std"]).sum()),
...
stage1_guard = evaluate_stage1_guard(
    group_metrics,
    by_well,
    finite_coverage,
    id_mismatch_count=id_mismatch_count,
    thresholds=dict(nested("audit.stage1_guard")),
    hidden_like_groups=list(role_columns),
)
...
input_sha = {
    ...
    "control_cache_decompressed": actual_control_sha,
...
summary = {
    "experiment": EXPERIMENT_NAME,
    "status": "stage1_raw_hmm_neutrality_completed",
    "route": nested("experiment.route"),
    ...
    "pf_stage": {
        "enabled_in_config": bool(nested("pf_stage.enabled")),
        "eligible": bool(stage1_guard["pf_stage_eligible"]),
        "executed": False,
    ...
    "decision": (
        "stage1_pass_pf_eligible_pending_separate_user_approval"
        if stage1_guard["passed"]
        else "stage1_fail_pf_closed"
    ),
    "notes": [
        "The saved exp209/exp205 raw typewell-GR exact-HMM control is loaded, not regenerated.",
        "Only raw-missing evaluation rows have their GR emission contribution set to zero.",
        "No self-GR descriptor or LightGBM unary is used; the exp209 transition remains active.",
        "Visible test missingness is descriptive only and is not score evidence.",
        "No likelihood-PF, run-length gate, raw-test inference, selector, or submission is produced.",
        "A passing guard records PF eligibility only; Stage 2 requires separate user approval.",
    ],
}
...
metrics_payload = {
    ...
    "cv": overall.get("neutral_rmse"),
    "control_cv": overall.get("control_rmse"),
    "delta_cv_neutral_minus_control": overall.get("delta_rmse_neutral_minus_control"),
    "public_lb": None,
    "private_lb": None,
    "metric": "rmse",
    "rows": summary["rows"],
