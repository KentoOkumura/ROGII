...
OUTPUT_PREFIX = "exp195_denoised_calibrated_matching_replacement_only_on_exp148"
...
def build_denoised_calibrated_matching_features(
    base_frame: pd.DataFrame,
    train_dir: str | Path,
    config: dict[str, Any] | None = None,
) -> tuple[pd.DataFrame, dict[str, list[str]], pd.DataFrame, dict[str, Any]]:
    config = config or {}
    prefix = str(config.get("prefix") or "dcm_")
    group_name = str(config.get("feature_group") or "denoised_calibrated_matching")
    key_cols = ["id", "well"]
    ...
    exp148_oof = np.full(len(base_frame), np.nan, dtype=np.float32)
    if exp148_frame is not None:
        exp148_join = base_frame[key_cols].merge(exp148_frame, on=key_cols, how="left", validate="one_to_one")
        exp148_oof = pd.to_numeric(exp148_join["exp148_oof_tvt"], errors="coerce").to_numpy(np.float32)

    train_dir = Path(train_dir)
    filters_config = list(config.get("filters") or [{"name": "raw", "kind": "raw"}])
    slope_clip_config = config.get("slope_clip", [-3.0, 3.0])
    slope_clip = (float(slope_clip_config[0]), float(slope_clip_config[1]))
    shifts = np.arange(
        float(config.get("shift_min_ft", -80.0)),
        float(config.get("shift_max_ft", 80.0)) + 0.5 * float(config.get("shift_step_ft", 4.0)),
        float(config.get("shift_step_ft", 4.0)),
        dtype=np.float32,
    )
    local_offsets = np.asarray(
        [int(value) for value in config.get("local_offsets_rows", [-24, -12, 0, 12, 24])],
        dtype=np.int32,
    )
    posterior_temperatures = [float(value) for value in config.get("posterior_temperatures", [8.0])]
    scan_stride = max(int(config.get("scan_stride_rows", 16)), 1)
    ...
    for well, positions_raw in base_frame.groupby("well", sort=False).indices.items():
        ...
        prior, prior_meta = _prefix_slope_prior(
            md=md,
            tvt_input=tvt_input,
            known_end=known_end,
            slope_window_rows=int(config.get("prefix_slope_window_rows", 80)),
        ...
        filters = _build_gr_filters(gr, filters_config)
        sorted_unique_rows = np.unique(np.clip(sub_rows, 0, len(horizontal) - 1))
        scan_rows = sorted_unique_rows[::scan_stride]
        scan_rows = np.unique(np.concatenate([scan_rows, sorted_unique_rows[:1], sorted_unique_rows[-1:]])).astype(np.int32)
        well_features = pd.DataFrame({"id": base_frame["id"].iloc[positions].to_numpy(), "well": str(well)})
        raw_cache: dict[str, np.ndarray] = {}
        raw_top1_tvt: np.ndarray | None = None

        backtest = _build_prefix_backtest_summary(
            known_end=known_end,
            md=md,
            tvt_input=tvt_input,
            filters=filters,
        ...
        for filter_name, filtered_gr, filter_meta in filters:
            scan = _scan_matching_surface(
                row_idx=scan_rows,
                prior_tvt_all=prior,
                horizontal_gr=filtered_gr,
            ...
            if not scan:
                continue
            base = f"{prefix}{filter_name}"
            top1_tvt = _interp_to_rows(sub_rows, scan_rows, scan["commit_top1_tvt"])
            top2_tvt = _interp_to_rows(sub_rows, scan_rows, scan["commit_top2_tvt"])
            if filter_name == "raw":
                raw_top1_tvt = top1_tvt
            for scan_name in [
                "top1_shift_ft",
                "top2_shift_ft",
                "top1_cost",
                "top2_cost",
                "top2_minus_top1_cost",
                "mode_separation_ft",
                "top2_is_local_min",
                "bimodal_flag",
                "shift_entropy_norm",
            ]:
                values = _interp_to_rows(sub_rows, scan_rows, scan[scan_name])
                well_features[f"{base}_{scan_name}"] = values
                if filter_name == "raw":
                    raw_cache[scan_name] = values
            well_features[f"{base}_commit_top1_minus_last_known_tvt"] = top1_tvt - last_tvt[positions]
            well_features[f"{base}_commit_top2_minus_last_known_tvt"] = top2_tvt - last_tvt[positions]
            well_features[f"{base}_top2_minus_top1_tvt"] = top2_tvt - top1_tvt
            for temperature in posterior_temperatures:
                suffix = _temp_suffix(float(temperature))
                posterior_tvt = _interp_to_rows(sub_rows, scan_rows, scan[f"posterior_mean_{suffix}_tvt"])
                posterior_p = _interp_to_rows(sub_rows, scan_rows, scan[f"posterior_p_top1_{suffix}"])
                posterior_entropy = _interp_to_rows(sub_rows, scan_rows, scan[f"posterior_entropy_{suffix}"])
                well_features[f"{base}_posterior_p_top1_{suffix}"] = posterior_p
                well_features[f"{base}_posterior_entropy_{suffix}"] = posterior_entropy
                well_features[f"{base}_posterior_{suffix}_minus_last_known_tvt"] = (
                    posterior_tvt - last_tvt[positions]
                )
                default_candidate = str(config.get("default_candidate") or "likpf_mean")
                for candidate_name, candidate_values in available_candidates:
                    if candidate_name == default_candidate:
                        well_features[f"{base}_posterior_{suffix}_minus_{candidate_name}"] = (
                            posterior_tvt - candidate_values[positions]
                        )
                        well_features[f"{base}_posterior_{suffix}_abs_minus_{candidate_name}"] = np.abs(
                            posterior_tvt - candidate_values[positions]
                        )
                if np.isfinite(exp148_oof[positions]).any():
                    diff = posterior_tvt - exp148_oof[positions]
                    well_features[f"{base}_posterior_{suffix}_minus_exp148_oof"] = np.where(
                        np.isfinite(diff),
                        diff,
                        0.0,
                    )
            for candidate_name, candidate_values in available_candidates:
                if candidate_name in {"pf_ancc", "beam_mean", "likpf_mean"}:
                    diff = top1_tvt - candidate_values[positions]
                    well_features[f"{base}_top1_minus_{candidate_name}"] = diff
                    well_features[f"{base}_top1_abs_minus_{candidate_name}"] = np.abs(diff)
            for key, value in backtest.items():
                if key.startswith(f"{filter_name}_"):
                    well_features[f"{prefix}{key}"] = np.float32(value)
            summary_rows.append(
                {
                    "well": str(well),
                    "filter": filter_name,
                    ...
                    "filter_metadata": json.dumps(_jsonable(filter_meta), sort_keys=True),
                    **{f"prior_{key}": value for key, value in prior_meta.items()},
                }
            )

        for smooth_name in [name for name, _values, _meta in filters if name != "raw"]:
            smooth_prefix = f"{prefix}{smooth_name}"
            raw_prefix = f"{prefix}raw"
            for metric in ["top1_cost", "top2_minus_top1_cost", "shift_entropy_norm"]:
                smooth_col = f"{smooth_prefix}_{metric}"
                raw_col = f"{raw_prefix}_{metric}"
                if smooth_col in well_features and raw_col in well_features:
                    well_features[f"{smooth_col}_minus_raw"] = (
                        well_features[smooth_col].to_numpy(np.float32)
                        - well_features[raw_col].to_numpy(np.float32)
                    )
            if raw_top1_tvt is not None and f"{smooth_prefix}_commit_top1_minus_last_known_tvt" in well_features:
                smooth_top1_tvt = (
                    last_tvt[positions]
                    + well_features[f"{smooth_prefix}_commit_top1_minus_last_known_tvt"].to_numpy(np.float32)
                )
                well_features[f"{smooth_prefix}_top1_tvt_minus_raw_top1_tvt"] = smooth_top1_tvt - raw_top1_tvt
                well_features[f"{smooth_prefix}_top1_tvt_abs_minus_raw_top1_tvt"] = np.abs(
                    smooth_top1_tvt - raw_top1_tvt
                )
        ...
        if available_candidates:
            ...
            candidate_range = (np.max(candidate_stack, axis=0) - np.min(candidate_stack, axis=0)).astype(np.float32)
            well_features[f"{prefix}candidate_tvt_std"] = candidate_std
            well_features[f"{prefix}candidate_tvt_range"] = candidate_range
            if f"{prefix}raw_shift_entropy_norm" in well_features:
                entropy = well_features[f"{prefix}raw_shift_entropy_norm"].to_numpy(np.float32)
                well_features[f"{prefix}candidate_std_x_raw_entropy"] = candidate_std * entropy
                well_features[f"{prefix}candidate_range_x_raw_entropy"] = candidate_range * entropy
        if f"{prefix}raw_shift_entropy_norm" in well_features:
            entropy = well_features[f"{prefix}raw_shift_entropy_norm"].to_numpy(np.float32)
            gap = well_features[f"{prefix}raw_top2_minus_top1_cost"].to_numpy(np.float32)
            distance = md_since[positions]
            well_features[f"{prefix}raw_entropy_x_log1p_md_since"] = entropy * np.log1p(distance)
            well_features[f"{prefix}raw_gap_x_log1p_md_since"] = gap * np.log1p(distance)
            well_features[f"{prefix}raw_inverse_gap_x_candidate_std"] = (
                (1.0 / np.maximum(np.abs(gap), 1.0))
                * well_features.get(f"{prefix}candidate_tvt_std", pd.Series(0.0, index=well_features.index)).to_numpy(np.float32)
            )
    ...
    for col in feature_cols:
        features[col] = pd.to_numeric(features[col], errors="coerce").fillna(0.0).astype(np.float32)
    _assert_finite_columns(features, feature_cols, "denoised calibrated matching feature frame")
        ...
        [
            {
                ...
                "scan_stride_rows": int(scan_stride),
                "filters": ",".join(str(item.get("name")) for item in filters_config),
                "posterior_temperatures": ",".join(_temp_suffix(value) for value in posterior_temperatures),
    ...
    meta = {
        "source_kind": "target_free_denoised_calibrated_gr_shift_scan",
        ...
        "shift_grid": {
            ...
            "step": float(config.get("shift_step_ft", 4.0)),
...
def build_sp45_bimodal_selector_features(
    ...
    raise NotImplementedError(
        "exp195 does not implement the legacy sp45 inference feature wrapper. "
        "Train-side DCM features require raw train_dir and are generated by "
        "build_denoised_calibrated_matching_features()."
    )
...
def run_denoised_calibrated_matching_replacement_only_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    dcm_feature_config: dict[str, Any] | None = None,
    ...
    dcm_features, dcm_group_columns, dcm_summary, dcm_meta = (
        build_denoised_calibrated_matching_features(
            full_frame,
            train_dir=train_dir,
            config=dcm_feature_config or {},
        )
    )
    dcm_feature_columns = [col for col in dcm_features.columns if col not in {"id", "well"}]
    if not full_frame["id"].equals(dcm_features["id"]) or not full_frame["well"].equals(
        dcm_features["well"]
    ):
        raise ValueError(
            "GR matching features are not row-order aligned with train feature frame"
        )
    _assign_aligned_float32_columns(
        full_frame,
        dcm_features,
    ...
    print(
        json.dumps(
            {
                "stage": "added_dcm_features",
                "rows": int(len(full_frame)),
                "dcm_features": int(len(dcm_feature_columns)),
            },
            sort_keys=True,
        ),
        flush=True,
    ...
    dcm_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_dcm_feature_summary.csv",
        index=False,
    )
            ...
            feature_schema_rows.append(
                {
                    ...
                    "is_dcm_feature": bool(feature in dcm_feature_columns),
                }
            )
    ...
    manifest = {
        "experiment": "exp195_denoised_calibrated_matching_replacement_only_on_exp148",
        ...
        "mode": "denoised_calibrated_matching_replacement_only_on_exp148_full_train_rows",
        "feature_source": feature_meta,
        "learned_likelihood_feature_source": learned_source_meta,
        "dcm_feature_source": dcm_meta,
        ...
        "dcm_feature_config": dcm_feature_config or {},
        "projection_feature_groups": projection_group_columns,
        "learned_feature_groups": learned_group_columns,
        "dcm_feature_groups": dcm_group_columns,
        "replacement_policy": {
            "kept_feature_groups": ["projection_correction", "u_disagreement"],
            "removed_feature_group": "learned_likelihood_confidence",
            "replacement_feature_group": "denoised_calibrated_matching",
            "removed_columns": learned_group_columns.get("learned_likelihood_confidence", []),
        },
    ...
    summary = {
        ...
        "replacement_policy": {
            ...
            "removed_column_count": int(
                len(learned_group_columns.get("learned_likelihood_confidence", []))
            ),
        },
        ...
        "artifacts": {
            ...
            "dcm_feature_summary": f"{OUTPUT_PREFIX}_dcm_feature_summary.csv",
...
def run_saved_model_inference(
    *,
    ...
    if not test_frame[["id", "well"]].reset_index(drop=True).equals(
        ...
        raise ValueError(
            "Raw-test GR matching features are not row-order aligned with test feature frame"
        )
    test_frame = pd.concat(
        [
    ...
    if configured_sp45_groups and {
        ...
        raise ValueError("GR matching feature groups differ from train manifest")
    feature_group_columns = {
        **feature_group_columns,
        **sp45_group_columns,
    }
