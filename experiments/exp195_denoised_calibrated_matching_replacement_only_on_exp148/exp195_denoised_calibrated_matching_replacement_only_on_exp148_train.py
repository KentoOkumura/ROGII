...
# %% [markdown]
# # exp195_denoised_calibrated_matching_replacement_only_on_exp148 train
#
# Full-train replacement-only feature audit that removes the exp148 learned-likelihood confidence block and uses denoised calibrated GR matching confidence features instead.
#
...
# 3. Train full-row GR matching replacement-only variant
...
from denoised_calibrated_matching_replacement_only_on_exp148 import (
    EXP145_TRAIN_ML_FEATURES,
    FULL_REPLAY_TRAIN_FEATURES,
    OUTPUT_PREFIX,
    find_artifact,
    run_denoised_calibrated_matching_replacement_only_on_exp148,
)
...
print("GR matching feature config:", cfg_get(config, "model.denoised_calibrated_matching_features"))
print("Replacement boundary:", "learned_likelihood_confidence ll_* block is loaded for inventory but excluded from active model features")
...
# %%
summary = run_denoised_calibrated_matching_replacement_only_on_exp148(
    ...
    dcm_feature_config=cfg_get(config, "model.denoised_calibrated_matching_features", {}),
...
dcm_summary = pd.read_csv(paths.artifacts_dir / f"{OUTPUT_PREFIX}_dcm_feature_summary.csv")
