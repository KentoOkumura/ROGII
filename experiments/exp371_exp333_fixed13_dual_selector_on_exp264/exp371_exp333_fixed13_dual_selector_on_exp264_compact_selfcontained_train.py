# %% [markdown]
# # exp371 exp333 fixed13 dual selector on exp264 — train
#
# Add the saved exp333 Stage 1 OOF as exactly one new candidate to the corrected
...
EXECUTE_NOTEBOOK = (
    os.environ.get("EXP371_IMPORT_ONLY", "0") != "1" and in_notebook_runtime()
)
...
# ## 4. Input resolution and SHA checks
#
# Exp333 is opened through four target-free columns only. Its colocated truth
# column is deliberately not read. The parent exp264 score is a post-fit audit
# input and never enters Stage A or selector fitting.

# %%
if EXECUTE_NOTEBOOK:
    if not bool(execution["run_approved"]):
        raise RuntimeError(
            "exp371 Kaggle train is not authorized. Set execution.run_approved "
    ...
    if execution.get("approved_scope") != (
        ...
        raise RuntimeError("exp371 approval scope does not match the fixed cost contract")
    ...
    exp333_cfg = CONFIG["data"]["exp333_oof"]
    exp333_path = resolve_file_by_sha(
        exp333_cfg["patterns"],
        roots,
        expected_file_sha256=exp333_cfg["expected_file_sha256"],
        expected_decompressed_sha256=exp333_cfg["expected_decompressed_sha256"],
        label="saved exp333 Stage 1 OOF",
    )
    exp333_oof, exp333_manifest = load_exp333_oof(
        exp333_path,
    ...
    write_json(OUTPUT_DIR / "exp371_exp333_oof_manifest.json", exp333_manifest)
...
# globally joins exp333 by `(well_id, row_idx)`, preserves its saved-exp226
...
# %%
if EXECUTE_NOTEBOOK:
    fixed13_cache = Exp333Fixed13CandidateCache(
        cache_root,
        CONTRACT,
        exp333_oof=exp333_oof,
        exp333_manifest=exp333_manifest,
    )
    ...
    exp333_repartition_manifest = fixed13_cache.selector_repartition_manifest(
        expected_rows=int(CONFIG["validation"]["expected_rows"])
    )
    if not bool(exp333_repartition_manifest["passed"]):
        raise RuntimeError(
            "exp333 global-key selector-fold repartition audit failed: "
            f"{exp333_repartition_manifest['checks']}"
        )
    exp333_manifest["selector_fold_repartition"] = exp333_repartition_manifest
    write_json(
        OUTPUT_DIR / "exp371_exp333_selector_fold_repartition.json",
        exp333_repartition_manifest,
    )
    write_json(OUTPUT_DIR / "exp371_exp333_oof_manifest.json", exp333_manifest)

    write_exp371_input_contract(
        OUTPUT_DIR / "exp371_input_contract.json",
        config=CONFIG,
        contract=CONTRACT,
        exp333_manifest=exp333_manifest,
...
if EXECUTE_NOTEBOOK:
    ...
    technical_checks = {
        ...
        "exp333_truth_or_error_columns_loaded": int(
            exp333_manifest["truth_or_error_columns_loaded"]
        )
        == 0,
        "exp333_global_key_selector_fold_repartition": bool(
            exp333_repartition_manifest["passed"]
        ),
        "exp333_source_fold_not_used_as_model_feature": not bool(
            exp333_repartition_manifest["source_fold_used_as_model_feature"]
        ),
    }
    if not all(technical_checks.values()):
        raise RuntimeError(f"exp371 technical checks failed: {technical_checks}")
...
if EXECUTE_NOTEBOOK:
    ...
    scope_metrics = pd.read_csv(
        OUTPUT_DIR / "exp371_fixed13_vs_fixed12_scope_metrics.csv"
    )
...
if EXECUTE_NOTEBOOK:
    ...
    summary = {
        ...
        "exp333_oof": exp333_manifest,
        "exp333_selector_fold_repartition": exp333_repartition_manifest,
    ...
    write_json(OUTPUT_DIR / "exp371_summary.json", summary)
    ...
    reproducibility.update(
        {
            ...
            "exp371_summary_sha256": sha256_file(
                OUTPUT_DIR / "exp371_summary.json"
            ),
        }
    )
