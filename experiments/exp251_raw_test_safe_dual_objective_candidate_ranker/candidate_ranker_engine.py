...
def recompute_multi_observation_features(
    ...
    *,
    ...
    horizontal_dir: Path | None = None,
) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    settings = get_nested(config, "augmentation.multi_observation_likelihood") or {}
    paths = ExperimentPaths()
    source_dir = Path(horizontal_dir) if horizontal_dir is not None else paths.train_data_dir
...
def _predict_clean_validation(
    *,
    ...
    for start in range(0, len(valid_idx), chunk_size):
        ...
        missing_features = [column for column in feature_columns if column not in chunk_features]
        if missing_features:
            raise ValueError(
                f"clean validation lacks selected train features: {missing_features[:20]}"
            )
...
def train_outer_oof_models(
    *,
    ...
    for fold, (train_idx, valid_idx) in enumerate(folds):
        ...
        model_features = (
            list(selected_feature_columns)
            if selected_feature_columns is not None
            else list(clean_features)
        )
        missing_selected = [column for column in model_features if column not in clean_features]
        if missing_selected:
            raise ValueError(
                f"selected raw-test-safe features are missing from train long frame: "
                f"{missing_selected[:20]}"
            )
        if schema is None:
            schema = model_features
        elif schema != model_features:
        ...
        for variant in variants:
            ...
            x_train, medians = _fit_imputer(train_long, model_features)
            x_valid = _apply_imputer(valid_long, model_features, medians)
            ...
            for objective, model, path in [
                ...
                for feature, importance in zip(
                    model_features, model.feature_importances_, strict=True
                ):
