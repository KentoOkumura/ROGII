...
def build_full_well_path_table(
    path_rows: pd.DataFrame,
    assignments: pd.DataFrame,
    cache: pd.DataFrame,
    *,
    ...
    table.insert(0, "id", table["row_id"])
    table["path_rank"] = pd.to_numeric(table["path_rank"], errors="coerce").astype("Int64")
    table["source_window_count"] = pd.to_numeric(
        table["source_window_count"],
        errors="coerce",
    ).astype("Int64")
    table["source_local_topk"] = int(local_topk)
    ...
    if "md_since" in cache.columns:
        cache_cols.append("md_since")
    cache_meta = cache[cache_cols].drop_duplicates("id").copy()
    table = table.merge(cache_meta, on="id", how="inner", validate="many_to_one")
    if "md_since" in table.columns:
        table = table.rename(columns={"md_since": "md_from_ps"})
    else:
        table["md_from_ps"] = np.nan
    ...
    table["overlap_weight"] = pd.to_numeric(table["overlap_weight"], errors="coerce")
    table["tvt_pred"] = pd.to_numeric(table["tvt_pred"], errors="coerce")
    table["md_from_ps"] = pd.to_numeric(table["md_from_ps"], errors="coerce")
...
def full_path_schema_frame(table: pd.DataFrame) -> pd.DataFrame:
    roles = {
        "id": "join key copied from row_id for compatibility with exp099 cache",
        "well": "well identifier",
        "row_id": "competition row id",
        "row_index": "numeric row suffix within well",
        "md_from_ps": "distance from known prefix sourced from exp099 cache",
        "path_rank": "full-well stitched candidate rank",
        "tvt_pred": "stitched heatmap MDN full-well candidate TVT",
        "source_window_count": "count of local windows contributing to the row",
        "overlap_weight": "sum of source local score probabilities at this row",
        ...
        "path_step_abs": "absolute first difference of tvt_pred inside full path",
        "curvature_abs": "absolute second difference of tvt_pred inside full path",
        ...
        "local_rank_mean": "mean local candidate rank used by this full path",
        "assignment_windows": "number of local windows assigned to this full path",
        "row_gap_after_previous": "row gap diagnostic inside full path",
    }
    rows = []
...
def contract_metrics_frame(
    ...
    *,
    ...
    cache_ids = int(cache["id"].nunique()) if "id" in cache else 0
...
def run_stitch_probe(
    *,
    ...
    for local_topk in local_topk_values:
        ...
        full_path = build_full_well_path_table(
            path_rows,
            assignments,
            cache,
            local_topk=local_topk,
        ...
        full_path_path = (
            paths.artifacts_dir / f"{OUTPUT_PREFIX}_{label}_full_well_candidate_paths.csv.gz"
        )
        full_schema_path = (
            paths.artifacts_dir / f"{OUTPUT_PREFIX}_{label}_full_well_path_schema.csv"
        )
        contract_path = (
            paths.artifacts_dir / f"{OUTPUT_PREFIX}_{label}_full_well_contract_metrics.csv"
        )
        ...
        output_paths = {
            ...
            "full_well_candidate_paths": str(full_path_path),
            "full_well_path_schema": str(full_schema_path),
            "full_well_contract_metrics": str(contract_path),
        ...
        output_sha = {
            ...
            "full_well_candidate_paths_csv_gz_sha256": sha256_path(full_path_path),
            "full_well_candidate_paths_csv_decompressed_sha256": sha256_path(
                full_path_path,
                decompressed=True,
            ),
            "full_well_path_schema_csv_sha256": sha256_path(full_schema_path),
            "full_well_contract_metrics_csv_sha256": sha256_path(contract_path),
    ...
    experiment_metrics = {
        ...
        "notes": [
            (
                ...
                "local paths into a selector-facing full-well path contract."
            ),
            (
