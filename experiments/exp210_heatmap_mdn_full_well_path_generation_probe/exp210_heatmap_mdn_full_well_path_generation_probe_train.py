...
# %% [markdown]
# # exp210_heatmap_mdn_full_well_path_generation_probe train
#
# Train-side diagnostic that turns exp208 dense heatmap MDN local paths into an
# explicit full-well candidate path artifact for downstream selector work.
...
# 3. Run full-well path generation diagnostic
...
from heatmap_mdn_full_well_path_generation_probe import (
...
print("Output topN full-well paths:", get_nested(config, "stitching.output_topn"))
...
print("Parent/control retraining:", get_nested(config, "model.training.control_or_parent_retraining"))
...
for topk_summary in summary["topk_summaries"]:
    ...
    print("Full-path contract:")
    print(json.dumps(topk_summary["full_path_contract"], indent=2, sort_keys=True))
    print("Evaluation:")
...
full_path_path = Path(primary["output_paths"]["full_well_candidate_paths"])
schema_path = Path(primary["output_paths"]["full_well_path_schema"])
contract_path = Path(primary["output_paths"]["full_well_contract_metrics"])
...
print("Primary full-well path contract metrics:")
display(contract_df)
print("Primary full-well path schema:")
display(schema_df)
print("Primary full-well candidate path preview:")
