        ...
        if not np.isnan(gr_v[i]):
            ...
            for j in range(N):
                ...
                if not np.isnan(gr_sm_v[i]):
                    ...
                    lsm = max(np.exp(-0.5 * ds * ds) if ds * ds < 600.0 else 0.0, 1e-300)
...
def _smooth(vals, fb, r):
    ...
    return (s.rolling(r * 2 + 1, center=True, min_periods=1).mean() if r > 0 else s).to_numpy(
        np.float32
    )
...
def beam_search(gr_h, tw_tvt, tw_gr, start_tvt, bs, mc, es, r):
    ...
    return tw_tvt[_beam_jit(sgr, tw_gr.astype(np.float64), si, bs, float(mc), float(es))].astype(
        np.float32
    )
...
@njit(cache=True)
def _pf_ancc_seeded(
    seed, md_v, z_v, gr_v, gg, vmin, step, gs, ls, ir, N, ALPHA, RN, PN, IS, RP, RR, RESAMP
):
    np.random.seed(seed)
    return _pf_ancc(
...
def lik_pf(
    hw,
    tw,
    n_particles=CFG.PF_PARTICLES,
    n_seeds=CFG.PF_SEEDS,
    ...
    if with_quality:
        q = {
            "pf_best_ll": float(liks.max()) / len(ev),
            "pf_ll_spread": float(liks.std()),
            "pf_pt_std": preds.std(0).astype(np.float32),
            "pf_gr_sig": gs,
        }
...
_m = np.linspace(1, 50, 20)
_z = np.zeros(20)
_g = np.full(20, 50.0)
_gg = np.linspace(45, 55, 100)
_pf_ancc(_m, _z, _g, _gg, 45.0, 0.1, 20.0, 50.0, 0.0, 8, 0.998, 0.002, 0.005, 0.3, 0.1, 0.001, 0.5)
_pf_z(
    _m,
    _z,
    _g,
...
_beam_jit(np.linspace(-1.0, 1.0, 30), np.linspace(-1.0, 1.0, 50), 25, 8, 15.0, 100.0)
_pf_lik_allseeds(
    _m, _z, _g, _gg, 45.0, 0.1, 20.0, 50.0, 0.0, 64, 4, 0, 0.998, 0.002, 0.005, 0.1, 0.001, 0.5, 4.5
)
...
class FormationPlaneKNN:
    def __init__(self, well_ids, data_dir):
        ...
        for wid in well_ids:
            try:
                df = pd.read_csv(
                    data_dir / f"{wid}__horizontal_well.csv", usecols=["X", "Y"] + FORMATIONS
                ).dropna()
            except:
                continue
            if len(df) == 0:
    ...
    def impute(self, xy_q, self_wid=None, k=PLANE_K):
        ...
        Xq = xy_q[:, 0]
        Yq = xy_q[:, 1]
        pred = (Xq[:, None] * coef[:, 0, :] + Yq[:, None] * coef[:, 1, :] + coef[:, 2, :]).astype(
            np.float32
        )
...
class DenseANCCImputer:
    def __init__(self, well_ids, data_dir, spw=DENSE_SPW):
        ...
        for wid in well_ids:
            try:
                df = pd.read_csv(
                    data_dir / f"{wid}__horizontal_well.csv", usecols=["X", "Y", "ANCC"]
                ).dropna()
            except:
                continue
            if len(df) == 0:
...
def build_well(hw_path, tw_path, is_train, likpf_map=None):
    ...
    gr_full = (
        hw.GR.astype(float).interpolate(limit_direction="both").fillna(float(np.nanmean(tw_gr)))
    )
    hgr = gr_full.iloc[ev.index[0] :].to_numpy(np.float32)
    kgr = gr_full.iloc[: len(kn)].to_numpy(np.float32)
    bpaths = {
        tag: beam_search(hgr, tw_tvt, tw_gr, last_tvt, bs, mc, es, r)
    ...
    all_sigs = (
        [pf_use] + list(bpaths.values()) + [sc8, sc15, sc25, sc_ens, tvt_fs["tvtF_ANCC"], tvt_dense]
    )
    ...
    gr_env = (
        gr_s.rolling(21, center=True, min_periods=1).max().iloc[ev.index].values.astype(np.float32)
    )
    gr_nrg = (
    ...
    feats = {
        ...
        **{
            f"tdpf{int(o)}": hgr - np.interp(pf_use + o, tw_tvt, tw_gr).astype(np.float32)
            for o in PF_OFFS
        },
...
def init_imputers(train_wids):
    global _FI, _DI
    _FI = FormationPlaneKNN(train_wids, CFG.DATA / "train")
    _DI = DenseANCCImputer(train_wids, CFG.DATA / "train")
...
def _likpf_rows(wid, split):
    ...
    for k, v in out.items():
        d["likpf_" + k.replace("pf_scale_", "scale_").replace("pf_mean", "mean")] = v.astype(
            np.float32
        )
...
def build_likpf(wids, split):
    ...
    res = Parallel(n_jobs=CFG.n_jobs, prefer="threads")(
        delayed(_likpf_rows)(w, split) for w in wids
    )
...
def train_stack(train_df, test_df, features):
    ...
    Xt = test_df[features].values.astype(np.float32)
    ...
    for i, p in enumerate(cb_configs(dev)):
        run(
            f"cb{i}",
            lambda p=p: CatBoostRegressor(**p),
            dict(early_stopping_rounds=250, use_best_model=True),
            False,
        )
    ...
    for tr, va in cv.split(OOF.values, y, groups=g):
        rid.fit(OOF.values[tr], y[tr])
        meta[va] = rid.predict(OOF.values[va])
    rid.fit(OOF.values, y)
    meta_test = rid.predict(TEST.values)
...
def list_test_wells(max_wells: int | None = None) -> list[str]:
    wids = sorted(
        p.stem.replace("__horizontal_well", "")
        for p in (CFG.DATA / "test").glob("*__horizontal_well.csv")
    )
    if max_wells is not None:
        wids = wids[: int(max_wells)]
    return wids
...
def build_replay_train_frames(
    max_wells: int | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    t0 = time.time()
    train_wids = list_train_wells(max_wells=max_wells)
    print(
        f"public replay train wells: {len(train_wids)} | n_jobs={CFG.n_jobs} | PF seeds={CFG.PF_SEEDS} | particles={CFG.PF_PARTICLES}",
        flush=True,
    )
    ...
    print(
        f"base features: rows={len(ravaghi_df):,} cols={len(ravaghi_df.columns)} elapsed={time.time() - t0:.1f}s",
        flush=True,
    )
    print(
        "building Pixiux public likelihood-PF replay features from raw competition files...",
        flush=True,
    )
    likpf_train = build_likpf(train_wids, "train")
    pixiux_df = add_likpf_features(ravaghi_df.copy(), likpf_train).reset_index(drop=True)
    print(
        f"pixiux features: rows={len(pixiux_df):,} cols={len(pixiux_df.columns)} elapsed={time.time() - t0:.1f}s",
        flush=True,
    )
    meta = {
...
def build_replay_inference_frames(
    max_wells: int | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    t0 = time.time()
    train_wids = list_train_wells(max_wells=max_wells)
    ...
    print(
        f"train base features: rows={len(train_base):,} cols={len(train_base.columns)} elapsed={time.time() - t0:.1f}s",
        flush=True,
    )
    ...
    print(
        f"test base features: rows={len(test_base):,} cols={len(test_base.columns)} elapsed={time.time() - t0:.1f}s",
        flush=True,
    )
...
def _fit_lgb_public_cv(
    ...
    print(
        f"training {variant}: rows={len(df):,} features={len(features)} device={devname} lgb_configs={len(configs)}",
        flush=True,
    )
        ...
        for fold, (tr, va) in enumerate(cv.split(X, y, groups=groups)):
            ...
            if model_output_dir is not None:
                ...
                model.booster_.save_model(
                    str(model_output_dir / model_file), num_iteration=best_iter
                )
                model_rows.append(
                    {
                        "variant": variant,
            ...
            print(
                f"  {variant} lgb{model_index} fold{fold}: rmse={fold_rmse:.6f} best_iter={best_iter}",
                flush=True,
            )
...
def _sample_fallback_prediction(
    data_dir: Path, sample_ids: pd.Series, default_value: float
) -> pd.Series:
...
def _predict_saved_lgb_ensemble(
    ...
    *,
    ...
    if model_name != "lgb_mean":
        ...
        ensemble_delta = np.mean(np.vstack([model_deltas[i] for i in model_items]), axis=0).astype(
            np.float32
        )
    prediction_rows.append(
        pd.DataFrame(
...
def _plot_mean_importance(mean_importance: pd.DataFrame, output_path: Path, top_n: int) -> None:
    ...
    fig, axes = plt.subplots(
        1, len(variants), figsize=(8.5 * len(variants), fig_height), squeeze=False
    )
    for ax, variant in zip(axes[0], variants, strict=False):
        subset = mean_importance[mean_importance["variant"] == variant].nlargest(
            top_n, "mean_importance"
        )
...
def run_public_replay_audit(
    *,
    ...
    for variant, frame in variants.items():
        ...
        metrics, importance, predictions = _fit_lgb_public_cv(
            frame, features, variant, model_output_dir=save_dir
        )
    ...
    importance_df.to_csv(
        output_dir / "ravaghi_vs_pixiux_public_replay_feature_importance.csv", index=False
    )
    mean_importance.to_csv(
        output_dir / "ravaghi_vs_pixiux_public_replay_feature_importance_mean.csv", index=False
    )
    predictions_df.to_csv(
        output_dir / "ravaghi_vs_pixiux_public_replay_oof_predictions.csv.gz",
        index=False,
        compression="gzip",
    )
    ...
    (output_dir / "ravaghi_vs_pixiux_public_replay_summary.json").write_text(
...
def run_public_replay_inference(
    *,
    ...
    sample_path = (
        ...
        else CFG.DATA / "sample_submission.csv"
    )
    ...
    pred_map = dict(
        zip(selected_test["id"].astype(str), selected_test["pred_tvt"].astype(float), strict=False)
    )
    ...
    schema_df.to_csv(
        output_dir / "ravaghi_vs_pixiux_public_replay_inference_feature_schema.csv", index=False
    )
    ...
    diagnostics_df.to_csv(
        output_dir / "ravaghi_vs_pixiux_public_replay_inference_metrics.csv", index=False
    )
    diagnostics = {
        "rows": int(len(sample)),
        "predicted_rows": int((~missing_mask).sum()),
    ...
    (output_dir / "ravaghi_vs_pixiux_public_replay_inference_summary.json").write_text(
        json.dumps(summary, indent=2)
    )
