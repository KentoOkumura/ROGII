# %% [markdown]
# # exp348 prefix GR unary window path-ranking SSM inference
#
# Stage C is intentionally unavailable. Training-only positive/negative banks,
# teacher boundaries, and ranking candidates are forbidden here. A current-test
# exact decoder may be added only after Stage 0, Stage A, and the full five-fold
# Stage B pass all fixed gates and the user separately approves inference.
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if bool(inference.get("enabled")):
        raise ValueError("exp348 Stage C inference is not approved")
    if bool(inference.get("create_submission")):
        raise ValueError("exp348 must not create a submission")
    ...
    forbidden = set(config.get("model", {}).get("forbidden", []))
    if "use_ranking_candidate_directly_as_validation_or_test_prediction" not in forbidden:
        raise ValueError("Stage C must forbid direct ranking-candidate prediction")
    return {
