# %% [markdown]
# # exp374 exp209 Student-t exact-HMM emission inference
#
# exp374 is a train-side fixed-df Student-t emission audit. Raw-test prediction,
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if contract["experiment"] != EXPERIMENT_NAME or contract["route"] != "pf_beam":
        raise ValueError("unexpected exp374 inference config")
    if contract["mode"] != "disabled_until_train_side_promotion_and_separate_approval":
        raise ValueError("exp374 inference mode contract changed")
    if any(
        contract[key]
        for key in (
            "inference_enabled",
        ...
        raise ValueError("exp374 inference and submission must remain disabled")
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp374 is a train-side Student-t exact-HMM emission audit; "
