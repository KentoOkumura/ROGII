# %% [markdown]
# # exp482 isolated GR shock prior hold — inference guard candidate
#
...
def validate_inference_disabled(config: Mapping[str, Any]) -> dict[str, bool]:
    ...
    if not contract["implementation_authorized"]:
        raise RuntimeError("exp482 implementation authorization is missing")
    if contract["inference_authorized"]:
        raise RuntimeError(
            "exp482 inference cannot be enabled before separately approved Stage 1 promotion"
        )
    if contract["submission_authorized"] or contract["create_submission"]:
        raise RuntimeError("exp482 submission must remain disabled")
    return contract
...
def run_inference(config: Mapping[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp482 inference is disabled: implementation approval covers only the "
        "compact Stage A0/A1 candidate and tests; canonical adoption, Kaggle run, "
        "full-OOF Stage 1, inference, and submission require separate approvals"
    )
