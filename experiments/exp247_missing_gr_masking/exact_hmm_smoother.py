...
OUTPUT_PREFIX = "exp247_missing_gr_masking"
VARIANT = "missing_gr_mask_only"
...
def load_lgb_prediction_series(
    root: str | Path, source_name: str, source_config: dict[str, Any]
    ...
    if pred_column is None:
        if "pred_tvt" in columns:
            ...
            raise ValueError(
                f"{source_name} prediction source needs pred_tvt or tvt column: {path}"
            )
    pred_column = str(pred_column)
    if id_column not in columns or pred_column not in columns:
        raise ValueError(
            f"{source_name} source missing columns id={id_column!r} pred={pred_column!r}: {path}"
        )
    ...
    before = len(frame)
    frame = frame.dropna(subset=["id", "pred_tvt"])
    duplicated = int(frame["id"].duplicated().sum())
    if duplicated:
        raise ValueError(
            f"{source_name} prediction source has duplicated ids after filtering: {duplicated}"
        )
...
def prepare_lgb_emission_variants(
    root: str | Path, config: dict[str, Any] | None
    ...
    if not active_sources or not sigma_values or not lambda_values:
        raise ValueError(
            "lgb_emission.enabled requires active_sources, sigma_grid, and lambda_grid"
        )

    max_variants = config.get("max_variants")
    max_variants = int(max_variants) if max_variants is not None else None
    sigma_floor = float(config.get("sigma_floor", min(sigma_values)))
    sigma_cap = float(config.get("sigma_cap", max(sigma_values)))
    emission_clip = float(config.get("emission_clip", 600.0))
    runtime_variants: list[dict[str, Any]] = []
    summary_variants: list[dict[str, Any]] = []
    loaded_sources: dict[str, tuple[pd.Series, dict[str, Any]]] = {}
    ...
    for source in active_sources:
        ...
        if source not in loaded_sources:
            loaded_sources[source] = load_lgb_prediction_series(
                root, source, sources_config[source]
            )
        predictions, source_meta = loaded_sources[source]
...
def build_gr_emission_loglik(
    ...
    *,
    ...
    """Build the exp221 GR unary and optionally neutralize raw-missing rows.

    Masking happens before any fixed LGB Gaussian unary is added. A masked row
    therefore contributes exactly zero GR log-likelihood for every TVT state,
    while transition and later unary terms remain active.
    """
    ...
    *,
    ...
    if lgb_tvt is not None and float(lgb_lambda) != 0.0:
        ...
        if len(lgb_obs) != len(eval_rows):
            raise ValueError(
                f"lgb_tvt length mismatch: expected {len(eval_rows)} got {len(lgb_obs)}"
            )
        if not np.isfinite(lgb_obs).all():
            raise ValueError("lgb_tvt contains non-finite values")
        sigma = float(lgb_sigma if lgb_sigma is not None else 0.0)
    ...
    *,
    ...
    summary = {
        ...
        "notes": [
            ...
            "submission.csv is ordered by sample_submission.csv and strict id "
    ...
    *,
    ...
    if outer_workers > 1:
        try:
            ...
            raise RuntimeError(
                "joblib is required when feature_cache.hmm.outer_workers > 1"
            ) from exc
        results = Parallel(n_jobs=outer_workers, prefer="threads")(
            delayed(build_one)(idx, well) for idx, well in enumerate(wells, start=1)
        )
    ...
    summary = {
        ...
        "mode": "lgb_oof_gaussian_emission_hmm_train_feature_cache",
        ...
        "numba_num_threads": int(effective_numba_num_threads)
        if effective_numba_num_threads
        else None,
        "well_generation": {
            "selected_wells": int(len(wells)),
            "ok_wells": int(len(ok_meta)),
        ...
        "notes": [
            ...
            "LGB OOF predictions are used as target-free Gaussian observation "
            "centers; true TVT is not used to tune row-wise sigma or lambda.",
            ...
            "exp072 likpf_mean and LGB baselines are compared only by the separate "
