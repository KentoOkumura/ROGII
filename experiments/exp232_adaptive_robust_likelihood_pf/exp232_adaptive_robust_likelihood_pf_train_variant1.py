...
# ---
# %% [markdown]
# # exp232 adaptive robust PF — T=4 CPU split run
#
# This notebook runs the complete eligible-well surface for `temp_t4` only.
# It is intentionally independent from the T=2 notebook so both CPU kernels
...
# 4. T=4 target-free likelihood / gate contract
# 5. T=4 PF generation with periodic progress
...
# %%
RUN_KEY = "temp_t4"
EXPECTED_NOTEBOOK_KIND = "train_variant1"
...
if run_spec.get("notebook_kind") != EXPECTED_NOTEBOOK_KIND:
    raise RuntimeError("temp_t4 split-run metadata does not match this notebook kind.")
if variants != [("temp_t4", 4.0)]:
    raise RuntimeError("train_variant1 must execute temp_t4 only.")
...
display(
    {
        ...
        "new_particle_likelihood": "exp(-0.5 * residual^2 / 4) only on gated rows",
