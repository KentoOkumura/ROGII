# %% [markdown]
# # exp351 exp306 L1 full convergence audit inference
#
# exp351 is a train-side, target-free solver audit. Prediction, raw-test
# inference, scientific scoring, and submission generation are outside its
# frozen scope.
...
def validate_disabled_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if contract["experiment"] != EXPERIMENT_NAME or contract["route"] != "pf_beam":
        raise ValueError("unexpected exp351 inference config")
    if contract["mode"] != "disabled_train_side_solver_audit_only":
        raise ValueError("exp351 inference mode contract changed")
    enabled = [
        key
        for key in (
            "inference_enabled",
    ...
    if enabled:
        raise ValueError(
            f"exp351 scientific scoring, inference, and submission must remain disabled: {enabled}"
        )
    return contract
...
def stop_disabled_inference(config: dict[str, Any]) -> None:
    ...
    raise RuntimeError(
        "exp351 is a train-side target-free L1 solver full audit; scientific "
        "scoring, prediction, inference, and submission are disabled"
    )
