...
VARIANT = "lgb_quantile_band_emission_hmm"
            ...
            for floor in sigma_floor_grid:
                for cap in sigma_cap_grid:
                    if float(floor) <= 0.0 or float(cap) < float(floor):
                        raise ValueError(f"invalid quantile sigma floor/cap: floor={floor} cap={cap}")
...
def run_train_feature_cache(
    *,
    ...
    summary = {
        ...
        "mode": "lgb_quantile_band_emission_hmm_train_feature_cache",
        ...
        "notes": [
            ...
            "LGB quantile OOF predictions are used as target-free Gaussian observation centers and row-wise sigma values; true TVT is not used to tune row-wise sigma or lambda.",
