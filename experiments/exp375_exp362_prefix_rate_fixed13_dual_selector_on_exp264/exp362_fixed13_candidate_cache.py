...
ADDED_CANDIDATE_ID = "prefix_rate_exact_hmm"
EXP362_ALLOWLIST = (
    "well_id",
    "row_idx",
    "fold",
...
def validate_fixed13_contract(contract: Mapping[str, Any]) -> None:
    ...
    if str(added.get("kind")) != "prefix_rate_exact_hmm":
        raise ValueError("exp362 candidate kind must remain prefix_rate_exact_hmm")
    if added.get("parents"):
        raise ValueError("exp362 candidate must not be reconstructed as a formula")
...
def exp362_prediction_content_sha256(frame: pd.DataFrame) -> str:
    """Hash the target-free allowlist after read, independent of gzip metadata."""

    ordered = frame.sort_values(
        ["fold", "well_id", "row_idx"], kind="stable"
    ).reset_index(drop=True)
    digest = hashlib.sha256()
    digest.update("|".join(EXP362_ALLOWLIST).encode())
...
def load_exp362_oof(
    ...
    *,
    ...
    if expected_file_sha256 and file_sha != str(expected_file_sha256):
        raise ValueError(f"exp362 OOF file SHA mismatch: {file_sha}")
    if decompressed_sha != str(expected_decompressed_sha256):
        raise ValueError(f"exp362 OOF decompressed SHA mismatch: {decompressed_sha}")
    if len(str(expected_prediction_logical_sha256)) != 64:
        raise ValueError("exp362 upstream prediction logical SHA must be a SHA256")
    header = pd.read_csv(path, nrows=0).columns.astype(str).tolist()
    missing = set(EXP362_ALLOWLIST) - set(header)
    if missing:
        raise ValueError(f"exp362 OOF allowlist columns are missing: {sorted(missing)}")
    frame = pd.read_csv(
        path,
        usecols=list(EXP362_ALLOWLIST),
        dtype={
            "well_id": str,
            "row_idx": np.int32,
            "fold": np.int8,
            "candidate_tvt": np.float64,
            "candidate_std": np.float64,
            "hmm_loglik": np.float64,
        },
    )
    if list(frame.columns) != list(EXP362_ALLOWLIST):
        frame = frame.loc[:, list(EXP362_ALLOWLIST)]
    if len(frame) != int(expected_rows):
        raise ValueError(f"exp362 OOF row mismatch: {len(frame)}")
    if frame["well_id"].nunique() != int(expected_wells):
        raise ValueError("exp362 OOF well count mismatch")
    if frame.duplicated(["well_id", "row_idx"]).any():
        raise ValueError("exp362 OOF contains duplicate well/row keys")
    if set(frame["fold"].astype(int)) != set(range(5)):
        raise ValueError("exp362 OOF source-fold coverage mismatch")
    well_fold_counts = frame.groupby("well_id", sort=False)["fold"].nunique()
    if not well_fold_counts.eq(1).all():
        raise ValueError("each exp362 well must have exactly one source outer fold")
    native = frame[
        ["candidate_tvt", "candidate_std", "hmm_loglik"]
    ].to_numpy(np.float64)
    if not np.isfinite(native).all():
        raise ValueError("exp362 OOF contains non-finite candidate/native confidence")
    if bool((frame["candidate_std"] < 0.0).any()):
        raise ValueError("exp362 OOF contains a negative candidate_std")
    well_loglik_counts = frame.groupby("well_id", sort=False)["hmm_loglik"].nunique()
    if not bool(well_loglik_counts.eq(1).all()):
        raise ValueError("exp362 hmm_loglik must be constant within each well")
    ...
    content_sha = exp362_prediction_content_sha256(frame)
    source_fold_rows = (
    ...
    manifest = {
        ...
        "post_read_allowlist_columns": list(EXP362_ALLOWLIST),
...
class Exp362Fixed13CandidateCache:
    def __init__(
        self,
        root: Path,
        contract: Mapping[str, Any],
        *,
        exp362_oof: pd.DataFrame,
        exp362_manifest: Mapping[str, Any],
    ):
        validate_fixed13_contract(contract)
        ...
        self.exp362_manifest = dict(exp362_manifest)
        self.exp362_by_key = exp362_oof.sort_values(
        ...
        if not self.exp362_by_key.index.is_unique:
            raise ValueError("exp362 global key index is not unique")
    ...
    def load_fold(self, fold: int) -> FoldBundle:
        ...
        added = self.exp362_by_key.reindex(selector_keys)
        missing = added[
            [
                "fold",
                "candidate_tvt",
        ...
        if missing.any():
            ...
            raise ValueError(
                f"exp362 global key join is missing exp263 fold {fold} rows: "
                f"{missing_keys}"
            )
        source_fold_sorted = added["fold"].to_numpy(np.int8)
        if not np.isin(source_fold_sorted, np.arange(5, dtype=np.int8)).all():
            raise ValueError(f"exp362 fold provenance is outside 0..4 in selector fold {fold}")
        ...
        if source_well_frame.groupby("well")["source_outer_fold"].nunique().max() != 1:
            raise ValueError("exp362 source fold changed within a well after key join")
        ...
        if not np.isfinite(prediction).all():
            raise ValueError(f"exp362 fold {fold} contains a non-finite prediction")
        ...
        if not bool(valid.all()):
            raise ValueError(f"exp362 fold {fold} native confidence is invalid")
        conf = base.base[KEY_COLUMNS].copy()
        conf["candidate_id"] = ADDED_CANDIDATE_ID
        conf["confidence_source"] = "exp362_prefix_rate_exact_hmm_posterior"
    ...
    def selector_repartition_manifest(self, *, expected_rows: int) -> dict[str, Any]:
        ...
        expected_source_fold_rows = {
            str(fold): int(value)
            for fold, value in self.exp362_manifest["source_fold_rows"].items()
        }
        ...
        return {
            ...
            "candidate_source_fold": "exp362_oof_fold",
...
def build_postfreeze_addone_novelty_readout(
    *,
    ...
    metrics_path = output_dir / "exp375_postfreeze_addone_novelty.csv"
    summary_path = output_dir / "exp375_postfreeze_addone_novelty.json"
...
def build_fixed13_integration_readout(
    *,
    ...
    if fixed_parity_max_abs > 1.0e-6:
        raise ValueError(
            f"fixed fallback errors changed after exp362 addition: {fixed_parity_max_abs}"
        )
        ...
        usage_rows.append(
            {
                "scope": scope,
                "rows": int(mask.sum()),
                "prefix_rate_top1_rows": int(selected.sum()),
                "prefix_rate_top1_fraction": float(selected.mean()),
            }
        )
        ...
        by_well_rows.append(
            {
                ...
                "fixed_fallback_rmse": fixed_rmse,
                "delta_fixed13_minus_parent": new_rmse - parent_rmse,
                "prefix_rate_top1_fraction": float(
    ...
    positive_usage_folds = int(
        sum(
            usage_lookup.loc[f"fold_{fold}", "prefix_rate_top1_fraction"] > 0.0
            for fold in range(5)
        )
    )
    hidden_deltas = [
    ...
    checks = {
        ...
        "prefix_rate_usage_pooled": float(
            usage_lookup.loc["pooled", "prefix_rate_top1_fraction"]
        )
        >= float(guard_config["minimum_added_candidate_primary_top1_fraction"]),
        "prefix_rate_usage_folds": positive_usage_folds
    ...
    gate = {
        ...
        "positive_prefix_rate_usage_folds": positive_usage_folds,
        "prefix_rate_usage_pooled": float(
            usage_lookup.loc["pooled", "prefix_rate_top1_fraction"]
        ),
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    scope_path = output_dir / "exp375_fixed13_vs_fixed12_scope_metrics.csv"
    usage_path = output_dir / "exp375_fixed13_candidate_usage.csv"
    by_well_path = output_dir / "exp375_fixed13_vs_fixed12_by_well.csv"
    gate_path = output_dir / "exp375_scientific_gate.json"
...
def write_exp375_input_contract(
    path: Path,
    *,
    ...
    payload = {
        ...
        "exp362_oof": dict(exp362_manifest),
        "parent_exp264_score": {
...
__all__ = [
    ...
    "exp362_prediction_content_sha256",
    "load_exp362_oof",
    "resolve_file_by_sha",
