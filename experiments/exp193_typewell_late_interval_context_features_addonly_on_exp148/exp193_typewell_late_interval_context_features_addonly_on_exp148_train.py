...
# %% [markdown]
# # exp193_typewell_late_interval_context_features_addonly_on_exp148 train
#
# Full-train add-only audit for target-free typewell late-interval context features on top of the exp148 learned-likelihood LightGBM surface.
#
...
# 3. Train full-row typewell context add-only variant
...
from typewell_late_interval_context_features_addonly_on_exp148 import (
    ...
    OUTPUT_PREFIX,
    find_artifact,
    run_typewell_late_interval_context_features_addonly_on_exp148,
)
...
print("Late range context parents:", cfg_get(config, "lineage.late_range_context_parents"))
...
print(
    "Typewell context feature config:",
    cfg_get(config, "model.typewell_late_interval_context_features"),
)
...
print(
    "learned feature preview rows:", len(learned_preview), "columns:", len(learned_preview.columns)
)
preview_cols = [
    c
    for c in [
...
# %%
summary = run_typewell_late_interval_context_features_addonly_on_exp148(
...
typewell_context_summary = pd.read_csv(
    paths.artifacts_dir / f"{OUTPUT_PREFIX}_typewell_context_feature_summary.csv"
)
...
display(typewell_context_summary)
