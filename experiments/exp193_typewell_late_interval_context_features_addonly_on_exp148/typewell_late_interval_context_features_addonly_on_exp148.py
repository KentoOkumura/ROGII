...
OUTPUT_PREFIX = "exp193_typewell_late_interval_context_features_addonly_on_exp148"
...
def build_sp45_bimodal_selector_features(
    ...
    raise NotImplementedError(
        "exp193 does not implement the legacy sp45 inference feature wrapper. "
        "Train-side typewell context features require raw train_dir and are "
        "generated by build_typewell_late_interval_context_features()."
    )
...
def run_typewell_late_interval_context_features_addonly_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    typewell_context_feature_config: dict[str, Any] | None = None,
    ...
    (
        typewell_context_features,
        typewell_context_group_columns,
        typewell_context_summary,
        typewell_context_meta,
    ) = build_typewell_late_interval_context_features(
        full_frame,
        train_dir=train_dir,
        config=typewell_context_feature_config or {},
    )
    typewell_context_feature_columns = [
        col for col in typewell_context_features.columns if col not in {"id", "well"}
    ]
    if not full_frame["id"].equals(typewell_context_features["id"]) or not full_frame[
        "well"
    ].equals(typewell_context_features["well"]):
        raise ValueError(
            "Typewell late interval context features are not row-order aligned with train feature frame"
        )
    _assign_aligned_float32_columns(
        full_frame,
        typewell_context_features,
    ...
    del typewell_context_features
    gc.collect()
    print(
        json.dumps(
            {
                "stage": "added_typewell_late_interval_context_features",
                "rows": int(len(full_frame)),
                "typewell_context_features": int(len(typewell_context_feature_columns)),
            },
            sort_keys=True,
        ),
        flush=True,
    ...
    typewell_context_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_typewell_context_feature_summary.csv",
        index=False,
    )
            ...
            feature_schema_rows.append(
                {
                    ...
                    "is_typewell_context_feature": bool(
                        feature in typewell_context_feature_columns
                    ),
                }
            )
    ...
    manifest = {
        "experiment": "exp193_typewell_late_interval_context_features_addonly_on_exp148",
        ...
        "mode": "typewell_late_interval_context_features_addonly_on_exp148_full_train_rows",
        "feature_source": feature_meta,
        "learned_likelihood_feature_source": learned_source_meta,
        "typewell_context_feature_source": typewell_context_meta,
        ...
        "typewell_context_feature_config": typewell_context_feature_config or {},
        "projection_feature_groups": projection_group_columns,
        "learned_feature_groups": learned_group_columns,
        "typewell_context_feature_groups": typewell_context_group_columns,
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "typewell_context_feature_summary": f"{OUTPUT_PREFIX}_typewell_context_feature_summary.csv",
...
def run_saved_model_inference(
    *,
    ...
    learned_feature_config: dict[str, Any] | None = None,
    typewell_context_feature_config: dict[str, Any] | None = None,
    variant_name: str = "typewell_late_interval_context_addonly",
    ...
    typewell_context_feature_config = typewell_context_feature_config or dict(
        manifest.get("typewell_context_feature_config") or {}
    )
    if projection_config.get("include_lgb_oof_features", False):
        raise NotImplementedError("LGB OOF U-projection features are disabled for exp193 inference")
    ...
    (
        ...
        test_frame,
        train_dir=test_dir,
        config=typewell_context_feature_config,
    )
    typewell_context_feature_columns = [
        col for col in typewell_context_features.columns if col not in {"id", "well"}
    ]
    if test_frame["id"].equals(typewell_context_features["id"]) and test_frame["well"].equals(
        typewell_context_features["well"]
    ):
        _assign_aligned_float32_columns(
            test_frame,
            typewell_context_features,
        ...
        before_typewell_rows = len(test_frame)
        test_frame = test_frame.merge(
            typewell_context_features,
            on=["id", "well"],
            how="inner",
        ...
        if len(test_frame) != before_typewell_rows:
            raise ValueError(
                "Raw-test typewell context features do not cover every replay test row: "
                f"{len(test_frame)} of {before_typewell_rows}"
            )
    del typewell_context_features
    gc.collect()
    ...
    configured_typewell_groups = manifest.get("typewell_context_feature_groups") or {}
    if configured_typewell_groups and {
        key: list(value) for key, value in typewell_context_group_columns.items()
    } != {key: list(value) for key, value in configured_typewell_groups.items()}:
        raise ValueError("Typewell context feature groups differ from train manifest")
    ...
    typewell_summary_path = (
        output_dir / f"{OUTPUT_PREFIX}_inference_typewell_context_feature_summary.csv"
    )
    ...
    typewell_context_summary.to_csv(typewell_summary_path, index=False)
    pd.DataFrame(
        [
            {
                "feature_index": int(index),
                ...
                "is_projection_feature": bool(feature in projection_feature_columns),
                "is_learned_likelihood_feature": bool(feature in learned_feature_columns),
                "is_typewell_context_feature": bool(feature in typewell_context_feature_columns),
            }
            for index, feature in enumerate(feature_columns)
        ]
    ...
    metrics = {
        ...
        "typewell_context_feature_count": int(len(typewell_context_feature_columns)),
    ...
    summary = {
        ...
        "mode": "saved_lgb_booster_inference_with_raw_test_feature_replay_and_typewell_context",
        ...
        "rawtest_typewell_context_feature_source": typewell_context_meta,
        "anchor_source": anchor_meta,
        "projection_feature_groups": projection_group_columns,
        "learned_feature_groups": learned_group_columns,
        "typewell_context_feature_groups": typewell_context_group_columns,
        ...
        "artifacts": {
            ...
            "typewell_context_feature_summary": typewell_summary_path.name,
