# %% [markdown]
# # exp388 exp374 Student-t fixed13 dual selector on exp264 — train
#
# Add the saved exp374 Student-t exact-HMM prediction as exactly one new
...
EXPERIMENT_NAME = "exp388_exp374_fixed13_dual_selector_on_exp264"
...
EXECUTE_NOTEBOOK = (
    os.environ.get("EXP388_IMPORT_ONLY", "0") != "1" and in_notebook_runtime()
)
...
# ## 4. Input resolution and SHA checks
#
# Exp374 is opened through the six target-free allowlisted columns only.
...
if EXECUTE_NOTEBOOK:
    if not bool(execution["run_approved"]):
        raise RuntimeError(
            "exp388 Kaggle train is not authorized. Set execution.run_approved "
    ...
    if execution.get("approved_scope") != (
        ...
        raise RuntimeError("exp388 approval scope does not match the fixed cost contract")
    ...
    exp374_cfg = CONFIG["data"]["exp374_predictions"]
    exp374_path = resolve_file_by_sha(
        exp374_cfg["patterns"],
        roots,
        expected_file_sha256=exp374_cfg.get("expected_file_sha256"),
        expected_decompressed_sha256=exp374_cfg["expected_decompressed_sha256"],
        label="saved exp374 Student-t exact-HMM predictions",
    )
    exp374_predictions, exp374_manifest = load_exp374_predictions(
        exp374_path,
        expected_rows=int(CONFIG["validation"]["expected_rows"]),
        expected_wells=int(CONFIG["validation"]["expected_wells"]),
        expected_file_sha256=exp374_cfg.get("expected_file_sha256"),
        expected_decompressed_sha256=exp374_cfg["expected_decompressed_sha256"],
        expected_prediction_logical_sha256=exp374_cfg[
            "expected_prediction_logical_sha256"
        ],
    )
    write_json(OUTPUT_DIR / "exp388_exp374_prediction_manifest.json", exp374_manifest)
        ...
        json.dumps(
            {
                ...
                "exp374_predictions": exp374_manifest,
                "parent_exp264_score": {
...
# globally joins exp374 by `(well_id, row_idx)` and repartitions rows by the
...
# %%
if EXECUTE_NOTEBOOK:
    fixed13_cache = Exp374Fixed13CandidateCache(
        cache_root,
        CONTRACT,
        exp374_predictions=exp374_predictions,
        exp374_manifest=exp374_manifest,
    )
    ...
    exp374_repartition_manifest = fixed13_cache.selector_repartition_manifest(
        expected_rows=int(CONFIG["validation"]["expected_rows"])
    )
    if not bool(exp374_repartition_manifest["passed"]):
        raise RuntimeError(
            "exp374 global-key selector-fold repartition audit failed: "
            f"{exp374_repartition_manifest['checks']}"
        )
    exp374_manifest["selector_fold_repartition"] = exp374_repartition_manifest
    write_json(
        OUTPUT_DIR / "exp388_exp374_selector_fold_repartition.json",
        exp374_repartition_manifest,
    )
    write_json(OUTPUT_DIR / "exp388_exp374_prediction_manifest.json", exp374_manifest)

    write_exp388_input_contract(
        OUTPUT_DIR / "exp388_input_contract.json",
        config=CONFIG,
        contract=CONTRACT,
        exp374_manifest=exp374_manifest,
...
if EXECUTE_NOTEBOOK:
    ...
    technical_checks = {
        ...
        "exp374_truth_or_error_columns_loaded": int(
            exp374_manifest["truth_or_error_columns_loaded"]
        )
        == 0,
        "exp374_global_key_selector_fold_repartition": bool(
            exp374_repartition_manifest["passed"]
        ),
        "exp374_source_fold_not_used_as_model_feature": not bool(
            exp374_repartition_manifest["source_fold_used_as_model_feature"]
        ),
        "exp374_native_confidence_finite": float(
            exp374_manifest["native_confidence_finite_fraction"]
        )
        == 1.0,
    }
    if not all(technical_checks.values()):
        raise RuntimeError(f"exp388 technical checks failed: {technical_checks}")
...
if EXECUTE_NOTEBOOK:
    ...
    scope_metrics = pd.read_csv(
        OUTPUT_DIR / "exp388_fixed13_vs_fixed12_scope_metrics.csv"
    )
...
if EXECUTE_NOTEBOOK:
    ...
    summary = {
        ...
        "exp374_predictions": exp374_manifest,
        "exp374_selector_fold_repartition": exp374_repartition_manifest,
    ...
    write_json(OUTPUT_DIR / "exp388_summary.json", summary)
    ...
    reproducibility.update(
        {
            ...
            "postfreeze_addone_novelty": novelty,
            "exp388_summary_sha256": sha256_file(
                OUTPUT_DIR / "exp388_summary.json"
            ),
        }
    )
