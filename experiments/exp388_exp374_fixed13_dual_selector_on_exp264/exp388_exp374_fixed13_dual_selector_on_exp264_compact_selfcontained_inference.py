# %% [markdown]
# # exp388 exp374 Student-t fixed13 dual selector on exp264 — inference
#
# Inference is intentionally disabled until the Stage C integration gate passes
# and downstream TVT training is separately approved and completed. Exp374 has
# no current-test Student-t candidate or native-confidence artifact yet.
...
def find_config() -> Path:
    ...
    if len(matches) == 1:
        return matches[0]
    raise FileNotFoundError("exp388 config.yaml did not resolve")
...
# %%
raise RuntimeError(
    "exp388 inference is disabled: Stage C must pass, then the fixed13 "
    "downstream TVT model and current-test Student-t exact-HMM candidate "
