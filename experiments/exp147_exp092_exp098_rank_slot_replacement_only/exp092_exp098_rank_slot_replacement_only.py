...
OUTPUT_PREFIX = "exp147_exp092_exp098_rank_slot_replacement_only"
...
def _frame_numeric(
    ...
    *,
    ...
    return pd.to_numeric(frame[column], errors="coerce").fillna(float(default)).to_numpy(np.float32)
...
def build_selector_rank_slot_features(
    ...
    *,
    ...
    for slot in range(top_k):
        ...
        result[f"{prefix}_candidate_minus_last_anchor"] = (slot_tvt - last_known).astype(np.float32)
    ...
    result["rank_score_top1_margin"] = (
        result["rank1_score"].to_numpy(np.float32) - result["rank2_score"].to_numpy(np.float32)
    ).astype(np.float32)
    group_columns["rank_slot_identity_score"].extend(
...
def feature_columns_for_variant(
    ...
    projection_group_columns: dict[str, list[str]],
    rank_slot_group_columns: dict[str, list[str]],
    variant: dict[str, Any],
) -> list[str]:
    columns = list(base_feature_columns)
    available_columns = set(base_feature_columns)
    for values in projection_group_columns.values():
        available_columns.update(values)
    for values in rank_slot_group_columns.values():
        available_columns.update(values)

    drop_columns = [str(col) for col in variant.get("drop_columns") or []]
    unknown_drop = sorted(set(drop_columns) - available_columns)
    if unknown_drop:
        raise ValueError(
            f"Unknown drop_columns for variant {variant.get('name')}: {unknown_drop[:40]}"
        )
    ...
    extra: list[str] = []
    for group in groups:
        if group not in projection_group_columns:
            raise ValueError(f"Unknown projection feature group for variant {variant}: {group}")
        extra.extend(projection_group_columns[group])
    for group in variant.get("rank_slot_feature_groups") or []:
        if group not in rank_slot_group_columns:
            raise ValueError(f"Unknown rank-slot feature group for variant {variant}: {group}")
        extra.extend(rank_slot_group_columns[group])
...
def _fit_one_variant_mode(
    *,
            ...
            metric_rows.append(
                {
                    ...
                    "features": int(len(feature_columns)),
                    "projection_feature_groups": ",".join(variant.get("feature_groups") or []),
                    "drop_columns": ",".join(variant.get("drop_columns") or []),
    ...
    mode_summary = {
        ...
        "projection_feature_groups": list(variant.get("feature_groups") or []),
        "drop_columns": list(variant.get("drop_columns") or []),
        "use_gpu": bool(mode_config.get("use_gpu", False)),
...
def run_exp092_exp098_rank_slot_replacement_only(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    if rank_slot_config.get("include_lgb_oof_features", False):
        raise NotImplementedError("LGB OOF rank-slot features are disabled for exp147")
    ...
    manifest = {
        "experiment": "exp147_exp092_exp098_rank_slot_replacement_only",
        ...
        "base_surface_parent": "exp073_gpu_reproducibility_guard_for_exp063_full_replay",
        "cache_parent": "exp072_exp063_full_replay_feature_cache",
        "mode": "exp092_exp098_rank_slot_replacement_only_from_exp072_cache",
...
def run_saved_model_inference(
    *,
    ...
    projection_config: dict[str, Any] | None = None,
    rank_slot_config: dict[str, Any] | None = None,
    variant_name: str = "u_projection_rank_slot_replacement_only",
    ...
    if projection_config.get("include_lgb_oof_features", False):
        raise NotImplementedError("LGB OOF U-projection features are disabled for exp147 inference")
    rank_slot_config = rank_slot_config or dict(manifest.get("rank_slot_config") or {})
    if rank_slot_config.get("include_lgb_oof_features", False):
        raise NotImplementedError("LGB OOF rank-slot features are disabled for exp147 inference")
    ...
    configured_rank_groups = manifest.get("rank_slot_feature_groups") or {}
    if configured_rank_groups and {
        key: list(value) for key, value in rank_group_columns.items()
    } != {key: list(value) for key, value in configured_rank_groups.items()}:
    ...
    rank_summary_path = output_dir / f"{OUTPUT_PREFIX}_inference_rank_slot_feature_summary.csv"
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "rank_slot_feature_summary": rank_summary_path.name,
        ...
        "known_followup_risk": (
            "Submit only if OOF, worst-well, near-row, bucket, and raw-test parity "
            "checks support the replacement-only variant."
        ),
        "elapsed_seconds": round(time.time() - t0, 3),
    }
...
run_u_projection_feature_ablation = run_exp092_exp098_rank_slot_replacement_only
