...
# %% [markdown]
# # exp184_heatmap_selector_compact_addonly_on_exp148 train
#
# Full-train add-only feature audit for exp184 heatmap selector selected-path confidence features on top of the exp148 learned-likelihood LightGBM surface.
#
...
# 3. Train full-row exp184 heatmap selector add-only variant
...
from heatmap_selector_compact_addonly_on_exp148 import (
    ...
    OUTPUT_PREFIX,
    find_artifact,
    run_heatmap_selector_compact_addonly_on_exp148,
)
...
print("Timeout-safe Kaggle train packages: train_lgb0, train_lgb1, train_lgb2")
...
selector_path = find_artifact(
    EXP184_SELECTOR_OOF_PREDICTIONS,
    cfg_get(config, "data.hmp184_selector_oof_predictions_local"),
)
heatmap_path = find_artifact(
    EXP182_VALIDATION_PREDICTIONS,
    cfg_get(config, "data.exp182_validation_predictions_local"),
)
print("exp072 full replay train cache:", cache_path)
print("exp145 full-train learned likelihood feature cache:", learned_path)
print("exp184 heatmap selector OOF selected path:", selector_path)
print("exp182 heatmap validation predictions:", heatmap_path)
...
selector_preview = pd.read_csv(selector_path, nrows=1000, dtype={"id": str, "well": str})
if {"variant", "mode"}.issubset(selector_preview.columns):
    selector_preview = selector_preview[
        selector_preview["variant"].astype(str).eq(
            str(cfg_get(config, "model.hmp184_selector_features.selected_variant"))
        )
        & selector_preview["mode"].astype(str).eq(
            str(cfg_get(config, "model.hmp184_selector_features.selected_mode", "viterbi"))
        )
    ].head(5)
else:
    selector_preview = selector_preview.head(5)
print("selector feature preview rows:", len(selector_preview), "columns:", len(selector_preview.columns))
display(selector_preview.head()[[
    "id",
    "well",
    "selected_candidate",
...
del base_preview, learned_preview, selector_preview
gc.collect()


# %% [markdown]
# ## 3. Train full-row heatmap compact add-only variant

# %%
summary = run_heatmap_selector_compact_addonly_on_exp148(
    ...
    hmp184_feature_config=cfg_get(config, "model.hmp184_selector_features", {}),
...
hmp184_summary = pd.read_csv(paths.artifacts_dir / f"{OUTPUT_PREFIX}_hmp184_selector_feature_summary.csv")
