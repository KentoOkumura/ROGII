...
EXP184_SELECTOR_OOF_PREDICTIONS = (
    "exp184_cnn_sdf_mtp_heatmap_path_features_on_exp158_oof_predictions.csv.gz"
)
EXP184_SELECTOR_SUMMARY = (
    "exp184_cnn_sdf_mtp_heatmap_path_features_on_exp158_summary.json"
)
EXP182_ARTIFACTS = (
    Path("experiments")
...
OUTPUT_PREFIX = "exp184_heatmap_selector_compact_addonly_on_exp148"
...
def read_exp182_heatmap_predictions(config: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    source = find_artifact(
        EXP182_VALIDATION_PREDICTIONS,
        config.get("exp182_validation_predictions_path"),
        local_artifacts=EXP182_ARTIFACTS,
    )
    sample_index = find_artifact(
        EXP182_SAMPLE_INDEX,
        config.get("exp182_sample_index_path"),
        local_artifacts=EXP182_ARTIFACTS,
    )
    summary = find_artifact(
        EXP182_SUMMARY,
        config.get("exp182_summary_path"),
        local_artifacts=EXP182_ARTIFACTS,
    )
    topk_ranks = [int(value) for value in config.get("topk_ranks", [1, 2, 3, 5, 10])]
    max_rank = max(max(topk_ranks), 10)
    allowed_specs = {
        str(config.get("primary_run_spec", "base_real_w128_b64_fullfold")),
        str(config.get("shuffled_run_spec", "base_shuffled_w128_b64_fullfold")),
        str(config.get("no_gr_run_spec", "base_no_gr_w128_b64_fullfold")),
    }
    ...
    forbidden = {
        ...
        or column in {"target_in_grid", "best_mode"}
        or ("oracle" in column)
    }
    usecols = sorted(required)
    leaked = sorted(set(usecols).intersection(forbidden))
    if leaked:
        raise ValueError(f"heatmap feature usecols include target-derived columns: {leaked}")
    ...
    return frame, {
        ...
        "sample_index_decompressed_sha256": sha256_gzip_decompressed(sample_index),
        "summary": str(summary),
...
def _positive_score_matrix(source: pd.DataFrame, max_rank: int = 10) -> np.ndarray:
    values = source[[f"pred_top{rank}_score" for rank in range(1, max_rank + 1)]].to_numpy(
        np.float32
    )
...
def _tvt_matrix(source: pd.DataFrame, max_rank: int = 10) -> np.ndarray:
    return source[[f"pred_top{rank}_tvt" for rank in range(1, max_rank + 1)]].replace(
...
def build_heatmap_source_features(
    ...
    *,
    tag: str,
    config: dict[str, Any],
    keep_tvt_columns: bool,
) -> tuple[pd.DataFrame, list[str]]:
    eps = float(config.get("confidence_eps", 1e-6))
    topk_ranks = [int(value) for value in config.get("topk_ranks", [1, 2, 3, 5, 10])]
    max_rank = max(max(topk_ranks), 10)
    ...
    generated: dict[str, np.ndarray] = {
        f"{tag}_top1_score": scores[:, 0],
        f"{tag}_top1_top2_score_margin": scores[:, 0] - scores[:, 1],
        f"{tag}_score_entropy": entropy.astype(np.float32),
        f"{tag}_score_top3_mass": (scores[:, :3].sum(axis=1) / score_sum).astype(np.float32),
        f"{tag}_path_step_abs_mean_ft": _numeric_array(source, "path_step_abs_mean_ft"),
        f"{tag}_path_step_abs_max_ft": _numeric_array(source, "path_step_abs_max_ft"),
        f"{tag}_prior_center_minus_last": (
            _numeric_array(source, "prior_center_tvt") - _numeric_array(source, "last_known_tvt")
        ).astype(np.float32),
    }
    if keep_tvt_columns:
        generated[f"{tag}_top1_tvt"] = tvts[:, 0]
        top3 = tvts[:, :3]
        generated[f"{tag}_top3_tvt_mean"] = np.nanmean(top3, axis=1).astype(np.float32)
        generated[f"{tag}_top3_tvt_std"] = np.nanstd(top3, axis=1).astype(np.float32)
        generated[f"{tag}_top3_tvt_range"] = (
            np.nanmax(top3, axis=1) - np.nanmin(top3, axis=1)
        ).astype(np.float32)
    ...
    for name, values in generated.items():
        ...
        column = f"hmp184_{name}"
        out[column] = safe
        columns.append(column)
    return out.groupby(["well", "row_center"], as_index=False, observed=True).mean(
        numeric_only=True
    ), columns
...
def interpolate_heatmap_features_for_frame(
    ...
    aligned = pd.DataFrame({"id": frame["id"].to_numpy(), "well": frame["well"].to_numpy()})
    ...
    for well, positions in frame.groupby("well", sort=False, observed=True).indices.items():
        ...
        source_group = source_by_well.get(str(well))
    ...
    aligned["hmp184_heatmap_source_valid"] = source_valid
    aligned["hmp184_heatmap_sparse_distance"] = np.where(
        np.isfinite(nearest_distance), nearest_distance, 1_000_000.0
    ).astype(np.float32)
    finite_dist = nearest_distance[np.isfinite(nearest_distance)]
    return aligned, {
        ...
        "nearest_sample_distance_rows_p95": float(np.nanquantile(finite_dist, 0.95))
        if len(finite_dist)
        else None,
    }


def build_heatmap_compact_row_features(
    base_frame: pd.DataFrame,
    selected_tvt: np.ndarray,
    config: dict[str, Any],
) -> tuple[pd.DataFrame, list[str], pd.DataFrame, dict[str, Any]]:
    heatmap_config = dict(config.get("heatmap_path_features") or {})
    if not heatmap_config.get("enabled", True):
        empty = base_frame[["id", "well"]].copy()
        return empty, [], pd.DataFrame(), {"enabled": False}
    predictions, source_meta = read_exp182_heatmap_predictions(heatmap_config)
    run_specs = {
    ...
    feature_frame = base_frame[["id", "well"]].copy()
    ...
    for tag, run_spec in run_specs.items():
        ...
        if tag == "real":
            join_columns.extend(["hmp184_heatmap_source_valid", "hmp184_heatmap_sparse_distance"])
        feature_frame = feature_frame.merge(
            aligned[join_columns],
            on="id",
            how="left",
            validate="one_to_one",
        )
        generated_columns.extend([column for column in join_columns if column != "id"])

    selected = np.asarray(selected_tvt, dtype=np.float32)
    real_top1 = feature_frame["hmp184_real_top1_tvt"].to_numpy(np.float32)
    real_top3_mean = feature_frame["hmp184_real_top3_tvt_mean"].to_numpy(np.float32)
    real_confidence = (
        0.40 * _rank01(feature_frame["hmp184_real_top1_score"].to_numpy(np.float32))
        + 0.30
        * _rank01(feature_frame["hmp184_real_top1_top2_score_margin"].to_numpy(np.float32))
        + 0.20
        * np.clip(1.0 - feature_frame["hmp184_real_score_entropy"].to_numpy(np.float32), 0.0, 1.0)
        + 0.10
        / (
            1.0
            + np.maximum(feature_frame["hmp184_real_top3_tvt_std"].to_numpy(np.float32), 0.0)
            / 25.0
        )
    ).astype(np.float32)
    shuffled_confidence = (
        0.45 * _rank01(feature_frame["hmp184_shuffled_top1_score"].to_numpy(np.float32))
        + 0.35
        * _rank01(feature_frame["hmp184_shuffled_top1_top2_score_margin"].to_numpy(np.float32))
        + 0.20
        * np.clip(
            1.0 - feature_frame["hmp184_shuffled_score_entropy"].to_numpy(np.float32),
            0.0,
            1.0,
        )
    ).astype(np.float32)
    no_gr_confidence = (
        0.45 * _rank01(feature_frame["hmp184_no_gr_top1_score"].to_numpy(np.float32))
        + 0.35
        * _rank01(feature_frame["hmp184_no_gr_top1_top2_score_margin"].to_numpy(np.float32))
        + 0.20
        * np.clip(
            1.0 - feature_frame["hmp184_no_gr_score_entropy"].to_numpy(np.float32),
            0.0,
            1.0,
        )
    ).astype(np.float32)
    far_threshold = float(heatmap_config.get("far_sparse_threshold_rows", 2048.0))
    compact = {
        "hmp184_far_from_sparse_flag": (
            feature_frame["hmp184_heatmap_sparse_distance"].to_numpy(np.float32) > far_threshold
        ).astype(np.float32),
        "hmp184_real_confidence_proxy": real_confidence,
        "hmp184_real_vs_shuffled_confidence_gap": (real_confidence - shuffled_confidence).astype(
            np.float32
        ),
        "hmp184_real_vs_no_gr_confidence_gap": (real_confidence - no_gr_confidence).astype(
            np.float32
        ),
        "hmp184_real_top1_minus_selected_abs": np.abs(real_top1 - selected).astype(np.float32),
        "hmp184_real_top3_mean_minus_selected_abs": np.abs(real_top3_mean - selected).astype(
            np.float32
        ),
        "hmp184_selected_minus_real_top1": (selected - real_top1).astype(np.float32),
        "hmp184_selected_abs_minus_real_top1": np.abs(selected - real_top1).astype(np.float32),
        "hmp184_real_top3_spread": feature_frame["hmp184_real_top3_tvt_range"].to_numpy(
            np.float32
        ),
    }
    for column, values in compact.items():
    ...
    keep_columns = [
        "id",
        "well",
        "hmp184_heatmap_source_valid",
        "hmp184_heatmap_sparse_distance",
        "hmp184_far_from_sparse_flag",
        "hmp184_real_top1_score",
        "hmp184_real_top1_top2_score_margin",
        "hmp184_real_score_entropy",
        "hmp184_real_confidence_proxy",
        "hmp184_real_top3_tvt_std",
        "hmp184_real_top3_spread",
        "hmp184_real_vs_shuffled_confidence_gap",
        "hmp184_real_vs_no_gr_confidence_gap",
        "hmp184_real_top1_minus_selected_abs",
        "hmp184_real_top3_mean_minus_selected_abs",
        "hmp184_selected_minus_real_top1",
        "hmp184_selected_abs_minus_real_top1",
    ]
    result = feature_frame[keep_columns].copy()
    feature_cols = [column for column in keep_columns if column not in {"id", "well"}]
    result[feature_cols] = result[feature_cols].fillna(0.0).astype(np.float32)
    _assert_finite_columns(result, feature_cols, "heatmap compact row feature frame")
    summary = pd.DataFrame(
        [
            {
                "feature_group": "heatmap_selector_compact",
                "source_rows": source_meta["rows"],
                "source_wells": source_meta["wells"],
                "joined_rows": int(len(result)),
                "joined_wells": int(result["well"].nunique()),
                "generated_features": int(len(feature_cols)),
                "source_valid_rate": interpolation_meta.get("real", {}).get("source_valid_rate"),
                "nearest_sample_distance_rows_p95": interpolation_meta.get("real", {}).get(
                    "nearest_sample_distance_rows_p95"
                ),
            }
        ]
    )
...
def load_hmp184_selector_oof_predictions(
    prediction_path: str | Path | None,
    *,
    variant: str,
    mode: str,
    key_frame: pd.DataFrame | None = None,
) -> tuple[pd.DataFrame, dict[str, Any]]:
    source = find_artifact(
        EXP184_SELECTOR_OOF_PREDICTIONS,
        prediction_path,
        local_artifacts=EXP184_TRAIN_ARTIFACTS,
    )
    usecols = [
        "id",
        "well",
    ...
    if key_frame is not None and len(key_frame) <= 200_000:
        wanted_ids = set(key_frame["id"].astype(str))
    ...
    for chunk in pd.read_csv(
        ...
        selected_chunk = chunk[chunk["variant"].eq(variant) & chunk["mode"].eq(mode)]
        if wanted_ids is not None and not selected_chunk.empty:
            selected_chunk = selected_chunk[selected_chunk["id"].isin(wanted_ids)]
        if not selected_chunk.empty:
            selected_chunks.append(selected_chunk.copy())
    selected = pd.concat(selected_chunks, ignore_index=True) if selected_chunks else pd.DataFrame()
    if selected.empty:
        available = (
            pd.read_csv(source, usecols=["variant", "mode"], chunksize=500_000)
            .__next__()[["variant", "mode"]]
            .drop_duplicates()
            .head(40)
            .to_dict("records")
        )
        raise ValueError(
            f"exp184 heatmap selector OOF variant={variant!r} mode={mode!r} not found; "
            f"available examples={available}"
        )
    duplicated = selected.duplicated(["id", "well"]).sum()
    if duplicated:
        raise ValueError(f"exp184 heatmap selector OOF has duplicated id/well rows: {duplicated}")
    unused_columns = {
        "variant",
        "mode",
        "true_tvt",
    ...
    return selected, {
        ...
        "source_sha256": sha256_file(source),
        "source_decompressed_sha256": sha256_gzip_decompressed(source),
        "source_kind": "hmp184_oof_best_viterbi_selected_path",
        "variant": variant,
        "mode": mode,
...
def load_optional_exp148_oof_predictions(
    ...
    *,
    ...
    header = pd.read_csv(source, nrows=0).columns.tolist()
    value_col = next(
        (col for col in ["pred_tvt", "prediction", "pred", "tvt"] if col in header),
        None,
    )
    ...
    usecols = [
        column
        for column in ["id", "well", "variant", "mode", "model", value_col]
        if column in header
    ]
    wanted_ids: set[str] | None = None
    if key_frame is not None and len(key_frame) <= 200_000:
    ...
    for chunk in pd.read_csv(
        ...
        for column, value in [("variant", variant), ("mode", mode), ("model", model)]:
            if column in chunk.columns:
                chunk = chunk[chunk[column].astype(str).eq(str(value))]
        if wanted_ids is not None and not chunk.empty:
            chunk = chunk[chunk["id"].isin(wanted_ids)]
        if not chunk.empty:
...
def build_heatmap_selector_compact_features(
    base_frame: pd.DataFrame,
    config: dict[str, Any] | None = None,
) -> tuple[pd.DataFrame, dict[str, list[str]], pd.DataFrame, dict[str, Any]]:
    config = config or {}
    prefix = str(config.get("prefix") or "hmp184_")
    key_cols = ["id", "well"]
    group_name = "heatmap_selector_compact"
    ...
    if missing_rate > max_missing_rate:
        raise ValueError(
            f"exp184 heatmap selector feature join missing_rate={missing_rate:.6f} "
    ...
    selected_is_pfbeam = selected_name.isin(pfbeam_names).to_numpy(np.float32)
    selected_is_pf_ancc = selected_name.eq("pf_ancc").to_numpy(np.float32)
    selected_family_code = np.where(selected_is_dense > 0.5, 2.0, 1.0).astype(np.float32)
    ...
    add_feature(f"{prefix}is_pfbeam_family", selected_is_pfbeam)
    add_feature(
        f"{prefix}selected_delta_from_last_norm_md",
        _safe_divide(selected_tvt - last_tvt, md_since + 1.0),
    )
    for name in [str(value) for value in config.get("candidate_diff_names", ["likpf_mean"])]:
        values = candidate_map.get(name)
        if values is None:
            continue
        diff = (selected_tvt - values).astype(np.float32)
        add_feature(f"{prefix}selected_minus_{name}", diff)
        add_feature(f"{prefix}selected_abs_minus_{name}", np.abs(diff).astype(np.float32))

    exp148_frame, exp148_meta = load_optional_exp148_oof_predictions(
        config.get("exp148_oof_predictions_path"),
        variant=str(config.get("exp148_oof_variant") or "learned_likelihood_confidence_addonly"),
        mode=str(config.get("exp148_oof_mode") or "gpu_repro_guard_dp_threads8"),
        model=str(config.get("exp148_oof_model") or "lgb_mean"),
        key_frame=base_frame[key_cols],
    )
    if exp148_frame is not None:
        exp148_join = base_frame[key_cols].merge(exp148_frame, on=key_cols, how="left", validate="one_to_one")
        exp148_oof = pd.to_numeric(exp148_join["exp148_oof_tvt"], errors="coerce").to_numpy(
            np.float32
        )
        valid_exp148 = np.isfinite(exp148_oof)
        add_feature(f"{prefix}selected_minus_exp148_oof", np.where(valid_exp148, selected_tvt - exp148_oof, 0.0))
        add_feature(f"{prefix}selected_abs_minus_exp148_oof", np.where(valid_exp148, np.abs(selected_tvt - exp148_oof), 0.0))
        add_feature(f"{prefix}exp148_oof_available", valid_exp148.astype(np.float32))
    else:
        exp148_meta = {"available": False, "reason": "exp148 OOF prediction artifact not found"}
    ...
    heatmap_features, heatmap_columns, heatmap_summary, heatmap_meta = build_heatmap_compact_row_features(
        base_frame,
        selected_tvt,
        config,
    )
    if heatmap_columns:
        if not base_frame[key_cols].reset_index(drop=True).equals(
            heatmap_features[key_cols].reset_index(drop=True)
        ):
            raise ValueError("heatmap compact features are not row-order aligned")
        for column in heatmap_columns:
            add_feature(column, heatmap_features[column].to_numpy(np.float32))
    del available_candidates, candidate_map, default_tvt, heatmap_features
    gc.collect()
    ...
    if feature_cols:
        _assert_finite_columns(
            feature_matrix,
            feature_cols,
            "exp184 heatmap selector compact feature frame",
        )
        ...
        [
            {
                ...
                "configured_candidate_tvt_specs": len(candidate_specs),
                "available_candidate_tvt_specs": available_candidate_count,
                "exp148_oof_available": bool(exp148_meta.get("available", False)),
                "heatmap_generated_features": int(len(heatmap_columns)),
                "generated_features": len(feature_cols),
            }
        ]
    )
    if not heatmap_summary.empty:
        summary = pd.concat([summary, heatmap_summary], ignore_index=True)
    meta = {
        "selector_source": selector_meta,
        "exp148_oof_source": exp148_meta,
        "heatmap_source": heatmap_meta,
        "generated_feature_count": int(len(feature_cols)),
        "generated_feature_columns": feature_cols,
    }
...
def build_sp45_bimodal_selector_features(
    ...
    features, groups, summary, _meta = build_heatmap_selector_compact_features(
        base_frame,
        config,
    )
...
def run_heatmap_selector_compact_addonly_on_exp148(
    *,
    output_dir: str | Path,
    train_dir: str | Path,
    cache_path: str | Path | None = None,
    ...
    hmp184_feature_config: dict[str, Any] | None = None,
    ...
    hmp184_features, hmp184_group_columns, hmp184_summary, hmp184_meta = (
        build_heatmap_selector_compact_features(
            full_frame,
            hmp184_feature_config or {},
        )
    )
    hmp184_feature_columns = [col for col in hmp184_features.columns if col not in {"id", "well"}]
    if not full_frame["id"].equals(hmp184_features["id"]) or not full_frame["well"].equals(
        hmp184_features["well"]
    ):
        raise ValueError(
            "exp184 heatmap selector features are not row-order aligned with train feature frame"
        )
    _assign_aligned_float32_columns(
        full_frame,
        hmp184_features,
        hmp184_feature_columns,
    )
    del hmp184_features
    gc.collect()
    print(
        json.dumps(
            {
                "stage": "added_hmp184_selector_features",
                "rows": int(len(full_frame)),
                "hmp184_selector_features": int(len(hmp184_feature_columns)),
            },
            sort_keys=True,
        ),
        flush=True,
    ...
    hmp184_summary.to_csv(
        output_dir / f"{OUTPUT_PREFIX}_hmp184_selector_feature_summary.csv",
        index=False,
    )
            ...
            feature_schema_rows.append(
                {
                    ...
                    "is_hmp184_selector_feature": bool(feature in hmp184_feature_columns),
                }
            )
    ...
    manifest = {
        "experiment": "exp184_heatmap_selector_compact_addonly_on_exp148",
        ...
        "selector_parent": "exp184_cnn_sdf_mtp_heatmap_path_features_on_exp158",
        "cache_parent": "exp072_exp063_full_replay_feature_cache",
        "mode": "heatmap_selector_compact_addonly_on_exp148_full_train_rows",
        "feature_source": feature_meta,
        "learned_likelihood_feature_source": learned_source_meta,
        "hmp184_selector_feature_source": hmp184_meta,
        ...
        "hmp184_feature_config": hmp184_feature_config or {},
        "projection_feature_groups": projection_group_columns,
        "learned_feature_groups": learned_group_columns,
        "hmp184_selector_feature_groups": hmp184_group_columns,
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "hmp184_selector_feature_summary": f"{OUTPUT_PREFIX}_hmp184_selector_feature_summary.csv",
...
def run_saved_model_inference(
    *,
    ...
    variant_name: str = "sp45_bimodal_selector_confidence_addonly",
    ...
    if not test_frame[["id", "well"]].reset_index(drop=True).equals(
        ...
        raise ValueError(
            "Raw-test exp184 heatmap selector features are not row-order aligned with test feature frame"
        )
    test_frame = pd.concat(
        [
    ...
    if configured_sp45_groups and {
        ...
        raise ValueError("exp184 heatmap selector feature groups differ from train manifest")
    feature_group_columns = {
        **feature_group_columns,
        **sp45_group_columns,
    }
