...
class CFG:
    ...
    SHOW_FIGS = False
    DETERMINISTIC = bool(int(os.environ.get("ROGII_DETERMINISTIC", "0")))
...
def stable_seed(value: str, offset: int = 0) -> int:
    digest = hashlib.blake2b(str(value).encode("utf-8"), digest_size=8).digest()
    return (int.from_bytes(digest, "little") + int(offset)) % 2_147_000_000
...
@njit(cache=True)
def _pf_ancc(md_v, z_v, gr_v, gg, vmin, step, gs, ls, ir, N, ALPHA, RN, PN, IS, RP, RR, RESAMP, seed_base):
...
def _pf_z(md_v, z_v, gr_v, gr_sm_v, gg_p, gg_s, vmin, step, gs, ip, iv, beta, icpt, zsig, N,
         MOM, VN, PN, GR_WT, RP, RV, RESAMP, seed_base):
    np.random.seed(seed_base)
...
def run_pf_ancc(hw, tw_tvt, tw_gr, N=ANCC_N, seed_base: int = 0):
    ...
    pts, std = _pf_ancc(ev.MD.values.astype(np.float64), ev.Z.values.astype(np.float64), ev.GR.values.astype(np.float64),
                        gg, gmin, gst, gs, ls, ir, N, ANCC_ALPHA, ANCC_RN, ANCC_PN, ANCC_IS, ANCC_RP, ANCC_RR, PF_RESAMP, int(seed_base))
    return pts.astype(np.float32), std.astype(np.float32)

def run_pf_z(hw, tw_tvt, tw_gr, N=PF_N, seed_base: int = 0):
    ...
    pts, std = _pf_z(ev.MD.values.astype(np.float64), ev.Z.values.astype(np.float64), ev.GR.values.astype(np.float64),
                     ...
                     PF_MOM, PF_VN, PF_PN, PF_GR_WT, PF_ROUGH_P, PF_ROUGH_V, PF_RESAMP, int(seed_base))
    return pts.astype(np.float32), std.astype(np.float32)
...
_pf_ancc(_m, _z, _g, _gg, 45., .1, 20., 50., 0., 8, .998, .002, .005, .3, .1, .001, .5, 123)
_pf_z(_m, _z, _g, _g, _gg, _gg, 45., .1, 20., 50., 0., -1., 0., .1, 8, .993, .005, .01, .3, .2, .003, .5, 456)
_beam_jit(np.linspace(-1., 1., 30), np.linspace(-1., 1., 50), 25, 8, 15., 100.)
_pf_lik_allseeds(_m, _z, _g, _gg, 45., .1, 20., 50., 0., 64, 4, 789, .998, .002, .005, .1, .001, .5, 4.5)
...
def build_well(hw_path, tw_path, is_train, likpf_map=None):
    ...
    seed_prefix = f"{split_name}:{wid}"
    pf_a, std_a = run_pf_ancc(hw, tw_tvt, tw_gr, seed_base=stable_seed(f"{seed_prefix}:pf_ancc"))
    if len(pf_a) == 0: return None
    pf_z, std_z = run_pf_z(hw, tw_tvt, tw_gr, seed_base=stable_seed(f"{seed_prefix}:pf_z"))
...
def _likpf_rows(wid, split):
    ...
    out, idx, _ = lik_pf(hw, tw, seed_base=stable_seed(f"{split}:{wid}:likpf"))
...
def build_likpf(wids, split):
    # Deterministic mode uses n_jobs=1 because the likelihood PF seeds NumPy RNG inside
    # a numba kernel; thread interleaving can otherwise change the effective random stream.
...
def _direct_candidate_series(test_df: pd.DataFrame) -> dict[str, pd.Series]:
    last = pd.to_numeric(test_df["last_known_tvt"], errors="coerce")
    ...
    for name in absolute_columns:
        if name in test_df.columns:
            candidates[name] = pd.to_numeric(test_df[name], errors="coerce")
    for name, column in delta_columns.items():
        if column in test_df.columns:
            candidates[name] = last + pd.to_numeric(test_df[column], errors="coerce")
    return candidates


def _resolve_reference_submission_paths(reference_submission_paths: list[str] | None) -> list[Path]:
    paths: list[Path] = []
    for value in reference_submission_paths or []:
        if not value:
            continue
        path = Path(value)
        if path.exists():
    ...
    kaggle_input = Path("/kaggle/input")
    if kaggle_input.exists():
        for path in sorted(kaggle_input.rglob("submission.csv")):
            # Skip the competition sample file; only kernel output submissions are useful references.
            if "sample" in path.name.lower() or "rogii-wellbore-geology-prediction" in str(path.parent):
                continue
            paths.append(path)
...
def _compare_submission_to_references(
    ...
    current = submission[["id", target_column]].copy()
    current["id"] = current["id"].astype(str)
    current[target_column] = pd.to_numeric(current[target_column], errors="coerce")
    for path in reference_paths:
        try:
            ref = pd.read_csv(path)
        except Exception as exc:
            rows.append({"reference_path": str(path), "status": f"read_failed: {exc}"})
            continue
        if "id" not in ref.columns or len(ref.columns) < 2:
            rows.append({"reference_path": str(path), "status": "invalid_columns"})
            continue
        ref_target = target_column if target_column in ref.columns else str(ref.columns[1])
        ref = ref[["id", ref_target]].copy()
        ref["id"] = ref["id"].astype(str)
        ref[ref_target] = pd.to_numeric(ref[ref_target], errors="coerce")
        merged = current.merge(ref, on="id", how="inner", suffixes=("_candidate", "_reference"))
        if merged.empty:
            rows.append({"reference_path": str(path), "status": "no_common_ids"})
            continue
        diff = merged[f"{target_column}_candidate"] - merged[f"{ref_target}_reference"]
        rows.append(
            {
                "reference_path": str(path),
                "status": "ok",
                "rows": int(len(merged)),
                "changed_rows": int((diff.abs() > 1e-9).sum()),
                "diff_rmse": float(np.sqrt(np.mean(np.square(diff)))),
                ...
                "corr": float(
                    np.corrcoef(
                        merged[f"{target_column}_candidate"].to_numpy(float),
                        merged[f"{ref_target}_reference"].to_numpy(float),
                    )[0, 1]
                )
                if len(merged) > 1
                else np.nan,
...
def run_pixiux_pf_beam_direct_submit_audit(
    *,
    data_dir: str | Path,
    output_dir: str | Path,
    submission_path: str | Path,
    ...
    candidate: str = "likpf_mean",
    sample_submission_path: str | Path | None = None,
    submission_target_column: str = "tvt",
    reference_submission_paths: list[str] | None = None,
) -> dict[str, Any]:
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    submission_path = Path(submission_path)
    ...
    t0 = time.time()
    test_df, feature_meta = build_replay_test_frame()
    candidates = _direct_candidate_series(test_df)
    if candidate not in candidates:
        available = ", ".join(sorted(candidates))
        raise ValueError(f"candidate={candidate} is unavailable. Available: {available}")

    candidate_df = test_df[["id", "well", "last_known_tvt"]].copy()
    for name, values in sorted(candidates.items()):
        candidate_df[name] = pd.to_numeric(values, errors="coerce").astype("float64")
        candidate_df[f"{name}_d"] = candidate_df[name] - candidate_df["last_known_tvt"].astype("float64")
    candidate_df.to_csv(
        output_dir / "pixiux_pf_beam_direct_candidate_predictions.csv.gz",
        index=False,
        compression="gzip",
    )
    ...
    pred_map = dict(zip(candidate_df["id"].astype(str), candidate_df[candidate].astype(float), strict=False))
    default_value = float(candidate_df[candidate].mean())
    fallback = _sample_fallback_prediction(CFG.DATA, sample["id"], default_value)
    mapped = sample["id"].astype(str).map(pred_map)
    missing_mask = mapped.isna()
    sample[submission_target_column] = mapped.fillna(fallback).astype("float64")
    ...
    tracker_test_meta = save_tracker_feature_frame(
        test_df,
        output_dir / "pixiux_pf_beam_direct_tracker_features_test.csv.gz",
    )
    candidate_metrics = []
    for name in sorted(candidates):
        values = pd.to_numeric(candidate_df[name], errors="coerce")
        candidate_metrics.append(
            {
                ...
                "delta_mean": float(candidate_df[f"{name}_d"].mean()),
                "delta_std": float(candidate_df[f"{name}_d"].std()),
            }
        )
    candidate_metrics_df = pd.DataFrame(candidate_metrics)
    candidate_metrics_df.to_csv(output_dir / "pixiux_pf_beam_direct_candidate_metrics.csv", index=False)

    reference_paths = _resolve_reference_submission_paths(reference_submission_paths)
    comparison_df = _compare_submission_to_references(sample, submission_target_column, reference_paths)
    comparison_df.to_csv(output_dir / "pixiux_pf_beam_direct_reference_comparison.csv", index=False)

    metrics_df = pd.DataFrame(
        [
            {
                "mode": "pixiux_pf_beam_direct_submit_audit",
                ...
                "reference_count": int(len(reference_paths)),
            }
        ]
    )
    metrics_df.to_csv(output_dir / "pixiux_pf_beam_direct_submission_metrics.csv", index=False)
    diagnostics = {
        "rows": int(len(sample)),
        "predicted_rows": int((~missing_mask).sum()),
        "fallback_rows": int(missing_mask.sum()),
    ...
    summary = {
        "experiment": "exp069_pixiux_pf_beam_direct_submit_audit",
        "mode": "pixiux_pf_beam_direct_submit_audit",
        "selected_candidate": candidate,
        "deterministic": bool(CFG.DETERMINISTIC),
        "n_jobs": int(CFG.n_jobs),
        "available_candidates": sorted(candidates),
        "feature_meta": feature_meta,
        "reusable_feature_frames": {
            "tracker_features_test": tracker_test_meta,
        },
        "submission": diagnostics,
        "reference_submission_paths": [str(path) for path in reference_paths],
        "reference_comparison_rows": int(len(comparison_df)),
        "excluded": [
        ...
        "artifacts": {
            "submission": str(submission_path.name),
            "metrics": "pixiux_pf_beam_direct_submission_metrics.csv",
            "candidate_metrics": "pixiux_pf_beam_direct_candidate_metrics.csv",
            "candidate_predictions": "pixiux_pf_beam_direct_candidate_predictions.csv.gz",
            "reference_comparison": "pixiux_pf_beam_direct_reference_comparison.csv",
            "tracker_features_test": "pixiux_pf_beam_direct_tracker_features_test.csv.gz",
        },
        "elapsed_seconds": round(time.time() - t0, 3),
    }
    (output_dir / "pixiux_pf_beam_direct_summary.json").write_text(json.dumps(summary, indent=2))
    print(json.dumps(summary, indent=2), flush=True)
    return summary
