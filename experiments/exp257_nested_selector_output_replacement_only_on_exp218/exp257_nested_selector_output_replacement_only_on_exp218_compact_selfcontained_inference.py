# %% [markdown]
# # exp257 nested selector output replacement-only inference (GPU)
...
# 6. Saved selector and exp257 final model contracts
# 7. Current-test context assembly and parity audit
# 8. exp218 current-test feature surface
# 9. Fold-matched selector replacement and saved-final inference
...
exp238_config_path = next(
    (path for path in exp238_config_candidates if path.exists()), None
)
if exp238_config_path is None:
    raise FileNotFoundError(
        f"cannot resolve exp238 inference config: {exp238_config_candidates}"
    )
...
SELECTOR_OUTPUT_PREFIX = (
    "exp238_nested_hmm_exp226_selector_rank_slot_addonly_on_exp218"
)
EXP099_TRAIN_CACHE = (
...
engine = import_file(
    "exp238_engine_exp257_replacement_inference",
    [
        PACKAGE_DIR
...
replacement_engine = import_file(
    "exp257_replacement_engine_inference",
...
def add_current_test_copcf_features(
    ...
    for column, values in generated.items():
        ...
        out[column] = numeric
    expected_count = int(
        EXP238_CONFIG["inference"]["rawtest_copcf_parity"]["copcf_feature_count"]
    )
...
# %%
parity_config = EXP238_CONFIG["inference"]["rawtest_copcf_parity"]
final_config = CONFIG["inference"]
override_config = final_config.get("user_override", {})
if not bool(override_config.get("authorized", False)):
    raise RuntimeError("exp257 guard-failed inference requires an explicit user override")
if bool(final_config["quality_guard_pass"]):
    raise RuntimeError("exp257 override metadata unexpectedly says the failed guard passed")
if bool(final_config["selector_training_during_inference"]):
    raise RuntimeError("exp257 inference must not train selectors")
if bool(final_config["final_training_during_inference"]):
    raise RuntimeError("exp257 inference must not train final boosters")
...
selector_summary_path = find_one(f"{SELECTOR_OUTPUT_PREFIX}_selector_summary.json")
selector_manifest_path = find_one(f"{SELECTOR_OUTPUT_PREFIX}_selector_model_manifest.csv")
final_manifest_path = find_one(f"{OUTPUT_PREFIX}_model_manifest.json")
...
feature_contract = replacement_engine.validate_feature_contract(
    final_feature_columns,
    expected_feature_count=int(final_config["final_feature_count"]),
)
selector_feature_columns = [
    column for column in final_feature_columns if column.startswith("nsel_")
]
base_feature_columns = list(final_feature_columns)
if selector_feature_columns:
    raise ValueError(f"replacement-only inference found forbidden nsel columns: {selector_feature_columns}")
    ...
    json.dumps(
        {
            ...
            "user_override": override_config,
            "same_fold_guard_pass": False,
...
for outer_fold in range(outer_fold_count):
    ...
    replacement, replacement_meta = replacement_engine.build_selector_output_replacements(
        test_frame,
        test_rows,
        score_sum,
        candidate_columns,
        error_floor=float(CONFIG["model"]["selector_output_adapter"]["error_floor"]),
    )
    if list(replacement.columns) != replacement_engine.REPLACEMENT_COLUMNS:
        raise ValueError(f"outer fold {outer_fold} replacement schema mismatch")
    replacement_audit.append({"outer_fold": outer_fold, **replacement_meta})
    x_matrix = final_test_frame[final_feature_columns].reset_index(drop=True).copy()
    for column in replacement_engine.REPLACEMENT_COLUMNS:
        x_matrix[column] = replacement[column].to_numpy(np.float32)
    x_matrix = x_matrix[final_feature_columns]
    ...
    display(replacement.head(3))
    del artifact, score_sum, replacement, x_matrix
    gc.collect()
...
replacement_contract_path = OUTPUT_DIR / f"{OUTPUT_PREFIX}_replacement_contract.csv"
replacement_audit_path = OUTPUT_DIR / f"{OUTPUT_PREFIX}_inference_replacement_audit.csv"
...
feature_contract.to_csv(final_schema_path, index=False)
replacement_engine.replacement_contract_frame().to_csv(
    replacement_contract_path, index=False
)
...
summary = {
    "status": "guard_failed_user_override_inference_completed_not_submitted",
    "mode": final_config["mode"],
    "user_override": override_config,
    "same_fold_exp238_guard_pass": False,
    ...
    "selector_output_replaced_feature_count": len(
        replacement_engine.REPLACEMENT_COLUMNS
    ),
    "selector_input_diagnostic_preserved_count": len(
        replacement_engine.PRESERVED_SELECTOR_INPUT_COLUMNS
    ),
    "nsel_feature_count": len(selector_feature_columns),
    ...
    "artifacts": {
        ...
        "replacement_contract": replacement_contract_path.name,
        "replacement_audit": replacement_audit_path.name,
    ...
    "sha256": {
        ...
        "replacement_contract": engine._sha(replacement_contract_path),
        "replacement_audit": engine._sha(replacement_audit_path),
    ...
    "notes": [
        ...
        "All 20 saved selectors and 15 saved exp257 final LightGBM models are SHA/schema checked.",
        ...
        (
            "The exp218 380-column schema is retained; 29 existing ll selector-output "
            "slots are overwritten from the 11-candidate nested selector scores."
        ),
        "No nsel add-only columns are introduced; 25 selector-input diagnostics are preserved.",
        (
            "The same-fold exp238 quality guard remains failed. Inference is executed only "
            "because the user explicitly requested an override; this does not promote exp257."
        ),
    ],
}
