# %% [markdown]
# # exp238 raw-test copcf parity saved-selector inference (CPU)
...
# 6. Saved selector contracts
# 7. Current-test context assembly and parity audit
# 8. Fold-specific saved-selector inference
...
if bool(parity_config["selector_training_during_inference"]):
    raise RuntimeError("raw-test copcf parity must not train selectors")
if bool(parity_config["final_training_during_inference"]):
    raise RuntimeError("raw-test copcf parity must not train final boosters")
if bool(parity_config["submission_requested"]):
    raise RuntimeError("raw-test copcf parity is audit-only and must not submit")
...
summary_path = OUTPUT_DIR / f"{OUTPUT_PREFIX}_rawtest_copcf_summary.json"
...
summary = {
    "status": "rawtest_copcf_parity_completed_not_submitted",
    "mode": parity_config["mode"],
    ...
    "notes": [
        ...
        "No selector/final model is fitted and no submission.csv is generated.",
        "Selector safety guard remains failed; these score surfaces are parity diagnostics only.",
    ],
}
