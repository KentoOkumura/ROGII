...
# ---
# %% [markdown]
# # exp211_affine_calibrated_gr_observation_pfbeam train
#
# Scoped train-side audit for replacing raw GR observation likelihood with
# prefix-only affine-calibrated GR in PF/Beam generation.
...
# 4. Affine observation and PF / Beam variant contract
# 5. Affine-calibrated GR PF / Beam generation audit
...
from affine_calibrated_gr_observation_pfbeam import (
    parse_filter_specs,
    read_exp072_eval_cache,
    run_affine_calibrated_gr_observation_pfbeam,
    select_target_wells,
)
...
if target_wells.empty:
    raise RuntimeError("No target wells selected for affine GR PF/Beam audit.")
...
if not any(spec.kind == "affine_calibrated" for spec in filters):
    raise RuntimeError("At least one affine_calibrated observation variant is required.")
...
# %%
result = run_affine_calibrated_gr_observation_pfbeam(config=config, paths=paths)
...
print("affine fallback summary")
display(
    pf_diagnostics.groupby(
        ["filter", "affine_fallback", "affine_fallback_reason"],
        dropna=False,
    )
...
display(
    ...
    .agg(
        ...
        affine_fallback_rate=("affine_fallback", "mean"),
