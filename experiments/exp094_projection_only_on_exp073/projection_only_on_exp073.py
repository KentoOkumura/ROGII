...
OUTPUT_PREFIX = "exp094_projection_only_on_exp073"
...
def apply_projection_variant(
    ...
    *,
    ...
    result["pred_tvt"] = (
        frame["pred_tvt"].to_numpy(np.float32) + float(beta) * correction
    ).astype(np.float32)
    ...
    result["projection_correction_applied"] = (float(beta) * correction).astype(np.float32)
    ...
    for degree in degrees:
        for beta in betas:
            for robust_c in robust_cs:
                variant = f"degree{degree}_beta{beta:g}_c{robust_c:g}"
