...
- 実験は `ml_model`、`pf_beam`、`ensemble` の 3 ルートで管理します。ML model の通常 CV 基準、PF/Beam の決定的ルール候補、ML + PF/Beam ensemble / public notebook replay は根拠を分けて記録します。
...
- 2026-07-27、ユーザー依頼によりexp226の保存済みgroup-safe OOF
  ...
  詳細は`docs/analysis/exp226_offset_root_cause_audit_20260727.md`を正とする。
...
- exp063 の Pixiux likelihood-PF diagnostics は古い自前 PF/Beam とは別物として扱う。通常の ML 改良では直接置換ではなく、prefix backtest、residual clip、sample weight、typewell / spatial prior の材料に限定する。`pf_beam_disagreement_error_map` は完了済みで、発見は `artifacts/pf_beam_disagreement_error_map/README.md` と `pf_beam_disagreement_*metrics.csv` を正とする。
...
古い依存関係の全量はこのファイルに残さない。完了済み実験の詳細、古い候補の失敗理由、日付つきログは `experiment_summary.md`、`submissions/SUBMISSIONS.md`、各実験の `SESSION_NOTES.md` / `result.md` を正とする。ここでは次の実験判断に直接効く依存だけを残す。
...
- 調査メモ: [`public_discussion_notebook_catchup_2026-07-05.md`](docs/notebooks/rogii-wellbore-geology-prediction/public_discussion_notebook_catchup_2026-07-05.md)
- 公開notebook archive:
  - [`hujile/rogii-maybe-you-like`](docs/notebooks/rogii-wellbore-geology-prediction/date_run_recent_20260705/hujile__rogii-maybe-you-like/) / https://www.kaggle.com/code/hujile/rogii-maybe-you-like
  - [`lightningv08/rogii-lb-7-168`](docs/notebooks/rogii-wellbore-geology-prediction/date_run_recent_20260705/lightningv08__rogii-lb-7-168/) / https://www.kaggle.com/code/lightningv08/rogii-lb-7-168
  - [`yusuketogashi/rogii-lb7156-baseline`](docs/notebooks/rogii-wellbore-geology-prediction/date_run_recent_20260705/yusuketogashi__rogii-lb7156-baseline/) / https://www.kaggle.com/code/yusuketogashi/rogii-lb7156-baseline
  - [`pilkwang/working-note-target-free-tvt-geosteering`](docs/notebooks/rogii-wellbore-geology-prediction/date_run_recent_20260705/pilkwang__working-note-target-free-tvt-geosteering/) / https://www.kaggle.com/code/pilkwang/working-note-target-free-tvt-geosteering
  - [`georgymamarin/fork-the-ruler-not-the-model`](docs/notebooks/rogii-wellbore-geology-prediction/date_run_recent_20260705/georgymamarin__fork-the-ruler-not-the-model/) / https://www.kaggle.com/code/georgymamarin/fork-the-ruler-not-the-model
  - [`busyaprime/persistence-is-the-geosteering-baseline-to-beat`](docs/notebooks/rogii-wellbore-geology-prediction/date_run_recent_20260705/busyaprime__persistence-is-the-geosteering-baseline-to-beat/) / https://www.kaggle.com/code/busyaprime/persistence-is-the-geosteering-baseline-to-beat
- Discussions:
...
| 状態 | アイデア | 実装範囲 | Kaggle train 前の確認 | 禁止事項 |
| --- | --- | --- | --- | --- |
| implementation complete・exp411 schedule prerequisite FAIL・run blocked | `exp226_hmm_guided_defensive_mixture_pf` / `exp420_exp226_hmm_guided_defensive_mixture_pf` | exp226 geometry rateとuntreated HMM innovation scheduleを元transition 50%の単一importance-corrected PFへ統合するcompact候補、fixed44/full orchestration、SHA freeze、truth-late gate、13 testsを実装済み | exp411の同一schedule / fixed32が方向一致`0.225397`、fold`0 / 5`、control発火`0.136119`、persistent-control active-well差`0.0`で事前固定gateをFAILした。現行契約の正規Notebook採用、package、Stage 0 fixed44を行わず、実装参照として保持する | schedule threshold/duration/refractoryやgateをsame-OOF救済しない。再開はexp411 scheduleをそのまま使わない独立仮説・別実験・別承認とし、現行exp420のfull、inference、submissionは禁止 |
| Stage A1 version 3 FAIL_CLOSED・branch閉鎖 | `prefix_anchored_mode_persistence_hmm_readout` / `exp391_prefix_anchored_mode_persistence_hmm_readout` | Kaggle private CPU version 3（id_no `128527913`）で固定16 HMM wellsを完走。HMM-supportedは1/19 events・1/5 folds、causeはposterior averaging 1 / transition 0 / K16 0 / fixed blend 3 / unresolved 15。same-pass parity最大0.35 ft、normalization誤差2.4568e-05、full換算870,045.814秒もFAIL。peak RSS 4.132145 GBとmode identity重複/衝突なしはPASSしたが、78,866 candidate rowsは全行fail closed | Stage B、inference、submissionへ進まない。再開には今回のgateを緩和しない独立したHMM内部原因の証拠と、新しい実験設計・実行承認が必要 | `jump_used`だけ、top1/top2 rank ID、exp236値とexp270 pathの同一posterior扱い、truth/errorによるevent/well/mode選択、parity/normalization/runtime/mechanism gate緩和、threshold/matching/fallback/blend救済、alternate mode平均、MAP/Viterbi直接採用、PF/Beam/K16変更、inference、submissionは禁止 |
| corrected mean4 selector PASS・downstream 15 GPU未承認・旧output quarantine | `pf_ancc_small_seed_mean_addonly_selector_audit` / `exp277_pf_ancc_small_seed_mean_addonly_selector_audit` | version 2はraw-test-only 88特徴、40 model SHA、25 partition契約、leakage/score guardを全PASS。mean4はprior比3指標を5/5 folds改善したが、corrected exp264 original `pf_ancc`比はexpected-errorだけ小幅改善、within10 2指標は小幅悪化でmixed | TVT価値を測るならversion 2 compactを固定し、`downstream_mean4_only` 1 variant × 3 configs × 5 folds = 15 GPU boosters、control再学習0を別途承認する。旧reported score/compact/importanceは使わない | exp276 0-booster再検証との順序を含め次の実行はユーザー判断。mean8/both/aggregate、旧output/旧380列control再利用、PF再生成、inference、submitは未承認 |
| 高・Stage 0 PASS、Stage 1 GPU version 4実行中 | `fold_safe_grwr_5_addonly_on_exp287` / `exp402_fold_safe_grwr_5_addonly_on_exp287` | 分割Stage 0は18/18 PASS。Stage 1は426列、1 variant × 3 configs × 5 folds = 15 T4 boosters、control再学習0。version 2のmount path defect、version 3のexp145 input不足はいずれも0 boosterで停止。version 4は物理T4 / SHA-qualified resolverに加え、固定11 inputとexp145必要3ファイルをfail-fast確認してRUNNING | 完了連絡後にOOF / fold / scope / by-well / importance / model manifest SHAを取得し、固定promotion gateを判定する。継続監視はしない | 旧exp218 GRWR値、entropy依存6列目、exp396 score-27、候補subset/formula/grid、sample weight、hard gate、direct correction、control再学習、same-OOF rescueは禁止。inferenceはpromotion PASS後、submissionはsubmit-check後の別承認 |
| version 4 COMPLETE・technical PASS / scientific FAIL・terminal close | `scale5_sigma_gr_likelihood_pf_ablation` / `exp404_scale5_sigma_gr_likelihood_pf_ablation` | x1.0/x1.3 common-seed paired predictionを完走。x1.0 / x1.3 RMSE `10.914522 / 11.174615`、gain `-0.260093 ft`、nonworse fold `1/5`、by-well p95 `+4.826467 ft`。parity・truth-late・SHA・実行量は全PASS | global scale5 x1.3を棄却して閉鎖。version 1凍結予測のx1.0列は、別途明示承認された独立ML実験exp413の入力としてSHA固定再利用を許可する。exp404自体のinference / submissionや新規PFは行わない | exp404内でのscale 3/8/12、temperature/multiplier/clip/seed/particle grid、meanへのprimary差替え、adaptive gate、HMM/Beam/ML/blend、same-OOF救済は禁止。exp413はx1.3 FAILを再分類せず、x1.0を固定12候補MLへ全面置換する別仮説 |
| Stage A version 1 COMPLETE・technical PASS / scientific FAIL・terminal close | `scale5_seed_aggregation_promotion_audit` / `exp417_scale5_seed_aggregation_promotion_audit` | exp404同一x1.0 500-particle ×128-seed bankの算術平均と固定temperature-5を0-PF saved-OOF監査。scale5はRMSE`10.914522073`対`11.594897884`、gain`0.680375810 ft`、5/5 folds、全固定scope、fixed HMM/LikPF 50:50を改善。input SHA、same-bank、truth-late、parity、artifact manifestは全PASS | direct by-wellは516改善 / 257悪化、delta p95`+2.941688483 ft`、worst well`70925e23 +25.311274575 ft`で固定上限`0.0 / 0.25 ft`をFAIL。`fixed_scale5_seed_aggregation_rejected_close_without_rescue`で閉じ、raw-test inference / submissionへ進まない | temperature/scale grid、best seed、median/mode/medoid/selector、roughening/process noise/GR sigma変更、well/row gate、same-OOF救済は禁止。exp413の独立ML全面置換branchを再分類しない |

| full 4 shard + merge v2 COMPLETE・technical PASS / scientific FAIL・terminal close・`exp429` | `self_gr_weak_boost_likelihood_pf_ablation` / `exp429_self_gr_weak_boost_likelihood_pf_ablation` | 固定1 scientific variant / 773 wells / 3,783,989 rows / 98,944 seed-well / 49,472,000 particle startsを完走。primaryはcandidate `11.127406421`対control `10.914522073`で`0.212884347 ft`悪化、改善fold`1/5`。arithmetic secondaryだけは`0.023416828 ft`改善した | by-well p95 `+0.770627049 ft`、worst well `+34.862601957 ft`、fixed HMM/PF 50:50 `+0.070319209 ft`、raw observed / 1000+ / hidden-like 2面も悪化。direct self-GR boostはwrong basinとwell-tailを増幅したと判断し、`terminal_close_without_self_gr_or_pf_rescue_grid`で閉鎖する | alpha/clip/window/top-k/temperature/GR sigma/particle/seed/transition/resampling、symmetric/negative boost、proposal/initialization/post-hoc switch、HMM/Beam/ML blend、truth/error gateのsame-OOF救済は禁止。inference/submissionへ進まない。独立候補は低優先feature-only readoutに限定 |
### 未着手バックログ

2026-08-03の優先度再評価では、full OOF tail FAILで終端閉鎖したexp500とdownstream
transfer FAILのexp502を未着手表へ戻さない。保存full OOF診断だけを使うexp500 harm
attributionとexp501 cross-fixed13原因readoutはP4に維持し、exp502由来のtransfer attributionも
0-model saved-artifact-onlyのP4へ置く。exp504もpooled gainとblock-rank mechanismは示したが
fold / hidden-like / well-tail gateをFAILしたため、保存生成物だけの原因分解をP4に置く。
exp505もpooled / 4 folds / 全scopeを改善したがmaterial tail縮小をFAILしたため未着手表へ
戻さず、fade selector tail不変の保存artifact原因readoutだけをP4へ追加する。
exp507はStage Dまで完了して固定gateをFAILしたため、実装済み表から削除して終端閉鎖した。
保存artifactだけのdownstream transfer原因readoutをP4へ追加し、既存P1/P2/P3を追い越さない。
2026-08-04にexp508は固定SG61/p3のStage Aを完了し、fold / scope / tail / startは全PASSしたが
pooled gain `0.006133728 ft`が固定`0.01 ft`を未達として終端閉鎖した。完了済みexp508を表から
削除し、PASS前提のwell routerも作成しない。window / tau / gate救済は追加しない。同日、
ユーザーは最終提出2枠をexp413単独にせず、strict public-core小blendとexact public
pre-override小blendへ固定する方針を選んだ。設計済みexp509をP0、第1枠、exp510をP1、第2枠へ
追加した。exp509は同日の別承認で、exp497 Stage I v4保存modelとdynamic hidden-safe exp413を
読むprediction-only compact self-contained候補を実装した。親の一時`submission.csv`をfinal名から
隔離し、strict / exp413 / prior blendを`0.002 / 0.020 / 0.020 ft`のcomponent別historical
equivalenceで監査した後、固定`0.8628352667 / 0.1371647333`式だけをfloat64で適用する。
dedicated 6 tests、exp497 dependency 30 tests、Jupytext、Ruff、strict validatorはPASS。
正規Notebook採用、Kaggle package/run、output取得、提出は未承認のため行っていない。
exp510は同日の別承認でartifact preflightと
compact self-contained候補inferenceを実装し、後続承認でvisible Kaggle version 2、output取得、
submit-check、code submissionまで実施した。visible technical gateはPASSしたがref `55225634`は
hidden rerunで失敗し、公開test固定exp413 sidecarのID不一致を高確度原因と診断した。
後続承認でexp413 v4をdynamic sample上で再生成するhidden-safe修正を実装し、同一kernel version 4を
14,151 rows / 3 wells、385.11秒で完走した。生成CSV readback後のfinal content / submission SHAは
version 2と完全一致し、technical gateはPASS。後続承認でref `55231514`を提出し、627分後に
COMPLETE、Public LB `7.201`、Kaggle UI `matching your best`を確認した。submitted payload、
同期的な最終blend/write、visible成分差分の再監査もPASSしたが、exp413比の測定可能なLB改善はない。
full-precision同値はAPI非公開で不明とし、overall / ML anchorはexp413を維持する。正規Notebook採用は
未承認とする。実装・提出・scoringが完了したため、exp510をアイデアバックログから削除する。
同日、exp506はprimary gateをFAILして終端したためP3から削除した。非選択controlのscope/tail
原因readoutは上記exp490横断調査で完了し、独立experimentへはしない。固定blendを非選択のまま
閉じ、algorithm preflight、ML add-only、factorial、evidence依存switchingをP2--P4へ追加する。
exp509/510の優先順位は変更しない。
同日、ユーザー提案によりexp226の周辺井戸local-linear K16 rateを「TVT予測後の
後処理」へ変換する`exp511_exp413_transductive_k16_neighbor_rate_postprocess`を実装・実行した。
Kaggle private CPU version 4は全技術条件をPASSし、exp413 `7.884802794405`から
`7.883964795206`へ`0.000837999 ft`改善したが、固定`0.01 ft`を未達し、nonworse foldも
`2/5`で固定`4/5`を未達として終端閉鎖した。scope、by-well p95/worst、continuityはPASS。
support成立は`350/12,368 segments = 2.8299%`、unique donor wells中央値5、選択距離中央値
`4,291.485 ft`で、強いexp413へ安定転用するにはpredicted-only近傍supportが疎だった。
alpha/clip/K/bandwidth/rho/theta/support/fade/scope/gateの同一OOF救済、独立rerun、inference、
submissionは行わない。実装済みexp511をバックログから削除し、同じK16 donor supportへ依存する
exp356は非退化supportの独立証拠待ちというP4 demotionを維持する。exp509/510の優先度は変えない。
2026-08-05、ユーザー判断によりhjyact公開Notebook
`Ultimate PF-Config Strategy | A Reproducible Score` version 2 / run `337064157`の完成版final predictionを
hidden sample上で動的再現し、exp413と等率で混ぜる
`exp512_hjyact_v2_final_10pct_hedge_on_exp413`を新規P1として実装した。ディレクトリ名の`10pct_hedge`は
初期設計の履歴名として維持する。hjyact source Public LBは
`6.568`、有効profileは`vp_balanced_modelpkg_005`。SP45/learned `0.60/0.40`、guarded overlap、balanced
visible-prefix、model-package guard、PF seed-branch hedgeを最終write順ごと固定し、
ユーザーの変更指示により`0.50 * exp413 + 0.50 * hjyact_v2_final`だけを評価する。visible output copy、特定well ID、現在の
14,151行/3 wellへの分岐、weight/profile/threshold grid、Public LB後の救済は禁止する。source final SHA
`b192d3f3...b9ded4a`とのvisible exact parityと2回のKaggle rerun SHA一致を先行gateにし、両成分を
dynamic sample上で再生成する。honest OOFはなく、visible testとPublic LB scoring subsetは別物として
扱う。6,859行の別名compact self-contained inference候補、source/model manifest、契約テストを作成し、
Jupytext、構文、Ruff F821、専用pytest 6件、`validate-exp`をPASSした。正規Notebook採用、Kaggle実行、提出は
未承認である。同日の追加指定で、hjyact learnedとexp413に共通する7-beam、NCC、formation/dense、
deterministic feature blockはfingerprint付きDAGでwellごとに1回だけ生成して共有する実装へ更新した。
hjyact learned likPFはGR sigma `x1.3` / seed0、exp413 likPFは`x1.0` / well別stable seedで異なるため
共有禁止とし、SP45 PF / 14-beamも独立生成する。保存modelは88ファイル/108推定器、新規booster 0、
source Ridge 5 fitsで固定した。共有によるproduction runtime短縮はKaggle実行後に
`candidate_reuse_manifest.json`でnode別に実測し、両componentの既知SHA一致を必須gateとする。その後、exp512は
別途承認されたKaggle inferenceをcanonical slugへversion 1--5までpushした。version 1は旧competition mount、
version 2は旧Ridge dataset mountで科学予測前に停止した。version 3--5はRidge 5 folds、hjyact final、
exp413 saved-model、0.50/0.50 prediction生成まで完了し、hjyact visible parityをPASSしたが、exp413 exact
content SHAだけをFAILした。K16係数固定とHaswell subprocess隔離後もobserved SHAは`3a9bbd1f...be68d87`で、
reference `875a1334...584dc4`との差は2,203行、最大`0.0165 ft`、RMSE`0.000753012 ft`。診断CSVの
提出形式とID順はPASSしたが、科学契約FAILのため外部提出していない。その後ユーザーはmax `0.02 ft` / RMSE
`0.001 ft`の数値許容を選択した。監査済みwitness SHAだけを許可し未知SHAはfail-closeするversion 6を実装し、
952,694-byte packageを同じKaggle GPU/internet-off kernelで完走した。hjyact exact SHA、exp413 witness、15/15
shared recordsのgeneration=1/cache hit=1、0.50/0.50 formula、14,151行submit-checkを全PASS。submission SHA
`b960c2b1...a713e23`はv3診断runとbyte-identical。外部提出と正規Notebook採用は未実行で、同一v6 package rerun
1/2かつhidden-well RNG未証明のためdeterministic anchorには昇格しない。static visible sidecarは使わない。
同日、ユーザー指示によりexp512で50%混合する完成版`hjyact_v2_final`を単独提出してLBを確認する
`exp513_hjyact_v2_final_standalone_public_lb_audit`をP0として設計確定した。exp512の公開component境界、
profile、SP45/learned `0.60/0.40`、guarded overlap、balanced visible-prefix、model-package guard、
PF seed-branch hedge、write順を固定し、exp413再生成、50/50 blend、cross-consumer candidate reuseだけを
除外する。source Public LB `6.568`は参照値であってexp513の実績ではない。visible output copy、特定well ID、
現在の14,151行/3 wellへの予測分岐、parameter/weight grid、LB後救済は禁止する。追加指示でexp512 v1/v2の
失敗を反映した5,097行 / 236,961 bytesの別名compact候補を実装し、旧competition/Ridge root直接assignmentを
禁止した。exp413 runtime、50/50 blend、shared-DAGを除外し、source/model manifestと専用7 testsを作成した。
Jupytext、構文、Ruff F821、親込みpytest 13件、strict validationはPASS。その後、437,126-byteの同一packageを
Kaggle GPU / internet-offで2回完走し、両runの14,151行visible finalがsource SHA
`b192d3f3...b9ded4a`と一致した。exp512 v1/v2のmount failureも再発していない。一方、後段override前の
SP45/learned blend統計はrun間で異なり、source由来のunseeded Numba PFをthread並列生成するhidden-well経路の
決定性は未確定である。正規Notebook採用、submit-check、competition submissionは未承認・未実行とし、
source RNG維持かwell別seed固定かの判断までfail-closeする。
同日、ユーザー指示によりexp512の9時間hidden inference制限を解くP0 runtime/scientific候補として、
`exp514_exp413_likpf_seed_bank_reuse_on_exp512`を設計確定した。exp413 `x1.0` / 500 particles /
128 seeds / well別stable SHA256のNumba bankをSP45より前に1 well 1回だけ生成し、同じraw seed pathsと
log-likelihoodからscale `3/5/8/12`、arithmetic mean、temperature-5 branch統計を派生する。
scale 5だけを既存exp413 feature/model graphへ、全scaleをknown prefix連結したfull-length配列とbranch統計として
既存SP45 selector/hedgeへ渡す。learned `x1.3`、Gold masked-prefix PF、`pf_ancc`、`pf_z`、Beamは共有しない。
exp413 scale5はparent content exactを必須にする一方、SP45 legacyとはRNG、seed namespace、first MD step、
typewell gridが異なるため同値高速化と表記せず、新しいscientific variantとしてpaired精度gateを置く。
visible 3 wellsはphysical override後にPF差が隠れるためpositive evidenceへ使わない。target-free fixed32 technical /
2-run後、当初はtarget-free fixed200でpooled`+0.02 ft`、4/5 folds、scope`+0.05 ft`、p95`+0.25 ft`、worst`+5 ft`の
all-ANDを判定し、raw-only 200-well end-to-end shadowでobserved`<=8.5h`、bootstrap p95`<=9h`を必須にする。
現行`11.5--15.0h`から重複SP45を算術控除した`7.3--9.4h`は参考値に限定する。続くユーザー指示で実装のみを承認し、
SHA固定generator、7,920行full inference候補、746行Stage A fixed32専用候補、7 contract testsを作成した。
exp073 kernel / interp / stable-seed AST一致、SP45 / exp413 duplicate call除去、consumer各1回ledger、
target-free Stage A入口、構文 / Ruff F821 / Jupytext / strict validationはPASSした。その後Stage A専用packageを
Kaggle private T4 version 1で完走し、32 wells × thread 1/4 × 各2 runのaggregate / branch / ledger SHAが
4 run完全一致した。実行時間は`2,363.410299秒`、truth read / booster / 親再学習 / submissionはすべて0で、
fixed32 technical gateはPASS。その後のユーザー指示でStage BはStage Aと同じtarget-free 32 wellsへ縮小した。
selection SHAを再利用し、gate閾値は維持するが、小規模screeningであって200-well一般化証拠とは扱わない。
legacy control 32 + shared candidate 32 = 64 well-bank生成、学習/booster 0のStage B v2はKaggleで正常完走した。
v1とfrozen prediction SHAは同一で、primary pooled RMSEはcontrol `9.010759361`、candidate
`9.060439859`、delta `+0.049680497 ft`。nonworse fold `2/5`、raw-GR observed
`+0.060618254 ft`、by-well p95 `+0.647871442 ft`も固定上限を超え、all-ANDをFAILした。
hidden-like 2面は`-0.117664920 ft`改善、worst wellは`+1.192163546 ft`でPASSしたが、事前規約により
scale/seed/selector/well subsetで救済せず終端する。Stage D v3はGold最大4-processとSP45/HJYACT決定論
176列共有により3 visible wellsを`839.696591秒`で完走し、v2の5出力SHA完全一致、submission check PASS、
200-well推定`6.174531--7.957332h`のruntime estimated PASSを得た。ただしこれはStage B精度FAILを
代替しないため、exp514のStage D submissionは提出しない。将来のruntime改善はlegacy SP45 stochastic
bankの科学条件を維持する実装最適化に限定し、exp413 stable-seed bankとの科学的共通化を再利用しない。
その後ユーザー実施のcode submission ref `55266559`はhidden rerun unhandled errorでscoreなしとなった。
静的監査ではStage D v3末尾のvisible-v2 5出力SHA guardがhiddenでも無条件に走る欠陥を確認したが、
hidden tracebackは取得不能で実際の最初の例外は未確定。visible parent peak RSS `25,124.602 MiB`がchildを
含まないmemory riskとStage B科学FAILも残るため、guard修正だけの再提出は行わない。
その後のユーザー指示でStage D v4の技術修正を実装した。Ridge train/OOF/modelをshared PF前に解放し、
shared PFとSP45を同一4-thread well streamingへ統合して全scale full payloadを各well消費後に破棄する。
SP45→HJYACT→exp413は全行deep copyを避けた所有権移譲とし、exp413まで保持するshared PF値は
`likpf_scale_5 / likpf_mean` float32だけに縮約した。visible v2の5 SHAはvisible sampleだけで検査し、
hiddenでは`SKIPPED_HIDDEN_DYNAMIC`とする。19 contract tests、Ruff、Jupytext、strict validationをPASS後、
canonical private T4 version 4をvisibleで完走した。v2の5出力SHAは完全一致、submission check PASS。
Ridge current RSSは`12,979.207→791.668 MiB`、exp413前は`953.895→865.621 MiB`、shared bank終了時は
`1,359.234→1,037.676 MiB`へ低下し、全well full payload保持はfalseだった。visible 3 wellsは
`879.386897秒`、200-well工程別外挿は`6.289658--8.057147h`でestimated PASS。ただしparent peak
`25,123.293 MiB`はRidge前peakを保持しchild processも含まないため、hidden OOM/完走保証ではない。
Stage B scientific FAILは変更せず、competition再提出は行っていない。
同日、ユーザーからexp512のcode submission完了とPublic LB `6.541`が報告され、Kaggle CLIでもref
`55255459`、submitted `2026-08-05 02:08:11.450000 UTC`、status `COMPLETE`、Public LB `6.541`を確認した。
exp413 / exp510 `7.201`から`0.660`、source公開値`6.568`から`0.027`改善したため、exp512を新しい全体・
アンサンブルPublic LB基準とする。数値score付きCOMPLETEは実際のhidden rerunがplatform上限内で完走した証拠だが、
正確なhidden runtimeとwell数はAPIから取得できない。その後、exact v6 sourceをcanonical kernel version 8として
再実行しlatest versionとした。v6/v8のhjyact、exp413、submissionはbyte-identicalで、same-v6 visible output gateは
2/2 PASS。一方、deterministic-GR intermediate content SHAは不一致で、hidden stochastic determinismも未証明。
latest v6 contractの工程別200-well外挿`14--18h`は保守的planning auditとして残し、visible 3 wellからhidden runtimeを
逆算しない。honest OOF、Private LB、full intermediate byte reproducibilityは未証明であり、Public LB結果だけで
private一般化やweight再調整を正当化しない。

| 優先度 | アイデア | 先行条件 / 依存 | 検証方法 | 注意点 |
| --- | --- | --- | --- | --- |
| 最優先・P0・Kaggle 2-run visible parity PASS・hidden RNG fail-close・`exp513` | `hjyact_v2_final_standalone_public_lb_audit` / `exp513_hjyact_v2_final_standalone_public_lb_audit`: exp512の完全なhjyact-v2 final componentを単独でhidden-safe再生成し、Public LBを1回だけ確認する | 親はexp512。source version 2 / run `337064157`、公開記載LB `6.568`、final SHA `b192d3f3...b9ded4a`を固定する。437,126-byte同一packageのv1/v2は両方COMPLETEし、visible final SHA一致。exp512 v1/v2の旧competition/Ridge mount失敗も解消 | runtime `819.939 / 816.881`秒、saved model 13 / estimators 33、新規booster 0。visible final 2-run parityはPASSしたが、pre-override blend統計が異なり、unseeded Numba PF + threaded well生成による未知well決定性は未成立。source RNG維持かwell別seed固定かを決め、後者なら別candidateのvisible parityと未知well相当2-runを再検証する | routeは`ensemble`、honest OOFなし。competition submit/LBは未実行。正規Notebook採用、submit-check、submitは別承認。visible output/static sidecar/cardinality分岐、LB後retuneは禁止。最終visible一致だけでhidden-readyと判定しない |
| 完了・Public LB `6.541`・v8 exact-v6 latest・visible output 2/2 PASS・hidden RNG WARN・`exp512` | `hjyact_v2_final_10pct_hedge_on_exp413` / `exp512_hjyact_v2_final_10pct_hedge_on_exp413`: hjyact version 2完成版finalとexp413をユーザー指定`0.50/0.50`で混ぜる。名前の10pctはlegacy | ユーザー承認のmax `0.02 ft` / RMSE `0.001 ft` gateを、v3--v5で監査済みのexp413 witness SHAだけに限定。exact v6 sourceをv8として再実行しlatest化。v6/v8のhjyact `b192d3f3...`、exp413 `3a9bbd1f...`、submission `b960c2b1...`はbyte-identical。user submission ref `55255459`はCOMPLETE / Public LB `6.541`で、exp413/510 `7.201`から`-0.660`、source公開`6.568`から`-0.027` | latest v8は88 model files/108 estimators、新規booster 0、scientific `1,193.477`秒。15 shared recordsはgeneration=1/cache hit=1/fallback 0、formula error 0、submit-check PASS。model-packageは5 modelsを実行したがp95 `26.700659 ft` guardでfinal weight 0。latest v6-contractの事前200-well外挿`約14–18h`はplanning FAIL。scored hidden rerunのCOMPLETEはplatform完走証拠だがscript version、正確なhidden runtime/well数は不明 | visible component/final 2-runはPASSしたが、deterministic-GR intermediate SHAとhidden-well stochastic determinism、honest OOF、Private LBは未完。visible 3-well runtimeからhidden時間を逆算しない。Public LB後のweight/profile変更、PF誤共有、visible/static sidecar、ID/cardinality分岐、Codexによる再提出、正規Notebook採用は禁止/未承認 |
| 完了・final slot 1・exp497 v2へ統合・exp509別run不要 | `exp413_strict_public_core_final_slot` / `exp509_exp413_strict_public_core_final_slot`: 保存exp413とexp497 strict public-coreを`0.8628352667 / 0.1371647333`で固定blendするprivate一般化優先枠 | exp497 Stage Eのpublic-core weightは5/5 positive、中央値`0.1371647333`だが、candidate gain`0.010314644 ft`、3/5 folds、hidden-like`+0.105138/+0.097410 ft`、p95/worst`+0.700720/+7.541588 ft`で科学的gate FAIL。ユーザーがexp413単独を避けるfinal portfolio overrideとして明示選択した。exp509候補と同じdynamic exp413 + 保存exp497 model + component parity + 固定式を、AGENTSのsame-exp inference原則に従ってexp497正規inferenceへ統合した。exp497 saved-model inference v2はKaggle T4でCOMPLETE、fit 0、14,151行outputとSHAを検証済み | exp497 FAILとselected train anchor exp413を維持したまま、truth-free全AND監査をPASS。exp509は履歴として保持するが、重複する正規Notebook採用/Kaggle runは行わない | Gold/contact/same-well/public output、weight再fit/grid、router、最終SG/warmup/projection、exp497再評価は禁止。external competition submitは別承認 |
...
| 低・P4・高リスクCPU PF・Stage 0全technical PASS・Stage 1別承認待ち・`exp487` | `time_varying_gr_affine_likelihood_pf` / `exp487_time_varying_gr_affine_likelihood_pf`: exp345 causal EKFとexp350 bidirectional RTSの`a_t,b_t` scheduleをPF particle emission centerへ適用する | Kaggle private CPU version 5（id_no `129180524`）で2×32=64 PF wells、8,192 seed-well、4,096,000 particle startsを完走し、全15 technical checksがPASS。fixed32記述RMSEはsaved exp404 control `9.616741`に対しcausal `12.634360`、RTS `13.391424`で大幅悪化した。static affine PF exp211の悪化と同方向で、raw GR二重利用riskへのnegative warningが強い | Stage 0契約上はStage 1 eligibleだが、自動昇格せずP4へdemote。別承認時だけ同じ固定条件で2×773=1,546 PF wellsを実行し、各variantを独立判定してwinnerを選ばない | a/b/process noise/slope/timing、causal/RTS blend、sigma/temperature、well/row gate、same-OOF winner、救済は禁止。base HMM/control PF rerun 0。Stage 1/inference/submissionは未承認・無効 |
| 高・P2・design frozen・direct physical LB・実装未承認・順序2・`exp453` | `huber_exact_hmm_direct_public_lb_audit` / `exp453_huber_exact_hmm_direct_public_lb_audit`: exp389固定`delta=1.345` Huber exact HMMを単体Public LBで評価する | exp389 saved-OOFはGaussian exact HMM `11.938287235`から`11.852741130`へ`+0.085546105 ft`、5/5 foldsと全固定scope改善。一方by-well p95 `+0.002234351 ft`、worst `+1.750248202 ft`でtail gateをFAILした。比較controlはexp434 `exact_hmm` | 1物理モデル・1実験・将来の実行対象inference Notebook 1本。Huber式だけを固定し、absolute-TVT grid 0.35、41 rate states、transition/prior/sigma/GR補完/posterior meanをexp209と同一にする。candidate HMMだけをdynamic test wellごとに1回decodeする | delta/scale/temperature/clip/sigma/grid/transitionの探索、Gaussian/Student-t/control再実行、blend/selector/gate/postprocess/ML、LB後の適応、自動昇格を禁止。実装、Kaggle run、submissionは各別承認 |
| 高・P2・design frozen・direct physical LB・実装未承認・順序3・`exp454` | `student_t_exact_hmm_direct_public_lb_audit` / `exp454_student_t_exact_hmm_direct_public_lb_audit`: exp374固定`df=4.0` Student-t exact HMMを単体Public LBで評価する | exp374 saved-OOFはGaussian exact HMM `11.938287235`から`11.720478702`へ`+0.217808533 ft`、4/5 folds、raw observed/missing、high-missing、1000+、hidden-like 2面を改善。一方by-well p95 `+0.982661344 ft`、worst `+35.015963236 ft`でtail gateをFAILした。比較controlはexp434 `exact_hmm` | 1物理モデル・1実験・将来の実行対象inference Notebook 1本。`-0.5*(df+1)*log1p(z^2/df)`だけを固定し、absolute-TVT grid 0.35、41 rate states、transition/prior/sigma/GR補完/posterior meanをexp209と同一にする。candidate HMMだけをdynamic test wellごとに1回decodeする | df/scale/temperature/clip/sigma/grid/transitionの探索、Gaussian/Huber/control再実行、blend/selector/gate/postprocess/ML、LB後の適応、自動昇格を禁止。実装、Kaggle run、submissionは各別承認 |
...
| 完了・terminal FAIL・高リスクCPU HMM+PF・`exp432` | `symmetric_datum_defensive_particle_reinjection` / `exp432_symmetric_datum_defensive_particle_reinjection`: exp412と同じ最初のpersistent rate-gap eventで、方向を使わずbase/minus/plus=`0.80/0.10/0.10`の±datum proposalを一度だけ混ぜ、`p0/q`で元PF targetを保つ | Kaggle private CPU Stage 0 v1（id_no `128974856`）で21/32 wellsがtrigger。no-event/parity/importance/truth-lateはPASSし、trigger後SSEは`12.0871%`改善したが、support外率は`0.4278 points`悪化、nonworse foldは`3/5`、worst controlは`+0.583073 ft`でmechanism AND gateをFAIL。exp419/425と合わせ、importance correctionや対称化だけでは安全なsupport回復を保証しない | `stage0_fail_closed`。full、inference、submissionへ進まず、fixed32上のmixture/datum/trigger/gate救済も行わない。再開には局所reinjectionがsupport悪化を回避できる新しいtarget-free根拠、新しい独立仮説、別実験、別承認を必須とする | 実行Notebookのruntime gateは4 PF shardsを反映せず単一kernel投影`91,746.964秒`でFAILしたが、mechanism FAILは独立に確定。現行実装と生成物SHAは参照として保持し、positive evidenceには使わない |
...
| 閉鎖・exp391 Stage A1 FAIL | `left_right_mode_consensus_confidence_readout` / `exp395_left_right_mode_consensus_confidence_readout`: 同じstable HMM mode lineageをheel側 / toe側のdisjoint GRで別々に採点し、posterior overlapだけをconfidenceにする | 先行条件だったexp391 Stage A1のHMM内部原因支持は1/19 events・1/5 foldsで、必要な60%以上・4/5 foldsを満たさなかった。parity、normalization、runtimeもFAILした | 実装・実行しない。独立したHMM内部原因の証拠が新たに得られた場合だけ別設計として再検討する | exp391のFAILを再分類しない。reverse TVT predictor、top1/top2 rank identity、TVT candidate、hard mode switch、補正、blend、selector、window/gap/mode threshold/weight grid、same-OOF rescue、inference、submissionは禁止 |
...
| blocked/demoted・P4・design-only・非退化support証拠待ち | `exp226_donor_covariance_sig_r_on_exp209` / `exp356_exp226_donor_covariance_sig_r_on_exp209`: 旧exp324のdonor rate covarianceでexp209 transition noiseだけを局所化する | exp362の近傍50 / bandwidth 500 ftでは`n_eff>=10`が0/12,368 segments、nearest-distance超過772でlocal scheduleが完全退化した。exp511でもunique donor wells>=8を満たしたのは350/12,368 segments（2.8299%）、中央値5、選択距離中央値4,291.485 ftに留まり、強いexp413へのgainは`0.000837999 ft`、2/5 foldsだった。同じK16 donor supportを使うため、独立したtruth-free readoutで非退化coverageを示すまで実装しない | 再開する場合も、まず0-HMMで`n_eff`、unique donor wells、nearest distance、fallback/clip coverageだけを固定contractで監査する。support非退化後にのみsaved exp209 transition residual NLL gain`>=1%`、4/5 folds、stress非悪化を評価する | exp362/511と同じsupport/bandwidth閾値の盲目的再利用、同一OOFでのK/近傍/bandwidth/support/shrink/clip grid、rate-mean変更、parent control再実行は禁止 |
...
| 保留・P4・known-prefix Stage 0・design-only | `stratified_registration_offset_pf` / `exp369_stratified_registration_offset_pf`: physical positionと`delta=[-6,-3,0,3,6] ft`を分離し各delta層を維持する | 500 particlesを初期`50/75/250/75/50`、resample後は各delta最低25。出力p、emission p+delta。exp230/297/345に加えexp359 full-window shift rankの全fold負方向と、5層によるphysical解像度低下から保留 | Stage 0はvisible prefix 128/64 rolling-originでNLL gain`>=1%`、4/5 folds、circular差`>=0.5%`、nonzero/boundary mass、符号安定性をAND評価。PASS・別承認時だけ1 treatment / `98,944` seed-well runs、exp072比`>=0.05 ft`とtail/delta-mass guard | 未実装・未実行。exp359 window score、deltaの出力加算、DTW/affine、delta/transition/quota grid、rate予測、particle/seed増加、blend、PF control replayは禁止 |
...
### `exp286_geop_hmm_sparse_addonly_candidate_on_exp264` 実装契約

この項目は2026-07-20に`exp286_geop_hmm_sparse_addonly_candidate_on_exp264`としてStage 0から
ユーザー明示指示によるfull-all-well 13候補Stage B、nested Stage C、downstream Stage Dまで完了した。
steeringは`.steering/20260719-exp286-geop-hmm-sparse-addonly-candidate-on-exp264/`、実験記録は
`experiments/exp286_geop_hmm_sparse_addonly_candidate_on_exp264/`を正とする。親は修正版
`exp264_exp263_candidate_confidence_dual_selector`、routeは`ensemble`。exp279はPF/Beam側の追加候補生成だけに使い、
exp264のML selector / downstreamと両方が予測生成に本質的に寄与するためである。Kaggle Stage 0 version 1は
full unionの3粒度・5/5 folds改善を確認したが、固定gateのwhole-well gain保持率が`27.710961% < 50%`で
candidate guardをFAILした。固定gate分岐は閉じたまま救済せず、full candidateをselectorへ渡すStage Bを
別の問いとしてversion 4で完了した。parent selector比のguardはPASS、fixed fallback比のhard guardはFAIL。
続くStage Cはparent12比`-0.203850 ft`、4/5 folds、score/leakage guard PASS。Stage Dはparent12
add-only比`-0.057027 ft`へpooled改善したが2/5 folds、worst-well `+5.862833 ft`で総合guard FAIL。
Stage C/Dの学習は完了済みとし、runtime replay、inference/submissionは無効のまま閉じる。

#### 仮説と変更境界

- `geop_hmm`のstandalone RMSE 10.035987やexp263固定配合への直接差し替え+0.478571 ftは採用根拠にしない。一方、exact HMM単体を全5 foldsで1.902300 ft改善し、exact HMMとの残差相関が0.3127、rowwiseで49.66%、wellwiseで374/773良かったことを「既存bankと異なる誤差を持つ追加候補」の根拠に限定する。
- 現行12候補、exact HMM、self-GR HMM、2つのlegal score domain、fallbackを変更しない。`geop_hmm`はprimitiveの13番目add-only candidateとし、primitive+pair domainは11から12、primitive+fixed domainは7から8へだけ拡張する。全13候補を一つのhard-selectable domainにせず、existing HMMの差し替え、固定blend、hard top1、PF rescueとして扱わない。
- Stage 0 sparse診断ではgate外をavailability=0としたが、Stage Bではgateを使わず全OOF行をavailability=1とする。候補値の補完は行わない。
- 事前gateは`geop_hmm`実行前に得られるexp226 geometry/GR delta、exact HMM sigma/loglik、exp226-exact/self-GR disagreement、既存bank disagreement、MD/tail/known-prefix contextだけから作る。true TVT/error/oracle、well ID、`geop_hmm-exact_hmm`差はgate入力に使わない。

#### 段階実行と性能guard

1. Stage 0は保存済みOOFだけの1 audit variant、LightGBM config / trained fold / booster `0 / 0 / 0`とする。13候補unionが現行12候補bankに対してrow、512-block、whole-well oracle RMSEをpooledですべて改善し、各指標4/5 folds以上で同方向になることを必須にする。
2. 事前gate scoreはfeature、単調方向、等重みrank、top-25% cutoffをtruth join前に凍結したが、full whole-well gain保持率27.71%でFAILした。この固定gateは救済しない。
3. ユーザー明示overrideにより、Stage Bはgateなしの全wellで1 config x 2 objectives x 5 folds = 10 CPU modelsだけを新規学習する。candidate one-hotを12から13へ拡張し、`geop_hmm`のID、kind/family、availability、generic proxy、posterior std/loglik confidenceを他候補と同じcandidate-longへ載せ、raw-test-safe schemaをStage Aで再監査する。既存12候補controlは再学習せず、保存済みparent hard RMSE 8.587004、fold、shared-12 scoreと直接比較する。13候補hard RMSE改善、3/5 folds改善、geop選択>0、13候補score guard PASSを必須とする。
4. ユーザー追加承認により、Stage Cは5 outer x 4 inner x 2 objectives = 40 CPU modelsを実行し、parent12 hard `8.652532`に対してfull13 `8.448682`、4/5 folds改善。25 partitions / compact 77列、score/leakage guardを全PASSした。
5. 同じ承認scopeのStage Dはclean273 + compact77 = 350列、3 configs x 5 folds = 15 T4 boostersだけを学習し、parent/controlは保存済みexp264結果を使って再学習しなかった。parent12 `8.460811`に対しfull13 `8.403784`へ改善したが、fold改善2/5、worst-well `+5.862833 ft`で総合guard FAIL。平均改善は記録するがtrain-side promotion、inference、submissionへ進めない。

#### 9時間hidden inference runtime guard（未実行・Stage D guard FAILで閉鎖）

以下は事前に固定した推論化条件である。Stage Dのfold/worst-well guardを通過しなかったため実行せず、
条件変更やruntimeだけのoverrideで再開しない。

- exp264 inference v4の424.511秒は公開3 wellsでの実測にすぎず、hidden約200 wellsの見積もりには使わない。exp279の全773 wells decode実測は合計18,045.296秒、平均23.34秒/well、最大47.77秒/wellであり、`geop_hmm`を200 wells全件へ追加すると単純換算で平均約1.30時間、観測最大換算で約2.65時間を単独で要する。この換算もhidden実測ではなく、全件生成を禁止する根拠としてだけ使う。
- runtime監査はtrue TVTを生成入力から落としたraw-test schemaのtrain-like 200-well shadow replayで行う。row数・well長の分布を合わせ、現行exp264のみと、同一入力・同一machineでtop-25%以下へ`geop_hmm`を追加した版をpaired計測する。per-stage / per-well時間から200-well aggregateをbootstrapし、p50 / p95 / observed upper boundをmanifestへ保存する。
- hidden推論候補化のhard gateは、事前gate対象を`min(floor(0.25 * N_wells), 50)`以下、各OOF fold coverage`<=30%`、geop追加p95`<=45分`、全candidate生成・40 selector model score・15 TVT model score・submission保存を含む総p95`<=7.5時間`とする。9時間上限に1.5時間の余白を残せない場合、または現行exp264 baseだけで上限を超える場合は推論化しない。
- Stage 0からruntime guardまでを全通過してもinference時の学習は0 boosterとする。full-well `geop_hmm` fallback、runtimeに応じた動的threshold緩和、途中生成物を使った候補補完、9時間上限ぎりぎりの提出は行わない。

### `exp264_exp263_candidate_confidence_dual_selector` 設計契約

実験は`experiments/exp264_exp263_candidate_confidence_dual_selector/`、steeringは`.steering/20260716-exp264-exp263-candidate-confidence-dual-selector/`に作成済み。候補・特徴・出力の機械可読な正は実験内`candidate_contract.yaml`、`feature_contract.yaml`、`output_contract.md`とする。

#### 候補と信頼度

- 候補source of truthはexp263。score対象はraw-test-ready 6 primitive、raw-test 5 pair、固定`exp226_w500_50_50`の12 surface。exp263の33 inventory、12 core、23 unique train/diagnostic surfaceとdeployable score bankを混同しない。
- 12本はすべてdual scoreを持つが、単一hard-selectable bankにしない。top1/top2/marginはprimitive+pair 11本domainとprimitive+fixed 7本domainで別々に作る。
- `blend_likpf_hmm_w500`は`likpf_mean__exact_hmm`のaliasなので独立candidate IDを作らない。exp264の12候補はすべて現行exp263 Stage 1でcurrent-test生成済みである。Stage 0 OOFにはあるが現行Stage 1出力に未収録の追加6 primitiveとouter-fold fitted formula 2本は、Stage 1拡張とparity確認前にはselectorへ混ぜない。
- candidate IDは文字列としてartifactへ残し、model入力は12 one-hot。family、primitive/pair/fixedもone-hotにし、ordinal `candidate_index`は禁止する。
- source-native confidenceは全候補に同じsigmaを要求しない。実在するsigma/loglik/entropy/margin/support/ESS/fallbackをNaN+validityで保持し、formulaは親別namespaced confidence、valid数、component range/stdを使う。全候補にavailability、shape、anchor距離、bank/formula disagreementのuniversal proxyを持たせ、共通の最終尺度を`pred_abs_error`と`p_within10`にする。
- 採用schemaがsource-native confidenceを使う場合、current-testはexp263 Stage 1の`confidence__<candidate_id>__<field>` parityを必須にする。未出力confidenceを暗黙の0または全NaNへ置換して推論しない。
- Stage A採用schemaに必要なcurrent-test namespaceは21列。exp226 2、self-GR HMM 10、likPF valid 1、
  exact HMM 4、PF-ANCC 2、Beam 2で固定し、exp264 inferenceは欠損・非finite・invalidをfeature生成前に停止する。

#### 特徴とlabel

- exp251 v4の295 candidate-long列はretain/recompute/remove/defer分類seedとして参照し、最終allowlistとして無条件継承しない。
- featureを`ctx__`、`cand__`、`conf__`、`bank__`、`formula__`、`id__`へ再編する。旧`sc_ens/hyb/tvt_dense*`固有列とordinal IDを落とし、candidate-relative、candidate-set contextはexp263 12 bankへ再計算する。初回実装はraw horizontal/typewellからtrain/current-test共通contextを作り、`copcf_*`は同一cross-fit/current-test generatorを接続するまでdeferする。
- labelはfeature schemaとfold固定後にtrue TVTを別joinして`candidate_abs_error`と`candidate_within10`を作る。true TVT/error/oracle、catalog RMSE、pair readout、outer eligibilityをfeatureへ入れない。
- Stage Aでconstant/all-missing/exact duplicateを除去する。|Pearson|または|Spearman| 0.999以上は報告し、exact/functional duplicateでない限り初回は自動dropしない。全特徴の説明、provenance、raw-test status、欠損率、objective×fold重要度をcatalogへ保存する。

#### 学習・出力・guard

1. Stage Aは0 booster。exp263 manifest `85e60ac1...a26bb9e`、3,783,989 rows、773 wells、12 identity/coverage、formula parity、confidence missing、forbidden feature 0、schema/content SHAを確認する。
   修正版Kaggle version 4は600,000 candidate-long rowsを監査し、150列から全欠損41、定数5、完全重複16を除いた88列をlogical schema SHA `aaef4ffd...ddd3a4`で凍結した。高相関14組はreport-only、採用側の完全重複0、構造的NaNを持つ採用特徴25列。raw contextはtrain 773/773・current-test 3/3 fileでavailability PASS。
2. Stage Bはstandard LightGBM固定、1 variant × 2 objectives × outer 5 = 10 CPU boosters。修正版version 5は
   expected-error MAE 3.795801、within10 logloss/Brier 0.359972/0.112451でpooled・5/5 folds改善しscore guard PASS。
   hard top1はfixed比+0.348673、0/5 folds改善でFAIL。45,407,868 candidate-long rows、3,783,989 compact rows、
   10/10 model SHAを監査済み。exp261/exp262でextra-treesが回帰・selector両面とも悪化したため
   random-threshold variantを使わず、同時config探索をしない。
3. canonical audit出力は候補別`pred_abs_error` / `p_within10`のcandidate-long Parquet。運用出力は同じfold/process内で候補別score、2 legal domainのtop1/top2/margin、score分散、entropy、top1 one-hotへcompact化する。推論時score CSVの保存・再読込とViterbiは不要。
4. score guardはcandidate別outer-train priorに対しexpected-error MAE、within10 logloss/Brierをpooledかつ4/5 folds以上で改善すること。candidate/fold/distance/confidence-validity別calibrationも保存する。
5. hard readoutはrowwise diagnosticだけ。固定blend 8.238331からoverall -0.02以上、3/5 folds改善、near/1000+/hidden-like各+0.02以内、worst-well +0.25以内を見る。hard readout failでもscore guard passならnested add-only候補には残せる。
6. 旧Stage C/Dは計算自体は完走したが、training-only formation 12特徴をouter-validで使用したfeature availability leakageにより全OOF判断を無効化した。旧score guard、7.805644、fold/distance/hidden-like/by-well差、重要度を性能根拠にしない。replacement-only、HMM+LGB exp221/234/240、hard inference、Viterbi、softmax TVT平均、competition submitは禁止する。
7. downstreamはユーザー判断で無効107列を落とした273列baseへ確定した。380列を元の方式で復旧せず、allowlist SHA `d01a73cc...77bf`をfit前に検証する。旧matched controlは使わず、修正版Stage Dはcontrol 15 + 347列add-only 15 = 30 GPU boostersを承認scopeどおり完走した。
8. 修正版Stage Cは88列schemaでouter 5 × inner 4 × 2 objectives = 40 CPU boostersを新規生成し完了した。
   score/leakage guardはPASS、hard guardはFAIL。40 modelと25 partitionのmanifest/SHAを監査し、
   74列compactだけをStage D add-only候補へ残す。
9. 修正版Stage D version 3はcontrol 10.476169、add-only 8.460811、5/5 folds改善。near / 1000+ /
   hidden-likeは改善したがworst-well +14.482873でguard FAIL。continuous compact仮説は支持されたが、
   corrected inference / submissionへは昇格しない。

2026-07-18の推論v3で、Stage Aがraw-test-safeと誤分類したtraining-only formation 6列のraw値・差分12特徴をStage B/C outer-validへ直接供給していたことが確定した。これはfeature availability leakageであり、Stage A/B/C、Stage C compact、Stage D add-only OOF、旧7.805644と全比較・重要度を無効化した。数値とSHAは失敗再現用にのみ保持する。exp263候補生成は別契約なので自動無効化しないが、修正版selectorへ使う前にparityを再監査する。

## データ配置

- 標準のローカルデータ置き場: `data/raw/`
- 外部データ置き場: `data/external/`
- 処理済みの再利用データ: `data/processed/`
- 実験で生成された特徴量: `experiments/<exp>/features/`
