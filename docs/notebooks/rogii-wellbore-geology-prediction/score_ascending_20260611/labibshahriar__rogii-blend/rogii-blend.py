...
# %% cell 17
# Auto-tune tau and w_sub1 on train OOF before defining PP
def _tune_pp(train_df, meta_oof):
    from scipy.optimize import minimize
    _base  = train_df['last_known_tvt'].values.astype(float)
    _ytrue = train_df['target'].values.astype(float) + _base
    _pf    = train_df['pf_ancc'].values.astype(float) - _base
    _lp    = train_df['likpf_scale_5'].values.astype(float) - _base
    _md    = train_df['md_since'].values.astype(float)

    def _score(params):
        tau, w_sub1, alpha = params
        if tau < 10 or tau > 300: return 999.
        if w_sub1 < 0.3 or w_sub1 > 0.9: return 999.
        if alpha < 0.7 or alpha > 1.3: return 999.
        _wu   = 1. - np.exp(-np.maximum(_md, 0.) / tau)
        _sub1 = alpha * _wu * meta_oof
        _delta = w_sub1 * _sub1 + (1 - w_sub1) * _lp
        return float(np.sqrt(np.mean((_base + _delta - _ytrue)**2)))

    res = minimize(_score, x0=[85., 0.60, 1.0], method='Nelder-Mead',
                   options={'xatol':0.5, 'fatol':0.001, 'maxiter':300})
    best_tau, best_w, best_alpha = res.x
    print(f"Tuned: tau={best_tau:.1f}, w_sub1={best_w:.3f}, alpha={best_alpha:.3f}, RMSE={res.fun:.4f}")
    return float(best_tau), float(best_w), float(best_alpha)
...
# This runs after train_stack() returns meta_oof — place this call
# right after the line: meta_oof, meta_test, OOF, TEST = train_stack(...)
# like this:
#   PP.tau, PP.w_sub1, PP.alpha = _tune_pp(train_df, meta_oof)
...
# Find optimal blend weight using train wells with known TVT
_blend_scores = []
for _row in _rows:
    _w = _row['w_sp45']
    _w_fle = 1.0 - _w
    
    # Load train wells that exist in both submissions
    _train_hw_files = sorted(_ov_glob.glob(str(_DATA/"train"/"*__horizontal_well.csv"))) if '_DATA' in dir() else []
    _train_wids_blend = [_ov_os.path.basename(f).split("__")[0] for f in _train_hw_files]
    
    _true_tvts = []
    _pred_tvts = []
    
    _sp45_map = dict(zip(_sp45['id'], _sp45['tvt'].astype(float)))
    _fle_map  = dict(zip(_fle['id'],  _fle['tvt'].astype(float)))
    
    for _wid in _train_wids_blend[:50]:  # sample 50 wells for speed
        try:
            _hw = _final_pd.read_csv(_DATA/"train"/f"{_wid}__horizontal_well.csv")
            _ev = _hw[_hw['TVT_input'].isna()]
            if 'TVT' not in _hw.columns or _ev['TVT'].isna().all(): continue
            for _i in _ev.index:
                _id = f"{_wid}_{_i}"
                if _id in _sp45_map and _id in _fle_map:
                    _pred_tvts.append(_w * _sp45_map[_id] + _w_fle * _fle_map[_id])
                    _true_tvts.append(float(_ev.loc[_i, 'TVT']))
        except: continue
    
    if len(_true_tvts) > 100:
        _score = float(_final_np.sqrt(_final_np.mean((_final_np.array(_pred_tvts) - _final_np.array(_true_tvts))**2)))
        _blend_scores.append((_w, _score))
        print(f"  w_sp45={_w:.2f} -> RMSE={_score:.4f}")

if _blend_scores:
    _best_w = min(_blend_scores, key=lambda x: x[1])[0]
    print(f"Best blend weight: w_sp45={_best_w:.2f}")
    _final_name = f'submission_sp45_fleongg_w{_best_w:.2f}.csv'
else:
    print("Blend scoring failed, falling back to 0.55")
