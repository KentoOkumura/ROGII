...
# %% cell 2
# Offline dependency bootstrap for script reruns.
import glob as _rk_glob
import subprocess as _rk_subprocess
import sys as _rk_sys

try:
    import koolbox as _rk_koolbox  # noqa: F401
except Exception:
    _rk_wheels = _rk_glob.glob("/kaggle/input/**/koolbox-*.whl", recursive=True)
    print("koolbox wheel candidates:", _rk_wheels[:5])
    if _rk_wheels:
        _rk_subprocess.check_call([
            _rk_sys.executable,
            "-m",
            "pip",
            "install",
...
SUBMISSION_PROFILE = 'projected_ridge_pf_pretrained_lgbm_blend'
...
PRETRAINED_LGBM_BLEND_PROJECTED_RIDGE_PF_WEIGHT = 0.55
