...
- exp389 compact sourceから必要なexact-HMM関数だけを抽出し、source/config SHAを照合する。
- Huber式、delta、grid、rate、sigma、補完、posterior meanのcontract testを作る。
- dynamic sample/ID/nonempty-well、finite、posterior、fallback 0、実行量、SHA guardを実装する。
- Gaussian/Student-t/PF/Beam/controlが実行されないことをtestする。
...
- さらに別のcompetition submission承認後、凍結順2番目として提出・監視する。
...
- exp453を採番し、標準experiment scaffoldを作成した。
- 候補をfixed `delta=1.345` Huber exact HMMの1本に固定した。
