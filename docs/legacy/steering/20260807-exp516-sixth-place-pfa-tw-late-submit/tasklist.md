...
- Kaggle inference version 2の公開commit runがT4 x2で完走した。anchor 1197.892秒、similarity 1.856秒、PF 13.483秒、current visible test 3井・14,151行。hidden rerunは数値スコア付きCOMPLETEだが、hidden runtime / well数 / output SHAはAPI非公開。
- Kaggle outputを取得し、submission checkerとmanual auditをPASSした。submission SHAは`feee82f8...80c`。
- kernel version 2を`LATE SUBMIT`固定messageで1回だけ提出した。ref `55326266`。
- scoringを28分監視し、Public `10.056`、Private `8.552`、status `COMPLETE`を記録した。
- 作者報告standaloneとの差と、negative resultが閉じるfinal-v96 `pfA × twGR`単体tuple、未評価の91候補final systemを分離して記録した。
