# catboost_pseudo_tail_ravaghi_features

- 候補名: `catboost_pseudo_tail_ravaghi_features`
- 状態: `検討メモ・設計不可`
- 対応する上位仮説: `未整理`
- 作成日: 2026-08-12
- 最終更新日: 2026-08-12
- 依頼原文: 旧 `KAGGLE_DIRECTION.md` の未着手表にあった記録を、内容を補完せず個別ファイルへ移行する。
- 期待する成果: `catboost_pseudo_tail_ravaghi_features`: CatBoost + Ravaghi features の pseudo-tail branch
...
- 移行前のアイデア: `catboost_pseudo_tail_ravaghi_features`: CatBoost + Ravaghi features の pseudo-tail branch
- 仮説が正しい場合に期待する観測: 未整理
- 仮説を棄却する観測: 未整理
...
- 先行条件 / 依存: Ravaghi feature set、CatBoost pipeline
- 変更するもの: 未整理
- 固定するもの: 未整理
...
categorical / ordered boosting の補完性と bucket readout を見る
...
runtime と reproducibility。exp073 / exp148 系と相関が高ければ停止
