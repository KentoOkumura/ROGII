# catboost_pseudo_tail_revisit

- 候補名: `catboost_pseudo_tail_revisit`
- 状態: `検討メモ・設計不可`
- 対応する上位仮説: `未整理`
- 作成日: 2026-08-12
- 最終更新日: 2026-08-12
- 依頼原文: 旧 `KAGGLE_DIRECTION.md` の未着手表にあった記録を、内容を補完せず個別ファイルへ移行する。
- 期待する成果: `catboost_pseudo_tail_revisit`: CatBoost pseudo-tail を再評価する
...
- 移行前のアイデア: `catboost_pseudo_tail_revisit`: CatBoost pseudo-tail を再評価する
- 仮説が正しい場合に期待する観測: 未整理
- 仮説を棄却する観測: 未整理
...
- 先行条件 / 依存: 既存 pseudo-tail CV/LB と特徴 schema。CatBoost が補完的な bucket を持つ場合のみ
- 変更するもの: 未整理
- 固定するもの: 未整理
...
pseudo-tail bucket、distance bucket、worst-well で LightGBM との差分を見る
...
単独改善狙いではなく diversity readout。学習コストと runtime を先に確認する
