# distance_temporal_huber_loss

- 候補名: `distance_temporal_huber_loss`
- 状態: `検討メモ・設計不可`
- 作成日: 2026-08-12
- 最終更新日: 2026-08-12
- 依頼原文: 旧 `KAGGLE_DIRECTION.md` の未着手表にあった記録を、内容を補完せず個別ファイルへ移行する。
- 期待する成果: `distance_temporal_huber_loss`: distance / temporal aware Huber loss を診断する
...
- 移行前のアイデア: `distance_temporal_huber_loss`: distance / temporal aware Huber loss を診断する
- 仮説が正しい場合に期待する観測: 未整理
- 仮説を棄却する観測: 未整理
...
- 先行条件 / 依存: LightGBM objective または sample weight 実装が必要
- 変更するもの: 未整理
- 固定するもの: 未整理
...
Huber delta と distance weight を少数固定で比較し、bucket 別に見る
...
custom loss の実装差分、runtime、CV/LB 乖離を確認する
