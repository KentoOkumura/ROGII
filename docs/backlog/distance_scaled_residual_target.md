# distance_scaled_residual_target

- 候補名: `distance_scaled_residual_target`
- 状態: `検討メモ・設計不可`
- 対応する上位仮説: `未整理`
- 作成日: 2026-08-12
- 最終更新日: 2026-08-12
- 依頼原文: 旧 `KAGGLE_DIRECTION.md` の未着手表にあった記録を、内容を補完せず個別ファイルへ移行する。
- 期待する成果: `distance_scaled_residual_target`: distance-scaled residual target を試す
...
- 移行前のアイデア: `distance_scaled_residual_target`: distance-scaled residual target を試す
- 仮説が正しい場合に期待する観測: 未整理
- 仮説を棄却する観測: 未整理
...
- 先行条件 / 依存: pseudo-tail または exp092 系の residual modeling branch
- 変更するもの: 未整理
- 固定するもの: 未整理
...
distance で residual を scale し、inverse transform 後の bucket RMSE を確認する
...
transform leakage、tail-only 過学習、near regression を guard にする
