...
EXP_DIR = (
    ROOT / "experiments" / "exp392_exp389_fixed13_dual_selector_on_exp264"
)
...
def test_exp389_input_and_execution_contract_is_frozen() -> None:
    config = read_yaml(EXP_DIR / "config.yaml")
    source = config["data"]["exp389_predictions"]
    assert source["filename"] == (
        "exp389_exp209_huber_exact_hmm_emission_predictions.csv.gz"
    )
    ...
    assert config["execution"]["kaggle_kernel_id_no"] == 128523057
    assert config["execution"]["kaggle_status"] == "KernelWorkerStatus.COMPLETE"
...
def test_exp389_loader_uses_allowlist_and_records_decompressed_sha(
    tmp_path: Path,
) -> None:
    frame = pd.DataFrame(
        {
    ...
    loaded, manifest = load_exp389_predictions(
        path,
        expected_rows=5,
        expected_wells=5,
    ...
    assert manifest["post_read_prediction_content_sha256"] == (
        exp389_prediction_content_sha256(loaded)
    )
...
def test_fixed13_cache_appends_exp389_in_base_row_order() -> None:
    ...
    cache = Exp389Fixed13CandidateCache.__new__(Exp389Fixed13CandidateCache)
    ...
    cache.exp389_manifest = {
        "candidate_requires_oof_fold": False
    }
    cache.exp389_by_key = exp389.set_index(["well_id", "row_idx"])[
        [
            "candidate_tvt",
            "candidate_std",
            "hmm_loglik",
...
def test_fixed13_cache_audits_target_free_global_key_repartition() -> None:
    ...
    exp389 = pd.DataFrame(exp389_rows)
    cache = Exp389Fixed13CandidateCache.__new__(Exp389Fixed13CandidateCache)
    cache.contract = contract()
    cache.ids = [*BASE_CANDIDATE_IDS, ADDED_CANDIDATE_ID]
    cache.specs = {
    ...
    cache.exp389_manifest = {"candidate_requires_oof_fold": False}
    cache.exp389_by_key = exp389.set_index(["well_id", "row_idx"])[
        [
            "candidate_tvt",
            "candidate_std",
...
def test_fixed13_readout_pairs_parent_and_reports_candidate_usage(
    ...
    assert guard["huber_usage_pooled"] == pytest.approx(1.0)
    assert guard["fixed_fallback_error_parity_max_abs_ft"] == pytest.approx(0.0)
    assert (tmp_path / "out" / "exp392_scientific_gate.json").exists()
...
def test_postfreeze_addone_novelty_is_diagnostic_only(tmp_path: Path) -> None:
    ...
    assert (
        tmp_path / "novelty" / "exp392_postfreeze_addone_novelty.csv"
    ).exists()
