...
Checks (in priority order):
  ...
  5. KAGGLE_TOKEN env var (common misconfiguration — auto-maps to KAGGLE_KEY)
  6. .env file in current directory
  7. ~/.kaggle/kaggle.json (legacy)
...
def check_credentials() -> bool:
    ...
    # 3. Check OAuth credentials from `kaggle auth login` (CLI only)
    oauth_credentials = Path.home() / ".kaggle" / "credentials.json"
    if oauth_credentials.exists():
        print(f"[OK] OAuth credentials found via {oauth_credentials}")
        mode = oct(oauth_credentials.stat().st_mode)[-3:]
        if mode != "600":
            print(f"[WARN] File permissions are {mode}, should be 600")
            print(f"       Run: chmod 600 {oauth_credentials}")
        print("[INFO] This is sufficient for kaggle CLI. Configure KAGGLE_API_TOKEN for kagglehub/MCP/headless workflows.")
        return True

    # 4. Check legacy KAGGLE_USERNAME + KAGGLE_KEY env vars
    ...
    # 5. Check for common misconfiguration: KAGGLE_TOKEN instead of KAGGLE_KEY
    ...
    # 7. Check ~/.kaggle/kaggle.json (legacy)
    kaggle_json = Path.home() / ".kaggle" / "kaggle.json"
    if kaggle_json.exists():
        try:
    ...
    print()
    print("  Option 2: Interactive kaggle CLI OAuth login")
    print("    kaggle auth login")
    print()
    print("  Option 3: Set KAGGLE_API_TOKEN env var")
    print('    export KAGGLE_API_TOKEN="your_api_token"')
    print()
    print("  Option 4 (legacy): Set username + key env vars")
    print('    export KAGGLE_USERNAME="your_username"')
    print('    export KAGGLE_KEY="your_api_key"')
    print()
    print("  Option 5 (legacy): Create kaggle.json")
    print("    mkdir -p ~/.kaggle")
