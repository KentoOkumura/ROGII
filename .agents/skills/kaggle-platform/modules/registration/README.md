...
|----------|--------|--------|-----------|
| `KAGGLE_API_TOKEN` | Access token string | "Generate New Token" button | **Yes for agents/CI/headless** |
| OAuth credentials | `~/.kaggle/credentials.json` | `kaggle auth login` | Local CLI only |
...
**Primary method for agents/automation:** A single API token from "Generate New Token" is all you need.
It works with kaggle CLI (>= 1.8.0), kagglehub (>= 0.4.1), and MCP Server.

**Interactive local CLI method:** Kaggle CLI v2.2.0+ supports OAuth:

```bash
kaggle auth login
```

This stores credentials in `~/.kaggle/credentials.json`. It is convenient for local CLI work, but API token files/env vars remain better for headless workflows and MCP.
...
## Step 2: Authenticate the CLI Locally (Optional OAuth)

For local terminal work with a browser:

```bash
kaggle auth login
```

Use `kaggle auth login --no-launch-browser` if the CLI cannot open a browser automatically. Do not paste `kaggle auth print-access-token` output into logs or chat.

## Step 3: Generate API Token (Primary for Agents)
...
## Step 4: Legacy API Key (Optional)
...
- **Never** commit `~/.kaggle/credentials.json`
- **Never** echo or print credential values in terminal output
- **Always** add `.env` and `.kaggle/` to `.gitignore`
- Set file permissions: `chmod 600 .env ~/.kaggle/kaggle.json ~/.kaggle/access_token ~/.kaggle/credentials.json`
