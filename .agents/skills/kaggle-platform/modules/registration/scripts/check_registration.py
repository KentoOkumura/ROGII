...
def check_registration() -> bool:
    ...
    if api_token:
        ...
        print("          Or run `kaggle auth login` for interactive CLI OAuth auth")

    # --- OAuth credentials from `kaggle auth login` ---
    oauth_path = _oauth_credentials_path()
    oauth_found = oauth_path.exists()
    if oauth_found:
        mode = oct(oauth_path.stat().st_mode)[-3:]
        if mode != "600":
            print(f"[WARN] {oauth_path} permissions are {mode}, should be 600")
            print(f"       Run: chmod 600 {oauth_path}")
        print(f"[OK] OAuth credentials: {oauth_path} (from kaggle auth login)")
        found_any = True
    else:
        print("[INFO] OAuth credentials not found (optional; run `kaggle auth login`)")
    ...
    if found_any:
        if api_token:
            ...
            print("For kagglehub, MCP, or headless automation, also configure KAGGLE_API_TOKEN.")
        else:
