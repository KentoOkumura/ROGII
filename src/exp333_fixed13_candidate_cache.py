...
ADDED_CANDIDATE_ID = "exp333_segment_offset"
EXP333_ALLOWLIST = ("well_id", "row_idx", "outer_fold", "tvt_pred_stage1")
...
def validate_fixed13_contract(contract: Mapping[str, Any]) -> None:
    ...
    if str(added.get("kind")) != "learned_segment_offset":
        raise ValueError("exp333 candidate kind must remain learned_segment_offset")
    if added.get("parents"):
        raise ValueError("exp333 candidate must not be reconstructed as a formula")
...
def exp333_prediction_content_sha256(frame: pd.DataFrame) -> str:
    ordered = frame.sort_values(
        ["outer_fold", "well_id", "row_idx"], kind="stable"
    ).reset_index(drop=True)
    digest = hashlib.sha256()
    digest.update("|".join(EXP333_ALLOWLIST).encode())
    for well, part in ordered.groupby("well_id", sort=True):
        ...
        digest.update(
            part["outer_fold"].to_numpy(np.int8).astype("<i1", copy=False).tobytes()
        )
        digest.update(
            part["tvt_pred_stage1"]
            .to_numpy(np.float64)
...
def load_exp333_oof(
    ...
    *,
    ...
    if file_sha != str(expected_file_sha256):
        raise ValueError(f"exp333 OOF file SHA mismatch: {file_sha}")
    if decompressed_sha != str(expected_decompressed_sha256):
        raise ValueError(f"exp333 OOF decompressed SHA mismatch: {decompressed_sha}")
    header = pd.read_csv(path, nrows=0).columns.astype(str).tolist()
    missing = set(EXP333_ALLOWLIST) - set(header)
    if missing:
        raise ValueError(f"exp333 OOF allowlist columns are missing: {sorted(missing)}")
    frame = pd.read_csv(
        path,
        usecols=list(EXP333_ALLOWLIST),
        dtype={
            "well_id": str,
            "row_idx": np.int32,
            "outer_fold": np.int8,
    ...
    if list(frame.columns) != list(EXP333_ALLOWLIST):
        frame = frame.loc[:, list(EXP333_ALLOWLIST)]
    if len(frame) != int(expected_rows):
        raise ValueError(f"exp333 OOF row mismatch: {len(frame)}")
    if frame["well_id"].nunique() != int(expected_wells):
        raise ValueError("exp333 OOF well count mismatch")
    if frame.duplicated(["well_id", "row_idx"]).any():
        raise ValueError("exp333 OOF contains duplicate well/row keys")
    if set(frame["outer_fold"].astype(int)) != set(range(5)):
        raise ValueError("exp333 OOF outer-fold coverage mismatch")
    well_fold_counts = frame.groupby("well_id", sort=False)["outer_fold"].nunique()
    if not well_fold_counts.eq(1).all():
        raise ValueError("each exp333 well must have exactly one source outer fold")
    prediction = frame["tvt_pred_stage1"].to_numpy(np.float64)
    if not np.isfinite(prediction).all():
        raise ValueError("exp333 OOF contains non-finite predictions")
    content_sha = exp333_prediction_content_sha256(frame)
    source_fold_rows = (
        frame.groupby("outer_fold", sort=True).size().astype(int).to_dict()
    )
    source_fold_wells = (
        frame[["well_id", "outer_fold"]]
        .drop_duplicates()
        .groupby("outer_fold", sort=True)
        .size()
        .astype(int)
        .to_dict()
    )
...
class Exp333Fixed13CandidateCache:
    def __init__(
        self,
        root: Path,
        contract: Mapping[str, Any],
        *,
        exp333_oof: pd.DataFrame,
        exp333_manifest: Mapping[str, Any],
    ):
        validate_fixed13_contract(contract)
        ...
        self.exp333_manifest = dict(exp333_manifest)
        self.exp333_by_key = exp333_oof.sort_values(
            ["well_id", "row_idx"], kind="stable"
        ).set_index(["well_id", "row_idx"])[["outer_fold", "tvt_pred_stage1"]]
        if not self.exp333_by_key.index.is_unique:
            raise ValueError("exp333 global key index is not unique")
    ...
    def load_fold(self, fold: int) -> FoldBundle:
        ...
        added = self.exp333_by_key.reindex(selector_keys)
        missing = added["tvt_pred_stage1"].isna() | added["outer_fold"].isna()
        if missing.any():
            missing_keys = [
                (str(well), int(row))
            ...
            raise ValueError(
                f"exp333 global key join is missing exp263 fold {fold} rows: "
                f"{missing_keys}"
            )
        source_fold_sorted = added["outer_fold"].to_numpy(np.int8)
        if not np.isin(source_fold_sorted, np.arange(5, dtype=np.int8)).all():
            raise ValueError(f"exp333 fold provenance is outside 0..4 in selector fold {fold}")
        ...
        if source_well_frame.groupby("well")["source_outer_fold"].nunique().max() != 1:
            raise ValueError("exp333 source fold changed within a well after key join")
        ...
        prediction_sorted = added["tvt_pred_stage1"].to_numpy(np.float32)
        prediction = prediction_sorted[inverse]
        if not np.isfinite(prediction).all():
            raise ValueError(f"exp333 fold {fold} contains a non-finite prediction")
        ...
        confidence[ADDED_CANDIDATE_ID] = base.base[KEY_COLUMNS].assign(
            ...
            confidence_source="exp333_no_source_native_confidence",
        )
        return FoldBundle(
            base=base.base,
            values=values,
    ...
    def selector_repartition_manifest(self, *, expected_rows: int) -> dict[str, Any]:
        ...
        expected_source_fold_rows = {
            str(fold): int(value)
            for fold, value in self.exp333_manifest["source_fold_rows"].items()
        }
        ...
        return {
            ...
            "candidate_source_fold": "exp333_saved_exp226_oof_fold",
...
def build_fixed13_integration_readout(
    *,
    ...
    if fixed_parity_max_abs > 1.0e-6:
        raise ValueError(
            f"fixed fallback errors changed after exp333 addition: {fixed_parity_max_abs}"
        )
        ...
        usage_rows.append(
            {
                "scope": scope,
                "rows": int(mask.sum()),
                "exp333_top1_rows": int(selected.sum()),
                "exp333_top1_fraction": float(selected.mean()),
            }
        )
    ...
    positive_usage_folds = int(
        sum(
            usage_lookup.loc[f"fold_{fold}", "exp333_top1_fraction"] > 0.0
            for fold in range(5)
        )
    )
    hidden_deltas = [
    ...
    checks = {
        ...
        "exp333_usage_pooled": float(
            usage_lookup.loc["pooled", "exp333_top1_fraction"]
        )
        >= float(guard_config["minimum_added_candidate_primary_top1_fraction"]),
        "exp333_usage_folds": positive_usage_folds
    ...
    gate = {
        ...
        "positive_exp333_usage_folds": positive_usage_folds,
        "exp333_usage_pooled": float(
            usage_lookup.loc["pooled", "exp333_top1_fraction"]
        ),
    ...
    scope_path = output_dir / "exp371_fixed13_vs_fixed12_scope_metrics.csv"
    usage_path = output_dir / "exp371_fixed13_candidate_usage.csv"
    by_well_path = output_dir / "exp371_fixed13_vs_fixed12_by_well.csv"
    gate_path = output_dir / "exp371_scientific_gate.json"
...
def write_exp371_input_contract(
    path: Path,
    *,
    ...
    payload = {
        ...
        "exp333_oof": dict(exp333_manifest),
        "parent_exp264_score": {
...
def exp371_stage_d_cost_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    """Validate the fixed 15-booster downstream contract for exp371."""

    from src.geop_hmm_selector_audit import stage_d_full13_cost_contract

    return stage_d_full13_cost_contract(config)


def run_exp371_fixed13_stage_d_addonly(
    *,
    config: Mapping[str, Any],
    stage_c_root: Path,
    ...
    """Train clean273 + exp371 fixed13 compact77 and compare with saved exp264."""

    from src.geop_hmm_selector_audit import run_fixed13_compact_stage_d_addonly
