...
ADDED_CANDIDATE_ID = "huber_exact_hmm"
EXP389_ALLOWLIST = (
    "id",
    "well_id",
    "row_idx",
...
EXP389_PREDICTION_COLUMN = EXP389_ALLOWLIST[3]
EXP389_STD_COLUMN = EXP389_ALLOWLIST[4]
EXP389_LOGLIK_COLUMN = EXP389_ALLOWLIST[5]
EXP389_PREPARED_COLUMNS = (
    "id",
    "well_id",
    "row_idx",
    "candidate_tvt",
...
def validate_fixed13_contract(contract: Mapping[str, Any]) -> None:
    ...
    if str(added.get("kind")) != "huber_exact_hmm":
        raise ValueError("exp389 candidate kind must remain huber_exact_hmm")
    if added.get("parents"):
        raise ValueError("exp389 candidate must not be reconstructed as a formula")
...
def exp389_prediction_content_sha256(frame: pd.DataFrame) -> str:
    ...
    digest = hashlib.sha256()
    digest.update("|".join(EXP389_PREPARED_COLUMNS).encode())
...
def load_exp389_predictions(
    ...
    *,
    ...
    if expected_file_sha256 and file_sha != str(expected_file_sha256):
        raise ValueError(f"exp389 prediction file SHA mismatch: {file_sha}")
    if decompressed_sha != str(expected_decompressed_sha256):
        raise ValueError(f"exp389 prediction decompressed SHA mismatch: {decompressed_sha}")
    if len(str(expected_prediction_logical_sha256)) != 64:
        raise ValueError("exp389 upstream prediction logical SHA must be a SHA256")
    header = pd.read_csv(path, nrows=0).columns.astype(str).tolist()
    missing = set(EXP389_ALLOWLIST) - set(header)
    if missing:
        raise ValueError(
            f"exp389 prediction allowlist columns are missing: {sorted(missing)}"
        )
    frame = pd.read_csv(
        path,
        usecols=list(EXP389_ALLOWLIST),
        dtype={
            "id": str,
            "well_id": str,
            "row_idx": np.int32,
            EXP389_PREDICTION_COLUMN: np.float64,
            EXP389_STD_COLUMN: np.float64,
            EXP389_LOGLIK_COLUMN: np.float64,
        },
    )
    if list(frame.columns) != list(EXP389_ALLOWLIST):
        frame = frame.loc[:, list(EXP389_ALLOWLIST)]
    frame = frame.rename(
        columns={
            EXP389_PREDICTION_COLUMN: "candidate_tvt",
            EXP389_STD_COLUMN: "candidate_std",
            EXP389_LOGLIK_COLUMN: "hmm_loglik",
        }
    )
    if len(frame) != int(expected_rows):
        raise ValueError(f"exp389 prediction row mismatch: {len(frame)}")
    if frame["well_id"].nunique() != int(expected_wells):
        raise ValueError("exp389 prediction well count mismatch")
    if frame.duplicated(["well_id", "row_idx"]).any():
        raise ValueError("exp389 prediction contains duplicate well/row keys")
    if frame["id"].duplicated().any():
        raise ValueError("exp389 prediction contains duplicate ids")
    expected_id = (
    ...
    if id_key_mismatches:
        raise ValueError(f"exp389 id/key mismatch rows: {id_key_mismatches}")
    native = frame[
        ["candidate_tvt", "candidate_std", "hmm_loglik"]
    ].to_numpy(np.float64)
    if not np.isfinite(native).all():
        raise ValueError(
            "exp389 prediction contains non-finite candidate/native confidence"
        )
    if bool((frame["candidate_std"] < 0.0).any()):
        raise ValueError("exp389 prediction contains a negative candidate_std")
    well_loglik_counts = frame.groupby("well_id", sort=False)["hmm_loglik"].nunique()
    if not bool(well_loglik_counts.eq(1).all()):
        raise ValueError("exp389 hmm_loglik must be constant within each well")
    ...
    content_sha = exp389_prediction_content_sha256(frame)
    manifest = {
        "path": str(path),
        "header_columns": header,
        "loaded_columns": list(EXP389_ALLOWLIST),
        ...
        "post_read_allowlist_columns": list(EXP389_ALLOWLIST),
...
class Exp389Fixed13CandidateCache:
    def __init__(
        self,
        root: Path,
        ...
        *,
        ...
        self.exp389_manifest = dict(exp389_manifest)
        self.exp389_by_key = exp389_predictions.sort_values(
        ...
        if not self.exp389_by_key.index.is_unique:
            raise ValueError("exp389 global key index is not unique")
    ...
    def load_fold(self, fold: int) -> FoldBundle:
        ...
        added = self.exp389_by_key.reindex(selector_keys)
        missing = added[
            [
                "candidate_tvt",
                "candidate_std",
        ...
        if missing.any():
            ...
            raise ValueError(
                f"exp389 global key join is missing exp263 fold {fold} rows: "
                f"{missing_keys}"
            )
        ...
        if not np.isfinite(prediction).all():
            raise ValueError(f"exp389 fold {fold} contains a non-finite prediction")
        ...
        if not bool(valid.all()):
            raise ValueError(f"exp389 fold {fold} native confidence is invalid")
        conf = base.base[KEY_COLUMNS].copy()
        conf["candidate_id"] = ADDED_CANDIDATE_ID
        conf["confidence_source"] = "exp389_huber_exact_hmm_posterior"
    ...
    def selector_repartition_manifest(self, *, expected_rows: int) -> dict[str, Any]:
        ...
        checks = {
            ...
            "target_free_candidate_requires_no_source_fold": not bool(
                self.exp389_manifest["candidate_requires_oof_fold"]
            ),
...
def build_postfreeze_addone_novelty_readout(
    *,
    ...
    metrics_path = output_dir / "exp392_postfreeze_addone_novelty.csv"
    summary_path = output_dir / "exp392_postfreeze_addone_novelty.json"
...
def build_fixed13_integration_readout(
    *,
    ...
    if fixed_parity_max_abs > 1.0e-6:
        raise ValueError(
            f"fixed fallback errors changed after exp389 addition: {fixed_parity_max_abs}"
        )
        ...
        usage_rows.append(
            {
                "scope": scope,
                "rows": int(mask.sum()),
                "huber_top1_rows": int(selected.sum()),
                "huber_top1_fraction": float(selected.mean()),
            }
        )
    ...
    positive_usage_folds = int(
        sum(
            usage_lookup.loc[f"fold_{fold}", "huber_top1_fraction"] > 0.0
            for fold in range(5)
        )
    )
    hidden_deltas = [
    ...
    checks = {
        ...
        "huber_usage_pooled": float(
            usage_lookup.loc["pooled", "huber_top1_fraction"]
        )
        >= float(guard_config["minimum_added_candidate_primary_top1_fraction"]),
        "huber_usage_folds": positive_usage_folds
    ...
    gate = {
        ...
        "positive_huber_usage_folds": positive_usage_folds,
        "huber_usage_pooled": float(
            usage_lookup.loc["pooled", "huber_top1_fraction"]
        ),
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    scope_path = output_dir / "exp392_fixed13_vs_fixed12_scope_metrics.csv"
    usage_path = output_dir / "exp392_fixed13_candidate_usage.csv"
    by_well_path = output_dir / "exp392_fixed13_vs_fixed12_by_well.csv"
    gate_path = output_dir / "exp392_scientific_gate.json"
...
def write_exp392_input_contract(
    path: Path,
    *,
    ...
    payload = {
        ...
        "exp389_predictions": dict(exp389_manifest),
        "parent_exp264_score": {
